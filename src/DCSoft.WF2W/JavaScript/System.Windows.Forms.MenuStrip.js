...
function __DCMenuGetItemHandle(menuItemData) {
    if (!menuItemData) return null;
    return menuItemData.dchandle || menuItemData.DCHandle || menuItemData.Handle || null;
}

function __DCMenuGetChildItems(menuItemData) {
    if (!menuItemData) return null;
    if (Array.isArray(menuItemData.Items)) return menuItemData.Items;
    if (Array.isArray(menuItemData.DropDownItems)) return menuItemData.DropDownItems;
    if (Array.isArray(menuItemData.MenuItems)) return menuItemData.MenuItems;
    return null;
}

function __DCMenuHasChildItems(menuItemData) {
    var childItems = __DCMenuGetChildItems(menuItemData);
...
function __DCRenderMenuMnemonicText(text) {
    if (window.__DCRenderMnemonic) {
        return window.__DCRenderMnemonic(text || "");
    }
    return {
        html: text || "",
        access: null
...
class SystemWindowsFormsMenuStripFactory extends SystemWindowsFormsControlFactory {
    Create(options) {
        options = options || {};

        var el = document.createElement("nav");
        el.setAttribute("dchandle", options.Handle);
        el.setAttribute("dctype", "System.Windows.Forms.MenuStrip");
    ...
    ApplyOptions(htmlElement, options) {
        ...
        var topItems = options.Items || options.MenuItems || options.DropDownItems;
        ...
        if (!document.getElementById("dc_mainmenu_root_styles")) {
            ...
            rootStyle.textContent = [
                'nav[dctype="System.Windows.Forms.MenuStrip"] ul#dc_menu_root_ul > li.dc-menu-root-highlight {',
                "    background: #e8e8e8;",
                "    border-radius: 4px;",
                "}",
                'nav[dctype="System.Windows.Forms.MenuStrip"] ul#dc_menu_root_ul > li {',
                "    list-style: none;",
                "}"
            ].join("\n");
        ...
        function triggerMenuItemClick(menuItemData) {
            var handle = __DCMenuGetItemHandle(menuItemData);
        ...
        for (var i = 0; i < topItems.length; i++) {
            ...
            var rendered = __DCRenderMenuMnemonicText(item.Text || "");
            ...
            (function (menuItemData, anchor, menuTarget) {
                ...
                anchor.addEventListener("click", async function (ev) {
                    ...
                    var childItems = __DCMenuGetChildItems(menuItemData);
                    var menuController = await __DCShowMenu(
                        anchor,
                        null,
                        null,
                        childItems || __DCMenuGetItemHandle(menuItemData)
                    );
                ...
                anchor.addEventListener("mouseenter", function () {
                    ...
                    if (!__DCMenuHasChildItems(menuItemData)) {
                        setRootHighlight(anchor);
                        return;
                    }
    ...
    SetWindowText(htmlElement, text) {
        ...
        if (!ul.__captionElement) {
            ...
            caption.className = "dc-menustrip-caption";
...
async function __DCShowMenu(targetElement, intOffsetX, intOffsetY, arrMenuItems) {
    ...
    function closeAllMenus() {
        ...
        document.querySelectorAll('nav[dctype="System.Windows.Forms.MenuStrip"] ul#dc_menu_root_ul > li.dc-menu-root-highlight').forEach(function (li) {
            li.classList.remove("dc-menu-root-highlight");
        });
    }
    ...
    function createMenuItem(itemData, menuContainer) {
        ...
        var mnemonic = __DCRenderMenuMnemonicText(itemData.Text || "");
        ...
        if (__DCMenuHasChildItems(itemData)) {
    ...
    function setupMenuItemEvents(item, menuContainer) {
        ...
        item.addEventListener("mouseenter", function (e) {
            ...
            if (__DCMenuHasChildItems(itemData) && !item.classList.contains("dc-menu-disabled")) {
        ...
        item.addEventListener("click", async function (e) {
            ...
            if (__DCMenuHasChildItems(itemData)) {
                ...
                var handle = __DCMenuGetItemHandle(itemData);
                if (handle != null) {
    ...
    async function navigateRight() {
        if (menuState.currentHoverItem) {
            ...
            if (itemData && __DCMenuHasChildItems(itemData)) {
    ...
    async function switchToNextMainMenuItem() {
        var mainMenuBar = document.querySelector('nav[dctype="System.Windows.Forms.MenuStrip"]');
    ...
    var keydownHandler = async function (e) {
        ...
        if (matchedItems.length === 1) {
            ...
            if (__DCMenuHasChildItems(matchedData)) {
                ...
                var matchedHandle = __DCMenuGetItemHandle(matchedData);
...
window.__DCControlTypes["System.Windows.Forms.MenuStrip"] = new SystemWindowsFormsMenuStripFactory();