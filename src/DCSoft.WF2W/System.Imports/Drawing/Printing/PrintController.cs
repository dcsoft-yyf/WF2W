...
namespace System.Drawing.Printing {
    ...
    public abstract class PrintController {
        ...
        internal async Task PrintAsync(PrintDocument document)
        {
            //IntSecurity.SafePrinting.Demand();
            // Most of the printing security is left to the individual print controller
            ...
            try
            {
                canceled = await PrintLoopAsync(document);
            }
            finally
            {
                try
        ...
        public static Func<string, int> EventBeginPrint = null;
        public static Func<int, string,int,int, bool> EventSetPageContent = null;
        public static Func<int, bool> EventEndPrint = null;
        ...
        private async Task<bool> PrintLoopAsync(PrintDocument document)
        {
            QueryPageSettingsEventArgs queryEvent = new QueryPageSettingsEventArgs((PageSettings)document.DefaultPageSettings.Clone());
            var strSVGBuilder = new System.Text.StringBuilder();
            var taskID = EventBeginPrint(document.DocumentName);
            try
            {
                for (; ; )
                {
                    ...
                    var intPageWidth = pageEvent.PageBounds.Width;
                    var intPageHeight = pageEvent.PageBounds.Height;
                    strSVGBuilder.Clear();
                    var myWriter = new StringWriter(strSVGBuilder);
                    Graphics graphics = Graphics.CreateSVG(myWriter,intPageWidth , intPageHeight);// OnStartPage(document, pageEvent);
                    ...
                    try
                    {
                        ...
                        var strSVG = strSVGBuilder.ToString();
                        EventSetPageContent(
                            taskID, 
                            strSVG, 
                            intPageWidth,
