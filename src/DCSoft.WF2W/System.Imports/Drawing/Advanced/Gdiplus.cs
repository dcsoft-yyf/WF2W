...
// If GDI+ moves their allocations to the standard heap we can revisit.
            ...
            internal static Exception StatusException(int status) {
                ...
                switch (status) {
                    case GenericError:       return new ExternalException(DCSR.GetString(DCSR.GdiplusGenericError), E_FAIL);
                    case InvalidParameter:   return new ArgumentException(DCSR.GetString(DCSR.GdiplusInvalidParameter));
                    case OutOfMemory:        return new OutOfMemoryException(DCSR.GetString(DCSR.GdiplusOutOfMemory));
                    case ObjectBusy:         return new InvalidOperationException(DCSR.GetString(DCSR.GdiplusObjectBusy));
                    case InsufficientBuffer: return new OutOfMemoryException(DCSR.GetString(DCSR.GdiplusInsufficientBuffer));
                    case NotImplemented:     return new NotImplementedException(DCSR.GetString(DCSR.GdiplusNotImplemented));
                    case Win32Error:         return new ExternalException(DCSR.GetString(DCSR.GdiplusGenericError), E_FAIL);
                    case WrongState:         return new InvalidOperationException(DCSR.GetString(DCSR.GdiplusWrongState));
                    case Aborted:            return new ExternalException(DCSR.GetString(DCSR.GdiplusAborted), E_ABORT);
                    case FileNotFound:       return new FileNotFoundException(DCSR.GetString(DCSR.GdiplusFileNotFound));
                    case ValueOverflow:      return new OverflowException(DCSR.GetString(DCSR.GdiplusOverflow));
                    case AccessDenied:       return new ExternalException(DCSR.GetString(DCSR.GdiplusAccessDenied), E_ACCESSDENIED);
                    case UnknownImageFormat: return new ArgumentException(DCSR.GetString(DCSR.GdiplusUnknownImageFormat));
                    case PropertyNotFound:   return new ArgumentException(DCSR.GetString(DCSR.GdiplusPropertyNotFoundError));
                    case PropertyNotSupported:return new ArgumentException(DCSR.GetString(DCSR.GdiplusPropertyNotSupportedError));


                    case FontFamilyNotFound:
                        Debug.Fail("We should be special casing FontFamilyNotFound so we can provide the font name");
                        return new ArgumentException(DCSR.GetString(DCSR.GdiplusFontFamilyNotFound, "?"));

                    case FontStyleNotFound:
                        Debug.Fail("We should be special casing FontStyleNotFound so we can provide the font name");
                        return new ArgumentException(DCSR.GetString(DCSR.GdiplusFontStyleNotFound, "?", "?"));

                    case NotTrueTypeFont:
                        Debug.Fail("We should be special casing NotTrueTypeFont so we can provide the font name");
                        return new ArgumentException(DCSR.GetString(DCSR.GdiplusNotTrueTypeFont_NoName));

                    case UnsupportedGdiplusVersion:
                        return new ExternalException(DCSR.GetString(DCSR.GdiplusUnsupportedGdiplusVersion), E_FAIL);

                    case GdiplusNotInitialized:
                        return new ExternalException(DCSR.GetString(DCSR.GdiplusNotInitialized), E_FAIL);
                }

                return new ExternalException(DCSR.GetString(DCSR.GdiplusUnknown), E_UNEXPECTED);
            }
