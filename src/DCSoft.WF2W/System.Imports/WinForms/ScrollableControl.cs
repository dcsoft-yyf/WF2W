    ...
    public class ScrollableControl : Control, IArrangedElement
    {
        ...
        public virtual bool AutoScroll
        {
            ...
            set
            {
                ...
                this.LogWF2WPropertyValue("AutoScroll", value);
            }
        }
        ...
        public Size AutoScrollMargin
        {
            ...
            set
            {
                ...
                {
                    throw new ArgumentOutOfRangeException("AutoScrollMargin", DCSR.GetString(DCSR.InvalidArgument, "AutoScrollMargin", value.ToString()));
                }
                SetAutoScrollMargin(value.Width, value.Height);
                this.LogWF2WPropertyValue("AutoScrollMargin", value);
            }
        }
        internal override Point GetAutoScrollPositionForPaint()
        {
            return this.AutoScrollPosition;
        }
        private bool UpdateDisplayRectangleDirect()
        {
            if (this.IsHandleCreated)
            {
                var strPos = DCWin32API.JSRuntime.Invoke<string>(
                        "__DCWin32API.GetDisplayRectangle",
                        this.Handle.ToInt32());
                if (strPos != null && strPos.Length > 0)
                {
                    var strItems = strPos.Split('|');
                    if( strItems.Length == 4 
                        && int.TryParse(strItems[0], out var vX )
                        && int.TryParse(strItems[1] ,out var vY )
                        && int.TryParse(strItems[2] ,out var vWidth )
                        && int.TryParse(strItems[3], out var vHeight ))
                    {
                        this.displayRect.X = -vX;
                        this.displayRect.Y = -vY;
                        this.displayRect.Width = vWidth;
                        this.displayRect.Height = vHeight;
                        base._ClientWidth = vWidth;
                        base._ClientHeight = vHeight;
                        return true;
                    }
                }
            }
        ...
        public Point AutoScrollPosition
        {
            get
            {
                if(this.UpdateDisplayRectangleDirect())
                {
                    return this.displayRect.Location;
                }
                ...
                if (value != this.userAutoScrollMinSize)
                {
                    ...
                    if (this.IsHandleCreated)
                    {
                        DCWin32API.JSRuntime.Invoke<object>(
                            "__DCWin32API.SetScrollAutoMinSize",
                            this.Handle.ToInt32(),
                            this.userAutoScrollMinSize.Width,
                            this.userAutoScrollMinSize.Height);
                    }
                    else
                    {
                        this.LogWF2WPropertyValue("AutoScrollMinSize", value);
                    }
                }
            }
        }
        ...
        private Rectangle GetDisplayRectInternal()
        {
            ...
            {
                this.displayRect = this.ClientRectangle;
            }
        ...
        //
        // 
        private ScrollEventArgs _LastScrollArgs = null;
        internal void SetDisplayRectangleDirect(int vLeft, int vTop, int vWidth, int vHeight)
        {
            this._LastScrollArgs = null;
            if (this.displayRect.Left != vLeft
                || this.displayRect.Top != vTop
                || this.displayRect.Width != vWidth
                || this.displayRect.Height != vHeight)
            {
                this.displayRect = new Rectangle(vLeft, vTop, vWidth, vHeight);
                var args = new ScrollEventArgs(ScrollEventType.EndScroll, vLeft);
                this.OnScroll(args);
                this._LastScrollArgs = args;
            }
        }
        internal bool HasScrollAsync()
        {
            return this.Events[EVENT_SCROLLAsync] != null;
        }
        internal async ValueTask RaiseEventScrollAsync()
        {
            var args = this._LastScrollArgs;
            this._LastScrollArgs = null;
            if (args != null)
            {
                await this.OnScrollAsync(args);
            }
        }
        protected void SetDisplayRectLocation(int x, int y)
        {
            ...
            {
                DCWin32API.JSRuntime.Invoke<object>("__DCWin32API.SetScrollPosition", this.Handle.ToInt32(), x, y);
                //Rectangle cr = this.ClientRectangle;
                //WinFormNativeMethods.RECT rcClip = WinFormNativeMethods.RECT.FromXYWH(cr.X, cr.Y, cr.Width, cr.Height);
                //WinFormNativeMethods.RECT rcUpdate = WinFormNativeMethods.RECT.FromXYWH(cr.X, cr.Y, cr.Width, cr.Height);
                //WinFormSafeNativeMethods.ScrollWindowEx(new HandleRef(this, Handle), xDelta, yDelta, 
                //                                 null,
                //                                 ref rcClip, 
                //                                 WinFormNativeMethods.NullHandleRef, 
                //                                 ref rcUpdate,
                //                                 WinFormNativeMethods.SW_INVALIDATE
                //                                 | WinFormNativeMethods.SW_ERASE
                //                                 | WinFormNativeMethods.SW_SCROLLCHILDREN);
            }
        ...
        private int ScrollThumbPosition(int fnBar)
        {
            ...
            si.fMask = WinFormNativeMethods.SIF_TRACKPOS;
            WinFormSafeNativeMethods.GetScrollInfo(new HandleRef(this, Handle), fnBar, si);
            return si.nTrackPos;
        }


        [SRCategory(DCSR.CatAction), SRDescription(DCSR.ScrollBarOnScrollDescr)]
        [System.Reflection.Obfuscation(Exclude = true, ApplyToMembers = true)]
        public event ScrollEventHandler Scroll
        {
            add
            {
                this.Events.AddHandler(EVENT_SCROLL, value);
            }
            remove
            {
                this.Events.RemoveHandler(EVENT_SCROLL, value);
            }
        }
        ...
        {
            var handler = (ScrollEventHandler)this.Events[EVENT_SCROLL];
            if (handler != null) handler(this, se);
        }

        private static readonly object EVENT_SCROLLAsync = new object();
        [SRCategory(DCSR.CatAction), SRDescription(DCSR.ScrollBarOnScrollDescr)]
        [System.Reflection.Obfuscation(Exclude = true, ApplyToMembers = true)]
        public event ScrollEventAsyncHandler ScrollAsync
        {
            add
            {
                this.Events.AddHandler(EVENT_SCROLLAsync, value);
            }
            remove
            {
                this.Events.RemoveHandler(EVENT_SCROLLAsync, value);
            }
        }

        [System.Reflection.Obfuscation(Exclude = true, ApplyToMembers = true)]
        internal async ValueTask OnScrollAsync(ScrollEventArgs se)
        {
            var handler = (ScrollEventAsyncHandler)this.Events[EVENT_SCROLLAsync];
            if (handler != null) await handler(this, se);
        }
        ...
        private void OnSetScrollPosition(object sender, EventArgs e)
        {
            if (!IsMirrored)
            {
                SendMessage(WinFormNativeMethods.WM_HSCROLL,
                            WinFormNativeMethods.Util.MAKELPARAM((RightToLeft == RightToLeft.Yes) ? WinFormNativeMethods.SB_RIGHT : WinFormNativeMethods.SB_LEFT, 0), 0);
            }
        }
        ...
        private void WmVScroll(ref Message m)
        {
            ...
            Rectangle client = ClientRectangle;
            bool thumbTrack = WinFormNativeMethods.Util.LOWORD(m.WParam) != WinFormNativeMethods.SB_THUMBTRACK;
            int pos = -displayRect.Y;
            int oldValue = pos;
            ...
            switch (WinFormNativeMethods.Util.LOWORD(m.WParam))
            {
                case WinFormNativeMethods.SB_THUMBPOSITION:
                case WinFormNativeMethods.SB_THUMBTRACK:
                    pos = ScrollThumbPosition(WinFormNativeMethods.SB_VERT);
                    break;
                case WinFormNativeMethods.SB_LINEUP:
                    if (pos > 0)
                    {
                    ...
                    break;
                case WinFormNativeMethods.SB_LINEDOWN:
                    ...
                    break;
                case WinFormNativeMethods.SB_PAGEUP:
                    ...
                    break;
                case WinFormNativeMethods.SB_PAGEDOWN:
                    ...
                    break;
                case WinFormNativeMethods.SB_TOP:
                    pos = 0;
                    break;
                case WinFormNativeMethods.SB_BOTTOM:
                    pos = maxPos;
                    break;
            }
        ...
        private void WmHScroll(ref Message m)
        {
            ...
            switch (WinFormNativeMethods.Util.LOWORD(m.WParam))
            {
                case WinFormNativeMethods.SB_THUMBPOSITION:
                case WinFormNativeMethods.SB_THUMBTRACK:
                    pos = ScrollThumbPosition(WinFormNativeMethods.SB_HORZ);
                    break;
                case WinFormNativeMethods.SB_LINEUP:
                ...
                case WinFormNativeMethods.SB_PAGEDOWN:
                    ...
                    break;
                case WinFormNativeMethods.SB_LEFT:
                    pos = 0;
                    break;
                case WinFormNativeMethods.SB_RIGHT:
                    pos = maxPos;
                    break;
            }
            if (GetScrollState(ScrollStateFullDrag) || WinFormNativeMethods.Util.LOWORD(m.WParam) != WinFormNativeMethods.SB_THUMBTRACK)
            {
                SetScrollState(ScrollStateUserHasScrolled, true);
                SetDisplayRectLocation(-pos, displayRect.Y);
                SyncScrollbars(AutoScroll);
        ...
        {
            ScrollEventType type = (ScrollEventType)WinFormNativeMethods.Util.LOWORD(m.WParam);
        ...
        protected override async Task WndProc(Message m)
        {
            switch (m.Msg)
            {
                ...
                case WinFormNativeMethods.WM_HSCROLL:
                    WmHScroll(ref m);
                    break;
                case WinFormNativeMethods.WM_SETTINGCHANGE:
        ...
        public class DockPaddingEdges : ICloneable
        {
            ...
            SRDescription(DCSR.PaddingAllDescr)
            ]
            public int All
            {
                get
            ...
            [
            RefreshProperties(RefreshProperties.All),
            SRDescription(DCSR.PaddingBottomDescr)
            ]
            public int Bottom
            {
                get
            ...
            [
            RefreshProperties(RefreshProperties.All),
            SRDescription(DCSR.PaddingLeftDescr)
            ]
            public int Left
            {
                get
            ...
            [
            RefreshProperties(RefreshProperties.All),
            SRDescription(DCSR.PaddingRightDescr)
            ]
            public int Right
            {
                get
            ...
            [
            RefreshProperties(RefreshProperties.All),
            SRDescription(DCSR.PaddingTopDescr)
            ]
            public int Top
            {
                get
