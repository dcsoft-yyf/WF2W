...
namespace System.Windows.Forms {
    ...
    internal sealed class WindowsFormsUtils {
        ...
        public static Point LastCursorPoint  {
            get {
                int lastXY = WinFormSafeNativeMethods.GetMessagePos();
                return new Point(WinFormNativeMethods.Util.SignedLOWORD(lastXY),WinFormNativeMethods.Util.SignedHIWORD(lastXY)) ;
            }
        }
        ...
        public static Graphics CreateMeasurementGraphics() {
            return System.Drawing.Graphics.CreateCanvas();
...
#if DEBUG      
            try {
                var strText = DCWin32API.GetWindowText(hwnd.ToInt32());
                 //int textLen = WinFormSafeNativeMethods.GetWindowTextLength(new HandleRef(null, hwnd));
                 //StringBuilder sb = new StringBuilder(textLen+1);
                 //WinFormUnsafeNativeMethods.GetWindowText(new HandleRef(null, hwnd), sb, sb.Capacity);
        ...
        public static HandleRef GetRootHWnd(HandleRef hwnd) {
            IntPtr rootHwnd = WinFormUnsafeNativeMethods.GetAncestor(new HandleRef(hwnd, hwnd.Handle), WinFormNativeMethods.GA_ROOT);
        ...
        public static Point TranslatePoint(Point point, Control fromControl, Control toControl) {
            WinFormNativeMethods.POINT pt = new WinFormNativeMethods.POINT(point.X, point.Y);
            WinFormUnsafeNativeMethods.MapWindowPoints(new HandleRef(fromControl, fromControl.Handle), new HandleRef(toControl, toControl.Handle), pt, 1);
        ...
        internal class TypedControlCollection : ReadOnlyControlCollection {
            ...
            public override void Add(Control value) {
                ...
                if (!typeOfControl.IsAssignableFrom(value.GetType())) {
                    throw new ArgumentException(String.Format(CultureInfo.CurrentCulture, DCSR.GetString(DCSR.TypedControlCollectionShouldBeOfType, typeOfControl.Name)), value.GetType().Name);
                }
                base.Add(value);
            }
        }
        ...
        internal struct DCMapping : IDisposable {
            ...
            public DCMapping(HandleRef hDC, Rectangle bounds) {
                ...
                HandleRef hOriginalClippingRegion            = WinFormNativeMethods.NullHandleRef;
                WinFormNativeMethods.RegionFlags originalRegionType = WinFormNativeMethods.RegionFlags.NULLREGION;
                ...
                success = WinFormSafeNativeMethods.GetViewportOrgEx(hDC, viewportOrg);
                ...
                HandleRef hClippingRegion = new HandleRef(null, WinFormSafeNativeMethods.CreateRectRgn(viewportOrg.x + bounds.Left, viewportOrg.y + bounds.Top, viewportOrg.x + bounds.Right, viewportOrg.y + bounds.Bottom));
                ...
                try
                {
                    ...
                    hOriginalClippingRegion = new HandleRef(this, WinFormSafeNativeMethods.CreateRectRgn(0, 0, 0, 0));
                    ...
                    int result = WinFormSafeNativeMethods.GetClipRgn(hDC, hOriginalClippingRegion);
                    Debug.Assert(result != -1, "GetClipRgn() failed.");

                    // Shift the viewpoint origint by coordinates specified in "bounds". 
                    WinFormNativeMethods.POINT lastViewPort = new WinFormNativeMethods.POINT();
                    success = WinFormSafeNativeMethods.SetViewportOrgEx(hDC, viewportOrg.x + bounds.Left, viewportOrg.y + bounds.Top, lastViewPort);
                    ...
                    if (result != 0)
                    {
                        ...
                        WinFormNativeMethods.RECT originalClipRect = new WinFormNativeMethods.RECT();
                        originalRegionType = (WinFormNativeMethods.RegionFlags)WinFormSafeNativeMethods.GetRgnBox(hOriginalClippingRegion, ref originalClipRect);
                        Debug.Assert(originalRegionType != WinFormNativeMethods.RegionFlags.ERROR, "ERROR returned from SelectClipRgn while selecting the original clipping region..");

                        if (originalRegionType == WinFormNativeMethods.RegionFlags.SIMPLEREGION)
                        {
                            ...
                            WinFormNativeMethods.RegionFlags combineResult = (WinFormNativeMethods.RegionFlags)WinFormSafeNativeMethods.CombineRgn(hClippingRegion, hClippingRegion, hOriginalClippingRegion, WinFormNativeMethods.RGN_AND);
                            Debug.Assert((combineResult == WinFormNativeMethods.RegionFlags.SIMPLEREGION) ||
                                            (combineResult == WinFormNativeMethods.RegionFlags.NULLREGION),
                    ...
                    else
                    {
                        ...
                        WinFormSafeNativeMethods.DeleteObject(hOriginalClippingRegion);
                        hOriginalClippingRegion = new HandleRef(null, IntPtr.Zero);
                        originalRegionType = WinFormNativeMethods.RegionFlags.SIMPLEREGION;
                    }
                ...
                catch (Exception ex)
                {
                    ...
                    if (hOriginalClippingRegion.Handle != IntPtr.Zero) {
                        success = WinFormSafeNativeMethods.DeleteObject(hOriginalClippingRegion);
