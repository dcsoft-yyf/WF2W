    ...
    public sealed class ImageListStreamer : ISerializable, IDisposable {
                    ...
                    if (dat != null)
                    {
                        ...
                        try
                        {
                            ...
                            lock (internalSyncObject) {
                                WinFormSafeNativeMethods.InitCommonControls();
                                nativeImageList = new ImageList.NativeImageList(
                                    WinFormSafeNativeMethods.ImageList_Read(new WinFormUnsafeNativeMethods.ComStreamFromDataStream(ms)));
                            }
                        }
                        finally
                        {
                        ...
                        {
                            throw new InvalidOperationException(DCSR.GetString(DCSR.ImageListStreamerLoadFailed));
                        }
                    }
#if DEBUG
                    }
        ...
        public void /*cpr: ISerializable*/GetObjectData(SerializationInfo si, StreamingContext context) {
            ...
            if (handle == IntPtr.Zero || !WriteImageList(handle, stream))
                throw new InvalidOperationException(DCSR.GetString(DCSR.ImageListStreamerSaveFailed));
        ...
        private bool WriteImageList(IntPtr imagelistHandle, Stream stream) {
            ...
            try {
                int hResult = WinFormSafeNativeMethods.ImageList_WriteEx(new HandleRef(this, imagelistHandle), WinFormNativeMethods.ILP_DOWNLEVEL, new WinFormUnsafeNativeMethods.ComStreamFromDataStream(stream));
                return (hResult == WinFormNativeMethods.S_OK);
            }
            catch (EntryPointNotFoundException) {
                // WriteEx wasn't found - that's fine - we will use Write.
            }

            return WinFormSafeNativeMethods.ImageList_Write(new HandleRef(this, imagelistHandle), new WinFormUnsafeNativeMethods.ComStreamFromDataStream(stream));
        }
