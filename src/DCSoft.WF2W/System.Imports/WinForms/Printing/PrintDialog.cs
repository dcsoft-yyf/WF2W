...
namespace System.Windows.Forms {
    ...
    public sealed class PrintDialog : CommonDialog {
        ...
        [
        DefaultValue(false),
        SRDescription(DCSR.PDallowCurrentPageDescr)
        ]
        ...
        DefaultValue(false),
        SRDescription(DCSR.PDallowPagesDescr)
        ]
        ...
        SRCategory(DCSR.CatBehavior), 
        DefaultValue(true),
        SRDescription(DCSR.PDallowPrintToFileDescr)
        ]
        ...
        SRCategory(DCSR.CatBehavior), 
        DefaultValue(false),
        SRDescription(DCSR.PDallowSelectionDescr)
        ]
        ...
        SRDescription(DCSR.PDprinterSettingsDescr)
        ]
        ...
        SRCategory(DCSR.CatBehavior), 
        DefaultValue(false),
        SRDescription(DCSR.PDprintToFileDescr)
        ]
        ...
        SRCategory(DCSR.CatBehavior), 
        DefaultValue(false),
        SRDescription(DCSR.PDshowHelpDescr)
        ]
        ...
        SRCategory(DCSR.CatBehavior), 
        DefaultValue(true),
        SRDescription(DCSR.PDshowNetworkDescr)
        ]
        ...
        [
        DefaultValue(false),
        SRDescription(DCSR.PDuseEXDialog)
        ]
        ...
        private int GetFlags() {
            ...
            if (!UseEXDialog || (Environment.OSVersion.Platform != System.PlatformID.Win32NT ||
                ...
                flags |= WinFormNativeMethods.PD_ENABLEPRINTHOOK;
            }

            if (!allowCurrentPage) flags |= WinFormNativeMethods.PD_NOCURRENTPAGE;
            if (!allowPages) flags |= WinFormNativeMethods.PD_NOPAGENUMS;
            if (!allowPrintToFile) flags |= WinFormNativeMethods.PD_DISABLEPRINTTOFILE;
            if (!allowSelection) flags |= WinFormNativeMethods.PD_NOSELECTION;

            flags |= (int) PrinterSettings.PrintRange;

            if (printToFile) flags |= WinFormNativeMethods.PD_PRINTTOFILE;
            if (showHelp) flags |= WinFormNativeMethods.PD_SHOWHELP;
            if (!showNetwork) flags |= WinFormNativeMethods.PD_NONETWORKBUTTON;
            if (PrinterSettings.Collate) flags |= WinFormNativeMethods.PD_COLLATE;
            return flags;
        }
        ...
        internal static WinFormNativeMethods.PRINTDLG CreatePRINTDLG() {
            WinFormNativeMethods.PRINTDLG data = null;
            if (IntPtr.Size == 4) {
                data = new WinFormNativeMethods.PRINTDLG_32();
            }
            else {
                data = new WinFormNativeMethods.PRINTDLG_64();
            }
        ...
        internal static WinFormNativeMethods.PRINTDLGEX CreatePRINTDLGEX() {
            WinFormNativeMethods.PRINTDLGEX data = new WinFormNativeMethods.PRINTDLGEX();
            data.lStructSize = Marshal.SizeOf(data);
            data.hwndOwner = IntPtr.Zero;
            data.hDevMode = IntPtr.Zero;
            data.hDevNames = IntPtr.Zero;
            ...
            data.pageRanges = WinFormUnsafeNativeMethods.GlobalAlloc(WinFormNativeMethods.GPTR,
                                                              data.nMaxPageRanges * Marshal.SizeOf(typeof(WinFormNativeMethods.PRINTPAGERANGE)));
            data.nMinPage = 0;
            data.nMaxPage = 9999;
            data.nCopies = 1;
            data.hInstance = IntPtr.Zero;
            ...
            data.nStartPage = WinFormNativeMethods.START_PAGE_GENERAL;
        ...
        protected override bool RunDialog(IntPtr hwndOwner) {
            ...
            if (!UseEXDialog || (Environment.OSVersion.Platform != System.PlatformID.Win32NT ||
                Environment.OSVersion.Version.Major < 5)) {
                WinFormNativeMethods.PRINTDLG data = CreatePRINTDLG();
                returnValue = ShowPrintDialog(hwndOwner, hookProcPtr, data);
            }
            else {
                WinFormNativeMethods.PRINTDLGEX data = CreatePRINTDLGEX();
        ...
        private bool ShowPrintDialog(IntPtr hwndOwner, WinFormNativeMethods.WndProc hookProcPtr, WinFormNativeMethods.PRINTDLG data) {
            ...
            try {
                ...
                if (AllowSomePages) {
                    if (PrinterSettings.FromPage < PrinterSettings.MinimumPage
                        ...
                        throw new ArgumentException(DCSR.GetString(DCSR.PDpageOutOfRange, "FromPage"));
                    if (PrinterSettings.ToPage < PrinterSettings.MinimumPage
                        || PrinterSettings.ToPage > PrinterSettings.MaximumPage)
                        throw new ArgumentException(DCSR.GetString(DCSR.PDpageOutOfRange, "ToPage"));
                ...
                if (!WinFormUnsafeNativeMethods.PrintDlg(data))
                    return false;
                ...
                PrintToFile = ((data.Flags & WinFormNativeMethods.PD_PRINTTOFILE) != 0);
                ...
                if ((data.Flags & WinFormNativeMethods.PD_USEDEVMODECOPIESANDCOLLATE) == 0) {
                    if (Environment.OSVersion.Version.Major >= 6) {
                        PrinterSettings.Copies = data.nCopies;
                        PrinterSettings.Collate = ((data.Flags & WinFormNativeMethods.PD_COLLATE) == WinFormNativeMethods.PD_COLLATE);
                    }
                }
        ...
        // VSWhidbey 93449: Due to the nature of PRINTDLGEX vs PRINTDLG, separate but similar methods
        // are required for showing the print dialog on Win2k and newer OS'.
        private bool ShowPrintDialog(IntPtr hwndOwner, WinFormNativeMethods.PRINTDLGEX data) {
            ...
            try {
                ...
                data.Flags &= ~(WinFormNativeMethods.PD_SHOWHELP | WinFormNativeMethods.PD_NONETWORKBUTTON);
 
                int hr = WinFormUnsafeNativeMethods.PrintDlgEx(data);
                if (WinFormNativeMethods.Failed(hr) || data.dwResultAction == WinFormNativeMethods.PD_RESULT_CANCEL) {
                    return false;
                }
                ...
                return (data.dwResultAction == WinFormNativeMethods.PD_RESULT_PRINT);
            }
            finally {
                ...
                if (data.pageRanges != IntPtr.Zero)
                    WinFormUnsafeNativeMethods.GlobalFree(new HandleRef(data, data.pageRanges));
            }
        }
