...
namespace System.Windows.Forms {
    ...
    public abstract class ToolStripDropDownItem : ToolStripItem {
        ...
        SRDescription(DCSR.ToolStripDropDownDescr)
        ]
            ...
            set { 
                if (dropDown != value) {
                    ...
                    if (dropDown != null) {
                        ...
                        dropDown.ItemClickedAsync -= DropDown_ItemClickedAsync;
        ...
        [Browsable(false)]
        [SRDescription(DCSR.ToolStripDropDownItemDropDownDirectionDescr)]
        ...
        SRDescription(DCSR.ToolStripDropDownClosedDecr)
        ]
        ...
        [
        SRCategory(DCSR.CatAction),
        SRDescription(DCSR.ToolStripDropDownOpeningDescr)
        ]
        ...
        [
        SRCategory(DCSR.CatAction),
        SRDescription(DCSR.ToolStripDropDownOpenedDescr)
        ]
        [System.Reflection.Obfuscation( Exclude = true , ApplyToMembers = true )]
        public event EventHandler DropDownOpened {
            add {
                this.Events.AddHandler(EventDropDownOpened, value);
            }
            remove {
                this.Events.RemoveHandler(EventDropDownOpened, value);
            }
        }

        private static readonly object EventDropDownOpenedAsync = new object();
        [
        SRCategory(DCSR.CatAction),
        SRDescription(DCSR.ToolStripDropDownOpenedDescr)
        ]
        [System.Reflection.Obfuscation(Exclude = true, ApplyToMembers = true)]
        public event EventHandlerAsync DropDownOpenedAsync
        {
            add
            {
                this.Events.AddHandler(EventDropDownOpenedAsync, value);
            }
            remove
            {
                this.Events.RemoveHandler(EventDropDownOpenedAsync, value);
            }
        }
        ...
        SRCategory(DCSR.CatData),
        SRDescription(DCSR.ToolStripDropDownItemsDescr)
        ]
        ...
        internal async ValueTask<ToolStripItemCollection> GetDropDownItemsAsync() {
            this.OnDropDownOpened(EventArgs.Empty);
            await this.OnDropDownOpenedAsync(EventArgs.Empty);
            return DropDown.Items;
        }
        [System.Reflection.Obfuscation(Exclude = true, ApplyToMembers = true )]
        [SRCategory(DCSR.CatAction)]
        ...
        private static readonly object EventDropDownItemClickedAsync = new object();
        [System.Reflection.Obfuscation(Exclude = true, ApplyToMembers = true)]
        [SRCategory(DCSR.CatAction)]
        public event ToolStripItemClickedAsyncEventHandler DropDownItemClickedAsync
        {
            add
            {
                Events.AddHandler(EventDropDownItemClickedAsync, value);
            }
            remove
            {
                Events.RemoveHandler(EventDropDownItemClickedAsync, value);
            }
        }
        ...
        private async Task DropDown_ItemClickedAsync(object sender, ToolStripItemClickedEventArgs e)
        {
            await OnDropDownItemClickedAsync(e);
        }
        ...
        protected internal virtual async ValueTask OnDropDownOpenedAsync(System.EventArgs e)
        {
            // only send the event if we're the thing that currently owns the DropDown.

            if (DropDown.OwnerItem == this)
            {
                var handler = (EventHandlerAsync)Events[EventDropDownOpenedAsync];
        ...
        protected internal virtual void OnDropDownItemClicked(ToolStripItemClickedEventArgs e) {
            ...
            if (this.DropDown.OwnerItem == this) {
                ToolStripItemClickedEventHandler handler = (ToolStripItemClickedEventHandler)this.Events[EventDropDownItemClicked];
                if (handler != null) handler(this, e);
            }
        }

        protected internal virtual async Task OnDropDownItemClickedAsync(ToolStripItemClickedEventArgs e)
        {
            // only send the event if we're the thing that currently owns the DropDown.

            if (this.DropDown.OwnerItem == this)
            {
                var handler = (ToolStripItemClickedAsyncEventHandler)this.Events[EventDropDownItemClickedAsync];
                if (handler != null) await handler(this, e);
            }
        }
        ...
        private void ShowDropDownInternal() {
            ...
            if (this.dropDown != null && !this.dropDown.Visible) {
                ...
                if (this.DropDown == this.ParentInternal) {
                    throw new InvalidOperationException(DCSR.GetString(DCSR.ToolStripShowDropDownInvalidOperation));
                }
