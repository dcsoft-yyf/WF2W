...
namespace System.Windows.Forms {
    ...
    public class DataObject : IDataObject, IComDataObject {
        ...
        private IntPtr GetCompatibleBitmap(Bitmap bm) {
            ...
            //// Get the screen DC.
            ////
            //IntPtr hDC = WinFormUnsafeNativeMethods.GetDC(WinFormNativeMethods.NullHandleRef);

            //// Create a compatible DC to render the source bitmap.
            ////
            //IntPtr dcSrc = WinFormUnsafeNativeMethods.CreateCompatibleDC(new HandleRef(null, hDC));
            //IntPtr srcOld = WinFormSafeNativeMethods.SelectObject(new HandleRef(null, dcSrc), new HandleRef(bm, hBitmap));

            //// Create a compatible DC and a new compatible bitmap. 
            ////
            //IntPtr dcDest = WinFormUnsafeNativeMethods.CreateCompatibleDC(new HandleRef(null, hDC));
            //IntPtr hBitmapNew = WinFormSafeNativeMethods.CreateCompatibleBitmap(new HandleRef(null, hDC), bm.Size.Width, bm.Size.Height);

            //// Select the new bitmap into a compatible DC and render the blt the original bitmap.
            ////
            //IntPtr destOld = WinFormSafeNativeMethods.SelectObject(new HandleRef(null, dcDest), new HandleRef(null, hBitmapNew));
            //WinFormSafeNativeMethods.BitBlt(new HandleRef(null, dcDest), 0, 0, bm.Size.Width, bm.Size.Height, new HandleRef(null, dcSrc), 0, 0, 0x00CC0020);

            //// Clear the source and destination compatible DCs.
            ////
            //WinFormSafeNativeMethods.SelectObject(new HandleRef(null, dcSrc), new HandleRef(null, srcOld));
            //WinFormSafeNativeMethods.SelectObject(new HandleRef(null, dcDest), new HandleRef(null, destOld));

            //WinFormUnsafeNativeMethods.DeleteCompatibleDC(new HandleRef(null, dcSrc));
            //WinFormUnsafeNativeMethods.DeleteCompatibleDC(new HandleRef(null, dcDest));
            //WinFormUnsafeNativeMethods.ReleaseDC(WinFormNativeMethods.NullHandleRef, new HandleRef(null, hDC));

            //WinFormSafeNativeMethods.DeleteObject(new HandleRef(bm, hBitmap));
...
/* gpr
       ...
       // [SecurityPermission(SecurityAction.Demand, Flags=SecurityPermissionFlag.UnmanagedCode)]
        ...
        void IComDataObject.DUnadvise(int dwConnection) {
            ...
            Marshal.ThrowExceptionForHR(WinFormNativeMethods.E_NOTIMPL);
        }
       ...
       // [SecurityPermission(SecurityAction.Demand, Flags=SecurityPermissionFlag.UnmanagedCode)]
        IEnumFORMATETC IComDataObject.EnumFormatEtc(DATADIR dwDirection) {
            ...
            if (dwDirection == DATADIR.DATADIR_GET) {
                ...
                throw new ExternalException(DCSR.GetString(DCSR.ExternalException), WinFormNativeMethods.E_NOTIMPL);
            }
        }
        ...
        void IComDataObject.GetData(ref FORMATETC formatetc, out STGMEDIUM medium) {
            ...
            if (GetTymedUseable(formatetc.tymed)) {
                if ((formatetc.tymed & TYMED.TYMED_HGLOBAL) != 0) {
                    ...
                    medium.unionmember = WinFormUnsafeNativeMethods.GlobalAlloc(WinFormNativeMethods.GMEM_MOVEABLE
                                                             | WinFormNativeMethods.GMEM_DDESHARE
                                                             | WinFormNativeMethods.GMEM_ZEROINIT,
                                                             1);
                    ...
                    try {
                        ...
                        WinFormUnsafeNativeMethods.GlobalFree(new HandleRef(medium, medium.unionmember));
        ...
        */
        ...
        private int SaveStreamToHandle(ref IntPtr handle, Stream stream) {
            if (handle != IntPtr.Zero) {
                WinFormUnsafeNativeMethods.GlobalFree(new HandleRef(null, handle));
            }
            int size = (int)stream.Length;
            handle = WinFormUnsafeNativeMethods.GlobalAlloc(WinFormNativeMethods.GMEM_MOVEABLE | WinFormNativeMethods.GMEM_DDESHARE, size);
            if (handle == IntPtr.Zero) {
                return (WinFormNativeMethods.E_OUTOFMEMORY);
            }
            IntPtr ptr = WinFormUnsafeNativeMethods.GlobalLock(new HandleRef(null, handle));
            if (ptr == IntPtr.Zero) {
                return (WinFormNativeMethods.E_OUTOFMEMORY);
            }
            try {
                ...
                WinFormUnsafeNativeMethods.GlobalUnlock(new HandleRef(null, handle));
            }
            return WinFormNativeMethods.S_OK;
        }
        ...
        private int SaveFileListToHandle(IntPtr handle, string[] files) {
            ...
            if (unicode) {
                ...
                for (int i=0; i<files.Length; i++) {
                    sizeInBytes += WinFormNativeMethods.Util.GetPInvokeStringLength(files[i]) + 1;
                }
                sizeInBytes ++;
            }
            ...
            //
            IntPtr newHandle = WinFormUnsafeNativeMethods.GlobalReAlloc(new HandleRef(null, handle),
                                                  sizeInBytes,
                                                  WinFormNativeMethods.GMEM_MOVEABLE | WinFormNativeMethods.GMEM_DDESHARE);
            if (newHandle == IntPtr.Zero) {
                return (WinFormNativeMethods.E_OUTOFMEMORY);
            }
            IntPtr basePtr = WinFormUnsafeNativeMethods.GlobalLock(new HandleRef(null, newHandle));
            ...
            for (int i=0; i<files.Length; i++) {
                if (unicode) {
                    ...
                    WinFormUnsafeNativeMethods.CopyMemoryW(currentPtr, files[i], files[i].Length*2);
                    ...
                    int pinvokeLen = WinFormNativeMethods.Util.GetPInvokeStringLength(files[i]);
                    WinFormUnsafeNativeMethods.CopyMemoryA(currentPtr, files[i], pinvokeLen);
            ...
            WinFormUnsafeNativeMethods.GlobalUnlock(new HandleRef(null, newHandle));
            return WinFormNativeMethods.S_OK;
        }
        ...
        private int SaveStringToHandle(IntPtr handle, string str, bool unicode) {
            ...
            if (unicode) {
                ...
                newHandle = WinFormUnsafeNativeMethods.GlobalReAlloc(new HandleRef(null, handle),
                                                  byteSize,
                                                  WinFormNativeMethods.GMEM_MOVEABLE
                                                  | WinFormNativeMethods.GMEM_DDESHARE
                                                  | WinFormNativeMethods.GMEM_ZEROINIT);
                if (newHandle == IntPtr.Zero) {
                    return (WinFormNativeMethods.E_OUTOFMEMORY);
                }
                IntPtr ptr = WinFormUnsafeNativeMethods.GlobalLock(new HandleRef(null, newHandle));
                if (ptr == IntPtr.Zero) {
                    return (WinFormNativeMethods.E_OUTOFMEMORY);
                }
                // NOTE: DllLib.copy(char[]...) converts to ANSI on Windows 95...
                char[] chars = str.ToCharArray(0, str.Length);
                WinFormUnsafeNativeMethods.CopyMemoryW(ptr, chars, chars.Length*2);
                ...
                int pinvokeSize = WinFormUnsafeNativeMethods.WideCharToMultiByte(0 /*CP_ACP*/, 0, str, str.Length, null, 0, IntPtr.Zero, IntPtr.Zero);

                byte[] strBytes = new byte[pinvokeSize];
                WinFormUnsafeNativeMethods.WideCharToMultiByte(0 /*CP_ACP*/, 0, str, str.Length, strBytes, strBytes.Length, IntPtr.Zero, IntPtr.Zero);
                ...
                WinFormUnsafeNativeMethods.CopyMemory(ptr, strBytes, pinvokeSize);
        ...
        private int SaveHtmlToHandle(IntPtr handle, string str) {
            ...
            try { 
                WinFormUnsafeNativeMethods.CopyMemory(ptr, bytes, bytes.Length);
                ...
                if (formats != null) {
                    ...
                    if (dataObject != null && dataObject.RestrictedFormats) {
                        if (!Clipboard.IsFormatValid(formats)) {
                            throw new SecurityException(DCSR.GetString(DCSR.ClipboardSecurityException));
                        }
                    }
        ...
        private class OleConverter : IDataObject {
            ...
            private Object GetDataFromOleIStream(string format) {
                ...
                if (WinFormNativeMethods.S_OK != QueryGetDataUnsafe(ref formatetc)){
                    return null;
                }
                ...
                if (medium.unionmember != IntPtr.Zero) {
                    WinFormUnsafeNativeMethods.IStream pStream = (WinFormUnsafeNativeMethods.IStream)Marshal.GetObjectForIUnknown(medium.unionmember);
                    Marshal.Release(medium.unionmember);
                    WinFormNativeMethods.STATSTG sstg = new WinFormNativeMethods.STATSTG();
                    pStream.Stat(sstg, WinFormNativeMethods.STATFLAG_DEFAULT );
                    int size = (int)sstg.cbSize;

                    IntPtr hglobal = WinFormUnsafeNativeMethods.GlobalAlloc(WinFormNativeMethods.GMEM_MOVEABLE
                                                      | WinFormNativeMethods.GMEM_DDESHARE
                                                      | WinFormNativeMethods.GMEM_ZEROINIT,
                                                      size);
                    IntPtr ptr = WinFormUnsafeNativeMethods.GlobalLock(new HandleRef(innerData, hglobal));
                    pStream.Read(ptr, size);
                    WinFormUnsafeNativeMethods.GlobalUnlock(new HandleRef(innerData, hglobal));
            ...
            private Object GetDataFromHGLOBLAL(string format, IntPtr hglobal) {
                ...
                if (hglobal != IntPtr.Zero) {
                    ...
                    WinFormUnsafeNativeMethods.GlobalFree(new HandleRef(null, hglobal));
                }
            ...
            private Object GetDataFromOleOther(string format) {
                ...
                if (medium.unionmember != IntPtr.Zero) {
                    ...
                    if (format.Equals(DataFormats.Bitmap)
                        ...
                        System.Internal.HandleCollector.Add(medium.unionmember, WinFormNativeMethods.CommonHandles.GDI);
                        ...
                        if (clipboardImage != null) {
                            ...
                            WinFormSafeNativeMethods.DeleteObject(new HandleRef(null, medium.unionmember));
                            firstImage.Dispose();
                        }
                        data = clipboardImage;
                    }
...
*/
            ...
            private Stream ReadByteStreamFromHandle(IntPtr handle, out bool isSerializedObject) {
                IntPtr ptr = WinFormUnsafeNativeMethods.GlobalLock(new HandleRef(null, handle));
                if (ptr == IntPtr.Zero){
                    throw new ExternalException(DCSR.GetString(DCSR.ExternalException), WinFormNativeMethods.E_OUTOFMEMORY);
                }
                try {
                    int size = WinFormUnsafeNativeMethods.GlobalSize(new HandleRef(null, handle));
                    byte[] bytes = new byte[size];
            ...
            private static Object ReadObjectFromHandleDeserializer(Stream stream) {
                throw new NotSupportedException("BinaryFormatter");
                //BinaryFormatter formatter = new BinaryFormatter();
            ...
            private string[] ReadFileListFromHandle(IntPtr hdrop) {
                ...
                string[] files = null;
                StringBuilder sb = new StringBuilder(WinFormNativeMethods.MAX_PATH);

                int count = WinFormUnsafeNativeMethods.DragQueryFile(new HandleRef(null, hdrop), unchecked((int)0xFFFFFFFF), null, 0);
                if (count > 0) {
                    files = new string[count];


                    for (int i=0; i<count; i++) {
                        int charlen = WinFormUnsafeNativeMethods.DragQueryFile(new HandleRef(null, hdrop), i, sb, sb.Capacity);
            ...
            public virtual void SetData(string format, bool autoConvert, Object data) {
                ...
                if (data is Bitmap && format.Equals(DataFormats.Dib)) {
                    if (autoConvert) {
                        ...
                        throw new NotSupportedException(DCSR.GetString(DCSR.DataObjectDibNotSupported));
                    }
                }
