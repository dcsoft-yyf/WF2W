    ...
    public class DataFormats {
        ...
        public static Format GetFormat(string format) {
            lock(internalSyncObject) {
                ...
                //
                int formatId = WinFormSafeNativeMethods.RegisterClipboardFormat(format);
                if (0 == formatId) {
                    throw new Win32Exception(Marshal.GetLastWin32Error(), DCSR.GetString(DCSR.RegisterCFFailed));
                }
        ...
        private static Format InternalGetFormat(string strName, ushort id) {
            lock(internalSyncObject) {
                ...
                //
                if (0 == WinFormSafeNativeMethods.GetClipboardFormatName(id, s, s.Capacity)) {
                    s.Length = 0;
                    if (strName == null) {
            ...
            if (0 == formatCount) {
                ...
                */
                ...
                formatList = new Format [] {
                    ...
                    new Format(UnicodeText,  WinFormNativeMethods.CF_UNICODETEXT),
                    new Format(Text,         WinFormNativeMethods.CF_TEXT),
                    new Format(Bitmap,       WinFormNativeMethods.CF_BITMAP),
                    new Format(MetafilePict, WinFormNativeMethods.CF_METAFILEPICT),
                    new Format(EnhancedMetafile,  WinFormNativeMethods.CF_ENHMETAFILE),
                    new Format(Dif,          WinFormNativeMethods.CF_DIF),
                    new Format(Tiff,         WinFormNativeMethods.CF_TIFF),
                    new Format(OemText,      WinFormNativeMethods.CF_OEMTEXT),
                    new Format(Dib,          WinFormNativeMethods.CF_DIB),
                    new Format(Palette,      WinFormNativeMethods.CF_PALETTE),
                    new Format(PenData,      WinFormNativeMethods.CF_PENDATA),
                    new Format(Riff,         WinFormNativeMethods.CF_RIFF),
                    new Format(WaveAudio,    WinFormNativeMethods.CF_WAVE),
                    new Format(SymbolicLink, WinFormNativeMethods.CF_SYLK),
                    new Format(FileDrop,     WinFormNativeMethods.CF_HDROP),
                    new Format(Locale,       WinFormNativeMethods.CF_LOCALE)
                };
