...
namespace System.Windows.Forms {
    ...
    public class ListViewItem : ICloneable, ISerializable {
        ...
        public bool Focused {
            get {
                if (listView != null && listView.IsHandleCreated) {
                    return(listView.GetItemState(Index, WinFormNativeMethods.LVIS_FOCUSED) != 0);
                }
                else return false;
            }
            ...
            set {
                if (listView != null && listView.IsHandleCreated) {
                    listView.SetItemState(Index, value ? WinFormNativeMethods.LVIS_FOCUSED : 0, WinFormNativeMethods.LVIS_FOCUSED);
                }
            }
        }
        ...
        SRCategory(DCSR.CatBehavior),
        SRDescription(DCSR.ListViewItemImageIndexDescr),
        ...
        [
        DefaultValue(0),
        SRDescription(DCSR.ListViewItemIndentCountDescr),
        ...
        public int IndentCount {
            ...
            set {
                if (value == indentCount)
                    return;
                if (value < 0) {
                    throw new ArgumentOutOfRangeException("IndentCount", DCSR.GetString(DCSR.ListViewIndentCountCantBeNegative));
                }
                indentCount = value;
                if (listView != null && listView.IsHandleCreated) {
        ...
        public bool Selected {
            get {
                if (listView != null && listView.IsHandleCreated) {
                    return(listView.GetItemState(Index, WinFormNativeMethods.LVIS_SELECTED) != 0);
                }
                else
            ...
            set {
                if (listView != null && listView.IsHandleCreated) {
                    listView.SetItemState(Index, value ? WinFormNativeMethods.LVIS_SELECTED: 0, WinFormNativeMethods.LVIS_SELECTED);
        ...
        TypeConverterAttribute(typeof(NoneExcludedImageIndexConverter)),
        DefaultValue(-1),
        SRDescription(DCSR.ListViewItemStateImageIndexDescr),
        SRCategory(DCSR.CatBehavior),
        RefreshProperties(RefreshProperties.Repaint),
        Editor("System.Windows.Forms.Design.ImageIndexEditor, " + AssemblyRef.SystemDesign, typeof(UITypeEditor)),
        ...
        public int StateImageIndex {
            get {
                if (listView != null && listView.IsHandleCreated) {
                    int state = listView.GetItemState(Index, WinFormNativeMethods.LVIS_STATEIMAGEMASK);
            ...
            set {
                ...
                if (listView != null && listView.IsHandleCreated) {
                    ...
                    listView.SetItemState(Index, state, WinFormNativeMethods.LVIS_STATEIMAGEMASK);
                }
        ...
        DesignerSerializationVisibility(DesignerSerializationVisibility.Hidden),        
        SRDescription(DCSR.ListViewItemSubItemsDescr),
        ...
        public void BeginEdit() {
            if (Index >= 0) {
                ...
                if (lv.LabelEdit == false)
                    throw new InvalidOperationException(DCSR.GetString(DCSR.ListViewBeginEditFailed));
                if (!lv.Focused)
                    lv.FocusInternal();
                WinFormUnsafeNativeMethods.SendMessage(new HandleRef(lv, lv.Handle), WinFormNativeMethods.LVM_EDITLABEL, Index, 0);
            }
        }                   
        ...
        /// </devdoc>
        internal void UpdateStateToListView(int index, ref WinFormNativeMethods.LVITEM lvItem, bool updateOwner) {
            ...
            if (StateSelected) {
                itemState |= WinFormNativeMethods.LVIS_SELECTED;
                stateMask |= WinFormNativeMethods.LVIS_SELECTED;
            }
            
            if (SavedStateImageIndex > -1) {
                itemState |= ((SavedStateImageIndex + 1) << 12);
                stateMask |= WinFormNativeMethods.LVIS_STATEIMAGEMASK;
            }                        
            ...
            if (listView.GroupsEnabled) {
                ...
                Debug.Assert(!updateOwner || listView.SendMessage(WinFormNativeMethods.LVM_ISGROUPVIEWENABLED, 0, 0) != IntPtr.Zero, "Groups not enabled");
                Debug.Assert(!updateOwner || listView.SendMessage(WinFormNativeMethods.LVM_HASGROUP, lvItem.iGroupId, 0) != IntPtr.Zero, "Doesn't contain group id: " + lvItem.iGroupId.ToString(CultureInfo.InvariantCulture));
            }            

            if (updateOwner) {
                WinFormUnsafeNativeMethods.SendMessage(new HandleRef(listView, listView.Handle), WinFormNativeMethods.LVM_SETITEM, 0, ref lvItem);
            }
        }
        ...
        internal void UpdateStateFromListView(int displayIndex, bool checkSelection) {
            if (listView != null && listView.IsHandleCreated && displayIndex != -1) {
                ...
                //
                WinFormNativeMethods.LVITEM lvItem = new WinFormNativeMethods.LVITEM();
                lvItem.mask = WinFormNativeMethods.LVIF_PARAM | WinFormNativeMethods.LVIF_STATE | WinFormNativeMethods.LVIF_GROUPID;
             
                if (checkSelection) {
                    lvItem.stateMask = WinFormNativeMethods.LVIS_SELECTED;
                }
                

                // we want to get all the information, including the state image mask
                lvItem.stateMask |= WinFormNativeMethods.LVIS_STATEIMAGEMASK;
                ...
                WinFormUnsafeNativeMethods.SendMessage(new HandleRef(listView, listView.Handle), WinFormNativeMethods.LVM_GETITEM, 0, ref lvItem);                
                
                // Update this class' information
                //
                if (checkSelection) {
                    StateSelected = (lvItem.state & WinFormNativeMethods.LVIS_SELECTED) != 0;
                }
                SavedStateImageIndex = ((lvItem.state & WinFormNativeMethods.LVIS_STATEIMAGEMASK) >> 12) - 1;
        ...
        public class ListViewSubItemCollection : IList {
            ...
            object IList.this[int index] {
                ...
                set {
                    if (value is ListViewSubItem) {
                        ...
                        throw new ArgumentException(DCSR.GetString(DCSR.ListViewBadListViewSubItem),"value");
                    }
                }
            }
            ...
            int IList.Add(object item) {
                if (item is ListViewSubItem) {
                    ...
                    throw new ArgumentException(DCSR.GetString(DCSR.ListViewSubItemCollectionInvalidArgument));
                }
            }
            ...
            private void EnsureSubItemSpace(int size, int index) {
                ...
                if (owner.SubItemCount == ListViewItem.MAX_SUBITEMS) {
                    throw new InvalidOperationException(DCSR.GetString(DCSR.ErrorCollectionFull));
                }
            ...
            void IList.Insert(int index, object item) {
                if (item is ListViewSubItem) {
                    ...
                    throw new ArgumentException(DCSR.GetString(DCSR.ListViewBadListViewSubItem),"item");
                }
            }
