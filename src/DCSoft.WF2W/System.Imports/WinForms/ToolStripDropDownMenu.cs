...
[System.Reflection.Obfuscation(Exclude = true, ApplyToMembers = false )]
    public class ToolStripDropDownMenu : ToolStripDropDown {
            ...
            [
            DefaultValue(true),
            SRDescription(DCSR.ToolStripDropDownMenuShowImageMarginDescr),
            ...
            [
            DefaultValue(false),
            SRDescription(DCSR.ToolStripDropDownMenuShowCheckMarginDescr),
            SRCategory(DCSR.CatAppearance)
            ]
            ...
            internal static ToolStripDropDownMenu FromHMenu(IntPtr hmenu, IWin32Window targetWindow) {
                ...
                int count = WinFormUnsafeNativeMethods.GetMenuItemCount(menuHandle);
                ...
                for (int i = 0; i < count; i++) {
                    ...
                    info.fMask = WinFormNativeMethods.MIIM_FTYPE;
                    info.fType = WinFormNativeMethods.MIIM_FTYPE;
                    WinFormUnsafeNativeMethods.GetMenuItemInfo(menuHandle, i, /*fByPosition=*/ true, info);
                    
                    if (info.fType == WinFormNativeMethods.MFT_SEPARATOR){
                        // its a separator.
                    	itemToAdd = new ToolStripSeparator();
                    }
                    else {
                        // its a menu item... lets fish out the command id
                     	info = new WinFormNativeMethods.MENUITEMINFO_T_RW();
                    	info.cbSize = Marshal.SizeOf(typeof(WinFormNativeMethods.MENUITEMINFO_T_RW));
                    	info.fMask = WinFormNativeMethods.MIIM_ID;
                    	info.fType = WinFormNativeMethods.MIIM_ID;
                    	WinFormUnsafeNativeMethods.GetMenuItemInfo(menuHandle, i, /*fByPosition=*/ true, info);
                    	...
                    	info = new WinFormNativeMethods.MENUITEMINFO_T_RW();
                    	info.cbSize = Marshal.SizeOf(typeof(WinFormNativeMethods.MENUITEMINFO_T_RW));
                    	info.fMask = WinFormNativeMethods.MIIM_SUBMENU;
                    	info.fType = WinFormNativeMethods.MIIM_SUBMENU;
                    	WinFormUnsafeNativeMethods.GetMenuItemInfo(menuHandle, i, /*fByPosition=*/ true, info);
