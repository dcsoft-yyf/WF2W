...
namespace System.Windows.Forms.Layout {
    ...
    internal class TableLayout : LayoutEngine {
        ...
        private void AssignRowsAndColumns(ContainerInfo containerInfo) {
            ...
            if (growStyle == TableLayoutPanelGrowStyle.FixedSize) {
                ...
                if (containerInfo.MinRowsAndColumns > numCols * numRows) {
                    throw new ArgumentException(DCSR.GetString(DCSR.TableLayoutPanelFullDesc));
                }    

                if ((minColumn > numCols) || (minRow > numRows)) {
                    throw new ArgumentException(DCSR.GetString(DCSR.TableLayoutPanelSpanDesc));
                }
        ...
        internal IArrangedElement GetControlFromPosition (IArrangedElement container, int column, int row) {
            if (row < 0) {
                throw new ArgumentException(DCSR.GetString(DCSR.InvalidArgument, "RowPosition", row.ToString(CultureInfo.CurrentCulture)));
            }
            if (column < 0) {
                throw new ArgumentException(DCSR.GetString(DCSR.InvalidArgument, "ColumnPosition", column.ToString(CultureInfo.CurrentCulture)));
            }
