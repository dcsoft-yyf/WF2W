    ...
    public sealed class VisualStyleRenderer {
        ...
        private static bool IsCombinationDefined(string className, int part) {
            ...
            if (!IsSupported) {
                if (!VisualStyleInformation.IsEnabledByUser) {
                    throw new InvalidOperationException(DCSR.GetString(DCSR.VisualStyleNotActive));
                }
                else {
                    throw new InvalidOperationException(DCSR.GetString(DCSR.VisualStylesDisabledInClientArea));
                }
            }
            ...
            if (hTheme != IntPtr.Zero) {
                ...
                if (part == 0) {
                    ...
                    returnVal = WinFormSafeNativeMethods.IsThemePartDefined(new HandleRef(null, hTheme), part, 0);
                }
            }
            ...
            if (!returnVal) {
                using (ThemeHandle tHandle = ThemeHandle.Create(className, false)) {
                    if (tHandle != null) {
                        returnVal = WinFormSafeNativeMethods.IsThemePartDefined(new HandleRef(null, tHandle.NativeHandle), part, 0);
                    }
        ...
        public VisualStyleRenderer(string className, int part, int state) {
            if (!IsCombinationDefined(className, part)) { //internally this call takes care of IsSupported. 
                throw new ArgumentException(DCSR.GetString(DCSR.VisualStylesInvalidCombination));
            }
        ...
        public void DrawBackground(IDeviceContext dc, Rectangle bounds) {
            ...
            using( WindowsGraphicsWrapper wgr = new WindowsGraphicsWrapper( dc, AllGraphicsProperties ) ){
                ...
                lastHResult = WinFormSafeNativeMethods.DrawThemeBackground( new HandleRef( this, Handle ), hdc, part, state, new WinFormNativeMethods.COMRECT( bounds ), null );
            }
        }
        ...
        public void DrawBackground(IDeviceContext dc, Rectangle bounds, Rectangle clipRectangle) {
            ...
            using( WindowsGraphicsWrapper wgr = new WindowsGraphicsWrapper( dc, AllGraphicsProperties ) ) {
                HandleRef hdc = new HandleRef( wgr, wgr.WindowsGraphics.DeviceContext.Hdc );
                lastHResult = WinFormSafeNativeMethods.DrawThemeBackground( new HandleRef( this, Handle ), hdc, part, state, new WinFormNativeMethods.COMRECT( bounds ), new WinFormNativeMethods.COMRECT( clipRectangle ) );
            }
        }
        ...
        public Rectangle DrawEdge(IDeviceContext dc, Rectangle bounds, Edges edges, EdgeStyle style, EdgeEffects effects) {
            ...
            WinFormNativeMethods.COMRECT rect = new WinFormNativeMethods.COMRECT();

            using( WindowsGraphicsWrapper wgr = new WindowsGraphicsWrapper( dc, AllGraphicsProperties ) ) {
                HandleRef hdc = new HandleRef( wgr, wgr.WindowsGraphics.DeviceContext.Hdc );
                lastHResult = WinFormSafeNativeMethods.DrawThemeEdge( new HandleRef( this, Handle ), hdc, part, state, new WinFormNativeMethods.COMRECT( bounds ), (int) style, (int) edges | (int) effects | EdgeAdjust, rect );
            }
        ...
        public void DrawImage(Graphics g, Rectangle bounds, ImageList imageList, int imageIndex) {
            ...
            if (imageIndex < 0 || imageIndex >= imageList.Images.Count) {
                throw new ArgumentOutOfRangeException("imageIndex", DCSR.GetString(DCSR.InvalidArgument, "imageIndex", imageIndex.ToString(CultureInfo.CurrentCulture)));
            }
        ...
        public void DrawParentBackground(IDeviceContext dc, Rectangle bounds, Control childControl) {
            ...
            if (childControl.Handle != IntPtr.Zero) {
                using( WindowsGraphicsWrapper wgr = new WindowsGraphicsWrapper( dc, AllGraphicsProperties ) ) {
                    HandleRef hdc = new HandleRef( wgr, wgr.WindowsGraphics.DeviceContext.Hdc );
                    lastHResult = WinFormSafeNativeMethods.DrawThemeParentBackground( new HandleRef( this, childControl.Handle ), hdc, new WinFormNativeMethods.COMRECT( bounds ) );
                }
            }
        }
        ...
        public void DrawText(IDeviceContext dc, Rectangle bounds, string textToDraw, bool drawDisabled, TextFormatFlags flags) {
            ...
            if (!String.IsNullOrEmpty(textToDraw)) {
                using( WindowsGraphicsWrapper wgr = new WindowsGraphicsWrapper( dc, AllGraphicsProperties ) ) {
                    HandleRef hdc = new HandleRef( wgr, wgr.WindowsGraphics.DeviceContext.Hdc );
                    lastHResult = WinFormSafeNativeMethods.DrawThemeText( new HandleRef( this, Handle ), hdc, part, state, textToDraw, textToDraw.Length, (int) flags, disableFlag, new WinFormNativeMethods.COMRECT( bounds ) );
                }
            }
        }
        ...
        public Rectangle GetBackgroundContentRectangle(IDeviceContext dc, Rectangle bounds) {
            ...
            using( WindowsGraphicsWrapper wgr = new WindowsGraphicsWrapper( dc, AllGraphicsProperties ) ) {
                HandleRef hdc = new HandleRef( wgr, wgr.WindowsGraphics.DeviceContext.Hdc );
                lastHResult = WinFormSafeNativeMethods.GetThemeBackgroundContentRect( new HandleRef( this, Handle ), hdc, part, state, new WinFormNativeMethods.COMRECT( bounds ), rect );
            }
        ...
        public Rectangle GetBackgroundExtent(IDeviceContext dc, Rectangle contentBounds) {
            ...
            using( WindowsGraphicsWrapper wgr = new WindowsGraphicsWrapper( dc, AllGraphicsProperties ) ) {
                HandleRef hdc = new HandleRef( wgr, wgr.WindowsGraphics.DeviceContext.Hdc );
                lastHResult = WinFormSafeNativeMethods.GetThemeBackgroundExtent( new HandleRef( this, Handle ), hdc, part, state, new WinFormNativeMethods.COMRECT( contentBounds ), rect );
            }
        ...
        public Region GetBackgroundRegion(IDeviceContext dc, Rectangle bounds) {
            ...
            using( WindowsGraphicsWrapper wgr = new WindowsGraphicsWrapper( dc, AllGraphicsProperties ) ) {
                HandleRef hdc = new HandleRef( wgr, wgr.WindowsGraphics.DeviceContext.Hdc );
                lastHResult = WinFormSafeNativeMethods.GetThemeBackgroundRegion( new HandleRef( this, Handle ), hdc, part, state, new WinFormNativeMethods.COMRECT( bounds ), ref hRegion );
            }
		...
		WinFormSafeNativeMethods.ExternalDeleteObject(new HandleRef(null, hRegion));
		return region;
        ...
        public bool GetBoolean(BooleanProperty prop) {
            ...
            bool val = false;
            lastHResult = WinFormSafeNativeMethods.GetThemeBool(new HandleRef(this, Handle), part, state, (int)prop, ref val);
            return val;
        }
        ...
        public Color GetColor(ColorProperty prop) {
            ...
            int color = 0;
            lastHResult = WinFormSafeNativeMethods.GetThemeColor(new HandleRef(this, Handle), part, state, (int)prop, ref color);
        ...
        public int GetEnumValue(EnumProperty prop) {
            ...
            int val = 0;
            lastHResult = WinFormSafeNativeMethods.GetThemeEnumValue(new HandleRef(this, Handle), part, state, (int)prop, ref val);
            return val;
        }
        ...
        public string GetFilename(FilenameProperty prop) {
            ...
            lastHResult = WinFormSafeNativeMethods.GetThemeFilename(new HandleRef(this, Handle), part, state, (int)prop, filename, filename.Capacity);
        ...
        public Font GetFont(IDeviceContext dc, FontProperty prop)
        {
            ...
            using( WindowsGraphicsWrapper wgr = new WindowsGraphicsWrapper( dc, AllGraphicsProperties ) ) {
                HandleRef hdc = new HandleRef( wgr, wgr.WindowsGraphics.DeviceContext.Hdc );
                lastHResult = WinFormSafeNativeMethods.GetThemeFont( new HandleRef( this, Handle ), hdc, part, state, (int) prop, logfont );
            }

            Font font = null;

            //check for a failed HR.
            if (WinFormNativeMethods.Succeeded(lastHResult)) {
        ...
        public int GetInteger(IntegerProperty prop) {
            ...
            int val = 0;
            lastHResult = WinFormSafeNativeMethods.GetThemeInt(new HandleRef(this, Handle), part, state, (int)prop, ref val);
            return val;
        }
        ...
        public Size GetPartSize(IDeviceContext dc, ThemeSizeType type) {
            ...
            WinFormNativeMethods.SIZE size = new WinFormNativeMethods.SIZE();

            using( WindowsGraphicsWrapper wgr = new WindowsGraphicsWrapper( dc, AllGraphicsProperties ) ) {
                HandleRef hdc = new HandleRef( wgr, wgr.WindowsGraphics.DeviceContext.Hdc );
                lastHResult = WinFormSafeNativeMethods.GetThemePartSize( new HandleRef( this, Handle ), hdc, part, state, null, type, size );
            }
        ...
        public Size GetPartSize(IDeviceContext dc, Rectangle bounds, ThemeSizeType type) {
            ...
            using( WindowsGraphicsWrapper wgr = new WindowsGraphicsWrapper( dc, AllGraphicsProperties ) ) {
                HandleRef hdc = new HandleRef( wgr, wgr.WindowsGraphics.DeviceContext.Hdc );
                lastHResult = WinFormSafeNativeMethods.GetThemePartSize( new HandleRef( this, Handle ), hdc, part, state, new WinFormNativeMethods.COMRECT( bounds ), type, size );
            }
        ...
        public Point GetPoint(PointProperty prop) {
            ...
            lastHResult = WinFormSafeNativeMethods.GetThemePosition(new HandleRef(this, Handle), part, state, (int)prop, point);
        ...
        public Padding GetMargins(IDeviceContext dc, MarginProperty prop) {
            ...
            WinFormNativeMethods.MARGINS margins = new WinFormNativeMethods.MARGINS();

            using( WindowsGraphicsWrapper wgr = new WindowsGraphicsWrapper( dc, AllGraphicsProperties ) ) {
                HandleRef hdc = new HandleRef( wgr, wgr.WindowsGraphics.DeviceContext.Hdc );
                lastHResult = WinFormSafeNativeMethods.GetThemeMargins( new HandleRef( this, Handle ), hdc, part, state, (int) prop, ref margins );
            }
        ...
        public string GetString(StringProperty prop) {
            ...
            lastHResult = WinFormSafeNativeMethods.GetThemeString(new HandleRef(this, Handle), part, state, (int)prop, aString, aString.Capacity);
        ...
        public Rectangle GetTextExtent(IDeviceContext dc, string textToDraw, TextFormatFlags flags) {
            ...
            using( WindowsGraphicsWrapper wgr = new WindowsGraphicsWrapper( dc, AllGraphicsProperties ) ) {
                HandleRef hdc = new HandleRef( wgr, wgr.WindowsGraphics.DeviceContext.Hdc );
                lastHResult = WinFormSafeNativeMethods.GetThemeTextExtent( new HandleRef( this, Handle ), hdc, part, state, textToDraw, textToDraw.Length, (int) flags, null, rect );
            }
        ...
        public Rectangle GetTextExtent(IDeviceContext dc, Rectangle bounds, string textToDraw, TextFormatFlags flags) {
            ...
            using( WindowsGraphicsWrapper wgr = new WindowsGraphicsWrapper( dc, AllGraphicsProperties ) ) {
                HandleRef hdc = new HandleRef( wgr, wgr.WindowsGraphics.DeviceContext.Hdc );
                lastHResult = WinFormSafeNativeMethods.GetThemeTextExtent( new HandleRef( this, Handle ), hdc, part, state, textToDraw, textToDraw.Length, (int) flags, new WinFormNativeMethods.COMRECT( bounds ), rect );
            }
        ...
        public TextMetrics GetTextMetrics(IDeviceContext dc) {
            ...
            using( WindowsGraphicsWrapper wgr = new WindowsGraphicsWrapper( dc, AllGraphicsProperties ) ) {
                HandleRef hdc = new HandleRef( wgr, wgr.WindowsGraphics.DeviceContext.Hdc );
                lastHResult = WinFormSafeNativeMethods.GetThemeTextMetrics( new HandleRef( this, Handle ), hdc, part, state, ref tm );
            }
        ...
        public HitTestCode HitTestBackground(IDeviceContext dc, Rectangle backgroundRectangle, Point pt, HitTestOptions options) {
            ...
            int htCode = 0;
            WinFormNativeMethods.POINTSTRUCT point = new WinFormNativeMethods.POINTSTRUCT(pt.X, pt.Y);

            using( WindowsGraphicsWrapper wgr = new WindowsGraphicsWrapper( dc, AllGraphicsProperties ) ) {
                HandleRef hdc = new HandleRef( wgr, wgr.WindowsGraphics.DeviceContext.Hdc );
                lastHResult = WinFormSafeNativeMethods.HitTestThemeBackground( new HandleRef( this, Handle ), hdc, part, state, (int) options, new WinFormNativeMethods.COMRECT( backgroundRectangle ), WinFormNativeMethods.NullHandleRef, point, ref htCode );
            }
        ...
        public HitTestCode HitTestBackground(IDeviceContext dc, Rectangle backgroundRectangle, IntPtr hRgn, Point pt, HitTestOptions options) {
            ...
            using( WindowsGraphicsWrapper wgr = new WindowsGraphicsWrapper( dc, AllGraphicsProperties ) ) {
                HandleRef hdc = new HandleRef( wgr, wgr.WindowsGraphics.DeviceContext.Hdc );
                lastHResult = WinFormSafeNativeMethods.HitTestThemeBackground( new HandleRef( this, Handle ), hdc, part, state, (int) options, new WinFormNativeMethods.COMRECT( backgroundRectangle ), new HandleRef( this, hRgn ), point, ref htCode );
            }
        ...
        public bool IsBackgroundPartiallyTransparent() {
            return (WinFormSafeNativeMethods.IsThemeBackgroundPartiallyTransparent(new HandleRef(this, Handle), part, state));
        }
        ...
        private class ThemeHandle : IDisposable {
            ...
            public static ThemeHandle Create(string className, bool throwExceptionOnFail) {
                ...
                try
                {
                    hTheme = WinFormSafeNativeMethods.OpenThemeData(new HandleRef(null, IntPtr.Zero), className);
                }
                catch (Exception e)
                {
                    ...
                    {
                        throw new InvalidOperationException(DCSR.GetString(DCSR.VisualStyleHandleCreationFailed), e);
                    }
                    else
                    {
                        return null;
                ...
                if (hTheme == IntPtr.Zero) {
                    if (throwExceptionOnFail) {
                        throw new InvalidOperationException(DCSR.GetString(DCSR.VisualStyleHandleCreationFailed));
                    }
                    else {
                        return null;
                    }
            ...
            public void Dispose() {
                if (_hTheme != IntPtr.Zero) {
                    WinFormSafeNativeMethods.CloseThemeData(new HandleRef(null, _hTheme));
                    _hTheme = IntPtr.Zero;
                }
