...
namespace System.Windows.Forms {
    ...
    internal sealed class ComponentManagerBroker : MarshalByRefObject {
        ...
        static ComponentManagerBroker() {
            int pid = WinFormSafeNativeMethods.GetCurrentProcessId();
        ...
        /// <devdoc>
        /// </devdoc>
        public WinFormUnsafeNativeMethods.IMsoComponentManager GetProxy(long pCM) {
            if (_proxy == null) {
                WinFormUnsafeNativeMethods.IMsoComponentManager original = (WinFormUnsafeNativeMethods.IMsoComponentManager)Marshal.GetObjectForIUnknown((IntPtr)pCM);
        ...
        ///    and returns it.
        /// </devdoc>
        internal static WinFormUnsafeNativeMethods.IMsoComponentManager GetComponentManager(IntPtr pOriginal) {
            ...
            lock(_syncObject) {
                ...
                if (_broker == null) {
                    ...
                    WinFormUnsafeNativeMethods.ICorRuntimeHost host = (WinFormUnsafeNativeMethods.ICorRuntimeHost) RuntimeEnvironment.GetRuntimeInterfaceAsObject(typeof(WinFormUnsafeNativeMethods.CorRuntimeHost).GUID, typeof(WinFormUnsafeNativeMethods.ICorRuntimeHost).GUID);
                    object domainObj;
                    int hr = host.GetDefaultDomain(out domainObj);
                    Debug.Assert(WinFormNativeMethods.Succeeded(hr), "ICorRuntimeHost failed to return the default domain.  The only way that should happen is if it hasn't been started yet, but if it hasn't been started how are we running managed code?");
                    AppDomain domain = domainObj as AppDomain;

                    if (domain == null) {
                        Debug.Assert(WinFormNativeMethods.Failed(hr) || domain != null, "ICorRuntimeHost::GetDefaultDomain succeeded but didn't retrn us an app domain.");
    ...
    ///    managed components.  
    /// </devdoc>
    internal class ComponentManagerProxy : MarshalByRefObject, WinFormUnsafeNativeMethods.IMsoComponentManager, WinFormUnsafeNativeMethods.IMsoComponent {

        private ComponentManagerBroker _broker;
        private WinFormUnsafeNativeMethods.IMsoComponentManager _original;
        private int _refCount;
        private int _creationThread;
        private IntPtr _componentId;
        private int _nextComponentId;
        private Dictionary<int, WinFormUnsafeNativeMethods.IMsoComponent> _components;
        private WinFormUnsafeNativeMethods.IMsoComponent _activeComponent;
        private int _activeComponentId;
        private WinFormUnsafeNativeMethods.IMsoComponent _trackingComponent;
        private int _trackingComponentId;

        internal ComponentManagerProxy(ComponentManagerBroker broker, WinFormUnsafeNativeMethods.IMsoComponentManager original) {
            _broker = broker;
            _original = original;
            _creationThread = WinFormSafeNativeMethods.GetCurrentThreadId();
            _refCount = 0;
        }
        ...
        private WinFormUnsafeNativeMethods.IMsoComponent Component {
            get {
        ...
        bool WinFormUnsafeNativeMethods.IMsoComponent.FDebugMessage(IntPtr hInst, int msg, IntPtr wparam, IntPtr lparam) {
            WinFormUnsafeNativeMethods.IMsoComponent c = Component;
        ...
        void WinFormUnsafeNativeMethods.IMsoComponent.OnEnterState(int uStateID, bool fEnter) {
            if (_components != null) {
                foreach (WinFormUnsafeNativeMethods.IMsoComponent c in _components.Values) {
        ...
        void WinFormUnsafeNativeMethods.IMsoComponent.OnActivationChange(WinFormUnsafeNativeMethods.IMsoComponent component, bool fSameComponent, int pcrinfo, bool fHostIsActivating, int pchostinfo, int dwReserved) {
            if (_components != null) {
                foreach (WinFormUnsafeNativeMethods.IMsoComponent c in _components.Values) {
        ...
        void WinFormUnsafeNativeMethods.IMsoComponent.Terminate() {
            if (_components != null && _components.Values.Count>0) {
                WinFormUnsafeNativeMethods.IMsoComponent[] components = new WinFormUnsafeNativeMethods.IMsoComponent[_components.Values.Count];
                _components.Values.CopyTo(components, 0);
                foreach (WinFormUnsafeNativeMethods.IMsoComponent c in components) {
                    c.Terminate();
                }
            }
        ...
        int WinFormUnsafeNativeMethods.IMsoComponentManager.QueryService(ref Guid guidService, ref Guid iid, out object ppvObj) {
            return _original.QueryService(ref guidService, ref iid, out ppvObj);
        }

        bool WinFormUnsafeNativeMethods.IMsoComponentManager.FDebugMessage(IntPtr hInst, int msg, IntPtr wparam, IntPtr lparam) {
            return _original.FDebugMessage(hInst, msg, wparam, lparam);
        }

        bool WinFormUnsafeNativeMethods.IMsoComponentManager.FRegisterComponent(WinFormUnsafeNativeMethods.IMsoComponent component, WinFormNativeMethods.MSOCRINFOSTRUCT pcrinfo, out IntPtr dwComponentID) {
            if (component == null) {
            ...
            if (_components == null) {
                _components = new Dictionary<int, WinFormUnsafeNativeMethods.IMsoComponent>();
            }
            ...
            while (_components.ContainsKey(_nextComponentId)) {
                ...
                if (_nextComponentId == int.MaxValue) {
                    if (outofMemory) {
                        throw new InvalidOperationException(DCSR.GetString(DCSR.ComponentManagerProxyOutOfMemory));
                    }
                    outofMemory = true;
                    _nextComponentId = 1;
                }
        ...
        bool WinFormUnsafeNativeMethods.IMsoComponentManager.FRevokeComponent(IntPtr dwComponentID) {
            ...
            if (_refCount == 1 && WinFormSafeNativeMethods.GetCurrentThreadId() == _creationThread) {
                if (!RevokeComponent()) {
                    return false;
                }
            }
        ...
        bool WinFormUnsafeNativeMethods.IMsoComponentManager.FUpdateComponentRegistration(IntPtr dwComponentID, WinFormNativeMethods.MSOCRINFOSTRUCT info) {
            if (_original == null) return false;
        ...
        void WinFormUnsafeNativeMethods.IMsoComponentManager.OnComponentEnterState(IntPtr dwComponentID, int uStateID, int uContext, int cpicmExclude, int rgpicmExclude,  int dwReserved) {
            if (_original == null) return;
            if (uContext == WinFormNativeMethods.MSOCM.msoccontextAll || uContext == WinFormNativeMethods.MSOCM.msoccontextMine) {
                if (_components != null) {
                    foreach (WinFormUnsafeNativeMethods.IMsoComponent comp in _components.Values) {
        ...
        bool WinFormUnsafeNativeMethods.IMsoComponentManager.FOnComponentExitState(IntPtr dwComponentID, int uStateID, int uContext, int cpicmExclude, int rgpicmExclude ) {
            if (_original == null) return false;
            if (uContext == WinFormNativeMethods.MSOCM.msoccontextAll || uContext == WinFormNativeMethods.MSOCM.msoccontextMine) {
                if (_components != null) {
                    foreach (WinFormUnsafeNativeMethods.IMsoComponent comp in _components.Values) {
        ...
        bool WinFormUnsafeNativeMethods.IMsoComponentManager.FPushMessageLoop(IntPtr dwComponentID, int reason, int pvLoopData) {
            if (_original == null) return false;
            return _original.FPushMessageLoop(_componentId, reason, pvLoopData);
        }

        bool WinFormUnsafeNativeMethods.IMsoComponentManager.FCreateSubComponentManager(object punkOuter, object punkServProv, ref Guid riid, out IntPtr ppvObj) {
        ...
        bool WinFormUnsafeNativeMethods.IMsoComponentManager.FGetActiveComponent(int dwgac, WinFormUnsafeNativeMethods.IMsoComponent[] ppic, WinFormNativeMethods.MSOCRINFOSTRUCT info, int dwReserved) {
            if (_original == null) return false;
