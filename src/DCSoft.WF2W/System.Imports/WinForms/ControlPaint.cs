...
namespace System.Windows.Forms {
    ...
    public sealed class ControlPaint {
        ...
        private static IntPtr CreateBitmapInfo(Bitmap bitmap, IntPtr hdcS) {
            WinFormNativeMethods.BITMAPINFOHEADER header = new WinFormNativeMethods.BITMAPINFOHEADER();
            header.biSize = Marshal.SizeOf(header);
            header.biWidth = bitmap.Width;
            header.biHeight = bitmap.Height;
            header.biPlanes = 1;
            header.biBitCount = 16;
            header.biCompression = WinFormNativeMethods.BI_RGB;
            // leave everything else 0

            // Set up color table --
            int entryCount = 0;
            IntPtr palette = WinFormSafeNativeMethods.CreateHalftonePalette(new HandleRef(null, hdcS));
            WinFormUnsafeNativeMethods.GetObject(new HandleRef(null, palette), 2, ref entryCount);
            int[] entries = new int[entryCount];
            WinFormSafeNativeMethods.GetPaletteEntries(new HandleRef(null, palette), 0, entryCount, entries);
            ...
            WinFormSafeNativeMethods.DeleteObject(new HandleRef(null, palette));
        ...
        public static IntPtr CreateHBitmap16Bit(Bitmap bitmap, Color background) {
            ...
            using( DeviceContext screenDc = DeviceContext.ScreenDC ){
                ...
                using( DeviceContext compatDc = DeviceContext.FromCompatibleDC( hdcS ) ){
                    ...
                    hBitmap = WinFormSafeNativeMethods.CreateDIBSection(new HandleRef(null, hdcS), new HandleRef(null, bitmapInfo), WinFormNativeMethods.DIB_RGB_COLORS,
                    ...
                    try {
                        IntPtr previousBitmap = WinFormSafeNativeMethods.SelectObject(new HandleRef(null, dc), new HandleRef(null, hBitmap));
                        if (previousBitmap == IntPtr.Zero) {
                            throw new Win32Exception();
                        }

                        WinFormSafeNativeMethods.DeleteObject( new HandleRef( null, previousBitmap ) );
       ...
       // [ResourceConsumption(ResourceScope.Machine)]
        public static IntPtr CreateHBitmapTransparencyMask(Bitmap bitmap) {
            ...
            IntPtr mask = WinFormSafeNativeMethods.CreateBitmap(size.Width, size.Height, 1, /* 1bpp */ 1, bits);
        ...
        public static IntPtr CreateHBitmapColorMask(Bitmap bitmap, IntPtr monochromeMask) {
            ...
            //IntPtr hdcS = WinFormUnsafeNativeMethods.GetDC(WinFormNativeMethods.NullHandleRef);
            //IntPtr source = WinFormUnsafeNativeMethods.CreateCompatibleDC(new HandleRef(null, hdcS));
            //IntPtr target = WinFormUnsafeNativeMethods.CreateCompatibleDC(new HandleRef(null, hdcS));
            //WinFormUnsafeNativeMethods.ReleaseDC(WinFormNativeMethods.NullHandleRef, new HandleRef(null, hdcS));
            //IntPtr previousSourceBitmap = WinFormSafeNativeMethods.SelectObject(new HandleRef(null, source), new HandleRef(null, monochromeMask));
            //IntPtr previousTargetBitmap = WinFormSafeNativeMethods.SelectObject(new HandleRef(null, target), new HandleRef(null, colorMask));
            ...
            ////
            //WinFormSafeNativeMethods.SetBkColor(new HandleRef(null, target), 0x00ffffff); // white
            //WinFormSafeNativeMethods.SetTextColor(new HandleRef(null, target), 0); // black
            //WinFormSafeNativeMethods.BitBlt(new HandleRef(null, target), 0, 0, size.Width, size.Height, new HandleRef(null, source),
            //                         0, 0, 0x220326); // RasterOp.SOURCE.Invert().AndWith(RasterOp.TARGET).GetRop());

            //WinFormSafeNativeMethods.SelectObject(new HandleRef(null, source), new HandleRef(null, previousSourceBitmap));
            //WinFormSafeNativeMethods.SelectObject(new HandleRef(null, target), new HandleRef(null, previousTargetBitmap));
            //WinFormUnsafeNativeMethods.DeleteCompatibleDC(new HandleRef(null, source));
            //WinFormUnsafeNativeMethods.DeleteCompatibleDC(new HandleRef(null, target));

            //return  System.Internal.HandleCollector.Add(colorMask, WinFormNativeMethods.CommonHandles.GDI);;
        }
        ...
        internal static IntPtr CreateHalftoneHBRUSH() {
            ...
            IntPtr hBitmap = WinFormSafeNativeMethods.CreateBitmap(8, 8, 1, 1, grayPattern);

            WinFormNativeMethods.LOGBRUSH lb = new WinFormNativeMethods.LOGBRUSH();
            lb.lbColor = ColorTranslator.ToWin32(Color.Black);
            lb.lbStyle = WinFormNativeMethods.BS_PATTERN;
            lb.lbHatch = hBitmap;
            IntPtr brush = WinFormSafeNativeMethods.CreateBrushIndirect(lb);
        ...
        internal static void CopyPixels(IntPtr sourceHwnd, IDeviceContext targetDC, Point sourceLocation, Point destinationLocation, Size blockRegionSize, CopyPixelOperation copyPixelOperation) {
            ...
            try {
                bool result = WinFormSafeNativeMethods.BitBlt(targetHDC, destinationLocation.X, destinationLocation.Y, destWidth, destHeight, 
                                                      screenHDC,
        ...
        public static void DrawBorder3D(Graphics graphics, int x, int y, int width, int height, Border3DStyle style, Border3DSide sides) {
            ...
            // Simulate DrawEdge with GDI+ pens.
            Rectangle rect = new Rectangle(x, y, width, height);

            // Pens analogous to system DrawEdge behavior
            Pen penLight = SystemPens.ControlLightLight;
            Pen penDark = SystemPens.ControlDarkDark;
            Pen penMidLight = SystemPens.ControlLight;
            Pen penMidDark = SystemPens.ControlDark;
            {
                // Helper local to draw a single side with two-tone effect
                void DrawSide(Border3DSide side, Pen first, Pen second) {
                    switch (side) {
                        case Border3DSide.Top:
                            graphics.DrawLine(first, rect.Left, rect.Top, rect.Right - 1, rect.Top);
                            graphics.DrawLine(second, rect.Left, rect.Top + 1, rect.Right - 1, rect.Top + 1);
                            break;
                        case Border3DSide.Left:
                            graphics.DrawLine(first, rect.Left, rect.Top, rect.Left, rect.Bottom - 1);
                            graphics.DrawLine(second, rect.Left + 1, rect.Top, rect.Left + 1, rect.Bottom - 1);
                            break;
                        case Border3DSide.Bottom:
                            graphics.DrawLine(first, rect.Left, rect.Bottom - 1, rect.Right - 1, rect.Bottom - 1);
                            graphics.DrawLine(second, rect.Left, rect.Bottom - 2, rect.Right - 1, rect.Bottom - 2);
                            break;
                        case Border3DSide.Right:
                            graphics.DrawLine(first, rect.Right - 1, rect.Top, rect.Right - 1, rect.Bottom - 1);
                            graphics.DrawLine(second, rect.Right - 2, rect.Top, rect.Right - 2, rect.Bottom - 1);
                            break;
                    }
                }

                // Determine pen pairing based on style
                // Raised: top/left light, bottom/right dark
                // Sunken: top/left dark, bottom/right light
                // Etched: outer dark, inner light (or mid tones)
                // Flat: single midDark line
                if ((style & Border3DStyle.Flat) == Border3DStyle.Flat) {
                    Pen flatPen = SystemPens.ControlDark;
                    if ((sides & Border3DSide.Top) != 0)
                        graphics.DrawLine(flatPen, rect.Left, rect.Top, rect.Right - 1, rect.Top);
                    if ((sides & Border3DSide.Left) != 0)
                        graphics.DrawLine(flatPen, rect.Left, rect.Top, rect.Left, rect.Bottom - 1);
                    if ((sides & Border3DSide.Bottom) != 0)
                        graphics.DrawLine(flatPen, rect.Left, rect.Bottom - 1, rect.Right - 1, rect.Bottom - 1);
                    if ((sides & Border3DSide.Right) != 0)
                        graphics.DrawLine(flatPen, rect.Right - 1, rect.Top, rect.Right - 1, rect.Bottom - 1);
                    return;
                }

                if ((style & Border3DStyle.Etched) == Border3DStyle.Etched) {
                    // Outer dark, inner light
                    if ((sides & Border3DSide.Top) != 0) {
                        graphics.DrawLine(penDark, rect.Left, rect.Top, rect.Right - 1, rect.Top);
                        graphics.DrawLine(penMidLight, rect.Left, rect.Top + 1, rect.Right - 1, rect.Top + 1);
                    }
                    if ((sides & Border3DSide.Left) != 0) {
                        graphics.DrawLine(penDark, rect.Left, rect.Top, rect.Left, rect.Bottom - 1);
                        graphics.DrawLine(penMidLight, rect.Left + 1, rect.Top, rect.Left + 1, rect.Bottom - 1);
                    }
                    if ((sides & Border3DSide.Bottom) != 0) {
                        graphics.DrawLine(penMidLight, rect.Left, rect.Bottom - 2, rect.Right - 1, rect.Bottom - 2);
                        graphics.DrawLine(penDark, rect.Left, rect.Bottom - 1, rect.Right - 1, rect.Bottom - 1);
                    }
                    if ((sides & Border3DSide.Right) != 0) {
                        graphics.DrawLine(penMidLight, rect.Right - 2, rect.Top, rect.Right - 2, rect.Bottom - 1);
                        graphics.DrawLine(penDark, rect.Right - 1, rect.Top, rect.Right - 1, rect.Bottom - 1);
                    }
                    return;
                }

                bool sunken = (style & Border3DStyle.Sunken) == Border3DStyle.Sunken;
                bool raised = (style & Border3DStyle.Raised) == Border3DStyle.Raised;
                if (sunken || raised) {
                    Pen firstTopLeft = sunken ? penDark : penLight;
                    Pen secondTopLeft = sunken ? penMidDark : penMidLight;
                    Pen firstBottomRight = sunken ? penLight : penDark;
                    Pen secondBottomRight = sunken ? penMidLight : penMidDark;

                    if ((sides & Border3DSide.Top) != 0)
                        DrawSide(Border3DSide.Top, firstTopLeft, secondTopLeft);
                    if ((sides & Border3DSide.Left) != 0)
                        DrawSide(Border3DSide.Left, firstTopLeft, secondTopLeft);
                    if ((sides & Border3DSide.Bottom) != 0)
                        DrawSide(Border3DSide.Bottom, firstBottomRight, secondBottomRight);
                    if ((sides & Border3DSide.Right) != 0)
                        DrawSide(Border3DSide.Right, firstBottomRight, secondBottomRight);
                    return;
                }

                // Fallback: treat unknown as Etched
                if ((sides & Border3DSide.Top) != 0)
                    DrawSide(Border3DSide.Top, penDark, penMidLight);
                if ((sides & Border3DSide.Left) != 0)
                    DrawSide(Border3DSide.Left, penDark, penMidLight);
                if ((sides & Border3DSide.Bottom) != 0)
                    DrawSide(Border3DSide.Bottom, penMidLight, penDark);
                if ((sides & Border3DSide.Right) != 0)
                    DrawSide(Border3DSide.Right, penMidLight, penDark);
            }
        }
        ...
        public static void DrawButton(Graphics graphics, int x, int y, int width, int height, ButtonState state) {
            DrawFrameControl(graphics, x, y, width, height, WinFormNativeMethods.DFC_BUTTON,
                             WinFormNativeMethods.DFCS_BUTTONPUSH | (int) state, Color.Empty, Color.Empty);
        }
        ...
        public static void DrawCaptionButton(Graphics graphics, int x, int y, int width, int height, CaptionButton button, ButtonState state) {
            DrawFrameControl(graphics, x, y, width, height, WinFormNativeMethods.DFC_CAPTION,
        ...
        public static void DrawCheckBox(Graphics graphics, int x, int y, int width, int height, ButtonState state) {
            ...
            if ((state & ButtonState.Flat) == ButtonState.Flat) {
                ...
                DrawFrameControl(graphics, x, y, width, height, WinFormNativeMethods.DFC_BUTTON,
                                 WinFormNativeMethods.DFCS_BUTTONCHECK | (int) state, Color.Empty, Color.Empty);
            }
        }
        ...
        public static void DrawComboButton(Graphics graphics, int x, int y, int width, int height, ButtonState state) {
            DrawFrameControl(graphics, x, y, width, height, WinFormNativeMethods.DFC_SCROLL,
                             WinFormNativeMethods.DFCS_SCROLLCOMBOBOX | (int) state, Color.Empty, Color.Empty);
        }
        ...
        private static void DrawFlatCheckBox(Graphics graphics, Rectangle rectangle, Color foreground, Brush background, ButtonState state) {
            ...
            if ((state & ButtonState.Checked) == ButtonState.Checked) {
                if (checkImage == null || checkImage.Width != rectangle.Width || checkImage.Height != rectangle.Height) {
                    ...
                    WinFormNativeMethods.RECT rcCheck = WinFormNativeMethods.RECT.FromXYWH(0, 0, rectangle.Width, rectangle.Height);
                    Bitmap bitmap = new Bitmap(rectangle.Width, rectangle.Height);
                    using (Graphics g2 = Graphics.FromImage(bitmap)) {
                        g2.Clear(Color.Transparent);
                        IntPtr dc = g2.GetHdc();
                        try {
                            WinFormSafeNativeMethods.DrawFrameControl(new HandleRef(null, dc), ref rcCheck,
                                                               WinFormNativeMethods.DFC_MENU, WinFormNativeMethods.DFCS_MENUCHECK);
                        }
                        finally {
        ...
        private static void DrawFrameControl(Graphics graphics, int x, int y, int width, int height, 
            ...
            WinFormNativeMethods.RECT rcFrame = WinFormNativeMethods.RECT.FromXYWH(0, 0, width, height);
            using (Bitmap bitmap = new Bitmap(width, height)) {
                using( Graphics g2 = Graphics.FromImage(bitmap) ) {
                    g2.Clear(Color.Transparent);

                    using( WindowsGraphics wg = WindowsGraphics.FromGraphics(g2) ){ // Get Win32 dc with Graphics properties applied to it.
                        WinFormSafeNativeMethods.DrawFrameControl(new HandleRef(wg, wg.DeviceContext.Hdc), ref rcFrame, kind, (int) state);
                    }
        ...
        */
        ...
        public static void DrawMenuGlyph(Graphics graphics, int x, int y, int width, int height, MenuGlyph glyph) {
            DrawFrameControl(graphics, x, y, width, height, WinFormNativeMethods.DFC_MENU, 
        ...
        {
            DrawFrameControl(graphics, x, y, width, height, WinFormNativeMethods.DFC_MENU, (int)glyph, foreColor, backColor);
        }
        ...
        public static void DrawMixedCheckBox(Graphics graphics, int x, int y, int width, int height, ButtonState state) {
            DrawFrameControl(graphics, x, y, width, height, WinFormNativeMethods.DFC_BUTTON,
                             WinFormNativeMethods.DFCS_BUTTON3STATE | (int) state, Color.Empty, Color.Empty);
        }
        ...
        public static void DrawRadioButton(Graphics graphics, int x, int y, int width, int height, ButtonState state) {
            DrawFrameControl(graphics, x, y, width, height, WinFormNativeMethods.DFC_BUTTON,
                             WinFormNativeMethods.DFCS_BUTTONRADIO | ((int)state), Color.Empty, Color.Empty);
        }
        ...
        public static void DrawReversibleFrame(Rectangle rectangle, Color backColor, FrameStyle style) {
            ...
            IntPtr dc = WinFormUnsafeNativeMethods.GetDCEx(new HandleRef(null, WinFormUnsafeNativeMethods.GetDesktopWindow()), WinFormNativeMethods.NullHandleRef, WinFormNativeMethods.DCX_WINDOW | WinFormNativeMethods.DCX_LOCKWINDOWUPDATE | WinFormNativeMethods.DCX_CACHE);
            IntPtr pen;

            switch (style) {
                case FrameStyle.Dashed:
                    pen = WinFormSafeNativeMethods.CreatePen(WinFormNativeMethods.PS_DOT, 1, ColorTranslator.ToWin32(backColor));
                    break;

                case FrameStyle.Thick:
                default:
                    pen = WinFormSafeNativeMethods.CreatePen(WinFormNativeMethods.PS_SOLID, 2, ColorTranslator.ToWin32(backColor));
                    break;
            }

            int prevRop2 = WinFormSafeNativeMethods.SetROP2(new HandleRef(null, dc), rop2);
            IntPtr oldBrush = WinFormSafeNativeMethods.SelectObject(new HandleRef(null, dc), new HandleRef(null, WinFormUnsafeNativeMethods.GetStockObject(WinFormNativeMethods.HOLLOW_BRUSH)));
            IntPtr oldPen = WinFormSafeNativeMethods.SelectObject(new HandleRef(null, dc), new HandleRef(null, pen));
            WinFormSafeNativeMethods.SetBkColor(new HandleRef(null, dc), ColorTranslator.ToWin32(graphicsColor));
            WinFormSafeNativeMethods.Rectangle(new HandleRef(null, dc), rectangle.X, rectangle.Y, rectangle.Right, rectangle.Bottom);

            WinFormSafeNativeMethods.SetROP2(new HandleRef(null, dc), prevRop2);
            WinFormSafeNativeMethods.SelectObject(new HandleRef(null, dc), new HandleRef(null, oldBrush));
            WinFormSafeNativeMethods.SelectObject(new HandleRef(null, dc), new HandleRef(null, oldPen));

            if (pen != IntPtr.Zero)
            { 
                WinFormSafeNativeMethods.DeleteObject(new HandleRef(null, pen));
            }
        ...
        public static void DrawReversibleLine(Point start, Point end, Color backColor) {
            ...
            IntPtr dc = WinFormUnsafeNativeMethods.GetDCEx(new HandleRef(null, WinFormUnsafeNativeMethods.GetDesktopWindow()), WinFormNativeMethods.NullHandleRef, WinFormNativeMethods.DCX_WINDOW | WinFormNativeMethods.DCX_LOCKWINDOWUPDATE | WinFormNativeMethods.DCX_CACHE);
            IntPtr pen = WinFormSafeNativeMethods.CreatePen(WinFormNativeMethods.PS_SOLID, 1, ColorTranslator.ToWin32(backColor));
            ...
            WinFormSafeNativeMethods.MoveToEx(new HandleRef(null, dc), start.X, start.Y, null);
            WinFormSafeNativeMethods.LineTo(new HandleRef(null, dc), end.X, end.Y);
        ...
        public static void FillReversibleRectangle(Rectangle rectangle, Color backColor) {
            ...
            IntPtr dc = WinFormUnsafeNativeMethods.GetDCEx(new HandleRef(null, WinFormUnsafeNativeMethods.GetDesktopWindow()), WinFormNativeMethods.NullHandleRef, WinFormNativeMethods.DCX_WINDOW | WinFormNativeMethods.DCX_LOCKWINDOWUPDATE | WinFormNativeMethods.DCX_CACHE);
            IntPtr brush = WinFormSafeNativeMethods.CreateSolidBrush(ColorTranslator.ToWin32(backColor));

            int prevRop2 = WinFormSafeNativeMethods.SetROP2(new HandleRef(null, dc), rop2);
            IntPtr oldBrush = WinFormSafeNativeMethods.SelectObject(new HandleRef(null, dc), new HandleRef(null, brush));

            // PatBlt must be the only Win32 function that wants height in width rather than x2,y2.
            WinFormSafeNativeMethods.PatBlt(new HandleRef(null, dc), rectangle.X, rectangle.Y, rectangle.Width, rectangle.Height, rop3);

            WinFormSafeNativeMethods.SetROP2(new HandleRef(null, dc), prevRop2);
            WinFormSafeNativeMethods.SelectObject(new HandleRef(null, dc), new HandleRef(null, oldBrush));
            WinFormSafeNativeMethods.DeleteObject(new HandleRef(null, brush));
            WinFormUnsafeNativeMethods.ReleaseDC(WinFormNativeMethods.NullHandleRef, new HandleRef(null, dc));
        }
        ...
        internal static bool FontToIFont(Font source, WinFormUnsafeNativeMethods.IFont target) {
            bool changed = false; 
