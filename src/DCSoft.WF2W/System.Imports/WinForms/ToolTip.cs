...
namespace System.Windows.Forms {
    ...
    public class ToolTip : Component, IExtenderProvider {
        ...
        public ToolTip() {
            ...
            auto = true;
            delayTimes[WinFormNativeMethods.TTDT_AUTOMATIC] = DEFAULT_DELAY;
            AdjustBaseFromAuto();
        }
        ...
        /// </devdoc>
        [
        SRDescription(DCSR.ToolTipActiveDescr),
        DefaultValue(true)
        ]
        public bool Active {
            get {
            ...
            set {
                if (active != value) {
                    ...
                    if (!DesignMode && GetHandleCreated()) {
                        WinFormUnsafeNativeMethods.SendMessage(new HandleRef(this, Handle), WinFormNativeMethods.TTM_ACTIVATE, (value==true)? 1: 0, 0);
                    }
                }
            }
        }
        ...
        SRDescription(DCSR.ToolTipAutomaticDelayDescr),
        DefaultValue(DEFAULT_DELAY)
        ]
        public int AutomaticDelay {
            get {
                return delayTimes[WinFormNativeMethods.TTDT_AUTOMATIC];
            }

            set {
                if (value < 0) {
                    throw new ArgumentOutOfRangeException("AutomaticDelay", DCSR.GetString(DCSR.InvalidLowBoundArgumentEx, "AutomaticDelay", (value).ToString(CultureInfo.CurrentCulture), (0).ToString(CultureInfo.CurrentCulture)));
                }
                SetDelayTime(WinFormNativeMethods.TTDT_AUTOMATIC, value);
            }
        }
        ...
        [
        RefreshProperties(RefreshProperties.All),
        SRDescription(DCSR.ToolTipAutoPopDelayDescr)
        ]
        public int AutoPopDelay {
            get {
                return delayTimes[WinFormNativeMethods.TTDT_AUTOPOP];
            }

            set {
                if (value < 0) {
                    throw new ArgumentOutOfRangeException("AutoPopDelay", DCSR.GetString(DCSR.InvalidLowBoundArgumentEx, "AutoPopDelay", (value).ToString(CultureInfo.CurrentCulture), (0).ToString(CultureInfo.CurrentCulture)));
                }
                SetDelayTime(WinFormNativeMethods.TTDT_AUTOPOP, value);
            }
        }
        ...
        /// </devdoc>
        [
        SRDescription(DCSR.ToolTipBackColorDescr),
        ...
        public Color BackColor {
            ...
            set {
                backColor = value;
                if (GetHandleCreated()) {
                    WinFormUnsafeNativeMethods.SendMessage(new HandleRef(this, Handle), WinFormNativeMethods.TTM_SETTIPBKCOLOR, ColorTranslator.ToWin32(backColor), 0);
                }
            }
        }
        ...
        protected virtual CreateParams CreateParams {
            ...
            get {
                ...
                if (showAlways) {
                    cp.Style = WinFormNativeMethods.TTS_ALWAYSTIP;
                }
                if (isBalloon) {
                    cp.Style |= WinFormNativeMethods.TTS_BALLOON;
                }
                if (!stripAmpersands) {
                    cp.Style |= WinFormNativeMethods.TTS_NOPREFIX;
                }
                if (!useAnimation) {
                    cp.Style |= WinFormNativeMethods.TTS_NOANIMATE;
                }
                if (!useFading) {
                    cp.Style |= WinFormNativeMethods.TTS_NOFADE;
                }
                cp.ExStyle = 0;
                cp.Caption = null;
        ...
        /// </devdoc>
        [
        SRDescription(DCSR.ToolTipForeColorDescr),
        ...
        public Color ForeColor {
            ...
            set {
                if (value.IsEmpty) {
                    throw new ArgumentException(DCSR.GetString(DCSR.ToolTipEmptyColor, "ForeColor"));
                }

                foreColor = value;
                if (GetHandleCreated()) {
                    WinFormUnsafeNativeMethods.SendMessage(new HandleRef(this, Handle), WinFormNativeMethods.TTM_SETTIPTEXTCOLOR, ColorTranslator.ToWin32(foreColor), 0);
                }
        ...
        /// </devdoc>
        [
        SRDescription(DCSR.ToolTipIsBalloonDescr),
        DefaultValue(false)
        ]
        ...
        private bool IsWindowActive(IWin32Window window)
        {
            ...
            if ((windowControl.ShowParams & 0xF) != WinFormNativeMethods.SW_SHOWNOACTIVATE)
            {
                IntPtr hWnd = WinFormUnsafeNativeMethods.GetActiveWindow();
                IntPtr rootHwnd =WinFormUnsafeNativeMethods.GetAncestor(new HandleRef(window, window.Handle), WinFormNativeMethods.GA_ROOT);
                if (hWnd != rootHwnd)
                {
        ...
        [
        RefreshProperties(RefreshProperties.All),
        SRDescription(DCSR.ToolTipInitialDelayDescr)
        ]
        public int InitialDelay {
            get {
                return delayTimes[WinFormNativeMethods.TTDT_INITIAL];
            }

            set {
                if (value < 0) {
                    throw new ArgumentOutOfRangeException("InitialDelay", DCSR.GetString(DCSR.InvalidLowBoundArgumentEx, "InitialDelay", (value).ToString(CultureInfo.CurrentCulture), (0).ToString(CultureInfo.CurrentCulture)));
                }
                SetDelayTime(WinFormNativeMethods.TTDT_INITIAL, value);
            }
        }
        ...
        DefaultValue(false),
        SRDescription(DCSR.ToolTipOwnerDrawDescr) 
        ]
        ...
        [
        RefreshProperties(RefreshProperties.All),
        SRDescription(DCSR.ToolTipReshowDelayDescr)
        ]
        public int ReshowDelay {
            get {
                return delayTimes[WinFormNativeMethods.TTDT_RESHOW];
            }
            set {
                if (value < 0) {
                    throw new ArgumentOutOfRangeException("ReshowDelay", DCSR.GetString(DCSR.InvalidLowBoundArgumentEx, "ReshowDelay", (value).ToString(CultureInfo.CurrentCulture), (0).ToString(CultureInfo.CurrentCulture)));
                }
                SetDelayTime(WinFormNativeMethods.TTDT_RESHOW, value);
            }
        }
        ...
        /// </devdoc>
        [
        SRDescription(DCSR.ToolTipStripAmpersandsDescr),
        Browsable(true),
        DefaultValue(false)
        ]
        ...
        SRDescription(DCSR.ToolTipToolTipIconDescr)
        ]
            ...
            set {
                if (toolTipIcon != value) {
                    ...
                    if (toolTipIcon > 0 && GetHandleCreated()) {
                        ...
                        WinFormUnsafeNativeMethods.SendMessage(new HandleRef(this, Handle), WinFormNativeMethods.TTM_SETTITLE, (int)toolTipIcon, title);
                        ...
                        WinFormUnsafeNativeMethods.SendMessage(new HandleRef(this, Handle), WinFormNativeMethods.TTM_UPDATE, 0, 0);
        ...
        public string ToolTipTitle
        {
            ...
            set
            {
                ...
                if (toolTipTitle != value)
                {
                    ...
                    //if (GetHandleCreated())
                    //{
                    //    WinFormUnsafeNativeMethods.SendMessage(new HandleRef(this, Handle), WinFormNativeMethods.TTM_SETTITLE, (int)toolTipIcon, toolTipTitle);
        ...
        /// </devdoc>
        [
        SRDescription(DCSR.ToolTipUseAnimationDescr),
        Browsable(true),
        DefaultValue(true)
        ]
        ...
        /// </devdoc>
        [
        SRDescription(DCSR.ToolTipUseFadingDescr),
        Browsable(true),
        DefaultValue(true)
        ]
        ...
        /// </devdoc>
        [SRCategory(DCSR.CatBehavior),SRDescription(DCSR.ToolTipDrawEventDescr)]
        ...
        /// </devdoc>
        [SRCategory(DCSR.CatBehavior),SRDescription(DCSR.ToolTipPopupEventDescr)]
        ...
        private void AdjustBaseFromAuto() {
            delayTimes[WinFormNativeMethods.TTDT_RESHOW] = delayTimes[WinFormNativeMethods.TTDT_AUTOMATIC] / RESHOW_RATIO;
            delayTimes[WinFormNativeMethods.TTDT_AUTOPOP] = delayTimes[WinFormNativeMethods.TTDT_AUTOMATIC] * AUTOPOP_RATIO;
            delayTimes[WinFormNativeMethods.TTDT_INITIAL] = delayTimes[WinFormNativeMethods.TTDT_AUTOMATIC];
        }
        ...
        private void CreateHandle() {
            ...
            if (ownerDraw) {
                ...
                style &= ~WinFormNativeMethods.WS_BORDER;
            ...
            Debug.Assert(WinFormNativeMethods.TTDT_AUTOMATIC == 0, "TTDT_AUTOMATIC != 0");

            if (auto) {
                SetDelayTime(WinFormNativeMethods.TTDT_AUTOMATIC, delayTimes[WinFormNativeMethods.TTDT_AUTOMATIC]);
                delayTimes[WinFormNativeMethods.TTDT_AUTOPOP] = GetDelayTime(WinFormNativeMethods.TTDT_AUTOPOP);
                delayTimes[WinFormNativeMethods.TTDT_INITIAL] = GetDelayTime(WinFormNativeMethods.TTDT_INITIAL);
                delayTimes[WinFormNativeMethods.TTDT_RESHOW] = GetDelayTime(WinFormNativeMethods.TTDT_RESHOW);
            }
            else {
            ...
            // Set active status
            //
            WinFormUnsafeNativeMethods.SendMessage(new HandleRef(this, Handle), WinFormNativeMethods.TTM_ACTIVATE, (active == true) ? 1 : 0, 0);
        ...
        private void SetToolInfo(Control ctl, string caption) {
            ...
            WinFormNativeMethods.TOOLINFO_TOOLTIP tool = GetTOOLINFO(ctl, caption, out allocatedString);
            try {
                int ret = (int)WinFormUnsafeNativeMethods.SendMessage(new HandleRef(this, Handle), WinFormNativeMethods.TTM_ADDTOOL, 0, tool);
                ...
                if (ret == 0) {
                    throw new InvalidOperationException(DCSR.GetString(DCSR.ToolTipAddFailed));
                }
            } finally {
        ...
        private void DestroyRegion(Control ctl) {
            ...
            if (created.ContainsKey(ctl)
                ...
                WinFormUnsafeNativeMethods.SendMessage(new HandleRef(this, Handle), WinFormNativeMethods.TTM_DELTOOL, 0, GetMinTOOLINFO(ctl));
                created.Remove(ctl);
            }
        }
        ...
        private int GetDelayTime(int type) {
            if (GetHandleCreated()) {
                return (int)WinFormUnsafeNativeMethods.SendMessage(new HandleRef(this, Handle), WinFormNativeMethods.TTM_GETDELAYTIME, type, 0);
            }
            else {
        ...
        /// </devdoc>
        /// <internalonly/>
        private WinFormNativeMethods.TOOLINFO_TOOLTIP GetMinTOOLINFO(Control ctl) {
            WinFormNativeMethods.TOOLINFO_TOOLTIP ti = new WinFormNativeMethods.TOOLINFO_TOOLTIP();
            ti.cbSize = Marshal.SizeOf(typeof(WinFormNativeMethods.TOOLINFO_TOOLTIP));
            ti.hwnd = ctl.Handle;
            ti.uFlags |= WinFormNativeMethods.TTF_IDISHWND;
            ti.uId = ctl.Handle;
            return ti;
        }
        ...
        /// </devdoc>
        /// <internalonly/>
        private WinFormNativeMethods.TOOLINFO_TOOLTIP GetTOOLINFO(Control ctl, string caption, out bool allocatedString) {
            allocatedString = false;
            WinFormNativeMethods.TOOLINFO_TOOLTIP ti = GetMinTOOLINFO(ctl);
            ti.cbSize = Marshal.SizeOf(typeof(WinFormNativeMethods.TOOLINFO_TOOLTIP));
            ti.uFlags |= WinFormNativeMethods.TTF_TRANSPARENT | WinFormNativeMethods.TTF_SUBCLASS;
            ...
            if (richParent != null && richParent.RightToLeft == RightToLeft.Yes && !ctl.IsMirrored) {
                ...
                ti.uFlags |= WinFormNativeMethods.TTF_RTLREADING;
            }
            
            if (ctl is TreeView || ctl is ListView) {
                TreeView tv = ctl as TreeView;
                if (tv != null && tv.ShowNodeToolTips) {
                    ti.lpszText = WinFormNativeMethods.InvalidIntPtr;
                }
                else {
                    ListView lv = ctl as ListView;
                    if (lv != null && lv.ShowItemToolTips) {
        ...
        private WinFormNativeMethods.TOOLINFO_TOOLTIP GetWinTOOLINFO(IntPtr hWnd) {
            WinFormNativeMethods.TOOLINFO_TOOLTIP ti = new WinFormNativeMethods.TOOLINFO_TOOLTIP();
            ti.cbSize = Marshal.SizeOf(typeof(WinFormNativeMethods.TOOLINFO_TOOLTIP));
            ti.hwnd = hWnd;
            ti.uFlags |= WinFormNativeMethods.TTF_IDISHWND | WinFormNativeMethods.TTF_TRANSPARENT | WinFormNativeMethods.TTF_SUBCLASS;
                    
            // RightToLeft reading order
            //
            Control richParent = TopLevelControl;
            if (richParent != null && richParent.RightToLeft == RightToLeft.Yes) {
                bool isWindowMirrored = ((unchecked((int)(long)WinFormUnsafeNativeMethods.GetWindowLong(new HandleRef(this, hWnd), WinFormNativeMethods.GWL_STYLE)) & WinFormNativeMethods.WS_EX_LAYOUTRTL) == WinFormNativeMethods.WS_EX_LAYOUTRTL);            
                //Indicates that the ToolTip text will be displayed in the opposite direction 
                //to the text in the parent window.                 
                if (!isWindowMirrored) {
                    ti.uFlags |= WinFormNativeMethods.TTF_RTLREADING;
        ...
        private IntPtr GetWindowFromPoint(Point screenCoords, ref bool success) {
            ...
            //    IntPtr hwndControl = WinFormUnsafeNativeMethods.WindowFromPoint(screenCoords.X, screenCoords.Y);
            ...
            while (!finalMatch) {
                ...
                IntPtr found = WinFormUnsafeNativeMethods.ChildWindowFromPointEx(new HandleRef(null, baseHwnd), pt.X, pt.Y, WinFormNativeMethods.CWP_SKIPINVISIBLE);
        ...
        private void SetDelayTime(int type, int time) {
            if (type == WinFormNativeMethods.TTDT_AUTOMATIC) {
                auto = true;
            }
            else {
                auto = false;
            ...
            if (GetHandleCreated() && time >= 0) {
                WinFormUnsafeNativeMethods.SendMessage(new HandleRef(this, Handle), WinFormNativeMethods.TTM_SETDELAYTIME, type, time);
        ...
        public void SetToolTip(Control control, string caption) {
            ...
            {
                DCWin32API.JSRuntime.Invoke<object>("__DCWin32API.SetToolTip", control.Handle.ToInt32(), this.toolTipTitle , caption );
            }
        ...
        private void SetToolTipInternal(Control control, TipInfo info) {
            ...
            if (!empty && !exists) {
                ...
                if (exists && !empty && handlesCreated && !DesignMode) {
                    bool allocatedString;
                    WinFormNativeMethods.TOOLINFO_TOOLTIP toolInfo = GetTOOLINFO(control, info.Caption, out allocatedString);
                    try {
                        WinFormUnsafeNativeMethods.SendMessage(new HandleRef(this, Handle), WinFormNativeMethods.TTM_SETTOOLINFO,
                                                        0, toolInfo);
                    } finally {
        ...
        private void ShowTooltip(string text, IWin32Window win, int duration) {
            ...
            if (associatedControl != null) {
                ...
                WinFormUnsafeNativeMethods.GetWindowRect(new HandleRef(associatedControl, associatedControl.Handle), ref r);
                ...
                if ( cursorLocation.X < r.left || cursorLocation.X > r.right ||
                    ...
                    WinFormNativeMethods.RECT visibleRect = new WinFormNativeMethods.RECT();
        ...
        public void Show(string text, IWin32Window window, int duration) {
            if (duration < 0) {
                throw new ArgumentOutOfRangeException("duration", DCSR.GetString(DCSR.InvalidLowBoundArgumentEx, "duration", (duration).ToString(CultureInfo.CurrentCulture), (0).ToString(CultureInfo.CurrentCulture)));
            }
        ...
        public void Show(string text, IWin32Window window, Point point) {
            ...
            if (HasAllWindowsPermission && IsWindowActive(window)) {
                //Set The ToolTips...
                WinFormNativeMethods.RECT r = new WinFormNativeMethods.RECT();
                WinFormUnsafeNativeMethods.GetWindowRect(new HandleRef(window, Control.GetSafeHandle(window)), ref r);
        ...
        private void SetTrackPosition(int pointX, int pointY)
        {
            ...
            {
                trackPosition = true;
                WinFormUnsafeNativeMethods.SendMessage(new HandleRef(this, this.Handle), WinFormNativeMethods.TTM_TRACKPOSITION, 0, WinFormNativeMethods.Util.MAKELONG(pointX, pointY));
            }
            finally
            {
                trackPosition= false;
        ...
        public void Hide(IWin32Window win) {
            ...
            if (HasAllWindowsPermission) {
                ...
                if (GetHandleCreated())  {                    
                    ...
                    WinFormUnsafeNativeMethods.SendMessage(new HandleRef(this, this.Handle), WinFormNativeMethods.TTM_TRACKACTIVATE, 0, GetWinTOOLINFO(hWnd));
                    WinFormUnsafeNativeMethods.SendMessage(new HandleRef(this, this.Handle), WinFormNativeMethods.TTM_DELTOOL, 0, GetWinTOOLINFO(hWnd));                   
                }
                StopTimer();
        ...
        private void SetTool(IWin32Window win, string text, TipInfo.Type type, Point position) {
            ...
            if (tool != null && tools.ContainsKey(tool)) {
                ...
                try {
                    ...
                    ti.hwnd = tool.Handle;
                    ti.uId = tool.Handle;
                    int ret = (int)WinFormUnsafeNativeMethods.SendMessage(new HandleRef(this, Handle), WinFormNativeMethods.TTM_GETTOOLINFO, 0, ti);
                    if (ret != 0) {
                        ti.uFlags |= WinFormNativeMethods.TTF_TRACK;

                        if (type == TipInfo.Type.Absolute || type == TipInfo.Type.SemiAbsolute) {
                            ti.uFlags |= WinFormNativeMethods.TTF_ABSOLUTE;
                        }
                    ...
                    WinFormUnsafeNativeMethods.SendMessage(new HandleRef(this, this.Handle), WinFormNativeMethods.TTM_SETTOOLINFO, 0, ti);
                    WinFormUnsafeNativeMethods.SendMessage(new HandleRef(this, this.Handle), WinFormNativeMethods.TTM_TRACKACTIVATE, 1, ti);
                } finally {
                ...
                IntPtr hWnd = Control.GetSafeHandle(win);
                owners[hWnd] = win;
                WinFormNativeMethods.TOOLINFO_TOOLTIP toolInfo = GetWinTOOLINFO(hWnd);
                toolInfo.uFlags |= WinFormNativeMethods.TTF_TRACK;

                if (type == TipInfo.Type.Absolute || type == TipInfo.Type.SemiAbsolute) {
                    toolInfo.uFlags |= WinFormNativeMethods.TTF_ABSOLUTE;
                }

                try {
                    toolInfo.lpszText = Marshal.StringToHGlobalAuto(text);
                    WinFormUnsafeNativeMethods.SendMessage(new HandleRef(this, this.Handle), WinFormNativeMethods.TTM_ADDTOOL, 0, toolInfo);
                    WinFormUnsafeNativeMethods.SendMessage(new HandleRef(this, this.Handle), WinFormNativeMethods.TTM_TRACKACTIVATE, 1, toolInfo);
                } finally {
        ...
        private void Reposition(Point tipPosition, Size tipSize) {
            ...
            WinFormSafeNativeMethods.SetWindowPos(new HandleRef(this, this.Handle),
            WinFormNativeMethods.HWND_TOPMOST,
            moveToLocation.X, moveToLocation.Y, tipSize.Width, tipSize.Height,
            WinFormNativeMethods.SWP_NOACTIVATE | WinFormNativeMethods.SWP_NOSIZE | WinFormNativeMethods.SWP_NOOWNERZORDER);
        }
        ...
        private void WmMove() {
            ...
            WinFormNativeMethods.TOOLINFO_TOOLTIP ti = new WinFormNativeMethods.TOOLINFO_TOOLTIP();
            ti.cbSize = Marshal.SizeOf(typeof(WinFormNativeMethods.TOOLINFO_TOOLTIP));
            int ret = (int)WinFormUnsafeNativeMethods.SendMessage(new HandleRef(this, Handle), WinFormNativeMethods.TTM_GETCURRENTTOOL, 0, ti);
            if (ret != 0)
            {
        ...
        private void WmMouseActivate(ref Message msg) {
            ...
            if (ret != 0) {
                ...
                WinFormNativeMethods.RECT r = new WinFormNativeMethods.RECT();
                WinFormUnsafeNativeMethods.GetWindowRect(new HandleRef(win, Control.GetSafeHandle(win)), ref r);
                Point cursorLocation = Cursor.Position;
                ...
                if (cursorLocation.X >= r.left && cursorLocation.X <= r.right &&
                    cursorLocation.Y >= r.top && cursorLocation.Y <= r.bottom) {
                    msg.Result = (IntPtr)WinFormNativeMethods.MA_NOACTIVATE;
                }
            }
        }
        ...
        private void WmWindowFromPoint(ref Message msg) {
            WinFormNativeMethods.POINT sc = (WinFormNativeMethods.POINT)msg.GetLParam(typeof(WinFormNativeMethods.POINT));
            ...
            if (ret != 0) {
                ...
                if (IsBalloon) {
                  // Get the text display rectangle
                   WinFormUnsafeNativeMethods.SendMessage(new HandleRef(this, Handle), WinFormNativeMethods.TTM_ADJUSTRECT, 1, ref r);
                ...
                if (currentTooltipSize != r.Size)
                {
                    ...
                    WinFormUnsafeNativeMethods.SendMessage(new HandleRef(this, Handle), WinFormNativeMethods.TTM_SETMAXTIPWIDTH, 0, maxwidth);
                }
                ...
                if (e.Cancel) { 
                    ...
                    WinFormNativeMethods.HWND_TOPMOST,
                    0, 0, 0, 0,
					WinFormNativeMethods.SWP_NOACTIVATE | WinFormNativeMethods.SWP_NOOWNERZORDER);
        ...
        private bool WmWindowPosChanged() {
            if (cancelled)
            {
               WinFormSafeNativeMethods.ShowWindow(new HandleRef(this, Handle), WinFormNativeMethods.SW_HIDE);
               return true;
            }
            return false;
        }
        ...
        private void WmPop() {
            ...
            if (ret != 0) {
                ...
                if ((tt.TipType & TipInfo.Type.Auto) != 0 || (tt.TipType & TipInfo.Type.SemiAbsolute) != 0) {
                   Screen screen = Screen.FromPoint(Cursor.Position);
                   WinFormUnsafeNativeMethods.SendMessage(new HandleRef(this, Handle), WinFormNativeMethods.TTM_SETMAXTIPWIDTH, 0, screen.WorkingArea.Width);
                }
        ...
        private void WndProc(ref Message msg) {
            ...
            case WinFormNativeMethods.WM_REFLECT + WinFormNativeMethods.WM_NOTIFY:
                 WinFormNativeMethods.NMHDR nmhdr = (WinFormNativeMethods.NMHDR) msg.GetLParam(typeof(WinFormNativeMethods.NMHDR));
                 if (nmhdr.code == WinFormNativeMethods.TTN_SHOW && !trackPosition) {
                     WmShow();
                 }
                 else if (nmhdr.code == WinFormNativeMethods.TTN_POP) {                    
                    WmPop();
            ...
            case WinFormNativeMethods.WM_MOUSEACTIVATE:
            ...
            case WinFormNativeMethods.TTM_WINDOWFROMPOINT:
                WmWindowFromPoint(ref msg);
                break;
   
            case WinFormNativeMethods.WM_PRINTCLIENT:
                goto case WinFormNativeMethods.WM_PAINT;
            ...
            case WinFormNativeMethods.WM_PAINT:
                if (ownerDraw && !isBalloon && !trackPosition)
                {
                    ...
                    IntPtr dc = WinFormUnsafeNativeMethods.BeginPaint(new HandleRef(this,Handle),ref ps);
                    ...
                    try {
                        ...
                        WinFormNativeMethods.TOOLINFO_TOOLTIP ti = new WinFormNativeMethods.TOOLINFO_TOOLTIP();
                        ti.cbSize = Marshal.SizeOf(typeof(WinFormNativeMethods.TOOLINFO_TOOLTIP));
                        int ret = unchecked( (int) (long)WinFormUnsafeNativeMethods.SendMessage(new HandleRef(this, Handle), WinFormNativeMethods.TTM_GETCURRENTTOOL, 0, ti));
                        if (ret != 0) {
                            IWin32Window win = (IWin32Window)owners[ti.hwnd];
                            ...
                            //IntSecurity.ObjectFromWin32Handle.Assert();
                            try {
                                font = Font.FromHfont(WinFormUnsafeNativeMethods.SendMessage(new HandleRef(this, Handle), WinFormNativeMethods.WM_GETFONT, 0, 0));
                            }
