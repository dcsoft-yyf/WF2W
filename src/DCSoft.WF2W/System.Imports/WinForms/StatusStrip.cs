...
namespace System.Windows.Forms {
    ...
    [ComVisible(true),
     ...
     SRDescription(DCSR.DescriptionStatusStrip)
    ]
[System.Reflection.Obfuscation(Exclude = true, ApplyToMembers = false )]
    public class StatusStrip : ToolStrip  {
                ...
                if (SizingGrip && IsHandleCreated) {
                    if (DesignMode) {
                        ...
                        if (rootHwnd.Handle != IntPtr.Zero) {
                            return !WinFormUnsafeNativeMethods.IsZoomed(rootHwnd); 
                        }
                    }
                }
                return false;
       ...
       DefaultValue(true),
       SRDescription(DCSR.StatusStripSizingGripDescr)
       ]
      ...
      protected override async  Task  WndProc(Message m)            {                
          if ((m.Msg == WinFormNativeMethods.WM_NCHITTEST) && SizingGrip) {
            ...
            if (sizeGripBounds.Contains(PointToClient(new Point(x, y)))) {
                ...
                if (rootHwnd.Handle != IntPtr.Zero && !WinFormUnsafeNativeMethods.IsZoomed(rootHwnd)) {
                    // get the client area of the topmost window.  If we're next to the edge then 
                    // the sizing grip is valid.
                    WinFormNativeMethods.RECT rootHwndClientArea = new WinFormNativeMethods.RECT();
                    WinFormUnsafeNativeMethods.GetClientRect(rootHwnd, ref rootHwndClientArea);
                    
                    // map the size grip FROM statusStrip coords TO the toplevel window coords.
                    WinFormNativeMethods.POINT gripLocation;
                    if (RightToLeft == RightToLeft.Yes) {
                        gripLocation = new WinFormNativeMethods.POINT(SizeGripBounds.Left, SizeGripBounds.Bottom);
                    }
                    else {
                        gripLocation = new WinFormNativeMethods.POINT(SizeGripBounds.Right, SizeGripBounds.Bottom);
                    }
                    WinFormUnsafeNativeMethods.MapWindowPoints(new HandleRef(this, this.Handle), rootHwnd, gripLocation, 1);
                    ...
                    if (RightToLeft != RightToLeft.Yes) {
                        if ((deltaRightEdge + deltaBottomEdge) < 2) {
                            m.Result = (IntPtr)WinFormNativeMethods.HTBOTTOMRIGHT;                   
                            return;                      
                        }
                    }
       ...
       private class RightToLeftLayoutGrip : Control {
            ...
            //[SecurityPermission(SecurityAction.LinkDemand, Flags=SecurityPermissionFlag.UnmanagedCode)]
            protected override async Task WndProc(Message m) {
                 if (m.Msg == WinFormNativeMethods.WM_NCHITTEST) {
                    int x = WinFormNativeMethods.Util.LOWORD(m.LParam);
                    int y = WinFormNativeMethods.Util.HIWORD(m.LParam);

                    if (ClientRectangle.Contains(PointToClient(new Point(x, y)))) {
                        m.Result = (IntPtr)WinFormNativeMethods.HTBOTTOMLEFT;
                        return;
                    }
