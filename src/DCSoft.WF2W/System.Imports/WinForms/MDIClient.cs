...
[System.Reflection.Obfuscation(Exclude = true, ApplyToMembers = false )]
    public sealed class MdiClient : Control {
        ...
        protected override CreateParams CreateParams {
           // [SecurityPermission(SecurityAction.LinkDemand, Flags=SecurityPermissionFlag.UnmanagedCode)]
            get {
                ...
                cp.Style |= WinFormNativeMethods.WS_VSCROLL | WinFormNativeMethods.WS_HSCROLL;
                cp.ExStyle |= WinFormNativeMethods.WS_EX_CLIENTEDGE;
                cp.Param = new WinFormNativeMethods.CLIENTCREATESTRUCT(IntPtr.Zero, 1);
        ...
        public void LayoutMdi(MdiLayout value) {
            ...
            switch (value) {
                case MdiLayout.Cascade:
                    SendMessage(WinFormNativeMethods.WM_MDICASCADE, 0, 0);
                    break;
                case MdiLayout.TileVertical:
                    SendMessage(WinFormNativeMethods.WM_MDITILE, WinFormNativeMethods.MDITILE_VERTICAL, 0);
                    break;
                case MdiLayout.TileHorizontal:
                    SendMessage(WinFormNativeMethods.WM_MDITILE, WinFormNativeMethods.MDITILE_HORIZONTAL, 0);
                    break;
                case MdiLayout.ArrangeIcons:
                    SendMessage(WinFormNativeMethods.WM_MDIICONARRANGE, 0, 0);
                    break;
            }
        }
                    ...
                    for (int i=0; i < Controls.Count; i++) {
                        ...
                        if (ctl != null && ctl is Form) {
                            ...
                            if (child.CanRecreateHandle() && child.WindowState == FormWindowState.Minimized) {
                                WinFormUnsafeNativeMethods.GetWindowPlacement(new HandleRef(child, child.Handle), ref wp);
                                ...
                                wp.flags = WinFormNativeMethods.WPF_SETMINPOSITION;
                                WinFormUnsafeNativeMethods.SetWindowPlacement(new HandleRef(child, child.Handle), ref wp);
                                wp.flags = 0;
                            }
                        }
                    }
        ...
        private void SetWindowRgn() {
            ...
            Rectangle bounds = Bounds;
            rgn1 = WinFormSafeNativeMethods.CreateRectRgn(0, 0, bounds.Width, bounds.Height);
            try {
                rgn2 = WinFormSafeNativeMethods.CreateRectRgn(-rect.left, -rect.top,
                                             bounds.Width - rect.right, bounds.Height - rect.bottom);
                try {
                    if (rgn1 == IntPtr.Zero || rgn2 == IntPtr.Zero)
                        throw new InvalidOperationException(DCSR.GetString(DCSR.ErrorSettingWindowRegion));

                    if (WinFormSafeNativeMethods.CombineRgn(new HandleRef(null, rgn1), new HandleRef(null, rgn1), new HandleRef(null, rgn2), WinFormNativeMethods.RGN_DIFF) == 0)
                        throw new InvalidOperationException(DCSR.GetString(DCSR.ErrorSettingWindowRegion));

                    if (WinFormUnsafeNativeMethods.SetWindowRgn(new HandleRef(this, Handle), new HandleRef(null, rgn1), true) == 0) {
                        throw new InvalidOperationException(DCSR.GetString(DCSR.ErrorSettingWindowRegion));
                    }
                    else {
                    ...
                    if (rgn2 != IntPtr.Zero) {
                        WinFormSafeNativeMethods.DeleteObject(new HandleRef(null, rgn2));
                    }
                }
            }
            finally {
                if (rgn1 != IntPtr.Zero) {
                    WinFormSafeNativeMethods.DeleteObject(new HandleRef(null, rgn1)); 
                }
            }
        }
        ...
        new public class ControlCollection : Control.ControlCollection {
            ...
            public override void Add(Control value) {
                ...
                if (!(value is Form) || !((Form)value).IsMdiChild) {
                    throw new ArgumentException(DCSR.GetString(DCSR.MDIChildAddToNonMDIParent), "value");
                }
