...
#pragma warning restore 0414
	...
	SRDescription(DCSR.ColumnHeaderDisplayIndexDescr)
	]
	    ...
	    set {
	        ...
	        if (value < 0 || value> (this.listview.Columns.Count - 1)) {
                    throw new ArgumentOutOfRangeException("DisplayIndex", DCSR.GetString(DCSR.ColumnHeaderBadDisplayIndex));
	        }
            ...
            set {
                ...
                if (imageIndexer.Index != value) {
                    ...
                    if (ListView != null && ListView.IsHandleCreated) {
                        ListView.SetColumnInfo(WinFormNativeMethods.LVCF_IMAGE, this);
                    }
                }
            }
        }
        ...
        [
        Browsable(false),
        SRDescription(DCSR.ColumnHeaderNameDescr)
        ]
        public string Name {
            get {
        ...
        [
        Localizable(true),
        SRDescription(DCSR.ColumnCaption)
        ]
        public string Text {
            get {
            ...
            set {
                ...
                if (listview != null) {
                    listview.SetColumnInfo(WinFormNativeMethods.LVCF_TEXT, this);
                }
            }
        ...
        /// </devdoc>
        [
        SRDescription(DCSR.ColumnAlignment),
        Localizable(true),
        ...
        public HorizontalAlignment TextAlign {
            ...
            set {
                ...
                if (listview != null) {
                    listview.SetColumnInfo(WinFormNativeMethods.LVCF_FMT, this);
                    listview.Invalidate();
                }
            }
        }
        ...
        /// </devdoc>
        [
        SRDescription(DCSR.ColumnWidth),
        Localizable(true),
        DefaultValue(60)
        ]
        public int Width {
            get {
                ...
                if (listview != null && listview.IsHandleCreated && !listview.Disposing && listview.View == View.Details) {
                    ...
                    //
                    IntPtr hwndHdr = WinFormUnsafeNativeMethods.SendMessage(new HandleRef(listview, listview.Handle), WinFormNativeMethods.LVM_GETHEADER, 0, 0);
                    if (hwndHdr != IntPtr.Zero) {
                        int nativeColumnCount = (int)WinFormUnsafeNativeMethods.SendMessage(new HandleRef(listview, hwndHdr), WinFormNativeMethods.HDM_GETITEMCOUNT, 0, 0);
                        if (Index < nativeColumnCount) {
                            width = (int)WinFormUnsafeNativeMethods.SendMessage(new HandleRef(listview, listview.Handle), WinFormNativeMethods.LVM_GETCOLUMNWIDTH, Index, 0);
                        }
                    }
                }
	...
	private void ResetText() {
        ...
        private void SetDisplayIndices(int[] cols) {
            ...
            if (this.listview.IsHandleCreated && !this.listview.Disposing) {
  	       WinFormUnsafeNativeMethods.SendMessage(new HandleRef(this.listview, this.listview.Handle), WinFormNativeMethods.LVM_SETCOLUMNORDERARRAY, cols.Length, cols);
	    }
        }
