...
namespace System.Windows.Forms {
    ...
    public class SendKeys {
        ...
        private static WinFormNativeMethods.HookProc hook;
        ...
        private static bool AddSimpleKey(char character, int repeat, IntPtr hwnd, int[] haveKeys, bool fStartNewChar, int cGrp) {
            int vk = WinFormUnsafeNativeMethods.VkKeyScan(character);

            if (vk != -1) {
                if (haveKeys[HAVESHIFT] == 0 && (vk & SHIFTKEYSCAN) != 0) {
                    AddEvent(new SKEvent(WinFormNativeMethods.WM_KEYDOWN, (int)Keys.ShiftKey, fStartNewChar, hwnd));
                    fStartNewChar = false;
                    haveKeys[HAVESHIFT] = UNKNOWN_GROUPING;
                }

                if (haveKeys[HAVECTRL] == 0 && (vk & CTRLKEYSCAN) != 0) {
                    AddEvent(new SKEvent(WinFormNativeMethods.WM_KEYDOWN, (int)Keys.ControlKey, fStartNewChar, hwnd));
                    fStartNewChar = false;
                    haveKeys[HAVECTRL] = UNKNOWN_GROUPING;
                }

                if (haveKeys[HAVEALT] == 0 && (vk & ALTKEYSCAN) != 0) {
                    AddEvent(new SKEvent(WinFormNativeMethods.WM_KEYDOWN, (int)Keys.Menu, fStartNewChar, hwnd));
                    fStartNewChar = false;
                ...
                int oemVal = WinFormSafeNativeMethods.OemKeyScan((short)(0xFF & (int)character));
                for (int i = 0; i < repeat; i++) {
                    AddEvent(new SKEvent(WinFormNativeMethods.WM_CHAR, character, (int)(oemVal & 0xFFFF), hwnd));
                }
            }
        ...
        private static void AddMsgsForVK(int vk, int repeat, bool altnoctrldown, IntPtr hwnd) {
            for (int i = 0; i < repeat; i++) {
                AddEvent(new SKEvent(altnoctrldown ? WinFormNativeMethods.WM_SYSKEYDOWN : WinFormNativeMethods.WM_KEYDOWN, vk, fStartNewChar, hwnd));
                // fStartNewChar = false;
                AddEvent(new SKEvent(altnoctrldown ? WinFormNativeMethods.WM_SYSKEYUP : WinFormNativeMethods.WM_KEYUP, vk, fStartNewChar, hwnd));
            }
        }
        ...
        private static void CancelMods(int [] haveKeys, int level, IntPtr hwnd) {
            if (haveKeys[HAVESHIFT] == level) {
                AddEvent(new SKEvent(WinFormNativeMethods.WM_KEYUP, (int)Keys.ShiftKey, false, hwnd));
                haveKeys[HAVESHIFT] = 0;
            }
            if (haveKeys[HAVECTRL] == level) {
                AddEvent(new SKEvent(WinFormNativeMethods.WM_KEYUP, (int)Keys.ControlKey, false, hwnd));
                haveKeys[HAVECTRL] = 0;
            }
            if (haveKeys[HAVEALT] == level) {
                AddEvent(new SKEvent(WinFormNativeMethods.WM_SYSKEYUP, (int)Keys.Menu, false, hwnd));
                haveKeys[HAVEALT] = 0;
            }
        }
        ...
        private static void InstallHook() {
            if (hhook == IntPtr.Zero) {
                hook = new WinFormNativeMethods.HookProc(new SendKeysHookProc().Callback);
                stopHook = false;
                hhook = WinFormUnsafeNativeMethods.SetWindowsHookEx(WinFormNativeMethods.WH_JOURNALPLAYBACK,
                                                 hook,
                                                 new HandleRef(null, WinFormUnsafeNativeMethods.GetModuleHandle(null)),
                                                 0);
                if (hhook == IntPtr.Zero)
                    throw new SecurityException(DCSR.GetString(DCSR.SendKeysHookFailed));
            }
        }
        ...
        private static void TestHook()
        {
            ...
            try
            {
                ...
                WinFormNativeMethods.HookProc hookProc = new WinFormNativeMethods.HookProc(EmptyHookCallback);
                IntPtr hookHandle = WinFormUnsafeNativeMethods.SetWindowsHookEx(WinFormNativeMethods.WH_JOURNALPLAYBACK,
                                                 hookProc,
                                                 new HandleRef(null, WinFormUnsafeNativeMethods.GetModuleHandle(null)),
                                                 0);
                ...
                {
                    WinFormUnsafeNativeMethods.UnhookWindowsHookEx(new HandleRef(null, hookHandle));
                }
            }
        ...
        private static byte[] GetKeyboardState() {
            ...
            WinFormUnsafeNativeMethods.GetKeyboardState(keystate);
            return keystate;
        }

        private static void SetKeyboardState(byte[] keystate) {
            WinFormUnsafeNativeMethods.SetKeyboardState(keystate);
        }
        ...
        private static void ParseKeys(string keys, IntPtr hwnd) {
            ...
            while (i < keysLen) {
                ...
                switch (ch) {
                    case '}':
                        ...
                        //
                        throw new ArgumentException(DCSR.GetString(DCSR.InvalidSendKeysString, keys));
                    ...
                    case '{':
                        ...
                        if (j >= keysLen) {
                            throw new ArgumentException(DCSR.GetString(DCSR.SendKeysKeywordDelimError));
                        }
                        ...
                        if (keys[j] != '}') {
                            throw new ArgumentException(DCSR.GetString(DCSR.InvalidSendKeysRepeat));
                        }
                        ...
                        if (vk != -1) {
                            ...
                            throw new ArgumentException(DCSR.GetString(DCSR.InvalidSendKeysKeyword, keys.Substring(i + 1, j - (i + 1))));
                        }
                    ...
                    case '+':
                        if (haveKeys[HAVESHIFT] != 0) throw new ArgumentException(DCSR.GetString(DCSR.InvalidSendKeysString, keys));
                    ...
                    case '^':
                        if (haveKeys[HAVECTRL]!= 0) throw new ArgumentException(DCSR.GetString(DCSR.InvalidSendKeysString, keys));
                    ...
                    case '%':
                        if (haveKeys[HAVEALT] != 0) throw new ArgumentException(DCSR.GetString(DCSR.InvalidSendKeysString, keys));

                        AddEvent(new SKEvent((haveKeys[HAVECTRL] != 0) ? WinFormNativeMethods.WM_KEYDOWN : WinFormNativeMethods.WM_SYSKEYDOWN,
                    ...
                    case '(':
                        ...
                        //
                        cGrp++;
                        if (cGrp > 3) throw new ArgumentException(DCSR.GetString(DCSR.SendKeysNestingError));
                    ...
                    case ')':
                        if (cGrp < 1) throw new ArgumentException(DCSR.GetString(DCSR.InvalidSendKeysString, keys));
            ...
            if (cGrp != 0)
                throw new ArgumentException(DCSR.GetString(DCSR.SendKeysGroupDelimError));
        ...
        private static void SendInput(byte[] oldKeyboardState, Queue previousEvents)
        {
            ...
            WinFormNativeMethods.INPUT[] currentInput = new WinFormNativeMethods.INPUT[2];

            // all events are Keyboard events
            currentInput[0].type = WinFormNativeMethods.INPUT_KEYBOARD;
            currentInput[1].type = WinFormNativeMethods.INPUT_KEYBOARD;

            // set KeyUp values for currentInput[1]
            currentInput[1].inputUnion.ki.wVk = (short) 0;
            currentInput[1].inputUnion.ki.dwFlags = WinFormNativeMethods.KEYEVENTF_UNICODE | WinFormNativeMethods.KEYEVENTF_KEYUP;
            ...
            int INPUTSize = Marshal.SizeOf(typeof(WinFormNativeMethods.INPUT));
            ...
            lock (events.SyncRoot)
            {
                ...
                bool blockInputSuccess = WinFormUnsafeNativeMethods.BlockInput(true);
                ...
                try
                {
                    ...
                    for (int i = 0; i < eventsTotal; i++)
                    {
                        ...
                        if (skEvent.wm == WinFormNativeMethods.WM_CHAR)
                        {
                            ...
                            currentInput[0].inputUnion.ki.dwFlags = WinFormNativeMethods.KEYEVENTF_UNICODE;
                            ...
                            eventsSent += WinFormUnsafeNativeMethods.SendInput(2, currentInput, INPUTSize) - 1;
                        }
                        else
                        {
                            ...
                            if (skEvent.wm == WinFormNativeMethods.WM_KEYUP || skEvent.wm == WinFormNativeMethods.WM_SYSKEYUP)
                            {
                                currentInput[0].inputUnion.ki.dwFlags |= WinFormNativeMethods.KEYEVENTF_KEYUP;
                            }

                            // Sets KEYEVENTF_EXTENDEDKEY flag if necessary
                            if (IsExtendedKey(skEvent))
                            {
                                currentInput[0].inputUnion.ki.dwFlags |= WinFormNativeMethods.KEYEVENTF_EXTENDEDKEY;
                            }

                            currentInput[0].inputUnion.ki.wVk = (short)skEvent.paramL;

                            // send only currentInput[0]
                            eventsSent += WinFormUnsafeNativeMethods.SendInput(1, currentInput, INPUTSize);
                ...
                finally
                {
                    ...
                    {
                        WinFormUnsafeNativeMethods.BlockInput(false);
                    }
                }
            }
        ...
        private static void AddCancelModifiersForPreviousEvents(Queue previousEvents)
        {
            ...
            while (previousEvents.Count > 0)
            {
                ...
                bool isOn;
                if ((skEvent.wm == WinFormNativeMethods.WM_KEYUP) ||
                    (skEvent.wm == WinFormNativeMethods.WM_SYSKEYUP))
                {
                    isOn = false;
                }
                else if ((skEvent.wm == WinFormNativeMethods.WM_KEYDOWN) ||
                         (skEvent.wm == WinFormNativeMethods.WM_SYSKEYDOWN))
                {
                    isOn = true;
                }
                else
            ...
            if (shift)
            {
                AddEvent(new SKEvent(WinFormNativeMethods.WM_KEYUP, (int)Keys.ShiftKey, false, IntPtr.Zero));
            }
            else if (ctrl)
            {
                AddEvent(new SKEvent(WinFormNativeMethods.WM_KEYUP, (int)Keys.ControlKey, false, IntPtr.Zero));
            }
            else if (alt)
            {
                AddEvent(new SKEvent(WinFormNativeMethods.WM_SYSKEYUP, (int)Keys.Menu, false, IntPtr.Zero));
            }
        }

        private static bool IsExtendedKey(SKEvent skEvent)
        {
            return (skEvent.paramL == WinFormNativeMethods.VK_UP) ||
                   (skEvent.paramL == WinFormNativeMethods.VK_DOWN) ||
                   (skEvent.paramL == WinFormNativeMethods.VK_LEFT) ||
                   (skEvent.paramL == WinFormNativeMethods.VK_RIGHT) ||
                   (skEvent.paramL == WinFormNativeMethods.VK_PRIOR) ||
                   (skEvent.paramL == WinFormNativeMethods.VK_NEXT) ||
                   (skEvent.paramL == WinFormNativeMethods.VK_HOME) ||
                   (skEvent.paramL == WinFormNativeMethods.VK_END) ||
                   (skEvent.paramL == WinFormNativeMethods.VK_INSERT) ||
                   (skEvent.paramL == WinFormNativeMethods.VK_DELETE);
        }
        ...
        {
            if (skEvent.wm == WinFormNativeMethods.WM_KEYDOWN)
            {
        ...
        private static void ResetKeyboardUsingSendInput(int INPUTSize)
        {
            ...
            WinFormNativeMethods.INPUT[] keyboardInput = new WinFormNativeMethods.INPUT[2];

            keyboardInput[0].type = WinFormNativeMethods.INPUT_KEYBOARD;
            keyboardInput[0].inputUnion.ki.dwFlags = 0;

            keyboardInput[1].type = WinFormNativeMethods.INPUT_KEYBOARD;
            keyboardInput[1].inputUnion.ki.dwFlags = WinFormNativeMethods.KEYEVENTF_KEYUP;

            // SendInputs to turn on or off these keys.  Inputs are pairs because KeyUp is sent for each one
            if (capslockChanged)
            {
                keyboardInput[0].inputUnion.ki.wVk = WinFormNativeMethods.VK_CAPITAL;
                keyboardInput[1].inputUnion.ki.wVk = WinFormNativeMethods.VK_CAPITAL;
                WinFormUnsafeNativeMethods.SendInput(2, keyboardInput, INPUTSize);
            }

            if (numlockChanged)
            {
                keyboardInput[0].inputUnion.ki.wVk = WinFormNativeMethods.VK_NUMLOCK;
                keyboardInput[1].inputUnion.ki.wVk = WinFormNativeMethods.VK_NUMLOCK;
                WinFormUnsafeNativeMethods.SendInput(2, keyboardInput, INPUTSize);
            }

            if (scrollLockChanged)
            {
                keyboardInput[0].inputUnion.ki.wVk = WinFormNativeMethods.VK_SCROLL;
                keyboardInput[1].inputUnion.ki.wVk = WinFormNativeMethods.VK_SCROLL;
                WinFormUnsafeNativeMethods.SendInput(2, keyboardInput, INPUTSize);
            }

            if (kanaChanged)
            {
                keyboardInput[0].inputUnion.ki.wVk = WinFormNativeMethods.VK_KANA;
                keyboardInput[1].inputUnion.ki.wVk = WinFormNativeMethods.VK_KANA;
                WinFormUnsafeNativeMethods.SendInput(2, keyboardInput, INPUTSize);
            }
        }
        ...
        private static void Send(string keys, Control control, bool wait) {
            ...
            if (!wait && !Application.MessageLoop) {
                throw new InvalidOperationException(DCSR.GetString(DCSR.SendKeysNoMessageLoop));
            }
        ...
        private static void UninstallJournalingHook() {
            if (hhook != IntPtr.Zero) {
                ...
                WinFormUnsafeNativeMethods.UnhookWindowsHookEx(new HandleRef(null, hhook));
                hhook = IntPtr.Zero;
            }
        }
        ...
        private class SKWindow : Control {
            ...
            protected override async Task WndProc( Message m) {
                if (m.Msg == WinFormNativeMethods.WM_CANCELJOURNAL) {
                    try {
        ...
        private class SendKeysHookProc {
            ...
            public virtual IntPtr Callback(int code, IntPtr wparam, IntPtr lparam) {
                WinFormNativeMethods.EVENTMSG eventmsg = (WinFormNativeMethods.EVENTMSG)WinFormUnsafeNativeMethods.PtrToStructure(lparam, typeof(WinFormNativeMethods.EVENTMSG));
                
    
                if (WinFormUnsafeNativeMethods.GetAsyncKeyState((int)Keys.Pause) != 0) {
                    SendKeys.stopHook = true;
                }
                
                //
                switch (code) {
                    case WinFormNativeMethods.HC_SKIP:
                    ...
                    case WinFormNativeMethods.HC_GETNEXT:
                        ...
                        eventmsg.time = WinFormSafeNativeMethods.GetTickCount();
                        Marshal.StructureToPtr(eventmsg, lparam, true);
                        break;
    
                    default:
                        if (code < 0)
                            WinFormUnsafeNativeMethods.CallNextHookEx(new HandleRef(null, SendKeys.hhook), code, wparam, lparam);
                        break;
                }
