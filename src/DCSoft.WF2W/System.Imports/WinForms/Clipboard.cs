    ...
    public sealed class Clipboard {
            ...
            if (formats != null) {
                if (formats.Length <= 4) {
                    for (int i = 0; i < formats.Length; i++) {
                        ...
                        if (format != WinFormNativeMethods.CF_TEXT &&
                            format !=  WinFormNativeMethods.CF_UNICODETEXT && 
        ...
        public static void SetDataObject(object data, bool copy, int retryTimes, int retryDelay) {
            if (Application.OleRequired() != System.Threading.ApartmentState.STA) {
                throw new System.Threading.ThreadStateException(DCSR.GetString(DCSR.ThreadMustBeSTA));
            }
            ...
            if (retryTimes < 0) {
                throw new ArgumentOutOfRangeException("retryTimes", DCSR.GetString(DCSR.InvalidLowBoundArgumentEx, "retryTimes", retryTimes.ToString(CultureInfo.CurrentCulture), (0).ToString(CultureInfo.CurrentCulture)));
            }

            if (retryDelay < 0) {
                throw new ArgumentOutOfRangeException("retryDelay", DCSR.GetString(DCSR.InvalidLowBoundArgumentEx, "retryDelay", retryDelay.ToString(CultureInfo.CurrentCulture), (0).ToString(CultureInfo.CurrentCulture)));
            }
            ...
            try
            {
                do {
                    if (data is IComDataObject) {
                        hr = WinFormUnsafeNativeMethods.OleSetClipboard((IComDataObject)data);
                    }
                    else {
                        hr = WinFormUnsafeNativeMethods.OleSetClipboard(dataObject);
                    }
                    if (hr != 0) {
                        if (retry == 0) {
                            ThrowIfFailed(hr);
                ...
                if (copy) {
                    retry = retryTimes;
                    do {
                        hr = WinFormUnsafeNativeMethods.OleFlushClipboard();
                        if (hr != 0) {
                            if (retry == 0) {
                                ThrowIfFailed(hr);
                            }
        ...
        private static IDataObject GetDataObject(int retryTimes, int retryDelay)
        {
            ...
            int hr, retry = retryTimes;
            do {
                hr = WinFormUnsafeNativeMethods.OleGetClipboard(ref dataObject);
                if (hr != 0) {
                    if (retry == 0) {
                        ThrowIfFailed(hr);
                    }
        ...
        public static void SetFileDropList(StringCollection filePaths) {
            ...
            {
                throw new ArgumentException(DCSR.GetString(DCSR.CollectionEmptyException));
            }
            ...
            foreach (string path in filePaths) {
                try {
                    ...
                    throw new ArgumentException(DCSR.GetString(DCSR.Clipboard_InvalidPath, path, "filePaths"), e);
                }
            }
        ...
        private static void ThrowIfFailed(int hr) {
            // 
            if (hr != 0) {
                ExternalException e = new ExternalException(DCSR.GetString(DCSR.ClipboardOperationFailed), hr);
                throw e;
            }
        }
    }
