...
namespace System.Windows.Forms {
    ...
    public sealed class ListViewGroup : ISerializable {
        ...
        /// </devdoc>
        public ListViewGroup() : this(DCSR.GetString(DCSR.ListViewGroupDefaultHeader, nextHeader++))
        {
        }
        ...
        SRDescription(DCSR.ListViewGroupNameDescr),
        Browsable(true),
        DefaultValue("")
        ]
        public string Name
        ...
        private void CheckListViewItems(ListViewGroup group) {
            for (int i = 0; i < group.Items.Count; i ++) {
                ...
                if (item.ListView != null && item.ListView != this.listView) {
                    throw new ArgumentException(DCSR.GetString(DCSR.OnlyOneControl, item.Text));
                }
            }
        }
