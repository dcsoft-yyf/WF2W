...
namespace System.Windows.Forms {
    internal class WebBrowserContainer : WinFormUnsafeNativeMethods.IOleContainer, WinFormUnsafeNativeMethods.IOleInPlaceFrame {
        //
        // Private fields:
        //
        ...
        //
        int WinFormUnsafeNativeMethods.IOleContainer.ParseDisplayName(Object pbc, string pszDisplayName, int[] pchEaten, Object[] ppmkOut) {
            if (ppmkOut != null)
                ppmkOut[0] = null;
             return WinFormNativeMethods.E_NOTIMPL;
        }

        int WinFormUnsafeNativeMethods.IOleContainer.EnumObjects(int grfFlags, out WinFormUnsafeNativeMethods.IEnumUnknown ppenum) {
            //ppenum = null;
        ...
        int WinFormUnsafeNativeMethods.IOleContainer.LockContainer(bool fLock) {
            return WinFormNativeMethods.E_NOTIMPL;
        }
        ...
        //
        IntPtr WinFormUnsafeNativeMethods.IOleInPlaceFrame.GetWindow() {
            return parent.Handle;
        }

        int WinFormUnsafeNativeMethods.IOleInPlaceFrame.ContextSensitiveHelp(int fEnterMode) {
            return WinFormNativeMethods.S_OK;
        }

        int WinFormUnsafeNativeMethods.IOleInPlaceFrame.GetBorder(WinFormNativeMethods.COMRECT lprectBorder) {
            return WinFormNativeMethods.E_NOTIMPL;
        }

        int WinFormUnsafeNativeMethods.IOleInPlaceFrame.RequestBorderSpace(WinFormNativeMethods.COMRECT pborderwidths) {
            return WinFormNativeMethods.E_NOTIMPL;
        }

        int WinFormUnsafeNativeMethods.IOleInPlaceFrame.SetBorderSpace(WinFormNativeMethods.COMRECT pborderwidths) {
            return WinFormNativeMethods.E_NOTIMPL;
        }

        int WinFormUnsafeNativeMethods.IOleInPlaceFrame.SetActiveObject(WinFormUnsafeNativeMethods.IOleInPlaceActiveObject pActiveObject, string pszObjName) {
            ...
            WinFormUnsafeNativeMethods.IOleObject oleObject = pActiveObject as WinFormUnsafeNativeMethods.IOleObject;
            if (oleObject != null) {
                WinFormUnsafeNativeMethods.IOleClientSite clientSite = null;
                try {
        ...
        int WinFormUnsafeNativeMethods.IOleInPlaceFrame.InsertMenus(IntPtr hmenuShared, WinFormNativeMethods.tagOleMenuGroupWidths lpMenuWidths) {
            return WinFormNativeMethods.S_OK;
        }

        int WinFormUnsafeNativeMethods.IOleInPlaceFrame.SetMenu(IntPtr hmenuShared, IntPtr holemenu, IntPtr hwndActiveObject) {
            return WinFormNativeMethods.E_NOTIMPL;
        }

        int WinFormUnsafeNativeMethods.IOleInPlaceFrame.RemoveMenus(IntPtr hmenuShared) {
            return WinFormNativeMethods.E_NOTIMPL;
        }

        int WinFormUnsafeNativeMethods.IOleInPlaceFrame.SetStatusText(string pszStatusText) {
            return WinFormNativeMethods.E_NOTIMPL;
        }

        int WinFormUnsafeNativeMethods.IOleInPlaceFrame.EnableModeless(bool fEnable) {
            return WinFormNativeMethods.E_NOTIMPL;
        }

        int WinFormUnsafeNativeMethods.IOleInPlaceFrame.TranslateAccelerator(ref WinFormNativeMethods.MSG lpmsg, short wID) {
        ...
        internal void AddControl(Control ctl) {
            if (containerCache.Contains(ctl))
                throw new ArgumentException(DCSR.GetString(DCSR.AXDuplicateControl, GetNameForControl(ctl)), "ctl");
