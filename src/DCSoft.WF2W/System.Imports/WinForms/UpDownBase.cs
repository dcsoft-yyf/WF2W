...
namespace System.Windows.Forms {
    ...
    public abstract class UpDownBase : ContainerControl {
        ...
        SRDescription(DCSR.UpDownBaseBorderStyleDescr)
        ]
        ...
        DefaultValue(true),
        SRDescription(DCSR.UpDownBaseInterceptArrowKeysDescr)
        ]
        ...
        DesignerSerializationVisibility(DesignerSerializationVisibility.Hidden),
        SRDescription(DCSR.UpDownBasePreferredHeightDescr)
        ]
        ...
        SRCategory(DCSR.CatBehavior),
        DefaultValue(false),
        SRDescription(DCSR.UpDownBaseReadOnlyDescr)
        ]
        ...
        SRDescription(DCSR.UpDownBaseTextAlignDescr)
        ]
        ...
        SRDescription(DCSR.UpDownBaseAlignmentDescr)
        ]
        ...
        protected override void OnMouseWheel(MouseEventArgs e) {
            ...
            Debug.Assert(this.wheelDelta > -WinFormNativeMethods.WHEEL_DELTA, "wheelDelta is too smal");
            Debug.Assert(this.wheelDelta < WinFormNativeMethods.WHEEL_DELTA, "wheelDelta is too big");
            this.wheelDelta += e.Delta;

            float partialNotches;
            partialNotches = (float)this.wheelDelta / (float)WinFormNativeMethods.WHEEL_DELTA;
            ...
            if (scrollBands != 0) {
               ...
               if (scrollBands > 0) {
                  ...
                  this.wheelDelta -= (int)((float)scrollBands * ((float)WinFormNativeMethods.WHEEL_DELTA / (float)wheelScrollLines));
               }
               else {
                  absScrollBands = -scrollBands;
                  while (absScrollBands > 0) {
        ...
        private MouseEventArgs TranslateMouseEvent(Control child, MouseEventArgs e) {
            if (child != null && IsHandleCreated) {
                ...
                WinFormNativeMethods.POINT point = new WinFormNativeMethods.POINT(e.X, e.Y);
                WinFormUnsafeNativeMethods.MapWindowPoints(new HandleRef(child, child.Handle), new HandleRef(this, Handle), point, 1);
            ...
            switch (m.Msg)
            {
                case WinFormNativeMethods.WM_SETFOCUS:
                    if (!HostedInWin32DialogManager) {
                        ...
                        if (TextBox.CanFocus){
                            WinFormUnsafeNativeMethods.SetFocus(new HandleRef(TextBox, TextBox.Handle));
                        }
        ...
        internal class UpDownEdit : TextBox{
            ...
            protected override void OnMouseUp(MouseEventArgs e) {
                ...
                if (e.Button == MouseButtons.Left) {
                    if (!parent.ValidationCancelled && WinFormUnsafeNativeMethods.WindowFromPoint(pt.X, pt.Y) == Handle) {
                        if (!doubleClickFired) {
                            parent.OnClick(me);
            ...
            internal class UpDownButtonsAccessibleObject : ControlAccessibleObject {
                ...
                internal class DirectionButtonAccessibleObject : AccessibleObject {
                    ...
                    public override string Name {
                        get {
                            if (up) {
                                return DCSR.GetString(DCSR.UpDownBaseUpButtonAccName);
                            }
                            return DCSR.GetString(DCSR.UpDownBaseDownButtonAccName);
                        }
                        set {
                        }
                    }
