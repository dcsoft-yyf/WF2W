    ...
    public class ComponentEditorForm : Form {
        ...
        public ComponentEditorForm(object component, Type[] pageTypes) : base() {
            ...
            if (!(component is IComponent)) {
               throw new ArgumentException(DCSR.GetString(DCSR.ComponentEditorFormBadComponent),"component");
            }
        ...
        internal virtual void ApplyChanges(bool lastApply) {
            if (dirty) {
                ...
                cancelButton.Text = DCSR.GetString(DCSR.CloseCaption);
                dirty = false;
        ...
        */
        ...
        private void OnConfigureUI() {
            ...
            if (site != null) {
                caption = DCSR.GetString(DCSR.ComponentEditorFormProperties, site.Name);
            }
            else {
                caption = DCSR.GetString(DCSR.ComponentEditorFormPropertiesNoName);
            }
            this.Text = caption;
            ...
            helpButton.Text = DCSR.GetString(DCSR.HelpCaption);
            ...
            applyButton.Text = DCSR.GetString(DCSR.ApplyCaption);
            ...
            cancelButton.Text = DCSR.GetString(DCSR.CancelCaption);
            ...
            okButton.Text = DCSR.GetString(DCSR.OKCaption);
        ...
        private sealed class ComponentEditorPageSite : IComponentEditorPageSite {
            ...
            internal ComponentEditorPageSite(Control parent, Type pageClass, IComponent component, ComponentEditorForm form) {
                ...
                try {
                    ...
                    throw new TargetInvocationException(DCSR.GetString(DCSR.ExceptionCreatingCompEditorControl, e.ToString()), e.InnerException);
                }
        ...
        internal sealed class PageSelector : TreeView {
            ...
            protected override CreateParams CreateParams {
                ...
                get {
                    ...
                    cp.ExStyle |= WinFormNativeMethods.WS_EX_STATICEDGE;
                    return cp;
                }
            }
            ...
            private void CreateDitherBrush() {
                ...
                IntPtr hbitmapTemp = WinFormSafeNativeMethods.CreateBitmap(8, 8, 1, 1, patternBits);
                ...
                if (hbitmapTemp != IntPtr.Zero) {
                    hbrushDither = WinFormSafeNativeMethods.CreatePatternBrush(new HandleRef(null, hbitmapTemp));

                    Debug.Assert(hbrushDither != IntPtr.Zero,
                                 "Unable to created dithered brush. Page selector UI will not be correct");

                    WinFormSafeNativeMethods.DeleteObject(new HandleRef(null, hbitmapTemp));
                }
            }

            private void DrawTreeItem(string itemText, int imageIndex, IntPtr dc, WinFormNativeMethods.RECT rcIn,
                ...
                if ((state & STATE_HOT) != 0)
                    hfontOld = WinFormSafeNativeMethods.SelectObject(new HandleRef(null, dc), new HandleRef(Parent, ((Control)Parent).FontHandle));

                // Fill the background
                if (((state & STATE_SELECTED) != 0) && (hbrushDither != IntPtr.Zero)) {
                    FillRectDither(dc, rcIn);
                    WinFormSafeNativeMethods.SetBkMode(new HandleRef(null, dc), WinFormNativeMethods.TRANSPARENT);
                }
                else {
                    WinFormSafeNativeMethods.SetBkColor(new HandleRef(null, dc), backColor);
                    IntUnsafeNativeMethods.ExtTextOut(new HandleRef(null, dc), 0, 0, WinFormNativeMethods.ETO_CLIPPED | WinFormNativeMethods.ETO_OPAQUE, ref rc, null, 0, null);
                }
                ...
                rc2.right = rc.right;
                WinFormSafeNativeMethods.SetTextColor(new HandleRef(null, dc), textColor);
                ...
                WinFormSafeNativeMethods.ImageList_Draw(new HandleRef(imagelist, imagelist.Handle), imageIndex, new HandleRef(null, dc),
                                       PADDING_HORZ, rc.top + (((rc.bottom - rc.top) - SIZE_ICON_Y) >> 1),
                                       WinFormNativeMethods.ILD_TRANSPARENT);
                ...
                if ((state & STATE_HOT) != 0) {
                    ...
                    // top left
                    savedColor = WinFormSafeNativeMethods.SetBkColor(new HandleRef(null, dc), ColorTranslator.ToWin32(SystemColors.ControlLightLight));
                    rc2.left = rc.left;
                    rc2.top = rc.top;
                    rc2.bottom = rc.top + 1;
                    rc2.right = rc.right;
                    IntUnsafeNativeMethods.ExtTextOut(new HandleRef(null, dc), 0, 0, WinFormNativeMethods.ETO_OPAQUE, ref rc2, null, 0, null);
                    rc2.bottom = rc.bottom;
                    rc2.right = rc.left + 1;
                    IntUnsafeNativeMethods.ExtTextOut(new HandleRef(null, dc), 0, 0, WinFormNativeMethods.ETO_OPAQUE, ref rc2, null, 0, null);

                    // bottom right
                    WinFormSafeNativeMethods.SetBkColor(new HandleRef(null, dc), ColorTranslator.ToWin32(SystemColors.ControlDark));
                    rc2.left = rc.left;
                    rc2.right = rc.right;
                    rc2.top = rc.bottom - 1;
                    rc2.bottom = rc.bottom;
                    ...
                    WinFormSafeNativeMethods.SetBkColor(new HandleRef(null, dc), savedColor);
                }

                if (hfontOld != IntPtr.Zero)
                    WinFormSafeNativeMethods.SelectObject(new HandleRef(null, dc), new HandleRef(null, hfontOld));
            }
            ...
            protected override void OnHandleCreated(EventArgs e) {
                ...
                itemHeight = (int)WinFormUnsafeNativeMethods.SendMessage(new HandleRef(this, Handle), WinFormNativeMethods.TVM_GETITEMHEIGHT, 0, 0);
                itemHeight += 2 * PADDING_VERT;
                WinFormUnsafeNativeMethods.SendMessage(new HandleRef(this, Handle), WinFormNativeMethods.TVM_SETITEMHEIGHT, itemHeight, 0);
            ...
            private void OnCustomDraw(ref Message m) {
                WinFormNativeMethods.NMTVCUSTOMDRAW nmtvcd = (WinFormNativeMethods.NMTVCUSTOMDRAW)m.GetLParam(typeof(WinFormNativeMethods.NMTVCUSTOMDRAW));

                switch (nmtvcd.nmcd.dwDrawStage) {
                    case WinFormNativeMethods.CDDS_PREPAINT:
                        m.Result = (IntPtr)(WinFormNativeMethods.CDRF_NOTIFYITEMDRAW | WinFormNativeMethods.CDRF_NOTIFYPOSTPAINT);
                        break;
                    case WinFormNativeMethods.CDDS_ITEMPREPAINT:
                        {
                            ...
                            if (itemNode != null) {
                                ...
                                if (((itemState & WinFormNativeMethods.CDIS_HOT) != 0) ||
                                   ((itemState & WinFormNativeMethods.CDIS_FOCUS) != 0))
                                   state |= STATE_HOT;
                                if ((itemState & WinFormNativeMethods.CDIS_SELECTED) != 0)
                        ...
                        break;
                    case WinFormNativeMethods.CDDS_POSTPAINT:
                        m.Result = (IntPtr)WinFormNativeMethods.CDRF_SKIPDEFAULT;
                        break;
                    default:
            ...
            protected override void OnHandleDestroyed(EventArgs e) {
                ...
                if (!RecreatingHandle && (hbrushDither != IntPtr.Zero)) {
                    WinFormSafeNativeMethods.DeleteObject(new HandleRef(this, hbrushDither));
                    hbrushDither = IntPtr.Zero;
                }
            }

            private void FillRectDither(IntPtr dc, WinFormNativeMethods.RECT rc) {
                IntPtr hbrushOld = WinFormSafeNativeMethods.SelectObject(new HandleRef(null, dc), new HandleRef(this, hbrushDither));

                if (hbrushOld != IntPtr.Zero) {
                    int oldTextColor, oldBackColor;

                    oldTextColor = WinFormSafeNativeMethods.SetTextColor(new HandleRef(null, dc), ColorTranslator.ToWin32(SystemColors.ControlLightLight));
                    oldBackColor = WinFormSafeNativeMethods.SetBkColor(new HandleRef(null, dc), ColorTranslator.ToWin32(SystemColors.Control));

                    WinFormSafeNativeMethods.PatBlt(new HandleRef(null, dc), rc.left, rc.top, rc.right - rc.left, rc.bottom - rc.top, WinFormNativeMethods.PATCOPY);
                    WinFormSafeNativeMethods.SetTextColor(new HandleRef(null, dc), oldTextColor);
                    WinFormSafeNativeMethods.SetBkColor(new HandleRef(null, dc), oldBackColor);
                }
            }

            //[SecurityPermission(SecurityAction.LinkDemand, Flags=SecurityPermissionFlag.UnmanagedCode)]
            protected override async Task WndProc(Message m) {
                if (m.Msg == WinFormNativeMethods.WM_REFLECT + WinFormNativeMethods.WM_NOTIFY) {
                    WinFormNativeMethods.NMHDR nmh = (WinFormNativeMethods.NMHDR)m.GetLParam(typeof(WinFormNativeMethods.NMHDR));
                    if (nmh.code == WinFormNativeMethods.NM_CUSTOMDRAW) {
                        OnCustomDraw(ref m);
                        return;
                    }
                }
