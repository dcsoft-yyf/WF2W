...
namespace System.Windows.Forms.ComponentModel.Com2Interop {
    ...
    internal class Com2PictureConverter : Com2DataTypeToManagedDataTypeConverter {
        ...
        public Com2PictureConverter(Com2PropertyDescriptor pd) {
            if (pd.DISPID == WinFormNativeMethods.ActiveX.DISPID_MOUSEICON || pd.Name.IndexOf("Icon") != -1) {
        ...
        public override object ConvertNativeToManaged(object nativeValue, Com2PropertyDescriptor pd) {
            ...
            Debug.Assert(nativeValue is WinFormUnsafeNativeMethods.IPicture, "nativevalue is not IPicture");

            WinFormUnsafeNativeMethods.IPicture nativePicture = (WinFormUnsafeNativeMethods.IPicture)nativeValue;
            ...
            if (handle != IntPtr.Zero) {
                switch (nativePicture.GetPictureType()) {
                    case  WinFormNativeMethods.Ole.PICTYPE_ICON:
                        pictureType = typeof(Icon);
                        lastManaged = Icon.FromHandle(handle);
                        break;
                    case   WinFormNativeMethods.Ole.PICTYPE_BITMAP:
        ...
        public override object ConvertManagedToNative(object managedValue, Com2PropertyDescriptor pd, ref bool cancelSet) {
            ...
            //    WinFormNativeMethods.PICTDESC pictdesc = null;
            //    bool own = false;

            //    if (lastManaged is Icon) {
            //        pictdesc = WinFormNativeMethods.PICTDESC.CreateIconPICTDESC(((Icon)lastManaged).Handle);
            //    }
            //    else if (lastManaged is Bitmap) {
            //        pictdesc = WinFormNativeMethods.PICTDESC.CreateBitmapPICTDESC(((Bitmap)lastManaged).GetHbitmap(), lastPalette);
            //        own = true;
            //    }
            //    else {
            ...
            //    WinFormUnsafeNativeMethods.IPicture pict = WinFormUnsafeNativeMethods.OleCreatePictureIndirect(pictdesc, ref g, own);
