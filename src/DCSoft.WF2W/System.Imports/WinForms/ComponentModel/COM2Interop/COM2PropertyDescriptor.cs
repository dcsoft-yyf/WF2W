...
namespace System.Windows.Forms.ComponentModel.Com2Interop {
    ...
    internal class Com2PropertyDescriptor : PropertyDescriptor, ICloneable{
        ...
        static Com2PropertyDescriptor() {
            ...
            //oleConverters[typeof(WinFormSafeNativeMethods.IFontDisp).GUID] = typeof(Com2FontConverter);
            //oleConverters[typeof(WinFormUnsafeNativeMethods.IFont).GUID] = typeof(Com2FontConverter);
            //oleConverters[typeof(WinFormUnsafeNativeMethods.IPictureDisp).GUID] = typeof(Com2PictureConverter);
            //oleConverters[typeof(WinFormUnsafeNativeMethods.IPicture).GUID] = typeof(Com2PictureConverter);
        }
        ...
        : base(name, attrs) {
            ...
            if (this.canShow && (propType == typeof(Object) || (valueConverter == null && propType == typeof(WinFormUnsafeNativeMethods.IDispatch)))) {
                this.typeHide = true;
            }
        }
        ...
        private TypeConverter GetBaseTypeConverter() {
            ...
            if (localConverter == null) {
               // we don't want to create the value editor for the IDispatch props because
                ...
                //
                if (!typeof(WinFormUnsafeNativeMethods.IDispatch).IsAssignableFrom(this.PropertyType)) {
        ...
        public Object GetNativeValue(Object component){
            ...
            if (component == null || !Marshal.IsComObject(component) || !(component is WinFormUnsafeNativeMethods.IDispatch))
                return null;

            WinFormUnsafeNativeMethods.IDispatch pDisp = (WinFormUnsafeNativeMethods.IDispatch)component;
            Object[] pVarResult = new Object[1];
            WinFormNativeMethods.tagEXCEPINFO pExcepInfo = new WinFormNativeMethods.tagEXCEPINFO();
            Guid g = Guid.Empty;

            int hr = pDisp.Invoke(this.dispid,
                                  ref g,
                                  WinFormSafeNativeMethods.GetThreadLCID(),
                                  WinFormNativeMethods.DISPATCH_PROPERTYGET,
                                  new WinFormNativeMethods.tagDISPPARAMS(),
                                  pVarResult,
                                  pExcepInfo, null);

            switch (hr) {
            case WinFormNativeMethods.S_OK:
            case WinFormNativeMethods.S_FALSE:
                ...
                return lastValue;
            case WinFormNativeMethods.DISP_E_EXCEPTION:
                //PrintExceptionInfo(pExcepInfo);
                return null;
            default:
                throw new ExternalException(DCSR.GetString(DCSR.DispInvokeFailed, "GetValue" , hr), hr);
            }
        }
        ...
        public override void SetValue(Object component, Object value) {
            ...
            if (this.readOnly) {
                throw new NotSupportedException(DCSR.GetString(DCSR.COM2ReadonlyProperty, this.Name ));
            }
            ...
            if (owner == null || !Marshal.IsComObject(owner) || !(owner is WinFormUnsafeNativeMethods.IDispatch)) {
                return;
            }
            ...
            WinFormUnsafeNativeMethods.IDispatch pDisp = (WinFormUnsafeNativeMethods.IDispatch)owner;

            WinFormNativeMethods.tagDISPPARAMS dp = new WinFormNativeMethods.tagDISPPARAMS();
            WinFormNativeMethods.tagEXCEPINFO excepInfo = new WinFormNativeMethods.tagEXCEPINFO();
            dp.cArgs = 1;
            dp.cNamedArgs = 1;
            int[] namedArgs = new int[]{WinFormNativeMethods.DISPID_PROPERTYPUT};
            ...
            try {
                ...
                WinFormSafeNativeMethods.VariantInit(new HandleRef(null, mem));
                ...
                try {
                    ...
                    int hr = pDisp.Invoke(this.dispid,
                                          ref g,
                                          WinFormSafeNativeMethods.GetThreadLCID(),
                                          WinFormNativeMethods.DISPATCH_PROPERTYPUT,
                                          dp,
                                          null,
                    ...
                    if (hr == WinFormNativeMethods.DISP_E_EXCEPTION && excepInfo.scode != 0) {
                        hr = excepInfo.scode;
                        errorInfo = excepInfo.bstrDescription;
                    }

                    switch (hr) {
                    case WinFormNativeMethods.E_ABORT:
                    case WinFormNativeMethods.OLE_E_PROMPTSAVECANCELLED:
                    ...
                    default:
                        ...
                        if (pDisp is WinFormUnsafeNativeMethods.ISupportErrorInfo) {
                            g = typeof(WinFormUnsafeNativeMethods.IDispatch).GUID;
                            if (WinFormNativeMethods.Succeeded(((WinFormUnsafeNativeMethods.ISupportErrorInfo)pDisp).InterfaceSupportsErrorInfo(ref g))) {
                                WinFormUnsafeNativeMethods.IErrorInfo pErrorInfo = null;
                                WinFormUnsafeNativeMethods.GetErrorInfo(0, ref pErrorInfo);
                                string info= null;
                                if (pErrorInfo != null) {
                                    if (WinFormNativeMethods.Succeeded(pErrorInfo.GetDescription(ref info))) {
                                        errorInfo = info;
                                    }
                                }
                            ...
                            int result = WinFormSafeNativeMethods.FormatMessage(WinFormNativeMethods.FORMAT_MESSAGE_FROM_SYSTEM | 
                                                                    WinFormNativeMethods.FORMAT_MESSAGE_IGNORE_INSERTS,
                            ...
                            if (result == 0) {   
                                errorInfo = String.Format(CultureInfo.CurrentCulture, DCSR.GetString(DCSR.DispInvokeFailed, "SetValue", hr));
                            }
                            else {       
                    ...
                    WinFormSafeNativeMethods.VariantClear(new HandleRef(null, mem));
