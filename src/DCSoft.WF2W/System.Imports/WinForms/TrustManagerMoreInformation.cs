...
namespace System.Security.Policy
{
    internal class TrustManagerMoreInformation : System.Windows.Forms.Form
    {
        ...
        private void FillContent(TrustManagerPromptOptions options, String publisherName)
        {
            ...
            {
                this.lblPublisherContent.Text = DCSR.GetString(DCSR.TrustManagerMoreInfo_UnknownPublisher);
            }
            else
            {
                this.lblPublisherContent.Text = DCSR.GetString(DCSR.TrustManagerMoreInfo_KnownPublisher, publisherName);
            }

            if ((options & (TrustManagerPromptOptions.RequiresPermissions | TrustManagerPromptOptions.WillHaveFullTrust)) != 0)
            {
                this.lblMachineAccessContent.Text = DCSR.GetString(DCSR.TrustManagerMoreInfo_UnsafeAccess);
            }
            else
            {
                this.lblMachineAccessContent.Text = DCSR.GetString(DCSR.TrustManagerMoreInfo_SafeAccess);
            }

            if ((options & TrustManagerPromptOptions.AddsShortcut) != 0)
            {
                this.Text = DCSR.GetString(DCSR.TrustManagerMoreInfo_InstallTitle);
                this.lblInstallationContent.Text = DCSR.GetString(DCSR.TrustManagerMoreInfo_WithShortcut);
            }
            else
            {
                this.Text = DCSR.GetString(DCSR.TrustManagerMoreInfo_RunTitle);
                this.lblInstallationContent.Text = DCSR.GetString(DCSR.TrustManagerMoreInfo_WithoutShortcut);
            }

            string source;
            if ((options & TrustManagerPromptOptions.LocalNetworkSource) != 0)
            {
                source = DCSR.GetString(DCSR.TrustManagerMoreInfo_LocalNetworkSource);
            }
            else if ((options & TrustManagerPromptOptions.LocalComputerSource) != 0)
            {
                source = DCSR.GetString(DCSR.TrustManagerMoreInfo_LocalComputerSource);
            }
            else if ((options & TrustManagerPromptOptions.InternetSource) != 0)
            {
                source = DCSR.GetString(DCSR.TrustManagerMoreInfo_InternetSource);
            }
            else if ((options & TrustManagerPromptOptions.TrustedSitesSource) != 0)
            {
                source = DCSR.GetString(DCSR.TrustManagerMoreInfo_TrustedSitesSource);
            }
            else
            {
                Debug.Assert((options & TrustManagerPromptOptions.UntrustedSitesSource) != 0);
                source = DCSR.GetString(DCSR.TrustManagerMoreInfo_UntrustedSitesSource);
            }
            this.lblLocationContent.Text = DCSR.GetString(DCSR.TrustManagerMoreInfo_Location, source);
        }
        ...
        private static void LoadWarningBitmap(TrustManagerWarningLevel warningLevel, System.Windows.Forms.PictureBox pictureBox)
        {
            ...
            switch (warningLevel)
            {
                case TrustManagerWarningLevel.Green:
                    ...
                    pictureBox.AccessibleDescription = string.Format(CultureInfo.CurrentCulture, DCSR.GetString(DCSR.TrustManager_WarningIconAccessibleDescription_LowRisk), pictureBox.AccessibleDescription);
                    break;
                case TrustManagerWarningLevel.Yellow:
                    bitmap = new Bitmap(typeof(System.Windows.Forms.Form), "TrustManagerWarningSm.bmp");
                    pictureBox.AccessibleDescription = string.Format(CultureInfo.CurrentCulture, DCSR.GetString(DCSR.TrustManager_WarningIconAccessibleDescription_MediumRisk), pictureBox.AccessibleDescription);
                    break;
                default:
                    ...
                    pictureBox.AccessibleDescription = string.Format(CultureInfo.CurrentCulture, DCSR.GetString(DCSR.TrustManager_WarningIconAccessibleDescription_HighRisk), pictureBox.AccessibleDescription);
                    break;
            }
            if (bitmap != null)
            {
