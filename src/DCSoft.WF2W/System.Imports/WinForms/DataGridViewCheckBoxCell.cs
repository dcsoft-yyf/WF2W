    ...
    public class DataGridViewCheckBoxCell : DataGridViewCell, IDataGridViewEditingCell
    {
        ...
        public virtual object EditingCellFormattedValue
        {
            ...
            set
            {
                ...
                {
                    throw new ArgumentException(DCSR.GetString(DCSR.DataGridViewCell_FormattedValueTypeNull));
                }
                if (value == null || !this.FormattedValueType.IsAssignableFrom(value.GetType()))
                {
                    ...
                    throw new ArgumentException(DCSR.GetString(DCSR.DataGridViewCheckBoxCell_InvalidValueType));
                }
        ...
        public virtual object GetEditingCellFormattedValue(DataGridViewDataErrorContexts context)
        {
            if (this.FormattedValueType == null)
            {
                throw new InvalidOperationException(DCSR.GetString(DCSR.DataGridViewCell_FormattedValueTypeNull));
            }
            if (this.FormattedValueType.IsAssignableFrom(defaultCheckStateType))
            {
                if ((this.flags & DATAGRIDVIEWCHECKBOXCELL_checked) != 0x00)
                {
                    ...
                    {
                        return DCSR.GetString(DCSR.DataGridViewCheckBoxCell_ClipboardChecked);
                    }
                ...
                else if ((this.flags & DATAGRIDVIEWCHECKBOXCELL_indeterminate) != 0x00)
                {
                    if ((context & DataGridViewDataErrorContexts.ClipboardContent) != 0)
                    {
                        return DCSR.GetString(DCSR.DataGridViewCheckBoxCell_ClipboardIndeterminate);
                    }
                ...
                else
                {
                    if ((context & DataGridViewDataErrorContexts.ClipboardContent) != 0)
                    {
                        return DCSR.GetString(DCSR.DataGridViewCheckBoxCell_ClipboardUnchecked);
                    }
            ...
            else if (this.FormattedValueType.IsAssignableFrom(defaultBooleanType))
            {
                ...
                if ((context & DataGridViewDataErrorContexts.ClipboardContent) != 0)
                {
                    return DCSR.GetString(ret ? DCSR.DataGridViewCheckBoxCell_ClipboardTrue : DCSR.DataGridViewCheckBoxCell_ClipboardFalse);
                }
                return ret;
            }
            else
                                                    ...
                                                    DataGridViewDataErrorContexts context)
        {
            ...
            if (ret != null && (context & DataGridViewDataErrorContexts.ClipboardContent) != 0)
            {
                if (ret is bool)
                {
                    ...
                    if (retBool)
                    {
                        return DCSR.GetString(this.ThreeState ? DCSR.DataGridViewCheckBoxCell_ClipboardChecked : DCSR.DataGridViewCheckBoxCell_ClipboardTrue);
                    }
                    else
                    {
                        return DCSR.GetString(this.ThreeState ? DCSR.DataGridViewCheckBoxCell_ClipboardUnchecked : DCSR.DataGridViewCheckBoxCell_ClipboardFalse);
                    }
                }
                        ...
                        if (paint && DataGridViewCell.PaintContentForeground(paintParts))
                        {
                            ...
                            if (checkState != CheckState.Unchecked)
                            {
                                ...
                                if (checkImage == null || checkImage.Width != fullSize.Width || checkImage.Height != fullSize.Height)
                                {
                                    ...
                                    WinFormNativeMethods.RECT rcCheck = WinFormNativeMethods.RECT.FromXYWH(0, 0, fullSize.Width, fullSize.Height);
                                    ...
                                    using (Graphics offscreen = Graphics.FromImage(bitmap))
                                    {
                                        ...
                                        try
                                        {
                                            WinFormSafeNativeMethods.DrawFrameControl(new HandleRef(offscreen, dc), ref rcCheck,
            ...
            public override string DefaultAction
            {
                get
                {
                    if (!this.Owner.ReadOnly)
                    {
                        ...
                        {
                            return DCSR.GetString(DCSR.DataGridView_AccCheckBoxCellDefaultActionCheck);
                        }
                        else
                        {
                            return DCSR.GetString(DCSR.DataGridView_AccCheckBoxCellDefaultActionUncheck);
                        }
                    }
                    else
                    {
