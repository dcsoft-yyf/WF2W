...
[System.Reflection.Obfuscation(Exclude = true, ApplyToMembers = false )]
    public class TrackBar : Control, ISupportInitialize {
        ...
        DefaultValue(true),
        SRDescription(DCSR.TrackBarAutoSizeDescr),
        Browsable(true),
        ...
        protected override CreateParams CreateParams {
            ...
            get {
                ...
                cp.ClassName = WinFormNativeMethods.WC_TRACKBAR;

                switch (tickStyle) {
                    case TickStyle.None:
                        cp.Style |= WinFormNativeMethods.TBS_NOTICKS;
                        break;
                    case TickStyle.TopLeft:
                        cp.Style |= (WinFormNativeMethods.TBS_AUTOTICKS | WinFormNativeMethods.TBS_TOP);
                        break;
                    case TickStyle.BottomRight:
                        cp.Style |= (WinFormNativeMethods.TBS_AUTOTICKS | WinFormNativeMethods.TBS_BOTTOM);
                        break;
                    case TickStyle.Both:
                        cp.Style |= (WinFormNativeMethods.TBS_AUTOTICKS | WinFormNativeMethods.TBS_BOTH);
                        break;
                }

                if (orientation == Orientation.Vertical) {
                    cp.Style |= WinFormNativeMethods.TBS_VERT; // HORIZ == 0
                }
        ...
        SRCategory(DCSR.CatBehavior),
        DefaultValue(5),
        SRDescription(DCSR.TrackBarLargeChangeDescr)
        ]
        public int LargeChange {
            ...
            set {
                if (value < 0) {
                    throw new ArgumentOutOfRangeException("LargeChange", DCSR.GetString(DCSR.TrackBarLargeChangeError, value));
                }
                ...
                if (largeChange != value) {
                    ...
                    if (IsHandleCreated)
                    {
                        SendMessage(WinFormNativeMethods.TBM_SETPAGESIZE, 0, value);
                    }
                }
            }
        }
        ...
        SRDescription(DCSR.TrackBarMaximumDescr)
        ]
        public int Maximum {
            get {
                return maximum;
        ...
        DefaultValue(0),
        RefreshProperties(RefreshProperties.All),
        SRDescription(DCSR.TrackBarMinimumDescr)
        ]
        public int Minimum {
            get {
                return minimum;
        ...
        Localizable(true),
        SRDescription(DCSR.TrackBarOrientationDescr)
        ]
        ...
        private int PreferredDimension {
            get {
                int cyhscroll = WinFormUnsafeNativeMethods.GetSystemMetrics(WinFormNativeMethods.SM_CYHSCROLL);
        ...
        private void RedrawControl()
        {
            if (IsHandleCreated)
            {
                ...
                SendMessage(WinFormNativeMethods.TBM_SETRANGEMAX, 1, maximum);
                Invalidate();
            }
        }
        ...
        SRCategory(DCSR.CatBehavior),
        DefaultValue(1),
        SRDescription(DCSR.TrackBarSmallChangeDescr)
        ]
        public int SmallChange {
            ...
            set {
                if (value < 0) {
                    throw new ArgumentOutOfRangeException("SmallChange", DCSR.GetString(DCSR.TrackBarSmallChangeError, value));
                }
                if (smallChange != value) {
                    smallChange = value;
                    if (IsHandleCreated)
                    {
                        SendMessage(WinFormNativeMethods.TBM_SETLINESIZE, 0, value);
                    }
                }
            }
        }
        ...
        SRDescription(DCSR.TrackBarTickStyleDescr)
        ]
        ...
        SRCategory(DCSR.CatAppearance),
        DefaultValue(1),
        SRDescription(DCSR.TrackBarTickFrequencyDescr)
        ]
        public int TickFrequency {
            ...
            set {
                ...
                if (tickFrequency != value) {
                    tickFrequency = value;
                    if (IsHandleCreated) {
                        SendMessage(WinFormNativeMethods.TBM_SETTICFREQ, value, 0);
                        Invalidate();
                    }
                }
            }
        ...
        DefaultValue(0),
        Bindable(true),
        SRDescription(DCSR.TrackBarValueDescr)
        ]
        public int Value {
            get {
                GetTrackBarValue();
        ...
        ///    <para>[To be supplied.]</para>
        /// </devdoc>
        [SRCategory(DCSR.CatBehavior), SRDescription(DCSR.TrackBarOnScrollDescr)]
        ...
        private void GetTrackBarValue() {
            if (IsHandleCreated) {
                value = unchecked( (int) (long)SendMessage(WinFormNativeMethods.TBM_GETPOS, 0, 0));
        ...
        protected override void OnHandleCreated(EventArgs e) {
            ...
            SendMessage(WinFormNativeMethods.TBM_SETRANGEMIN, 0, minimum);
            SendMessage(WinFormNativeMethods.TBM_SETRANGEMAX, 0, maximum);
            SendMessage(WinFormNativeMethods.TBM_SETTICFREQ, tickFrequency, 0);
            SendMessage(WinFormNativeMethods.TBM_SETPAGESIZE, 0, largeChange);
            SendMessage(WinFormNativeMethods.TBM_SETLINESIZE, 0, smallChange);
            SetTrackBarPosition();
            AdjustSize();
        }
        ...
        protected override void OnMouseWheel(MouseEventArgs e) {
            ...
            Debug.Assert(this.cumulativeWheelData > -WinFormNativeMethods.WHEEL_DELTA, "cumulativeWheelData is too small");
            Debug.Assert(this.cumulativeWheelData < WinFormNativeMethods.WHEEL_DELTA, "cumulativeWheelData is too big");
            this.cumulativeWheelData += e.Delta;

            float partialNotches;
            partialNotches = (float)this.cumulativeWheelData / (float)WinFormNativeMethods.WHEEL_DELTA;
            ...
            if (scrollBands != 0) {
               ...
               if (scrollBands > 0) {
                  ...
                  this.cumulativeWheelData -= (int)((float)scrollBands * ((float)WinFormNativeMethods.WHEEL_DELTA / (float)wheelScrollLines));
               }
               else {
                  absScrollBands = -scrollBands;
        ...
        private void SetTrackBarPosition() {
            if (IsHandleCreated) {
                ...
                SendMessage(WinFormNativeMethods.TBM_SETPOS, 1, reflectedValue);
            }
        }
        ...
        protected override async  Task WndProc(Message m) {
            switch (m.Msg) {
                case WinFormNativeMethods.WM_REFLECT+WinFormNativeMethods.WM_HSCROLL:
                case WinFormNativeMethods.WM_REFLECT+WinFormNativeMethods.WM_VSCROLL:
                    switch (WinFormNativeMethods.Util.LOWORD(m.WParam)) {
                        case WinFormNativeMethods.TB_LINEUP:
                        case WinFormNativeMethods.TB_LINEDOWN:
                        case WinFormNativeMethods.TB_PAGEUP:
                        case WinFormNativeMethods.TB_PAGEDOWN:
                            //case NativeMethods.TB_THUMBPOSITION:
                        case WinFormNativeMethods.TB_THUMBTRACK:
                        case WinFormNativeMethods.TB_TOP:
                        case WinFormNativeMethods.TB_BOTTOM:
                        case WinFormNativeMethods.TB_ENDTRACK:
                            if (value != Value) {
                                OnScroll(EventArgs.Empty);
                                OnValueChanged(EventArgs.Empty);
                            }
