...
namespace System.Resources.Extensions
{
    ...
    public partial class DCDeserializingResourceReader
    {
        static DCDeserializingResourceReader()
        {
...
#pragma warning restore SYSLIB0011
        ...
        private object DeserializeObject(int typeIndex)
        {
                ...
                case SerializationFormat.BinaryFormatter:
                    {
                        ...
                        if (length < 0)
                        {
                            throw new BadImageFormatException(DCResourceSR.Format(DCResourceSR.BadImageFormat_ResourceDataLengthInvalid, length));
                        }
                ...
                case SerializationFormat.TypeConverterByteArray:
                    {
                        ...
                        if (converter == null)
                        {
                            throw new TypeLoadException(DCResourceSR.Format(DCResourceSR.TypeLoadException_CannotLoadConverter, type));
                        }
                ...
                case SerializationFormat.TypeConverterString:
                    {
                        ...
                        if(  type.IsEnum )
                        {
                            if ( Enum.TryParse( type , stringData , out value ))
                            {
                                break;
                            }
                        }
                ...
                case SerializationFormat.ActivatorStream:
                    {
                        ...
                        {
                            value = new System.Drawing.Bitmap(stream);
                            ((System.Drawing.Bitmap)value)._FromAssemblyResource = true;
                        }
                        else
                        {
            ...
            if (value.GetType() != type)
                throw new BadImageFormatException(DCResourceSR.Format(DCResourceSR.BadImageFormat_ResType_SerBlobMismatch, type.FullName, value.GetType().FullName));
            return value;
        }
    }
}
