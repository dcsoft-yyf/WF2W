...
#endif
{
    ...
    static partial class IntUnsafeNativeMethods 
    {
        ...
        //[ResourceExposure(ResourceScope.None)]
        /// <summary>
        /// 模拟 win32 API GetDeviceCaps，返回 Windows 11 下的默认值
        /// </summary>
        ...
        {
            // Windows 11 常见显示参数（逻辑默认值）
            const int defaultWidthPx = 1920;
            const int defaultHeightPx = 1080;
            const int defaultDpi = 96;
            const int defaultBitsPerPixel = 32;
            const int defaultPlanes = 1;

            // 由像素和 DPI 推导物理尺寸（毫米）
            int widthMm = (int)Math.Round(defaultWidthPx * 25.4 / defaultDpi);
            int heightMm = (int)Math.Round(defaultHeightPx * 25.4 / defaultDpi);

            switch (nIndex)
            {
                case 0:   // DRIVERVERSION
                    return 0x0A00; // 10.0
                ...
                case 112: // PHYSICALOFFSETX
                    return 0;

                case 113: // PHYSICALOFFSETY
                    return 0;

                case 114: // SCALINGFACTORX
                    return 1;

                case 115: // SCALINGFACTORY
                    return 1;

                case 116: // VREFRESH
                    return 60;

                case 117: // DESKTOPVERTRES（按 Win32 常量定义）
                    return defaultHeightPx;

                case 118: // DESKTOPHORZRES（按 Win32 常量定义）
                    return defaultWidthPx;

                case 119: // BLTALIGNMENT
                    return 0;
