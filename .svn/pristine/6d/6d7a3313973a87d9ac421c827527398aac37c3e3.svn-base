...
namespace System.Windows.Forms {
    ...
    public class Splitter : Control  {
        ...
        public override DockStyle Dock {
            ...
            set {
                ...
                if (!(value == DockStyle.Top || value == DockStyle.Bottom || value == DockStyle.Left || value == DockStyle.Right)) {
                    throw new ArgumentException(DCSR.GetString(DCSR.SplitterInvalidDockEnum));
                }
        ...
        Localizable(true),
        DefaultValue(25),
        SRDescription(DCSR.SplitterMinExtraDescr)
        ]
        public int MinExtra {
            get {
                return minExtra;
        ...
        Localizable(true),
        DefaultValue(25),
        SRDescription(DCSR.SplitterMinSizeDescr)
        ]
        public int MinSize {
            get {
                return minSize;
        ...
        SRDescription(DCSR.SplitterSplitPositionDescr)
        ]
        ...
        private void DrawSplitHelper(int splitSize) {
            ...
            IntPtr dc = WinFormUnsafeNativeMethods.GetDCEx(new HandleRef(ParentInternal, parentHandle), WinFormNativeMethods.NullHandleRef, WinFormNativeMethods.DCX_CACHE | WinFormNativeMethods.DCX_LOCKWINDOWUPDATE);
            IntPtr halftone = ControlPaint.CreateHalftoneHBRUSH();
            IntPtr saveBrush = WinFormSafeNativeMethods.SelectObject(new HandleRef(ParentInternal, dc), new HandleRef(null, halftone));
            WinFormSafeNativeMethods.PatBlt(new HandleRef(ParentInternal, dc), r.X, r.Y, r.Width, r.Height, WinFormNativeMethods.PATINVERT);
            WinFormSafeNativeMethods.SelectObject(new HandleRef(ParentInternal, dc), new HandleRef(null, saveBrush));
            WinFormSafeNativeMethods.DeleteObject(new HandleRef(null, halftone));
            WinFormUnsafeNativeMethods.ReleaseDC(new HandleRef(ParentInternal, parentHandle), new HandleRef(null, dc));
        }
        ...
        private class SplitterMessageFilter : IMessageFilter 
        {
            ...
            public bool PreFilterMessage(ref Message m) {
                if (m.Msg >= WinFormNativeMethods.WM_KEYFIRST && m.Msg <= WinFormNativeMethods.WM_KEYLAST) {
                    if (m.Msg == WinFormNativeMethods.WM_KEYDOWN && unchecked((int)(long)m.WParam) == (int)Keys.Escape) {
