...
namespace System.Windows.Forms {
    ...
    public class ToolStripMenuItem : ToolStripDropDownItem {
        ...
        SRDescription(DCSR.CheckBoxCheckedDescr)
        ]
        public bool Checked {
            get {
        ...
        internal Image CheckedImage {
         ...
         SRDescription(DCSR.ToolStripButtonCheckOnClickDescr)
         ]
        ...
        RefreshProperties(RefreshProperties.All),
        SRDescription(DCSR.CheckBoxCheckStateDescr)
        ]
        ...
        /// </devdoc>
        [SRDescription(DCSR.CheckBoxOnCheckedChangedDescr)]
        ...
        /// property changes.</para>
        /// </devdoc>
        [SRDescription(DCSR.CheckBoxOnCheckStateChangedDescr)]
        ...
        [
        SRDescription(DCSR.ToolStripMenuItemShortcutKeyDisplayStringDescr),
        SRCategory(DCSR.CatAppearance),
        DefaultValue(null),
        Localizable(true)
        ]
        ...
        private bool GetNativeMenuItemEnabled() {
            ...
            WinFormNativeMethods.MENUITEMINFO_T_RW info = new WinFormNativeMethods.MENUITEMINFO_T_RW();
            info.cbSize = Marshal.SizeOf(typeof(WinFormNativeMethods.MENUITEMINFO_T_RW));
            info.fMask = WinFormNativeMethods.MIIM_STATE;
            info.fType = WinFormNativeMethods.MIIM_STATE;
            info.wID = nativeMenuCommandID;
            WinFormUnsafeNativeMethods.GetMenuItemInfo(new HandleRef(this, nativeMenuHandle), nativeMenuCommandID, /*fByPosition instead of ID=*/ false, info);

            return ((info.fState & WinFormNativeMethods.MFS_DISABLED) == 0);
        }
        ...
        private string GetNativeMenuItemTextAndShortcut() {
            ...
            WinFormNativeMethods.MENUITEMINFO_T_RW info = new WinFormNativeMethods.MENUITEMINFO_T_RW();
            info.fMask = WinFormNativeMethods.MIIM_STRING;
            info.fType = WinFormNativeMethods.MIIM_STRING;
            info.wID = nativeMenuCommandID;
        ...
        private Image GetNativeMenuItemImage(){
            ...
            WinFormNativeMethods.MENUITEMINFO_T_RW info = new WinFormNativeMethods.MENUITEMINFO_T_RW();
            info.fMask = WinFormNativeMethods.MIIM_BITMAP;
            info.fType = WinFormNativeMethods.MIIM_BITMAP;
            info.wID = nativeMenuCommandID;
            WinFormUnsafeNativeMethods.GetMenuItemInfo(new HandleRef(this, nativeMenuHandle), nativeMenuCommandID, /*fByPosition instead of ID=*/ false, info);
      
            if (info.hbmpItem != IntPtr.Zero && info.hbmpItem.ToInt32() > WinFormNativeMethods.HBMMENU_POPUP_MINIMIZE) {
                ...
                switch (info.hbmpItem.ToInt32()) {
                    case WinFormNativeMethods.HBMMENU_MBAR_CLOSE:
                    case WinFormNativeMethods.HBMMENU_MBAR_CLOSE_D:
                    case WinFormNativeMethods.HBMMENU_POPUP_CLOSE:
                        buttonToUse = (int)CaptionButton.Close;
                        break;

                    case WinFormNativeMethods.HBMMENU_MBAR_MINIMIZE:
                    case WinFormNativeMethods.HBMMENU_MBAR_MINIMIZE_D:
                    case WinFormNativeMethods.HBMMENU_POPUP_MINIMIZE:
                        buttonToUse = (int)CaptionButton.Minimize;
                        break;

                    case WinFormNativeMethods.HBMMENU_MBAR_RESTORE:
                    case WinFormNativeMethods.HBMMENU_POPUP_RESTORE:
                        buttonToUse = (int)CaptionButton.Restore;
                        break;

                    case WinFormNativeMethods.HBMMENU_POPUP_MAXIMIZE:
                        buttonToUse = (int)CaptionButton.Maximize;
                        break;

                    case WinFormNativeMethods.HBMMENU_SYSTEM:
                        // 
                    case WinFormNativeMethods.HBMMENU_CALLBACK:
        ...
        protected override void OnClick(EventArgs e) {
            ...
            if (nativeMenuCommandID != -1) {
                ...
                if ((nativeMenuCommandID & 0xF000) != 0) {
                    ...
                    WinFormUnsafeNativeMethods.PostMessage( new HandleRef(this, targetWindowHandle), WinFormNativeMethods.WM_SYSCOMMAND,nativeMenuCommandID, 0);
                }
                else {
                    // These are user added items like ".Net Window..."
                     
                    // be consistent with sending a WM_SYSCOMMAND, use POST not SEND.
                    WinFormUnsafeNativeMethods.PostMessage( new HandleRef(this, targetWindowHandle), WinFormNativeMethods.WM_COMMAND, nativeMenuCommandID, 0);
                }
                this.Invalidate();
            }
