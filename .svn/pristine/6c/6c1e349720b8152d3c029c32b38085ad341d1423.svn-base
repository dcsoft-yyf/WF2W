...
/**
 * 绘制内容并将内容转换为图片数据
 * @param {Uint8Array} bsData
 * @param {string} 图片格式，可以为 "png,jpg,bmp"，默认为jpg
 * @returns {Uint8Array} 图片数据
 */
async function __DCPaintToImage(bsData, strFormat) {
    if (bsData == null || bsData.length == 0) {
        return null;
    }
    var reader = new DCBinaryReader(bsData);
    var intWidth = reader.ReadInt16();
    var intHeight = reader.ReadInt16();
    var element = document.createElement("CANVAS");
    element.width = intWidth;
    element.height = intHeight;
    element.style.width = intWidth + "px";
    element.style.height = intHeight + "px";
    var ctx = element.getContext("2d");
    var intBackImageFormat = reader.ReadByte();
    var bsBackImage = reader.ReadByteArray();
    if (bsBackImage != null && bsBackImage.length > 0) {
        // 在这里加载bsBackImage图片数据，然后绘制到element上面，这本身是异步函数，要异步等待图片绘制完毕,intBackImageFormat指明图片格式
        var strMimeType = null;
        switch (intBackImageFormat) {
            ...
            default: strMimeType = "image/png"; break;
        }

        var blob = new Blob([bsBackImage], { type: strMimeType });
        var strImgUrl = URL.createObjectURL(blob);
    ...
    var drawer = new DCGraphicsDrawer(canvasElement, reader);
    await drawer.RunTask();
    // 将element转换为指定格式的图片数据并返回
    var fmt = (strFormat || "jpg").toLowerCase();
    var outMimeType = "image/jpeg";
    if (fmt == "png") outMimeType = "image/png";
    else if (fmt == "bmp") outMimeType = "image/bmp";

    var outBlob = await new Promise(function (resolve) {
        element.toBlob(function (b) { resolve(b); }, outMimeType);
    });
    ...
    var arrBuffer = await outBlob.arrayBuffer();
    element = null;
    return new Uint8Array(arrBuffer);
};
/**
 * 绘制控件内容
 * @param {number} intHandle 控件句柄
 * @param {Uint8Array} bsData 图形数据
 */
function __DCPaintControlContent(intHandle, bsData) {
    var element = window.__DCWin32API.DCGetControlByHandle(intHandle, true);
    if (element != null) {
        var fact = element.__DCFactory;
        if (fact == null) return null;
        var canvasElement = fact.GetCanvasElement.call(fact ,element);
        ...
        try {
            ...
            var bolDoubleBuffer = reader.ReadBoolean();
            var bolClearAll = reader.ReadBoolean();
            var offsetX = reader.ReadInt32();
            var oofsetY = reader.ReadInt32();
            var drawer = new DCGraphicsDrawer(canvasElement, reader);
            if (bolClearAll) {
                ...
                drawer.RectangleForClear = clipRect;
            }
            if (bolDoubleBuffer) {
            //    drawer.EnableDoubleBuffer(clipRect.Left, clipRect.Top, clipRect.Width, clipRect.Height);
            }
            drawer.RunTask();
        }
        catch (e) {
...
class DCBinaryReader {
    ...
    ReadByteArrayRaw(intLen) {
        if (this.Position >= this.View.byteLength - intLen) {
            // console.log("out range$$$");
            return null;
        }
        if (intLen == 0) {
            ...
            var result = this.DataBuffer.slice(this.Position, this.Position + intLen);
            this.Position += intLen;
            return result;
        }
    }
    ReadString() {
    ...
    constructor(vCanvasElement, vCodes) {
        ...
        this.Functions[6] = function (reader) {
            ...
            if (w < 0) {
                ...
                w = - w;
                this.ctx.save();
                this.ctx.rect(x - 1, y - 100, w + 2, 2000);
                this.ctx.clip();
                this.ctx.fillText(
                    str,
                    x - 0.5,
        ...
        this.Functions[22] = function (reader) {
            ...
            this.ctx.drawImage(img, v1, v2, v3, v4);
        };
        //23 SetPageUnit
        ...
        //29 ImageTable
        this.Functions[29] = async function (reader) {
            // 加载图片列表
            var len = reader.ReadInt16();
            //this.ImageTable = new Array();
            ...
            for (var iCount = 0; iCount < len; iCount++) {
                ...
                var strImgUrl = null;
                var bolDeleteUrl = false;
                if (imgType == 5) {
                    var bsTemp = reader.ReadByteArray();
                    var flag = bsTemp[0];
                    strImgUrl = (new TextDecoder()).decode(bsTemp.slice(1));
                    bolDeleteUrl = false;
                }
                else {
                    var strMimeType = null;
                    ...
                    strImgUrl = URL.createObjectURL(blob2);
                    bolDeleteUrl = true;
                }
                var imgElemnent = document.createElement("IMG");// new Image();
                this.ImageTable.push(imgElemnent);
                imgElemnent.src = strImgUrl;
                if (imgType != 5) {
                    imgElemnent.srcTemp2 = strImgUrl;
                }

                await new Promise((resolve, reject) => {
                    if (imgElemnent.complete && imgElemnent.naturalWidth !== 0) {
                        if (bolDeleteUrl) {
                            URL.revokeObjectURL(imgElemnent.src);
                        }
                        resolve(imgElemnent);
                    }
                    else {
                        ...
                        imgElemnent.onerror = function (e) {
                            console.log(e);
                            if (bolDeleteUrl) {
                                URL.revokeObjectURL(imgElemnent.src);
                            }
        ...
        //30 BrushTable
        this.Functions[30] = async function (reader) {
            var len = reader.ReadInt16();
            this.BrushTable = new Array();
            for (var iCount = 0; iCount < len; iCount++) {
                ...
                else if (strBrush.charAt(0) == '^') {
                    this.BrushTable.push(_DC_CreateHatchBrush(strBrush));
                }
                else {
                    // 普通纯色画刷
        ...
        this.Functions[38] = null;
        this.Functions[39] = null;
        // 40 ClearRect
    ...
    async RunTask() {
        ...
        if (this.DoubleBuffer == true) {
            ...
            if (this.RectangleForClear != null) {
                var rect9 = this.RectangleForClear;
                this.RectangleForClear = null;
                if (rect9.Width > 2 && rect9.Height > 2) {
                    this.ctx.clearRect(
                        rect9.Left + 1,
                        rect9.Top + 1,
                        rect9.Width - 2,
        ...
        while (thisReader.EOF == false && this._Cancel != true) {
            ...
            if (fIndex >= 0 && fIndex < fLength) {
                ...
                if (fIndex == 29 || fIndex == 30) {
                    await func.call(thisClass, thisReader);
                }
                else {
