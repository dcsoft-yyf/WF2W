...
namespace System.Windows.Forms {
    ...
    [SRDescription(DCSR.DescriptionPrintPreviewControl)]
[System.Reflection.Obfuscation(Exclude = true, ApplyToMembers = false )]
    public class PrintPreviewControl : Control {
        ...
        DefaultValue(false),
        SRDescription(DCSR.PrintPreviewAntiAliasDescr)
        ]
        ...
        SRCategory(DCSR.CatBehavior),
        DefaultValue(true),
        SRDescription(DCSR.PrintPreviewAutoZoomDescr)
        ]
        ...
        SRCategory(DCSR.CatBehavior),
        DefaultValue(null),
        SRDescription(DCSR.PrintPreviewDocumentDescr)
        ]
        ...
        SRDescription(DCSR.PrintPreviewColumnsDescr)
        ]
        public int Columns {
            get { return columns;}
            set {
                if (value < 1 ) {
                    throw new ArgumentOutOfRangeException("Columns", DCSR.GetString(DCSR.InvalidLowBoundArgumentEx, "Columns", value.ToString(CultureInfo.CurrentCulture), (1).ToString(CultureInfo.CurrentCulture)));
                }
        ...
        SRDescription(DCSR.ControlWithScrollbarsPositionDescr)
        ]
        ...
        [
        DefaultValue(1),
        SRDescription(DCSR.PrintPreviewRowsDescr),
        SRCategory(DCSR.CatBehavior)
        ]
        public int Rows {
            get { return rows;}
            set {
                ...
                if (value < 1 ) {
                    throw new ArgumentOutOfRangeException("Rows", DCSR.GetString(DCSR.InvalidLowBoundArgumentEx, "Rows", value.ToString(CultureInfo.CurrentCulture), (1).ToString(CultureInfo.CurrentCulture)));
                }
        ...
        SRDescription(DCSR.ControlRightToLeftDescr)
        ]
        ...
        [
        DefaultValue(0),
        SRDescription(DCSR.PrintPreviewStartPageDescr),
        SRCategory(DCSR.CatBehavior)
        ]
        public int StartPage {
            ...
            set {
                if (value < 0 ) {
                    throw new ArgumentOutOfRangeException("StartPage", DCSR.GetString(DCSR.InvalidLowBoundArgumentEx, "StartPage", value.ToString(CultureInfo.CurrentCulture), (0).ToString(CultureInfo.CurrentCulture)));
                }
        ...
        /// </devdoc>
        [SRCategory(DCSR.CatPropertyChanged), SRDescription(DCSR.RadioButtonOnStartPageChangedDescr)]
        ...
        Browsable(false), EditorBrowsable(EditorBrowsableState.Never),
        DesignerSerializationVisibility(DesignerSerializationVisibility.Hidden),
        SRDescription(DCSR.ControlWithScrollbarsVirtualSizeDescr)
        ]
        ...
        [
        SRCategory(DCSR.CatBehavior),
        SRDescription(DCSR.PrintPreviewZoomDescr),
        ...
        public double Zoom {
            ...
            set {
                if (value <= 0)
                    throw new ArgumentException(DCSR.GetString(DCSR.PrintPreviewControlZoomNegative));
                autoZoom = false;
                zoom = value;
                InvalidateLayout();
            }
        ...
        private int AdjustScroll(Message m, int pos, int maxPos, bool horizontal) {
            switch (WinFormNativeMethods.Util.LOWORD(m.WParam)) {
                ...
                case WinFormNativeMethods.SB_THUMBTRACK:
                    ...
                    int direction = horizontal ? WinFormNativeMethods.SB_HORZ : WinFormNativeMethods.SB_VERT;
                    if (WinFormSafeNativeMethods.GetScrollInfo(new HandleRef(this, m.HWnd), direction, si))
                    {
                        pos = si.nTrackPos;
                    }
                    else
                    {
                        pos = WinFormNativeMethods.Util.HIWORD(m.WParam);
                    }
                    break;
        ...
        private void ComputeLayout() {
            ...
            screendpi = new Point(WinFormUnsafeNativeMethods.GetDeviceCaps(new HandleRef(tempGraphics, dc), WinFormNativeMethods.LOGPIXELSX),
                                  WinFormUnsafeNativeMethods.GetDeviceCaps(new HandleRef(tempGraphics, dc), WinFormNativeMethods.LOGPIXELSY));
        ...
        private void ComputePreview() {
            ...
            if (document == null)
                ...
                document.PrintController = new PrintControllerWithStatusDialog(previewController, 
                                                                               DCSR.GetString(DCSR.PrintControllerWithStatusDialog_DialogTitlePreview));
                    ...
                    if (pageInfo != null || exceptionPrinting) {
                        ...
                        try {
                            if (exceptionPrinting) {
                                pevent.Graphics.DrawString(DCSR.GetString(DCSR.PrintPreviewExceptionPrinting), Font, brush, ClientRectangle, format);
                            }
                            else {
                                pevent.Graphics.DrawString(DCSR.GetString(DCSR.PrintPreviewNoPages), Font, brush, ClientRectangle, format);
                            }
                        }
                        finally {
                            brush.Dispose();
        ...
        private void SetPositionNoInvalidate(Point value) {
            ...
            WinFormNativeMethods.RECT scroll = WinFormNativeMethods.RECT.FromXYWH(rect.X, rect.Y, rect.Width, rect.Height);
            ...
            WinFormUnsafeNativeMethods.SetScrollPos(new HandleRef(this, Handle), WinFormNativeMethods.SB_HORZ, position.X, true);
            WinFormUnsafeNativeMethods.SetScrollPos(new HandleRef(this, Handle), WinFormNativeMethods.SB_VERT, position.Y, true);
        }
        ...
        internal void SetVirtualSizeNoInvalidate(Size value) {
            ...
            WinFormNativeMethods.SCROLLINFO info = new WinFormNativeMethods.SCROLLINFO();
            info.fMask = WinFormNativeMethods.SIF_RANGE | WinFormNativeMethods.SIF_PAGE;
            info.nMin = 0;
            info.nMax = Math.Max(Height, virtualSize.Height) - 1;
            info.nPage = Height;
            WinFormUnsafeNativeMethods.SetScrollInfo(new HandleRef(this, Handle), WinFormNativeMethods.SB_VERT, info, true);

            info.fMask = WinFormNativeMethods.SIF_RANGE | WinFormNativeMethods.SIF_PAGE;
            info.nMin = 0;
            info.nMax = Math.Max(Width, virtualSize.Width) - 1;
            info.nPage = Width;
            WinFormUnsafeNativeMethods.SetScrollInfo(new HandleRef(this, Handle), WinFormNativeMethods.SB_HORZ, info, true);
        }
