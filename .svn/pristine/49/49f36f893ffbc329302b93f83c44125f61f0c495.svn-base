...
namespace System.Windows.Forms {
    ...
    public sealed class NotifyIcon : Component {
        ...
        private const int WM_TRAYMOUSEMESSAGE = WinFormNativeMethods.WM_USER + 1024;
        private static int WM_TASKBARCREATED = WinFormSafeNativeMethods.RegisterWindowMessage("TaskbarCreated");
        ...
        Localizable(true),
        DefaultValue(""),
        SRDescription(DCSR.NotifyIconBalloonTipTextDescr),
        ...
        SRDescription(DCSR.NotifyIconBalloonTipIconDescr)
        ]
        ...
        Localizable(true),
        DefaultValue(""),
        SRDescription(DCSR.NotifyIconBalloonTipTitleDescr)
        ]
        ...
        /// </devdoc>
        [SRCategory(DCSR.CatAction), SRDescription(DCSR.NotifyIconOnBalloonTipClickedDescr)]
        ...
        /// </devdoc>
        [SRCategory(DCSR.CatAction), SRDescription(DCSR.NotifyIconOnBalloonTipClosedDescr)]
        ...
        /// </devdoc>
        [SRCategory(DCSR.CatAction), SRDescription(DCSR.NotifyIconOnBalloonTipShownDescr)]
        ...
        SRDescription(DCSR.NotifyIconMenuDescr)
        ]
        ...
        Localizable(true),
        DefaultValue(null),
        SRDescription(DCSR.NotifyIconIconDescr)
        ]
        public Icon Icon {
            get {
                return icon;
        ...
        Localizable(true),
        DefaultValue(""),
        SRDescription(DCSR.NotifyIconTextDescr),
        Editor("System.ComponentModel.Design.MultilineStringEditor, " + AssemblyRef.SystemDesign, typeof(System.Drawing.Design.UITypeEditor))
        ]
        public string Text {
            get {
            ...
            set {
                ...
                if (value != null && !value.Equals(this.text)) {
                    if (value != null && value.Length > 63) {
                        throw new ArgumentOutOfRangeException("Text", value, DCSR.GetString(DCSR.TrayIcon_TextTooLong));
                    }
                    this.text = value;
                    if (added) {
                        UpdateIcon(true);
        ...
        Localizable(true),
        DefaultValue(false),
        SRDescription(DCSR.NotifyIconVisDescr)
        ]
        public bool Visible {
            get {
                return visible;
        ...
        /// </devdoc>
        [SRCategory(DCSR.CatAction), SRDescription(DCSR.ControlOnClickDescr)]
        ...
        ///     Occurs when the user clicks the icon in the system tray.
        /// </devdoc>
        [SRCategory(DCSR.CatAction), SRDescription(DCSR.NotifyIconMouseClickDescr)]
        ...
        /// </devdoc>
        [SRCategory(DCSR.CatAction), SRDescription(DCSR.NotifyIconMouseDoubleClickDescr)]
        ...
        protected override void Dispose(bool disposing) {
            if (disposing) {
                ...
                if (window != null && window.Handle != IntPtr.Zero) {
                    WinFormUnsafeNativeMethods.PostMessage(new HandleRef(window, window.Handle), WinFormNativeMethods.WM_CLOSE, 0, 0);
        ...
        public void ShowBalloonTip(int timeout, string tipTitle, string tipText, ToolTipIcon tipIcon) {
            ...
            if (timeout < 0) {
               throw new ArgumentOutOfRangeException("timeout", DCSR.GetString(DCSR.InvalidArgument, "timeout", (timeout).ToString(CultureInfo.CurrentCulture)));
            }

            if (string.IsNullOrEmpty(tipText))
            {
                throw new ArgumentException(DCSR.GetString(DCSR.NotifyIconEmptyOrNullTipText));
            }
           ...
           if (added ) {
                ...
                WinFormNativeMethods.NOTIFYICONDATA data = new WinFormNativeMethods.NOTIFYICONDATA();
                if (window.Handle == IntPtr.Zero) {
                   window.CreateHandle(new CreateParams());
                }
                data.hWnd = window.Handle;
                data.uID = id;
                data.uFlags = WinFormNativeMethods.NIF_INFO;
                data.uTimeoutOrVersion = timeout;
                data.szInfoTitle = tipTitle;
                data.szInfo = tipText;
                switch (tipIcon) {
                   case ToolTipIcon.Info: data.dwInfoFlags = WinFormNativeMethods.NIIF_INFO; break;
                   case ToolTipIcon.Warning: data.dwInfoFlags = WinFormNativeMethods.NIIF_WARNING; break;
                   case ToolTipIcon.Error: data.dwInfoFlags = WinFormNativeMethods.NIIF_ERROR; break;
                   case ToolTipIcon.None: data.dwInfoFlags = WinFormNativeMethods.NIIF_NONE; break;
                }
                WinFormUnsafeNativeMethods.Shell_NotifyIcon(WinFormNativeMethods.NIM_MODIFY, data);
           }
        }
        ...
        private void ShowContextMenu() {
            ...
            if (contextMenu != null || contextMenuStrip != null) {
                ...
                WinFormUnsafeNativeMethods.SetForegroundWindow(new HandleRef(window, window.Handle));
                ...
                if (contextMenu != null) {
                    ...
                    WinFormSafeNativeMethods.TrackPopupMenuEx(new HandleRef(contextMenu, contextMenu.Handle),
                                             WinFormNativeMethods.TPM_VERTICAL | WinFormNativeMethods.TPM_RIGHTALIGN,
                                             pt.x,
                                             pt.y,
                    ...
                    WinFormUnsafeNativeMethods.PostMessage(new HandleRef(window, window.Handle), WinFormNativeMethods.WM_NULL, IntPtr.Zero, IntPtr.Zero);
                }
        ...
        private void UpdateIcon(bool showIconInTray) {
            lock(syncObj) {
                ...
                data.uFlags = WinFormNativeMethods.NIF_MESSAGE;
                if (showIconInTray) {
                    if (window.Handle == IntPtr.Zero) {
                        window.CreateHandle(new CreateParams());
                    }
                ...
                data.szTip = null;
                if (icon != null) {
                    data.uFlags |= WinFormNativeMethods.NIF_ICON;
                    data.hIcon = icon.Handle;
                }
                data.uFlags |= WinFormNativeMethods.NIF_TIP;
                data.szTip = text;

                if (showIconInTray && icon != null) {
                    if (!added) {
                        WinFormUnsafeNativeMethods.Shell_NotifyIcon(WinFormNativeMethods.NIM_ADD, data);
                        added = true;
                    }
                    else {
                        WinFormUnsafeNativeMethods.Shell_NotifyIcon(WinFormNativeMethods.NIM_MODIFY, data);
                    ...
                    WinFormUnsafeNativeMethods.Shell_NotifyIcon(WinFormNativeMethods.NIM_DELETE, data);
                    added = false;
                }
            }
        }
                ...
                case WM_TRAYMOUSEMESSAGE:
                    switch ((int)msg.LParam) {
                        ...
                        case WinFormNativeMethods.WM_RBUTTONUP:
                            ...
                            break;
                        case WinFormNativeMethods.NIN_BALLOONSHOW:
                            OnBalloonTipShown();
                            break;
                        case WinFormNativeMethods.NIN_BALLOONHIDE:
                            OnBalloonTipClosed();
                            break;
                        case WinFormNativeMethods.NIN_BALLOONTIMEOUT:
                            OnBalloonTipClosed();
                            break;
                        case WinFormNativeMethods.NIN_BALLOONUSERCLICK:
                            OnBalloonTipClicked();
                            break;
                    }
                    break;
