       ...
       // [SecurityPermissionAttribute(SecurityAction.Demand,SerializationFormatter=true)]
        ...
        static string GenerateMessage(PrinterSettings settings) {
            if (settings.IsDefaultPrinter) {
                return DCSR.GetString(DCSR.InvalidPrinterException_NoDefaultPrinter);
            }
            else {
                try {
                    return DCSR.GetString(DCSR.InvalidPrinterException_InvalidPrinter, settings.PrinterName);
                }
                catch (SecurityException) {
                    return DCSR.GetString(DCSR.InvalidPrinterException_InvalidPrinter, DCSR.GetString(DCSR.CantTellPrinterName));
                }
            }
        }
    }
