            ...
            if (!Control.UseVisualStyleBackColor) {
                ...
                if (color.A == 255 && e.HDC != IntPtr.Zero) {
                    ...
                    if (DisplayInformation.BitsPerPixel > 8) {
                        WinFormNativeMethods.RECT r = new WinFormNativeMethods.RECT(bounds.X, bounds.Y, bounds.Right, bounds.Bottom);
                        WinFormSafeNativeMethods.FillRect(new HandleRef(e, e.HDC), ref r, new HandleRef(this, Control.BackColorBrush));
                        painted = true;
                    }
                }
