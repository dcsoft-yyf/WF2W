...
namespace System.Windows.Forms {
    ...
    public class StatusBarPanel : Component, ISupportInitialize {
        ...
        Localizable(true),
        SRDescription(DCSR.StatusBarPanelAlignmentDescr)
        ]
        ...
        SRDescription(DCSR.StatusBarPanelAutoSizeDescr)
        ]
        ...
        SRDescription(DCSR.StatusBarPanelBorderStyleDescr)
        ]
        ...
        DefaultValue(null),
        Localizable(true),
        SRDescription(DCSR.StatusBarPanelIconDescr)
        ]
        public Icon Icon {
            ...
            set {
                ...
                if (Created) {
                    ...
                    this.parent.SendMessage(WinFormNativeMethods.SB_SETICON, (IntPtr)GetIndex(), handle);
        ...
        Localizable(true),
        RefreshProperties(RefreshProperties.All),
        SRDescription(DCSR.StatusBarPanelMinWidthDescr)
        ]
        public int MinWidth {
            get {
            ...
            set {
                if (value < 0) {
                    throw new ArgumentOutOfRangeException("MinWidth", DCSR.GetString(DCSR.InvalidLowBoundArgumentEx, "MinWidth", value.ToString(CultureInfo.CurrentCulture), (0).ToString(CultureInfo.CurrentCulture)));
                }
	...
	[
        SRCategory(DCSR.CatAppearance),
        Localizable(true),
        SRDescription(DCSR.StatusBarPanelNameDescr)
        ]
        public string Name {
            get {
        ...
        SRDescription(DCSR.StatusBarPanelStyleDescr)
        ]
        ...
        Localizable(true),
        DefaultValue(""),
        SRDescription(DCSR.StatusBarPanelTextDescr)
        ]
        public string Text {
            get {
                if (text == null) {
        ...
        Localizable(true),
        DefaultValue(""),
        SRDescription(DCSR.StatusBarPanelToolTipTextDescr)
        ]
        ...
        SRDescription(DCSR.StatusBarPanelWidthDescr)
        ]
        public int Width {
            get {
                return this.width;
            ...
            set {
                if (!initializing && value < this.minWidth)
                    throw new ArgumentOutOfRangeException("Width", DCSR.GetString(DCSR.WidthGreaterThanMinWidth));
            ...
            if (Created) {
                ...
                switch (borderStyle) {
                    case StatusBarPanelBorderStyle.None:
                        border |= WinFormNativeMethods.SBT_NOBORDERS;
                        break;
                    case StatusBarPanelBorderStyle.Sunken:
                        break;
                    case StatusBarPanelBorderStyle.Raised:
                        border |= WinFormNativeMethods.SBT_POPOUT;
                        break;
                }
                switch (style) {
                    ...
                    case StatusBarPanelStyle.OwnerDraw:
                        border |= WinFormNativeMethods.SBT_OWNERDRAW;
                        break;
                }
                ...
                int result = (int) WinFormUnsafeNativeMethods.SendMessage(new HandleRef(parent, parent.Handle), WinFormNativeMethods.SB_SETTEXT, (IntPtr)wparam, sendText);

                if (result == 0)
                    throw new InvalidOperationException(DCSR.GetString(DCSR.UnableToSetPanelText));

                if (this.icon != null && style != StatusBarPanelStyle.OwnerDraw) {
                    this.parent.SendMessage(WinFormNativeMethods.SB_SETICON, (IntPtr)GetIndex(), this.icon.Handle);
                }
                else {
                    this.parent.SendMessage(WinFormNativeMethods.SB_SETICON, (IntPtr)GetIndex(), IntPtr.Zero);
                }

                if (style == StatusBarPanelStyle.OwnerDraw) {
                    WinFormNativeMethods.RECT rect = new WinFormNativeMethods.RECT();
                    result = (int) WinFormUnsafeNativeMethods.SendMessage(new HandleRef(parent, parent.Handle), WinFormNativeMethods.SB_GETRECT, (IntPtr)GetIndex(), ref rect);
