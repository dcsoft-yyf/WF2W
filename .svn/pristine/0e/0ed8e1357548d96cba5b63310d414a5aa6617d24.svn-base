...
namespace System.Windows.Forms {
    ...
    public class ThreadExceptionDialog : Form {
        ...
        public ThreadExceptionDialog(Exception t) {
            ...
            if (w != null) {
                messageRes = DCSR.ExDlgWarningText;
            ...
            if (t is SecurityException) {
                messageText = DCSR.GetString(messageRes, t.GetType().Name, Trim(messageText));
            }
            else {
                messageText = DCSR.GetString(messageRes, Trim(messageText));
            }

            StringBuilder detailsTextBuilder = new StringBuilder();
            string newline = "\r\n";
            string separator = DCSR.GetString(DCSR.ExDlgMsgSeperator);
            string sectionseparator = DCSR.GetString(DCSR.ExDlgMsgSectionSeperator);
            if (Application.CustomThreadExceptionHandlerAttached) {
                detailsTextBuilder.Append(DCSR.GetString(DCSR.ExDlgMsgHeaderNonSwitchable));
            }
            else {
                detailsTextBuilder.Append(DCSR.GetString(DCSR.ExDlgMsgHeaderSwitchable));
            }
            detailsTextBuilder.Append(string.Format(CultureInfo.CurrentCulture, sectionseparator, DCSR.GetString(DCSR.ExDlgMsgExceptionSection)));
            detailsTextBuilder.Append(t.ToString());
            detailsTextBuilder.Append(newline);
            detailsTextBuilder.Append(newline);
            detailsTextBuilder.Append(string.Format(CultureInfo.CurrentCulture, sectionseparator, DCSR.GetString(DCSR.ExDlgMsgLoadedAssembliesSection)));
            //new FileIOPermission(PermissionState.Unrestricted).Assert();
            try {
                foreach (Assembly asm in AppDomain.CurrentDomain.GetAssemblies()) {
                    AssemblyName name = asm.GetName();
                    string fileVer = DCSR.GetString(DCSR.NotAvailable);
                    ...
                    try {
                        ...
                        if (name.EscapedCodeBase != null && name.EscapedCodeBase.Length > 0) {
                            ...
                            if (codeBase.Scheme == "file") {
                                fileVer = FileVersionInfo.GetVersionInfo(WinFormNativeMethods.GetLocalPath(name.EscapedCodeBase)).FileVersion;
                            }
                        }
                    }
                    catch(System.IO.FileNotFoundException){
                    }
                    detailsTextBuilder.Append(DCSR.GetString(DCSR.ExDlgMsgLoadedAssembliesEntry, name.Name, name.Version, fileVer, name.EscapedCodeBase));
            ...
            detailsTextBuilder.Append(string.Format(CultureInfo.CurrentCulture, sectionseparator, DCSR.GetString(DCSR.ExDlgMsgJITDebuggingSection)));
            if (Application.CustomThreadExceptionHandlerAttached) {
                detailsTextBuilder.Append(DCSR.GetString(DCSR.ExDlgMsgFooterNonSwitchable));
            }
            else {
                detailsTextBuilder.Append(DCSR.GetString(DCSR.ExDlgMsgFooterSwitchable));
            }
            ...
            try {
                ...
                if (activeForm == null || activeForm.Text.Length == 0) {
                    Text = DCSR.GetString(DCSR.ExDlgCaption);
                }
                else {
                    Text = DCSR.GetString(DCSR.ExDlgCaption2, activeForm.Text);
                }
            }
            finally {
                //CodeAccessPermission.RevertAssert();
