...
namespace System.Windows.Forms {
    ...
    public class Help {
        ...
        public static void ShowPopup(Control parent, string caption, Point location) {
            ...
            WinFormNativeMethods.HH_POPUP pop = new WinFormNativeMethods.HH_POPUP();
            ...
            try {
                pop.pszText = pszText;
                pop.idString = 0;
                pop.pt = new WinFormNativeMethods.POINT(location.X, location.Y);
        ...
        private static void ShowHTML10Help(Control parent, string url, HelpNavigator command, object param) {
            ...
            if (file == null || file.IsFile) {
                ...
                uint requiredStringSize = WinFormUnsafeNativeMethods.GetShortPathName(localPath, newPath, 0);
                if (requiredStringSize > 0) {
                    //It's able to make it a short path.  Happy day.
                    newPath.Capacity = (int)requiredStringSize;
                    requiredStringSize = WinFormUnsafeNativeMethods.GetShortPathName(localPath, newPath, requiredStringSize);
            ...
            if (parent != null) {
                ...
                handle = new HandleRef(null, WinFormUnsafeNativeMethods.GetActiveWindow());
            }
            ...
            if (stringParam != null) {
                ...
                if (stringHtmlParam != null) {
                    WinFormSafeNativeMethods.HtmlHelp(handle, pathAndFileName, htmlCommand, stringHtmlParam);
                }
                else if (htmlParam is int)  {
                    WinFormSafeNativeMethods.HtmlHelp(handle, pathAndFileName, htmlCommand, (int)htmlParam);
                }
                else if (htmlParam is WinFormNativeMethods.HH_FTS_QUERY) {
                    WinFormSafeNativeMethods.HtmlHelp(handle, pathAndFileName, htmlCommand, (WinFormNativeMethods.HH_FTS_QUERY)htmlParam);
                }
                else if (htmlParam is WinFormNativeMethods.HH_AKLINK) {
                    // According to MSDN documentation, we have to ensure that the help window is up
                    // before we call ALINK lookup.
                    //
                    WinFormSafeNativeMethods.HtmlHelp(WinFormNativeMethods.NullHandleRef, pathAndFileName, HH_DISPLAY_TOPIC, (string)null);
                    WinFormSafeNativeMethods.HtmlHelp(handle, pathAndFileName, htmlCommand, (WinFormNativeMethods.HH_AKLINK)htmlParam);
                }
                else {
                    Debug.Fail("Cannot handle HTML parameter of type: " + htmlParam.GetType());
                    WinFormSafeNativeMethods.HtmlHelp(handle, pathAndFileName, htmlCommand, (string)param);
                }
            }
            else if (param == null ) {
                WinFormSafeNativeMethods.HtmlHelp(handle, pathAndFileName, MapCommandToHTMLCommand(command, null, out htmlParam), 0);
            }
            else if (param is WinFormNativeMethods.HH_POPUP) {
                WinFormSafeNativeMethods.HtmlHelp(handle, pathAndFileName, HH_DISPLAY_TEXT_POPUP, (WinFormNativeMethods.HH_POPUP)param);
            }
            else if (param.GetType() == typeof(Int32)) {
                throw new ArgumentException(DCSR.GetString(DCSR.InvalidArgument, "param", "Integer"));
            }
        }
        ...
        private static void ShowHTMLFile(Control parent, string url, HelpNavigator command, object param) {
            ...
            if (file == null) {
                throw new ArgumentException(DCSR.GetString(DCSR.HelpInvalidURL, url), "url");
            }
            ...
            WinFormUnsafeNativeMethods.ShellExecute_NoBFM(handle, null, file.ToString(), null, null, WinFormNativeMethods.SW_NORMAL);
        }
        ...
        private static Uri Resolve(string partialUri) {
            ...
            if (file != null && file.Scheme == "file") {
                string localPath = WinFormNativeMethods.GetLocalPath(partialUri);
        ...
        private static int MapCommandToHTMLCommand(HelpNavigator command, string param, out object htmlParam) {
            ...
            switch (command) {
                ...
                case HelpNavigator.Find: {
                    WinFormNativeMethods.HH_FTS_QUERY ftsQuery = new WinFormNativeMethods.HH_FTS_QUERY();
	        ...
	        case HelpNavigator.TopicId: {
                ...
                case HelpNavigator.AssociateIndex: {
                    WinFormNativeMethods.HH_AKLINK alink = new WinFormNativeMethods.HH_AKLINK();
