...
namespace System.Windows.Forms 
{
    ...
    public sealed class FolderBrowserDialog : CommonDialog
    {
        ...
        private WinFormUnsafeNativeMethods.BrowseCallbackProc callback;
        ...
        DefaultValue(true),
        Localizable(false),
        SRCategory(DCSR.CatFolderBrowsing),
        SRDescription(DCSR.FolderBrowserDialogShowNewFolderButton)
        ]
        ...
        Localizable(true),
        SRCategory(DCSR.CatFolderBrowsing),
        SRDescription(DCSR.FolderBrowserDialogSelectedPath)
        ]
        ...
        Localizable(false),
        SRCategory(DCSR.CatFolderBrowsing),
        SRDescription(DCSR.FolderBrowserDialogRootFolder),
        ...
        Localizable(true),
        SRCategory(DCSR.CatFolderBrowsing),
        SRDescription(DCSR.FolderBrowserDialogDescription)
        ]
        ...
        /// </devdoc>
        private static WinFormUnsafeNativeMethods.IMalloc GetSHMalloc()
        {
            WinFormUnsafeNativeMethods.IMalloc[] malloc = new WinFormUnsafeNativeMethods.IMalloc[1];
            WinFormUnsafeNativeMethods.Shell32.SHGetMalloc(malloc);
            return malloc[0];
        }
        ...
        protected override bool RunDialog(IntPtr hWndOwner) 
        {
            ...
            WinFormUnsafeNativeMethods.Shell32.SHGetSpecialFolderLocation(hWndOwner, (int) rootFolder, ref pidlRoot);
            if (pidlRoot == IntPtr.Zero)
            {
                WinFormUnsafeNativeMethods.Shell32.SHGetSpecialFolderLocation(hWndOwner, WinFormNativeMethods.CSIDL_DESKTOP, ref pidlRoot);
                if (pidlRoot == IntPtr.Zero)
                {
                    throw new InvalidOperationException(DCSR.GetString(DCSR.FolderBrowserDialogNoRootFolder));
                }
            }

            int mergedOptions = unchecked( (int) (long)WinFormUnsafeNativeMethods.BrowseInfos.NewDialogStyle);
            if (!showNewFolderButton)
            {
                mergedOptions += unchecked( (int) (long)WinFormUnsafeNativeMethods.BrowseInfos.HideNewFolderButton);
            }
            ...
            try
            {
                ...
                WinFormUnsafeNativeMethods.BROWSEINFO bi = new WinFormUnsafeNativeMethods.BROWSEINFO();
    
                pszDisplayName = Marshal.AllocHGlobal(WinFormNativeMethods.MAX_PATH * Marshal.SystemDefaultCharSize);
                pszSelectedPath = Marshal.AllocHGlobal(WinFormNativeMethods.MAX_PATH * Marshal.SystemDefaultCharSize);
                this.callback = new WinFormUnsafeNativeMethods.BrowseCallbackProc(this.FolderBrowserDialog_BrowseCallbackProc);
                ...
                // And show the dialog
                pidlRet = WinFormUnsafeNativeMethods.Shell32.SHBrowseForFolder(bi);
    
                if (pidlRet != IntPtr.Zero)
                {
                    // Then retrieve the path from the IDList
                    WinFormUnsafeNativeMethods.Shell32.SHGetPathFromIDList(pidlRet, pszSelectedPath);
            ...
            {
                WinFormUnsafeNativeMethods.CoTaskMemFree(pidlRoot);
                if (pidlRet != IntPtr.Zero)
                {
                    WinFormUnsafeNativeMethods.CoTaskMemFree(pidlRet);
                }
                                                           ...
                                                           IntPtr lpData)
        {
            switch (msg)
            {
                case WinFormNativeMethods.BFFM_INITIALIZED: 
                    ...
                    if (selectedPath.Length != 0) 
                    {
                        ...
                        WinFormUnsafeNativeMethods.SendMessage(new HandleRef(null, hwnd), (int) WinFormNativeMethods.BFFM_SETSELECTION, 1, selectedPath);
                    }
                    break;
                case WinFormNativeMethods.BFFM_SELCHANGED: 
                    ...
                    {
                        IntPtr pszSelectedPath = Marshal.AllocHGlobal(WinFormNativeMethods.MAX_PATH * Marshal.SystemDefaultCharSize);
                        // Try to retrieve the path from the IDList
                        bool isFileSystemFolder = WinFormUnsafeNativeMethods.Shell32.SHGetPathFromIDList(selectedPidl, pszSelectedPath);
                        Marshal.FreeHGlobal(pszSelectedPath);
                        WinFormUnsafeNativeMethods.SendMessage(new HandleRef(null, hwnd), (int) WinFormNativeMethods.BFFM_ENABLEOK, 0, isFileSystemFolder ? 1 : 0);
                    }
                    break;
            }
            return 0;
