...
#endif
{
    ...
    sealed partial class WindowsGraphics : MarshalByRefObject, IDisposable, IDeviceContext
    {
        ...
        {     
            ...
            // Ensure proposedSize has sensible minimums
            if (proposedSize.Width <= 0) proposedSize.Width = 1;
            if (proposedSize.Height <= 0) proposedSize.Height = 1;

            // Map IntTextFormatFlags to StringFormat
            using (Graphics g = Graphics.FromHdc(this.dc.Hdc))
            {
                StringFormat sf = new StringFormat(StringFormat.GenericDefault);

                // Horizontal alignment
                if ((flags & IntTextFormatFlags.HorizontalCenter) != 0)
                {
                    sf.Alignment = StringAlignment.Center;
                }
                else if ((flags & IntTextFormatFlags.Right) != 0)
                {
                ...
                // Vertical alignment
                if ((flags & IntTextFormatFlags.VerticalCenter) != 0)
                {
                    sf.LineAlignment = StringAlignment.Center;
                }
                else if ((flags & IntTextFormatFlags.Bottom) != 0)
                {
                ...
                bool singleLine = (flags & IntTextFormatFlags.SingleLine) != 0;
                if (singleLine)
                {
                    sf.FormatFlags |= StringFormatFlags.NoWrap;
                }
                if ((flags & IntTextFormatFlags.WordBreak) == 0)
                {
                    // If no word break requested, allow win32-like breaking by characters when constrained
                    sf.FormatFlags |= StringFormatFlags.NoClip; // prefer not to clip; MeasureString will still return needed size
                }

                // Direction
                if ((flags & IntTextFormatFlags.RightToLeft) != 0)
                {
                    sf.FormatFlags |= StringFormatFlags.DirectionRightToLeft;
                }

                // Measure trailing spaces similar to TextFormatFlags
                sf.FormatFlags |= StringFormatFlags.MeasureTrailingSpaces;

                // Convert WindowsFont to System.Drawing.Font
                Font sysFont = null;
                try
                {
                    WindowsFont wfont = font ?? this.dc.Font;
                    sysFont = wfont.ToFont();

                    SizeF measured;
                    if (proposedSize.Width >= MaxSize.Width && proposedSize.Height >= MaxSize.Height)
                    {
                        measured = g.MeasureString(text, sysFont, int.MaxValue, sf);
                    }
                    else if (proposedSize.Width >= MaxSize.Width)
                    {
                        // unconstrained width, constrain height minimally
                        measured = g.MeasureString(text, sysFont, new SizeF(float.MaxValue, proposedSize.Height), sf);
                    }
                    else
                    {
                        measured = g.MeasureString(text, sysFont, new SizeF(proposedSize.Width, proposedSize.Height), sf);
                    }

                    // Round up to integer pixels
                    int w = (int)Math.Ceiling(measured.Width);
                    int h = (int)Math.Ceiling(measured.Height);
                    return new Size(w, h);
                }
                finally
                {
