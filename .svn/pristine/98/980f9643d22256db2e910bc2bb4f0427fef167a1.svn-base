...
namespace System.Windows.Forms
{
    ...
    public partial class FileDialog
    {
        ...
        internal bool UseVistaDialogInternal
        {
            get 
            {
                if (WinFormUnsafeNativeMethods.IsVista && this._autoUpgradeEnabled && SettingsSupportVistaDialog)
                {
        ...
        private FileDialogNative.FOS GetOptions()
        {
            ...
            const int UnexpectedOptions = 
                WinFormNativeMethods.OFN_USESHELLITEM //This is totally bogus (only used in FileDialog by accident to ensure that places are shown
              | WinFormNativeMethods.OFN_SHOWHELP //If ShowHelp is true, we don't use the Vista Dialog
              | WinFormNativeMethods.OFN_ENABLEHOOK //These shouldn't be set in options (only set in the flags for the legacy dialog)
              | WinFormNativeMethods.OFN_ENABLESIZING //These shouldn't be set in options (only set in the flags for the legacy dialog)
              | WinFormNativeMethods.OFN_EXPLORER //These shouldn't be set in options (only set in the flags for the legacy dialog)
            ;
        ...
        private bool HandleVistaFileOk(FileDialogNative.IFileDialog dialog)
        {
            ...
            finally
            {
                ...
                {
                    if (0 != (options & WinFormNativeMethods.OFN_HIDEREADONLY))
                    {
                        ...
                        options &= ~ WinFormNativeMethods.OFN_READONLY;
                    }
                }
            }
            return ok;
        ...
        private class VistaDialogEvents : FileDialogNative.IFileDialogEvents
        {
            ...
            {
                return this._dialog.HandleVistaFileOk(pfd) ? WinFormNativeMethods.S_OK : WinFormNativeMethods.S_FALSE;
            }
        ...
        internal static FileDialogNative.IShellItem GetShellItemForPath(string path)
        {
            ...
            uint zero = 0;
            if (0 <= WinFormUnsafeNativeMethods.Shell32.SHILCreateFromPath(path, out pidl, ref zero))
            {
                if (0 <= WinFormUnsafeNativeMethods.Shell32.SHCreateShellItem(
