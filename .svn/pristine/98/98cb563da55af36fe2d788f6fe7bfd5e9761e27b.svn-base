...
namespace System.Windows.Forms {
    ...
    public class ToolBarButton : Component {
        ...
        SRDescription(DCSR.ToolBarButtonMenuDescr)
        ]
        public Menu DropDownMenu {
            ...
            set {
                ...
                if (value != null && !(value is ContextMenu)) {
                    throw new ArgumentException(DCSR.GetString(DCSR.ToolBarButtonInvalidDropDownMenuType));
                }
                dropDownMenu = value;
            }
        }
        ...
        DefaultValue(true),
        Localizable(true),
        SRDescription(DCSR.ToolBarButtonEnabledDescr)
        ]
        public bool Enabled {
            get {
                return enabled;
            ...
            set {
                if (enabled != value) {
                    ...
                    if (parent != null && parent.IsHandleCreated) {
                        parent.SendMessage(WinFormNativeMethods.TB_ENABLEBUTTON, FindButtonIndex(),
                            enabled ? 1 : 0);
                    }
                }
            }
        ...
        Localizable(true),
        SRDescription(DCSR.ToolBarButtonImageIndexDescr)
        ]
        public int ImageIndex {
            ...
            set {
                if (ImageIndexer.Index != value) {
                    if (value < -1)
                        throw new ArgumentOutOfRangeException("ImageIndex", DCSR.GetString(DCSR.InvalidLowBoundArgumentEx, "ImageIndex", (value).ToString(CultureInfo.CurrentCulture), -1));
        ...
        [
        DefaultValue(false),
        SRDescription(DCSR.ToolBarButtonPartialPushDescr)
        ]
        public bool PartialPush {
            get {
                if (parent == null || !parent.IsHandleCreated)
                    ...
                    if ((int)parent.SendMessage(WinFormNativeMethods.TB_ISBUTTONINDETERMINATE, FindButtonIndex(), 0) != 0)
                        partialPush = true;
                    else
                        partialPush = false;
        ...
        [
        DefaultValue(false),
        SRDescription(DCSR.ToolBarButtonPushedDescr)
        ]
        public bool Pushed {
            get {
                if (parent == null || !parent.IsHandleCreated)
        ...
        public Rectangle Rectangle {
            get {
                if (parent != null) {
                    ...
                    WinFormUnsafeNativeMethods.SendMessage(new HandleRef(parent, parent.Handle), WinFormNativeMethods.TB_GETRECT, FindButtonIndex(), ref rc);
        ...
        SRDescription(DCSR.ToolBarButtonStyleDescr),
        RefreshProperties(RefreshProperties.Repaint)
        ]
        ...
        Localizable(true),
        DefaultValue(""),
        SRDescription(DCSR.ToolBarButtonTextDescr)
        ]
        public string Text {
            get {
        ...
        Localizable(true),
        DefaultValue(""),
        SRDescription(DCSR.ToolBarButtonToolTipTextDescr)
        ]
        ...
        DefaultValue(true),
        Localizable(true),
        SRDescription(DCSR.ToolBarButtonVisibleDescr)
        ]
        public bool Visible {
            get {
                return visible;
        ...
        internal int GetButtonWidth() {
            ...
            WinFormNativeMethods.TBBUTTONINFO button = new WinFormNativeMethods.TBBUTTONINFO();
            button.cbSize = Marshal.SizeOf(typeof(WinFormNativeMethods.TBBUTTONINFO));
            button.dwMask = WinFormNativeMethods.TBIF_SIZE;
            
            int buttonID = (int)WinFormUnsafeNativeMethods.SendMessage(new HandleRef(Parent, Parent.Handle), WinFormNativeMethods.TB_GETBUTTONINFO, commandId, ref button);
            if (buttonID != -1) {
        ...
        {
            if ((int)parent.SendMessage(WinFormNativeMethods.TB_ISBUTTONCHECKED, FindButtonIndex(), 0) != 0) {
                pushed = true;
            }
            else {
                pushed = false;
        ...
        /// </devdoc>
        internal WinFormNativeMethods.TBBUTTON GetTBBUTTON(int commandId) {

            WinFormNativeMethods.TBBUTTON button = new WinFormNativeMethods.TBBUTTON();
            ...
            //
            button.fsState = 0;
            if (enabled) button.fsState |= WinFormNativeMethods.TBSTATE_ENABLED;
            if (partialPush && style == ToolBarButtonStyle.ToggleButton) button.fsState |= WinFormNativeMethods.TBSTATE_INDETERMINATE;
            if (pushed) button.fsState |= WinFormNativeMethods.TBSTATE_CHECKED;
            if (!visible) button.fsState |= WinFormNativeMethods.TBSTATE_HIDDEN;

            // set the button style
            //
            switch (style) {
                case ToolBarButtonStyle.PushButton:
                    button.fsStyle = WinFormNativeMethods.TBSTYLE_BUTTON;
                    break;
                case ToolBarButtonStyle.ToggleButton:
                    button.fsStyle = WinFormNativeMethods.TBSTYLE_CHECK;
                    break;
                case ToolBarButtonStyle.Separator:
                    button.fsStyle = WinFormNativeMethods.TBSTYLE_SEP;
                    break;
                case ToolBarButtonStyle.DropDownButton:
                    button.fsStyle = WinFormNativeMethods.TBSTYLE_DROPDOWN;
                    break;
        ...
        /// </devdoc>
        internal WinFormNativeMethods.TBBUTTONINFO GetTBBUTTONINFO(bool updateText, int newCommandId) {

            WinFormNativeMethods.TBBUTTONINFO button = new WinFormNativeMethods.TBBUTTONINFO();
            button.cbSize = Marshal.SizeOf(typeof(WinFormNativeMethods.TBBUTTONINFO));
            button.dwMask = WinFormNativeMethods.TBIF_IMAGE
                            | WinFormNativeMethods.TBIF_STATE | WinFormNativeMethods.TBIF_STYLE;
            ...
            // update the text.
            if (updateText) {
                button.dwMask |= WinFormNativeMethods.TBIF_TEXT;
            }
            ...
            if (newCommandId != commandId) {
                ...
                button.dwMask |= WinFormNativeMethods.TBIF_COMMAND;
            }
