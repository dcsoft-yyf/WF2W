...
namespace System.Windows.Forms.ComponentModel.Com2Interop {
    ...
    internal class Com2IPerPropertyBrowsingHandler : Com2ExtendedBrowsingHandler {
        public override Type Interface {
            get {
                return typeof(WinFormNativeMethods.IPerPropertyBrowsing);
            }
        }
        ...
        private Guid GetPropertyPageGuid(WinFormNativeMethods.IPerPropertyBrowsing target, int dispid) {
        ...
        internal static string GetDisplayString(WinFormNativeMethods.IPerPropertyBrowsing ppb, int dispid, ref bool success) {
        ...
        private void OnGetBaseAttributes(Com2PropertyDescriptor sender, GetAttributesEvent attrEvent) {
            WinFormNativeMethods.IPerPropertyBrowsing target = sender.TargetObject as WinFormNativeMethods.IPerPropertyBrowsing;
            if (target != null) {
                ...
                if (sender.CanShow && validPropPage) {
                    if (typeof(WinFormUnsafeNativeMethods.IDispatch).IsAssignableFrom(sender.PropertyType)) {
        ...
        private void OnGetDisplayValue(Com2PropertyDescriptor sender, GetNameItemEvent gnievent) {
            try {
                if (sender.TargetObject is WinFormNativeMethods.IPerPropertyBrowsing) {
                    ...
                    string displayString = GetDisplayString((WinFormNativeMethods.IPerPropertyBrowsing)sender.TargetObject, sender.DISPID, ref success);
        ...
        private void OnGetTypeConverterAndTypeEditor(Com2PropertyDescriptor sender, GetTypeConverterAndTypeEditorEvent gveevent) {
            if (sender.TargetObject is WinFormNativeMethods.IPerPropertyBrowsing) {
                WinFormNativeMethods.IPerPropertyBrowsing ppb = (WinFormNativeMethods.IPerPropertyBrowsing)sender.TargetObject;

                bool hasStrings = false;

                // check for enums
                WinFormNativeMethods.CA_STRUCT caStrings = new WinFormNativeMethods.CA_STRUCT();
                WinFormNativeMethods.CA_STRUCT caCookies = new WinFormNativeMethods.CA_STRUCT();
             ...
             public override object ConvertTo(ITypeDescriptorContext context, CultureInfo culture, object value, Type destType) {
                 ...
                 if (destType == typeof(string) && !itemsEnum.arraysFetched) {
                     ...
                     if (curValue == value || (curValue != null && curValue.Equals(value))) {
                         bool success = false;
                         string val = GetDisplayString((WinFormNativeMethods.IPerPropertyBrowsing)itemsEnum.target.TargetObject, itemsEnum.target.DISPID, ref success);
                         if (success) {
                             return val;
                         }
                     }
         ...
         private class Com2IPerPropertyBrowsingEnum : Com2Enum {
             ...
             private void EnsureArrays() {
                ...
                try {
                    ...
                    WinFormNativeMethods.IPerPropertyBrowsing ppb = (WinFormNativeMethods.IPerPropertyBrowsing)target.TargetObject;
                    int itemCount = 0;
                    ...
                    if (nameItems.Length > 0){
                        ...
                        WinFormNativeMethods.VARIANT var = new WinFormNativeMethods.VARIANT();
                        int cookie;
                        ...
                        for (int i = nameItems.Length - 1; i >= 0; i--) {
                            ...
                            var.vt = (short)WinFormNativeMethods.tagVT.VT_EMPTY;
                            int hr = ppb.GetPredefinedValue(target.DISPID, cookie, var);
                            if (hr == WinFormNativeMethods.S_OK && var.vt != (short)WinFormNativeMethods.tagVT.VT_EMPTY) {
             ...
             public override string ToString(object v) {
                ...
                if (target.IsCurrentValue(v)) {
                    ...
                    string displayString = Com2IPerPropertyBrowsingHandler.GetDisplayString((WinFormNativeMethods.IPerPropertyBrowsing)target.TargetObject, target.DISPID, ref success);
