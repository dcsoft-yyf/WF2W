...
[System.Reflection.Obfuscation(Exclude = true, ApplyToMembers = false )]
    public class ToolStripPanel : ContainerControl, IArrangedElement {
        ...
        [SRCategory(DCSR.CatAppearance), SRDescription(DCSR.ToolStripRendererChanged)]
        // PM team has reviewed and decided on naming changes already
        [SuppressMessage("Microsoft.Naming", "CA1704:IdentifiersShouldBeSpelledCorrectly")]
        ...
        SRDescription(DCSR.ToolStripPanelRowsDescr)
        ]
            ...
            private class FeedbackDropDown : ToolStripDropDown {
                    ...
                    if (!IsDisposed) {
                        if (_numPaintsServiced == 0) { // protect against re-entrancy.
                            try {
                                ...
                                while (WinFormUnsafeNativeMethods.PeekMessage(ref msg, new HandleRef(this, IntPtr.Zero),
                                                           WinFormNativeMethods.WM_PAINT, WinFormNativeMethods.WM_PAINT,
                                                           WinFormNativeMethods.PM_REMOVE)) {
                                    WinFormSafeNativeMethods.UpdateWindow(new HandleRef(null, msg.hwnd));    
                ...
                protected override async  Task WndProc(Message m){                
                    if (m.Msg == WinFormNativeMethods.WM_NCHITTEST){
                        m.Result = (IntPtr)WinFormNativeMethods.HTTRANSPARENT;
                    }
        ...
        public void Join(ToolStrip toolStripToDrag, int row) {
            if (row < 0) {
                throw new ArgumentOutOfRangeException("row", DCSR.GetString(DCSR.IndexOutOfRange, row.ToString(CultureInfo.CurrentCulture)));
            }
