...
[System.Reflection.Obfuscation(Exclude = true, ApplyToMembers = false )]
    public class ToolBar : Control {
        ...
        Localizable(true),
        SRDescription(DCSR.ToolBarAppearanceDescr)
        ]
        ...
        DefaultValue(true),
        Localizable(true),
        SRDescription(DCSR.ToolBarAutoSizeDescr),
        Browsable(true), 
        ...
        SRDescription(DCSR.ToolBarBorderStyleDescr)
        ]
        ...
        Localizable(true),
        SRDescription(DCSR.ToolBarButtonsDescr),
        ...
        Localizable(true),
        SRDescription(DCSR.ToolBarButtonSizeDescr)
        ]
        public Size ButtonSize {
            get {
                if (buttonSize.IsEmpty) {
                    ...
                    if (IsHandleCreated && buttons != null && buttonCount > 0) {
                        int result = unchecked( (int) (long)SendMessage(WinFormNativeMethods.TB_GETBUTTONSIZE, 0, 0));
                        if (result > 0) {
                            return new Size(WinFormNativeMethods.Util.LOWORD(result), WinFormNativeMethods.Util.HIWORD(result));
                        }
                    }
            ...
            set {
                ...
                if (value.Width < 0 || value.Height < 0)
                    throw new ArgumentOutOfRangeException("ButtonSize", DCSR.GetString(DCSR.InvalidArgument, "ButtonSize", value.ToString()));
        ...
        protected override CreateParams CreateParams {
            ...
            get {
                ...
                cp.ClassName = WinFormNativeMethods.WC_TOOLBAR;

                // windows forms has it's own docking code.
                //
                cp.Style |= WinFormNativeMethods.CCS_NOPARENTALIGN
                            | WinFormNativeMethods.CCS_NORESIZE;
                // | NativeMethods.WS_CHILD was commented out since setTopLevel should be able to work.

                if (!Divider) cp.Style |= WinFormNativeMethods.CCS_NODIVIDER ;
                if (Wrappable) cp.Style |= WinFormNativeMethods.TBSTYLE_WRAPPABLE;
                if (ShowToolTips && !DesignMode) cp.Style |= WinFormNativeMethods.TBSTYLE_TOOLTIPS;
                ...
                switch (appearance) {
                    ...
                    case ToolBarAppearance.Flat:
                        cp.Style |= WinFormNativeMethods.TBSTYLE_FLAT;
                        break;
                }
                ...
                switch (textAlign) {
                    ...
                    case ToolBarTextAlign.Right:
                        cp.Style |= WinFormNativeMethods.TBSTYLE_LIST;
                        break;
                }
        ...
        SRCategory(DCSR.CatAppearance),
        DefaultValue(true),
        SRDescription(DCSR.ToolBarDividerDescr)
        ]
        public bool Divider {
            get {
        ...
        SRCategory(DCSR.CatAppearance),
        Localizable(true),
        SRDescription(DCSR.ToolBarDropDownArrowsDescr)
        ]
        ...
        SRCategory(DCSR.CatBehavior),
        DefaultValue(null),
        SRDescription(DCSR.ToolBarImageListDescr)
        ]
        ...
        SRDescription(DCSR.ToolBarImageSizeDescr)
        ]
        ...
        internal int PreferredHeight {
            get {
                ...
                if (buttons == null || buttonCount == 0 || !IsHandleCreated) {
                    ...
                    SendMessage(WinFormNativeMethods.TB_GETRECT, firstVisible, ref rect);
                ...
                if (Wrappable && IsHandleCreated) {
                    height = height * unchecked( (int) (long)SendMessage(WinFormNativeMethods.TB_GETROWS, 0, 0));
                }
                ...
                if (maxWidth == -1) {
                    ...
                    if (!IsHandleCreated || buttons == null)
                        ...
                        for (int x = 0; x < buttonCount; x++) {
                            SendMessage(WinFormNativeMethods.TB_GETRECT, 0, ref rect);
        ...
        DefaultValue(false),
        Localizable(true),
        SRDescription(DCSR.ToolBarShowToolTipsDescr)
        ]
        ...
        Localizable(true),
        SRDescription(DCSR.ToolBarTextAlignDescr)
        ]
        ...
        DefaultValue(true),
        Localizable(true),
        SRDescription(DCSR.ToolBarWrappableDescr)
        ]
        ...
        /// </devdoc>
        [SRCategory(DCSR.CatBehavior), SRDescription(DCSR.ToolBarButtonClickDescr)]
        ...
        /// </devdoc>
        [SRCategory(DCSR.CatBehavior), SRDescription(DCSR.ToolBarButtonDropDownDescr)]
        ...
        protected override void CreateHandle() {
            if (!RecreatingHandle) {
                ...
                try {
                    ...
                    icc.dwICC = WinFormNativeMethods.ICC_BAR_CLASSES;
        ...
        private void ForceButtonWidths() {
            ...
            if (buttons != null && buttonSize.IsEmpty && IsHandleCreated) {
                ...
                for (int x = 0; x < buttonCount; x++) {
                    ...
                    WinFormNativeMethods.TBBUTTONINFO tbbi = new WinFormNativeMethods.TBBUTTONINFO();
                    tbbi.cbSize = Marshal.SizeOf(typeof(WinFormNativeMethods.TBBUTTONINFO));
                    ...
                    tbbi.dwMask = WinFormNativeMethods.TBIF_SIZE;
                    WinFormUnsafeNativeMethods.SendMessage(new HandleRef(this, Handle), WinFormNativeMethods.TB_SETBUTTONINFO, x, ref tbbi);
                }
            }
        }
        ...
        private void InsertButton(int index, ToolBarButton value) {
            ...
            Insert(index, value);
            if (IsHandleCreated) {
                WinFormNativeMethods.TBBUTTON tbbutton = value.GetTBBUTTON(index);
                WinFormUnsafeNativeMethods.SendMessage(new HandleRef(this, Handle), WinFormNativeMethods.TB_INSERTBUTTON, index, ref tbbutton);
            }
            UpdateButtons();
        }
        ...
        internal void InternalSetButton(int index, ToolBarButton value, bool recreate, bool updateText) {
            ...
            if (IsHandleCreated) {
                WinFormNativeMethods.TBBUTTONINFO tbbi = value.GetTBBUTTONINFO(updateText, index);
                WinFormUnsafeNativeMethods.SendMessage(new HandleRef(this, Handle), WinFormNativeMethods.TB_SETBUTTONINFO, index, ref tbbi);
                ...
                if (recreate) {
                    ...
                    //
                    SendMessage(WinFormNativeMethods.TB_AUTOSIZE, 0, 0);
        ...
        protected override void OnHandleCreated(EventArgs e) {
            ...
            //
            SendMessage(WinFormNativeMethods.TB_BUTTONSTRUCTSIZE, Marshal.SizeOf(typeof(WinFormNativeMethods.TBBUTTON)), 0);

            // set up some extra goo
            //
            if (DropDownArrows)
                SendMessage(WinFormNativeMethods.TB_SETEXTENDEDSTYLE, 0, WinFormNativeMethods.TBSTYLE_EX_DRAWDDARROWS);

            // if we have an imagelist, add it in now.
            //
            if (imageList != null)
                SendMessage(WinFormNativeMethods.TB_SETIMAGELIST, 0, imageList.Handle);
                ...
                try {
                    ...
                    for (int x = 0; x < buttonCount; x++) {
                        if (buttons[x].Text.Length > 0) {
                            ...
                            buttons[x].stringIndex = SendMessage(WinFormNativeMethods.TB_ADDSTRING, 0, addString);
                        }
                        else {
                    ...
                    //
                    int cb = Marshal.SizeOf(typeof(WinFormNativeMethods.TBBUTTON));
                    int count = buttonCount;
                    ptbbuttons = Marshal.AllocHGlobal(checked(cb * count));

                    for (int x = 0; x < count; x++) {

                        WinFormNativeMethods.TBBUTTON tbbutton = buttons[x].GetTBBUTTON(x);
                        Marshal.StructureToPtr(tbbutton, (IntPtr)(checked((long)ptbbuttons + (cb * x))), true);
                        buttons[x].parent = this;
                    }

                    SendMessage(WinFormNativeMethods.TB_ADDBUTTONS, count, ptbbuttons);
        ...
        private void SendToolbarButtonSizeMessage() {
            SendMessage(WinFormNativeMethods.TB_SETBUTTONSIZE, 0, WinFormNativeMethods.Util.MAKELPARAM((int)(buttonSize.Width*currentScaleDX), (int)(buttonSize.Height*currentScaleDY)));
        }
        ...
        internal void SetToolTip(ToolTip toolTip) {
            WinFormUnsafeNativeMethods.SendMessage(new HandleRef(this, this.Handle), WinFormNativeMethods.TB_SETTOOLTIPS, new HandleRef(toolTip, toolTip.Handle), 0);
        ...
        private void WmNotifyDropDown(ref Message m) {
            WinFormNativeMethods.NMTOOLBAR nmTB = (WinFormNativeMethods.NMTOOLBAR)m.GetLParam(typeof(WinFormNativeMethods.NMTOOLBAR));

            ToolBarButton tbb = (ToolBarButton)buttons[nmTB.iItem];
            if (tbb == null)
                throw new InvalidOperationException(DCSR.GetString(DCSR.ToolBarButtonNotFound));
            ...
            if (menu != null) {
                ...
                WinFormNativeMethods.TPMPARAMS tpm = new WinFormNativeMethods.TPMPARAMS();

                SendMessage(WinFormNativeMethods.TB_GETRECT, nmTB.iItem, ref rc);
                ...
                if ((menu.GetType()).IsAssignableFrom(typeof(ContextMenu))) {
                    ...
                    WinFormUnsafeNativeMethods.MapWindowPoints(new HandleRef(nmTB.hdr, nmTB.hdr.hwndFrom), WinFormNativeMethods.NullHandleRef, ref rc, 2);
                    ...
                    WinFormSafeNativeMethods.TrackPopupMenuEx(
                                                  new HandleRef(menu, menu.Handle),
                                                  WinFormNativeMethods.TPM_LEFTALIGN |
                                                  WinFormNativeMethods.TPM_LEFTBUTTON |
        ...
        private void WmNotifyNeedTextA(ref Message m) {
            ...
            WinFormNativeMethods.TOOLTIPTEXTA ttt = (WinFormNativeMethods.TOOLTIPTEXTA) m.GetLParam(typeof(WinFormNativeMethods.TOOLTIPTEXTA));
            int commandID = (int)ttt.hdr.idFrom;
            ToolBarButton tbb = (ToolBarButton) buttons[commandID];
        ...
        private void WmNotifyHotItemChange(ref Message m) {
            ...
            WinFormNativeMethods.NMTBHOTITEM nmTbHotItem = (WinFormNativeMethods.NMTBHOTITEM)m.GetLParam(typeof(WinFormNativeMethods.NMTBHOTITEM));
            if (WinFormNativeMethods.HICF_ENTERING == (nmTbHotItem.dwFlags & WinFormNativeMethods.HICF_ENTERING))
                this.hotItem = nmTbHotItem.idNew;
            else if (WinFormNativeMethods.HICF_LEAVING == (nmTbHotItem.dwFlags & WinFormNativeMethods.HICF_LEAVING))
                this.hotItem = -1;
            else if (WinFormNativeMethods.HICF_MOUSE == (nmTbHotItem.dwFlags & WinFormNativeMethods.HICF_MOUSE))
                this.hotItem = nmTbHotItem.idNew;
            else if (WinFormNativeMethods.HICF_ARROWKEYS == (nmTbHotItem.dwFlags & WinFormNativeMethods.HICF_ARROWKEYS))
                this.hotItem = nmTbHotItem.idNew;
            else if (WinFormNativeMethods.HICF_ACCELERATOR == (nmTbHotItem.dwFlags & WinFormNativeMethods.HICF_ACCELERATOR))
                this.hotItem = nmTbHotItem.idNew;
            else if (WinFormNativeMethods.HICF_DUPACCEL == (nmTbHotItem.dwFlags & WinFormNativeMethods.HICF_DUPACCEL))
                this.hotItem = nmTbHotItem.idNew;
            else if (WinFormNativeMethods.HICF_RESELECT == (nmTbHotItem.dwFlags & WinFormNativeMethods.HICF_RESELECT))
                this.hotItem = nmTbHotItem.idNew;
            else if (WinFormNativeMethods.HICF_LMOUSE == (nmTbHotItem.dwFlags & WinFormNativeMethods.HICF_LMOUSE))
                this.hotItem = nmTbHotItem.idNew;
            else if (WinFormNativeMethods.HICF_TOGGLEDROPDOWN == (nmTbHotItem.dwFlags & WinFormNativeMethods.HICF_TOGGLEDROPDOWN))
                this.hotItem = nmTbHotItem.idNew;
        }
        ...
        private void WmReflectCommand(ref Message m) {
            ...
            int id = WinFormNativeMethods.Util.LOWORD(m.WParam);
        ...
        protected override async  Task WndProc(Message m) {
            switch (m.Msg) {
                case WinFormNativeMethods.WM_COMMAND + WinFormNativeMethods.WM_REFLECT:
                    WmReflectCommand(ref m);
                    break;

                case WinFormNativeMethods.WM_NOTIFY:
                case WinFormNativeMethods.WM_NOTIFY + WinFormNativeMethods.WM_REFLECT:
                    WinFormNativeMethods.NMHDR note = (WinFormNativeMethods.NMHDR) m.GetLParam(typeof(WinFormNativeMethods.NMHDR));
                    switch (note.code) {
                        case WinFormNativeMethods.TTN_NEEDTEXTA:
                            // MSDN:
                        ...
                        case WinFormNativeMethods.TTN_NEEDTEXTW:
                        ...
                        case WinFormNativeMethods.TTN_SHOW:
                            ...
                            WinFormNativeMethods.WINDOWPLACEMENT wndPlacement = new WinFormNativeMethods.WINDOWPLACEMENT();
                            int nRet = WinFormUnsafeNativeMethods.GetWindowPlacement(new HandleRef(null, note.hwndFrom), ref wndPlacement);
                            ...
                            if (wndPlacement.rcNormalPosition_left == 0 &&
                                ...
                                WinFormNativeMethods.POINT leftTop = new WinFormNativeMethods.POINT(x, y);
                                WinFormUnsafeNativeMethods.ClientToScreen(new HandleRef(this, this.Handle), leftTop);
                                ...
                                WinFormSafeNativeMethods.SetWindowPos(new HandleRef(null, note.hwndFrom), WinFormNativeMethods.NullHandleRef, leftTop.x, leftTop.y, 0, 0, WinFormNativeMethods.SWP_NOSIZE | WinFormNativeMethods.SWP_NOZORDER | WinFormNativeMethods.SWP_NOACTIVATE);
                                m.Result = (IntPtr)1;
                                return;
                            }
                            break;

                        case WinFormNativeMethods.TBN_HOTITEMCHANGE:
                            WmNotifyHotItemChange(ref m);
                            break;


                        case WinFormNativeMethods.TBN_QUERYINSERT:
                            m.Result = (IntPtr)1;
                            break;

                        case WinFormNativeMethods.TBN_DROPDOWN:
        ...
        public class ToolBarButtonCollection : IList {
            ...
            object IList.this[int index] {
                ...
                set {
                    if (value is ToolBarButton) {
                        ...
                        throw new ArgumentException(DCSR.GetString(DCSR.ToolBarBadToolBarButton),"value");
                    }
                }
            }
            /// <include file='doc\ToolBar.uex' path='docs/doc[@for="ToolBar.ToolBarButtonCollection.this"]/*' />
            ...
            int IList.Add(object button) {
                if (button is ToolBarButton) {
                    ...
                    throw new ArgumentException(DCSR.GetString(DCSR.ToolBarBadToolBarButton),"button");
                }
            }
            ...
            public void Clear() {
                ...
                for (int x = owner.buttonCount; x > 0; x--) {
                    if (owner.IsHandleCreated) {
                        owner.SendMessage(WinFormNativeMethods.TB_DELETEBUTTON, x - 1, 0);
                    }
                    owner.RemoveAt(x - 1);
                }
           ...
           private bool IsValidIndex(int index) {
            ...
            public void RemoveAt(int index) {
                ...
                if (owner.IsHandleCreated) {
                    owner.SendMessage(WinFormNativeMethods.TB_DELETEBUTTON, index, 0);
                }
