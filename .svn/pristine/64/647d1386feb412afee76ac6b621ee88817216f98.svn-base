...
namespace System.Windows.Forms {
    ...
    internal class MdiControlStrip : MenuStrip {
            ...
            public MdiControlStrip(IWin32Window target) {
                IntPtr hMenu= WinFormUnsafeNativeMethods.GetSystemMenu(new HandleRef(this, Control.GetSafeHandle(target)), /*bRevert=*/false);
                this.target = target;

                // The menu item itself takes care of enabledness and sending WM_SYSCOMMAND messages to the target.
                minimize    = new ControlBoxMenuItem(hMenu, WinFormNativeMethods.SC_MINIMIZE, target);
                close       = new ControlBoxMenuItem(hMenu, WinFormNativeMethods.SC_CLOSE,    target);
                restore     = new ControlBoxMenuItem(hMenu, WinFormNativeMethods.SC_RESTORE,  target);
...
*/
            ...
            private Image GetTargetWindowIcon() {
                ...
                IntPtr hIcon = WinFormUnsafeNativeMethods.SendMessage(new HandleRef(this, Control.GetSafeHandle(target)), WinFormNativeMethods.WM_GETICON, WinFormNativeMethods.ICON_SMALL, 0);
            ...
            private void OnSystemMenuDropDownOpening(object sender, EventArgs e) {
                if (!system.HasDropDownItems && (target != null)) {
                    system.DropDown = ToolStripDropDownMenu.FromHMenu(WinFormUnsafeNativeMethods.GetSystemMenu(new HandleRef(this, Control.GetSafeHandle(target)), /*bRevert=*/false), target);
                }
