    ...
    public class NativeWindow : MarshalByRefObject, IWin32Window {
...
#endif
        IntPtr handle;
        WinFormNativeMethods.WndProc windowProc;
        ...
        internal void ForceExitMessageLoop() {
            ...
            if (handle != IntPtr.Zero) {
                ...
                if (WinFormUnsafeNativeMethods.IsWindow(new HandleRef(null, handle))) {
                    int lpdwProcessId;  //unused
                    int id = WinFormSafeNativeMethods.GetWindowThreadProcessId(new HandleRef(null, handle), out lpdwProcessId);
                    ...
                    if (threadHandle != IntPtr.Zero) {
                        int exitCode = 0;
                        WinFormSafeNativeMethods.GetExitCodeThread(new HandleRef(null, threadHandle), out exitCode);
                        if (!AppDomain.CurrentDomain.IsFinalizingForUnload() && exitCode == WinFormNativeMethods.STATUS_PENDING) {
                            IntPtr result;
                            if (WinFormUnsafeNativeMethods.SendMessageTimeout(new HandleRef(null, handle),
                                WinFormNativeMethods.WM_UIUNSUBCLASS, IntPtr.Zero, IntPtr.Zero,
                                WinFormUnsafeNativeMethods.SMTO_ABORTIFHUNG, 100, out result) == IntPtr.Zero) {
            ...
            if (h != IntPtr.Zero && ownedHandle) {
                ...
                //
                WinFormUnsafeNativeMethods.PostMessage(new HandleRef(this, h), WinFormNativeMethods.WM_CLOSE, 0, 0);
            }
        }
        ...
        internal static void AddWindowToIDTable(object wrapper, IntPtr handle) {
            ...
            WinFormUnsafeNativeMethods.SetWindowLong(new HandleRef(wrapper, handle), WinFormNativeMethods.GWL_ID, new HandleRef(wrapper, (IntPtr)globalID));
            globalID++;
        ...
        private static int AdjustWndProcFlagsFromConfig(int wndProcFlags) {
            if ( false ){//WindowsFormsSection.GetSection().JitDebugging) {
        ...
        internal void AssignHandle(IntPtr handle, bool assignUniqueID) {
            ...
            lock (this) {
                ...
                //    userDefWindowProc = WinFormUnsafeNativeMethods.GetProcAddress(new HandleRef(null, WinFormUnsafeNativeMethods.GetModuleHandle("user32.dll")), defproc);
                //    if (userDefWindowProc == IntPtr.Zero) {
                //        throw new Win32Exception();
                //    }
                //}

                //defWindowProc = WinFormUnsafeNativeMethods.GetWindowLong(new HandleRef(this, handle), WinFormNativeMethods.GWL_WNDPROC);
                ...
                //    windowProc = new WinFormNativeMethods.WndProc(this.DebuggableCallback);
                //}
                //else {
                //    Debug.WriteLineIf(WndProcChoice.TraceVerbose, "Using normal wndproc");
                //    windowProc = new WinFormNativeMethods.WndProc(this.Callback);
                //}

                AddWindowToTable(handle, this);

                //WinFormUnsafeNativeMethods.SetWindowLong(new HandleRef(this, handle), WinFormNativeMethods.GWL_WNDPROC, windowProc);
                //windowProcPtr = WinFormUnsafeNativeMethods.GetWindowLong(new HandleRef(this, handle), WinFormNativeMethods.GWL_WNDPROC);
                //Debug.Assert(defWindowProc != windowProcPtr, "Uh oh! Subclassed ourselves!!!");
                //if (assignUniqueID &&
                //    (unchecked((int)((long)WinFormUnsafeNativeMethods.GetWindowLong(new HandleRef(this, handle), WinFormNativeMethods.GWL_STYLE))) & WinFormNativeMethods.WS_CHILD) != 0 &&
                //     unchecked((int)((long)WinFormUnsafeNativeMethods.GetWindowLong(new HandleRef(this, handle), WinFormNativeMethods.GWL_ID))) == 0) {
                //    WinFormUnsafeNativeMethods.SetWindowLong(new HandleRef(this, handle), WinFormNativeMethods.GWL_ID, new HandleRef(this, handle));
                //}
        ...
        private IntPtr Callback(IntPtr hWnd, int msg, IntPtr wparam, IntPtr lparam) {
            ...
            try {
                ...
                if (msg == WinFormNativeMethods.WM_NCDESTROY) ReleaseHandle(false);
                if (msg == WinFormNativeMethods.WM_UIUNSUBCLASS) ReleaseHandle(true);
            }
        ...
        private void CheckReleased() {
            if (handle != IntPtr.Zero) {
                throw new InvalidOperationException(DCSR.GetString(DCSR.HandleAlreadyExists));
            }
        }
        ...
        public virtual void CreateHandle(CreateParams cp) {
            ...
            if ((cp.Style & WinFormNativeMethods.WS_CHILD) != WinFormNativeMethods.WS_CHILD
            ...
            lock (this) {
                ...
                lock (createWindowSyncObject) {
                    ...
                    IntPtr modHandle = WinFormUnsafeNativeMethods.GetModuleHandle(null);
                    ...
                    try {
                        ...
                        createResult = WinFormUnsafeNativeMethods.CreateWindowEx(cp.ExStyle, windowClass.windowClassName,
                                                                          cp.Caption, cp.Style, cp.X, cp.Y, cp.Width, cp.Height, new HandleRef(cp, cp.Parent), WinFormNativeMethods.NullHandleRef,
                                                                          new HandleRef(null, modHandle), cp);

                        lastWin32Error = Marshal.GetLastWin32Error();
                    }
                    catch (NullReferenceException e) {
                        throw new OutOfMemoryException(DCSR.GetString(DCSR.ErrorCreatingHandle), e);
                    }
                    ...
                    if (createResult == IntPtr.Zero) {
                        throw new Win32Exception(lastWin32Error, DCSR.GetString(DCSR.ErrorCreatingHandle));
                    }
                    ownHandle = true;
                    this.handle = new IntPtr(createResult);
                    if( cp.ControlInstance != null )
                    {
                        DCWin32API.RegisterControl(this.handle.ToInt32() , cp.ControlInstance);
                    }
                    System.Internal.HandleCollector.Add(createResult, WinFormNativeMethods.CommonHandles.Window);
                }
            }
        }
        ...
        {
            DCWin32API.DefWindowProc(m.HWnd.ToInt32(), m.Msg, m.WParam.ToInt32(), m.LParam.ToInt32());
            ...
            //                    m.Result = WinFormUnsafeNativeMethods.DefWindowProc(m.HWnd, m.Msg, m.WParam, m.LParam);
            //                    return;
            //                }
            //                m.Result = WinFormUnsafeNativeMethods.CallWindowProc(defWindowProc, m.HWnd, m.Msg, m.WParam, m.LParam);
            //            }
            //            else {
        ...
        public virtual void DestroyHandle() {
            ...
            lock (this) {
                if (handle != IntPtr.Zero) {
                    if (!WinFormUnsafeNativeMethods.DestroyWindow(new HandleRef(this, handle))) {
                        UnSubclass();
                        //then post a close and let it do whatever it needs to do on its own.
                        WinFormUnsafeNativeMethods.PostMessage(new HandleRef(this, handle), WinFormNativeMethods.WM_CLOSE, 0, 0);
                    }
                    handle = IntPtr.Zero;
                    ownHandle = false;
                }
                ...
                lock (internalSyncObject) {
                    for (int i = 0; i < hashBuckets.Length; i++) {
                        ...
                        if (b.handle != IntPtr.Zero && b.handle != new IntPtr(-1)) {
                            ...
                            WinFormUnsafeNativeMethods.SetWindowLong(href, WinFormNativeMethods.GWL_WNDPROC, new HandleRef(null, userDefWindowProc));
                            WinFormUnsafeNativeMethods.SetClassLong(href, WinFormNativeMethods.GCL_WNDPROC, userDefWindowProc);
                            WinFormUnsafeNativeMethods.PostMessage(href, WinFormNativeMethods.WM_CLOSE, 0, 0);
            ...
            if (handle != IntPtr.Zero) {
                ...
                lock (this) {
                    if (handle != IntPtr.Zero) {
                        ...
                        if (ownHandle) {
                            System.Internal.HandleCollector.Remove(handle, WinFormNativeMethods.CommonHandles.Window);
                            ownHandle = false;
                        }
        ...
        */
        ...
        internal static void SetUnhandledExceptionModeInternal(UnhandledExceptionMode mode, bool threadScope) {
            ...
            if (!threadScope && anyHandleCreatedInApp) {
                throw new InvalidOperationException(DCSR.GetString(DCSR.ApplicationCannotChangeApplicationExceptionMode));
            }
            if (threadScope && anyHandleCreated) {
                throw new InvalidOperationException(DCSR.GetString(DCSR.ApplicationCannotChangeThreadExceptionMode));
            }
...
//            //
//            IntPtr currentWinPrc = WinFormUnsafeNativeMethods.GetWindowLong(new HandleRef(this, handle), WinFormNativeMethods.GWL_WNDPROC);
...
//                    //
//                    WinFormUnsafeNativeMethods.SetWindowLong(href, WinFormNativeMethods.GWL_WNDPROC, new HandleRef(this, defWindowProc));
//                }
//                else {
//                    if (finalizing) {
...
//                        // userDefWindowProc.
//                        WinFormUnsafeNativeMethods.SetWindowLong(href, WinFormNativeMethods.GWL_WNDPROC, new HandleRef(this, userDefWindowProc));
//                    }
//                    else {
...
//                        // have re-subclassed.
//                        WinFormUnsafeNativeMethods.SetWindowLong(href, WinFormNativeMethods.GWL_WNDPROC, previousWindow.windowProc);
//                    }
//                }
//            }
//            else {
...
//            }
        ...
        private class WindowClass {
            ...
            internal WinFormNativeMethods.WndProc windowProc;
            ...
            public IntPtr Callback(IntPtr hWnd, int msg, IntPtr wparam, IntPtr lparam) {
                ...
                WinFormUnsafeNativeMethods.SetWindowLong(new HandleRef(null, hWnd), WinFormNativeMethods.GWL_WNDPROC, new HandleRef(this, defWindowProc));
            ...
            private void RegisterClass() {
                //WinFormNativeMethods.WNDCLASS_D wndclass = new WinFormNativeMethods.WNDCLASS_D();
                ...
                //    //
                //    wndclass.hbrBackground = WinFormUnsafeNativeMethods.GetStockObject(WinFormNativeMethods.HOLLOW_BRUSH); //(IntPtr)(NativeMethods.COLOR_WINDOW + 1);
                ...
                //}
                //else {
                //    WinFormNativeMethods.WNDCLASS_I wcls = new WinFormNativeMethods.WNDCLASS_I();
                //    bool ok = WinFormUnsafeNativeMethods.GetClassInfo(WinFormNativeMethods.NullHandleRef, className, wcls);
                ...
                //wndclass.hInstance = WinFormUnsafeNativeMethods.GetModuleHandle(null);
                //wndclass.lpszClassName = windowClassName;

                //short atom = WinFormUnsafeNativeMethods.RegisterClass(wndclass);
                //if (atom == 0) {

                //    int err = Marshal.GetLastWin32Error();
                //    if (err == WinFormNativeMethods.ERROR_CLASS_ALREADY_EXISTS) {
                ...
                //        // to throw.
                //        WinFormNativeMethods.WNDCLASS_I wcls = new WinFormNativeMethods.WNDCLASS_I();
                //        bool ok = WinFormUnsafeNativeMethods.GetClassInfo(new HandleRef(null, WinFormUnsafeNativeMethods.GetModuleHandle(null)), windowClassName, wcls);
                ...
                //            if (WinFormUnsafeNativeMethods.UnregisterClass(windowClassName, new HandleRef(null, WinFormUnsafeNativeMethods.GetModuleHandle(null)))) {
                //                atom = WinFormUnsafeNativeMethods.RegisterClass(wndclass);
                ...
                //                    wndclass.lpszClassName = windowClassName;
                //                    atom = WinFormUnsafeNativeMethods.RegisterClass(wndclass);
                //                } while (atom == 0 && Marshal.GetLastWin32Error() == WinFormNativeMethods.ERROR_CLASS_ALREADY_EXISTS);
                //            }
                //        }
                //    }
            ...
            private void UnregisterClass() {
                //if (registered && WinFormUnsafeNativeMethods.UnregisterClass(windowClassName, new HandleRef(null, WinFormUnsafeNativeMethods.GetModuleHandle(null)))) {
                //    windowProc = null;
                //    registered = false;
                //}
            }
