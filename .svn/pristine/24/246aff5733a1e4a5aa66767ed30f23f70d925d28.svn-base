...
namespace System.Windows.Forms
{
    ...
    public sealed partial class Application
    {
        ...
        public static void OpenUrl(string strUrl, string strTarget)
        {
            if (strUrl != null && strUrl.Length > 0)
            {
                DCWin32API.JSRuntime.InvokeVoidAsync("window.open", strUrl, strTarget);
            }
        }

        [System.Reflection.Obfuscation(Exclude = true, ApplyToMembers = true)]
        public static void OpenUrl(string strBaseUrl , string strUrl, string strTarget)
        {
            if (strUrl != null && strUrl.Length > 0)
            {
                DCWin32API.JSRuntime.InvokeVoidAsync("window.open", strUrl, strTarget);
        ...
        private static readonly List<Func<Task>> _ActionForInvokeAsyncs = new List<Func<Task>>();
        /// <summary>
        /// 执行一个异步任务
        /// </summary>
        /// <param name="action">任务对象</param>
        ...
        public static void InvokeAsync( Func<Task> action )
        {
            ...
            _ActionForInvokeAsyncs.Add(action);
            DCWin32API.JSRuntime.Invoke<object>("__DCWin32API.InvokeApplicationAsyncMethod");
        }

        internal static async ValueTask InvokeAsyncInternal()
        {
            if (_ActionForInvokeAsyncs.Count > 0)
            {
                var first = _ActionForInvokeAsyncs[0];
                _ActionForInvokeAsyncs.RemoveAt(0);
                if (first != null)
                {
                    await first();
                }
        ...
        internal static bool ComCtlSupportsVisualStyles
        {
            get
            {
                ...
                IntPtr hModule = WinFormUnsafeNativeMethods.GetModuleHandle(ExternDll.Comctl32);
                if (hModule != IntPtr.Zero)
                {
                    try
                    {
                        IntPtr pFunc = WinFormUnsafeNativeMethods.GetProcAddress(new HandleRef(null, hModule), "ImageList_WriteEx");
                ...
                else
                {
                    ...
                    hModule = WinFormUnsafeNativeMethods.LoadLibrary(ExternDll.Comctl32);
                    if (hModule != IntPtr.Zero)
                    {
                        try
                        {
                        ...
                        {
                            WinFormUnsafeNativeMethods.FreeLibrary(new HandleRef(null, hModule));
                        }
                    }
                }
                return false;
        ...
        internal static string _ExecutablePath = null;
        [System.Reflection.Obfuscation(Exclude = true, ApplyToMembers = true)]
        ...
        internal static string _ProductName = null;
        [System.Reflection.Obfuscation(Exclude = true, ApplyToMembers = true)]
        ...
        internal static string _ProductVersion = null;
        [System.Reflection.Obfuscation(Exclude = true, ApplyToMembers = true)]
        ...
        public static string SafeTopLevelCaptionFormat
        {
            get
            {
                ...
                {
                    safeTopLevelCaptionSuffix = DCSR.GetString(DCSR.SafeTopLevelCaptionFormat); // 0 - original, 1 - zone, 2 - site
                }
        ...
        internal static string _StartupPath = null;
        ...
        public static VisualStyleState VisualStyleState
        {
            get
            {
                ...
                VisualStyleState vState = (VisualStyleState)WinFormSafeNativeMethods.GetThemeAppProperties();
                return vState;
            }
            ...
            set
            {
                if (VisualStyleInformation.IsSupportedByOS)
                {
                    ...
                    WinFormSafeNativeMethods.SetThemeAppProperties((int)value);
                    ...
                    WinFormSafeNativeMethods.EnumThreadWindowsCallback callback = new WinFormSafeNativeMethods.EnumThreadWindowsCallback(Application.SendThemeChanged);
                    WinFormSafeNativeMethods.EnumWindows(callback, IntPtr.Zero);
        ...
        {
            int processId;
            int thisPID = WinFormSafeNativeMethods.GetCurrentProcessId();
            WinFormSafeNativeMethods.GetWindowThreadProcessId(new HandleRef(null, handle), out processId);
            if (processId == thisPID && WinFormSafeNativeMethods.IsWindowVisible(new HandleRef(null, handle)))
            {

                SendThemeChangedRecursive(handle, IntPtr.Zero);
                WinFormSafeNativeMethods.RedrawWindow(new HandleRef(null, handle),
        ...
        private static bool SendThemeChangedRecursive(IntPtr handle, IntPtr lparam)
        {
            ...
            WinFormUnsafeNativeMethods.EnumChildWindows(new HandleRef(null, handle),
                new WinFormNativeMethods.EnumChildrenCallback(Application.SendThemeChangedRecursive),
                WinFormNativeMethods.NullHandleRef);

            //then do myself.
            WinFormUnsafeNativeMethods.SendMessage(new HandleRef(null, handle), WinFormNativeMethods.WM_THEMECHANGED, 0, 0);
        ...
        {
            ThreadContext.FromCurrent().RunMessageLoop(WinFormNativeMethods.MSOCM.msoloopDoEvents, null);
        }
        ...
        {
            ThreadContext.FromCurrent().RunMessageLoop(WinFormNativeMethods.MSOCM.msoloopDoEventsModal, null);
        }
        private static bool _EnableVisualStyles = false;
        [System.Reflection.Obfuscation(Exclude = true, ApplyToMembers = true)]
        ...
        //    useVisualStyles = WinFormUnsafeNativeMethods.ThemingScope.CreateActivationContext(assemblyFileName, nativeResourceID);
        //}
        ...
        private static ThreadContext GetContextForHandle(HandleRef handle)
        {
            ...
            int pid;
            int id = WinFormSafeNativeMethods.GetWindowThreadProcessId(handle, out pid);
        ...
        {
            //Debug.Assert(WinFormUnsafeNativeMethods.IsWindow(handle), "Handle being parked is not a valid window handle");
            //Debug.Assert(((int)WinFormUnsafeNativeMethods.GetWindowLong(handle, WinFormNativeMethods.GWL_STYLE) & WinFormNativeMethods.WS_CHILD) != 0, "Only WS_CHILD windows should be parked.");
        ...
        public static void Restart()
        {
            ...
            //        ExitInternal();
            //        WinFormUnsafeNativeMethods.CorLaunchApplication(hostType, appFullName, 0, null, 0, null, new WinFormUnsafeNativeMethods.PROCESS_INFORMATION());
            //    }
            //    else
            //    {
            //        // Regular app case
        ...
        {
            ThreadContext.FromCurrent().RunMessageLoop(WinFormNativeMethods.MSOCM.msoloopMain, new ApplicationContext());
        }

        [System.Reflection.Obfuscation(Exclude = true, ApplyToMembers = true)]
        public static void Run(Form mainForm)
        {
            mainForm._FromApplicationRun = true;
            mainForm.Visible = true;
            mainForm.Activate();
            //ThreadContext.FromCurrent().RunMessageLoop(WinFormNativeMethods.MSOCM.msoloopMain, new ApplicationContext(mainForm));
        }
        ...
        {
            ThreadContext.FromCurrent().RunMessageLoop(WinFormNativeMethods.MSOCM.msoloopMain, context);
        }
        ...
        {
            ThreadContext.FromCurrent().RunMessageLoop(WinFormNativeMethods.MSOCM.msoloopModalForm, new ModalApplicationContext(form));
        }
        ...
        public static bool SetSuspendState(PowerState state, bool force, bool disableWakeEvent)
        {
            ...
            return WinFormUnsafeNativeMethods.SetSuspendState(state == PowerState.Hibernate, force, disableWakeEvent);
        }
        ...
        /// </devdoc>
        private class ComponentManager : WinFormUnsafeNativeMethods.IMsoComponentManager
        {
            ...
            {
                public WinFormUnsafeNativeMethods.IMsoComponent component;
                public WinFormNativeMethods.MSOCRINFOSTRUCT componentInfo;
            }

            private Hashtable oleComponents;
            private int cookieCounter = 0;
            private WinFormUnsafeNativeMethods.IMsoComponent activeComponent = null;
            private WinFormUnsafeNativeMethods.IMsoComponent trackingComponent = null;
            ...
            /// </devdoc>
            int WinFormUnsafeNativeMethods.IMsoComponentManager.QueryService(
                                                 ...
                                                 out object ppvObj)
            {
                ...
                ppvObj = null;
                return WinFormNativeMethods.E_NOINTERFACE;
            ...
            /// </devdoc>
            bool WinFormUnsafeNativeMethods.IMsoComponentManager.FDebugMessage(
                                                   IntPtr hInst,
                                                   int msg,
                                                   IntPtr wparam,
                                                   IntPtr lparam)
            ...
            /// </devdoc>
            bool WinFormUnsafeNativeMethods.IMsoComponentManager.FRegisterComponent(WinFormUnsafeNativeMethods.IMsoComponent component,
            ...
            ///      Return TRUE if successful, FALSE otherwise.
            /// </devdoc>
            bool WinFormUnsafeNativeMethods.IMsoComponentManager.FRevokeComponent(IntPtr dwComponentID)
            {
            ...
            ///      Return TRUE if successful, FALSE otherwise.
            /// </devdoc>
            bool WinFormUnsafeNativeMethods.IMsoComponentManager.FUpdateComponentRegistration(
                                                                  IntPtr dwComponentID,
                                                                  WinFormNativeMethods.MSOCRINFOSTRUCT info
                                                                  )
            {
                int dwLocalComponentID = unchecked((int)(long)dwComponentID);
            ...
            /// </devdoc>
            bool WinFormUnsafeNativeMethods.IMsoComponentManager.FOnComponentActivate(IntPtr dwComponentID)
            {
            ...
            ///      Return TRUE if successful, FALSE otherwise.
            /// </devdoc>
            bool WinFormUnsafeNativeMethods.IMsoComponentManager.FSetTrackingComponent(IntPtr dwComponentID, bool fTrack)
            {
            ...
            /// </devdoc>
            void WinFormUnsafeNativeMethods.IMsoComponentManager.OnComponentEnterState(
                                                           IntPtr dwComponentID,
                                                           int uStateID,
                                                           int uContext,
                                                           int cpicmExclude,
                                                           ...
                                                           int dwReserved)
            {
                ...
                if (uContext == WinFormNativeMethods.MSOCM.msoccontextAll || uContext == WinFormNativeMethods.MSOCM.msoccontextMine)
                {
            ...
            /// </devdoc>
            bool WinFormUnsafeNativeMethods.IMsoComponentManager.FOnComponentExitState(
                                                           IntPtr dwComponentID,
                                                           int uStateID,
                                                           int uContext,
                                                           int cpicmExclude,
            ...
            /// </devdoc>
            bool WinFormUnsafeNativeMethods.IMsoComponentManager.FInState(int uStateID, IntPtr pvoid)
            {
            ...
            /// </devdoc>
            bool WinFormUnsafeNativeMethods.IMsoComponentManager.FContinueIdle()
            {
                ...
                //
                WinFormNativeMethods.MSG msg = new WinFormNativeMethods.MSG();
                return !WinFormUnsafeNativeMethods.PeekMessage(ref msg, WinFormNativeMethods.NullHandleRef, 0, 0, WinFormNativeMethods.PM_NOREMOVE);
            }
            ...
            ///      cleanup).
            /// </devdoc>
            bool WinFormUnsafeNativeMethods.IMsoComponentManager.FPushMessageLoop(
                                                      IntPtr dwComponentID,
                                                      int reason,
                                                      ...
                                                      )
            {
                ...
                WinFormUnsafeNativeMethods.IMsoComponent prevActive = this.activeComponent;
                ...
                try
                {
                    ...
                    //
                    WinFormNativeMethods.MSG msg = new WinFormNativeMethods.MSG();
                    WinFormNativeMethods.MSG[] rgmsg = new WinFormNativeMethods.MSG[] { msg };
                    bool unicodeWindow = false;
                    WinFormUnsafeNativeMethods.IMsoComponent requestingComponent;
                    ...
                    while (continueLoop)
                    {
                        ...
                        //
                        WinFormUnsafeNativeMethods.IMsoComponent component;
                        ...
                        bool peeked = WinFormUnsafeNativeMethods.PeekMessage(ref msg, WinFormNativeMethods.NullHandleRef, 0, 0, WinFormNativeMethods.PM_NOREMOVE);
                        ...
                        if (peeked)
                        {
                            ...
                            if (continueLoop)
                            {
                                if (msg.hwnd != IntPtr.Zero && WinFormSafeNativeMethods.IsWindowUnicode(new HandleRef(null, msg.hwnd)))
                                {
                                    unicodeWindow = true;
                                    WinFormUnsafeNativeMethods.GetMessageW(ref msg, WinFormNativeMethods.NullHandleRef, 0, 0);
                                }
                                else
                                {
                                    unicodeWindow = false;
                                    WinFormUnsafeNativeMethods.GetMessageA(ref msg, WinFormNativeMethods.NullHandleRef, 0, 0);
                                }

                                if (msg.message == WinFormNativeMethods.WM_QUIT)
                                {
                                    ...
                                    if (reason != WinFormNativeMethods.MSOCM.msoloopMain)
                                    {
                                        WinFormUnsafeNativeMethods.PostQuitMessage((int)msg.wParam);
                                    }
                                ...
                                {
                                    WinFormUnsafeNativeMethods.TranslateMessage(ref msg);
                                    if (unicodeWindow)
                                    {
                                        WinFormUnsafeNativeMethods.DispatchMessageW(ref msg);
                                    }
                                    else
                                    {
                                        WinFormUnsafeNativeMethods.DispatchMessageA(ref msg);
                                    }
                                }
                            }
                        }
                        else
                        {
                            ...
                            // for us to do.
                            //
                            if (reason == WinFormNativeMethods.MSOCM.msoloopDoEvents ||
                                reason == WinFormNativeMethods.MSOCM.msoloopDoEventsModal)
                            {
                                break;
                            }
                            ...
                            if (continueLoop)
                            {
                                if (continueIdle)
                                {
                                    ...
                                    WinFormUnsafeNativeMethods.MsgWaitForMultipleObjectsEx(0, IntPtr.Zero, 100, WinFormNativeMethods.QS_ALLINPUT, WinFormNativeMethods.MWMO_INPUTAVAILABLE);
                                }
                                else
                                {
                                    ...
                                    //
                                    if (!WinFormUnsafeNativeMethods.PeekMessage(ref msg, WinFormNativeMethods.NullHandleRef, 0, 0, WinFormNativeMethods.PM_NOREMOVE))
                                    {
                                        WinFormUnsafeNativeMethods.WaitMessage();
                                    }
                                }
                            }
                        }
            ...
            /// </devdoc>
            bool WinFormUnsafeNativeMethods.IMsoComponentManager.FCreateSubComponentManager(
                                                                object punkOuter,
                                                                object punkServProv,
                                                                ref Guid riid,
                                                                out IntPtr ppvObj)
            ...
            ///      some error occurred.
            /// </devdoc>
            bool WinFormUnsafeNativeMethods.IMsoComponentManager.FGetParentComponentManager(out WinFormUnsafeNativeMethods.IMsoComponentManager ppicm)
            {
                ppicm = null;
                return false;
            }
            ...
            ///      dwReserved is reserved for future use and should be zero.
            /// </devdoc>
            bool WinFormUnsafeNativeMethods.IMsoComponentManager.FGetActiveComponent(
                                                         int dwgac,
                                                         WinFormUnsafeNativeMethods.IMsoComponent[] ppic,
                                                         WinFormNativeMethods.MSOCRINFOSTRUCT info,
                                                         int dwReserved)
            {

                WinFormUnsafeNativeMethods.IMsoComponent component = null;

                if (dwgac == WinFormNativeMethods.MSOCM.msogacActive)
                {
                    component = activeComponent;
                }
                else if (dwgac == WinFormNativeMethods.MSOCM.msogacTracking)
                {
                    component = trackingComponent;
                }
                else if (dwgac == WinFormNativeMethods.MSOCM.msogacTrackingOrActive)
                {
                    if (trackingComponent != null)
                    {
                        component = trackingComponent;
        ...
        /// </devdoc>
        /// <internalonly/>
        internal sealed class ThreadContext : MarshalByRefObject, WinFormUnsafeNativeMethods.IMsoComponent
        {
            ...
            //
            private WinFormUnsafeNativeMethods.IMsoComponentManager componentManager;
            ...
            private WinFormNativeMethods.MSG tempMsg = new WinFormNativeMethods.MSG();
            ...
            public ThreadContext()
            {
                ...
                //WinFormUnsafeNativeMethods.DuplicateHandle(new HandleRef(null, WinFormSafeNativeMethods.GetCurrentProcess()), new HandleRef(null, WinFormSafeNativeMethods.GetCurrentThread()),
                //                                    new HandleRef(null, WinFormSafeNativeMethods.GetCurrentProcess()), ref address, 0, false,
                //                                    WinFormNativeMethods.DUPLICATE_SAME_ACCESS);

                handle = DCWin32API.AllocHandle(null);//  address;

                id = DCWin32API.AllocHandle(null);// WinFormSafeNativeMethods.GetCurrentThreadId();
                messageLoopCount = 0;
            ...
            /// </devdoc>
            internal WinFormUnsafeNativeMethods.IMsoComponentManager ComponentManager
            {
                get
                {
                    ...
                    if (componentManager == null)
                    {
                        ...
                        try
                        {
                            WinFormUnsafeNativeMethods.IMsoComponentManager msocm = null;
                            ...
                            if (WinFormNativeMethods.Succeeded(WinFormUnsafeNativeMethods.CoRegisterMessageFilter(WinFormNativeMethods.NullHandleRef, ref msgFilterPtr)) && msgFilterPtr != (IntPtr)0)
                            {

                                IntPtr dummy = (IntPtr)0;
                                WinFormUnsafeNativeMethods.CoRegisterMessageFilter(new HandleRef(null, msgFilterPtr), ref dummy);

                                object msgFilterObj = Marshal.GetObjectForIUnknown(msgFilterPtr);
                                Marshal.Release(msgFilterPtr);

                                WinFormUnsafeNativeMethods.IOleServiceProvider sp = msgFilterObj as WinFormUnsafeNativeMethods.IOleServiceProvider;
                                if (sp != null)
                                {
                                    try
                                    {
                                        ...
                                        if (WinFormNativeMethods.Succeeded(hr) && retval != IntPtr.Zero)
                                        {
                                            ...
                                            try
                                            {
                                                Guid IID_IMsoComponentManager = typeof(WinFormUnsafeNativeMethods.IMsoComponentManager).GUID;
                                            ...
                                            if (WinFormNativeMethods.Succeeded(hr) && pmsocm != IntPtr.Zero)
                                            {
                            ...
                            if (msocm != null && componentID == INVALID_ID)
                            {
                                ...
                                WinFormNativeMethods.MSOCRINFOSTRUCT info = new WinFormNativeMethods.MSOCRINFOSTRUCT();
                                info.cbSize = Marshal.SizeOf(typeof(WinFormNativeMethods.MSOCRINFOSTRUCT));
                                info.uIdleTimeInterval = 0;
                                info.grfcrf = WinFormNativeMethods.MSOCM.msocrfPreTranslateAll | WinFormNativeMethods.MSOCM.msocrfNeedIdleTime;
                                info.grfcadvf = WinFormNativeMethods.MSOCM.msocadvfModal;
            ...
            internal void BeginModalMessageLoop(ApplicationContext context)
            {
                ...
                try
                {
                    WinFormUnsafeNativeMethods.IMsoComponentManager cm = ComponentManager;
                    if (cm != null)
                    {
                        cm.OnComponentEnterState((IntPtr)componentID, WinFormNativeMethods.MSOCM.msocstateModal, WinFormNativeMethods.MSOCM.msoccontextAll, 0, 0, 0);
                    }
                }
                finally
                {
                ...
                lock (this)
                {
                    try
                    {
                        ...
                        {  // make sure that we are not reentrant
                            ...
                            {
                                bool ourThread = WinFormSafeNativeMethods.GetCurrentThreadId() == id;
                                    ...
                                    if (ourThread)
                                    {
                                        ...
                                        finally
                                        {
                                            if (GetState(STATE_OLEINITIALIZED) && !GetState(STATE_EXTERNALOLEINIT))
                                            {
                                                ...
                                                WinFormUnsafeNativeMethods.OleUninitialize();
                                            }
                                        }
                                    }
                                }
                                finally
                                {
                                    ...
                                    {
                                        WinFormUnsafeNativeMethods.CloseHandle(new HandleRef(this, handle));
                                        handle = IntPtr.Zero;
                                    }
            ...
            private void DisposeParkingWindow()
            {
                if (parkingWindow != null && parkingWindow.IsHandleCreated)
                {
                    ...
                    //
                    int pid;
                    int hwndThread = WinFormSafeNativeMethods.GetWindowThreadProcessId(new HandleRef(parkingWindow, parkingWindow.Handle), out pid);
            ...
            internal void EndModalMessageLoop(ApplicationContext context)
            {
                ...
                try
                {
                    ...
                    if (cm != null)
                    {
                        cm.FOnComponentExitState((IntPtr)componentID, WinFormNativeMethods.MSOCM.msocstateModal, WinFormNativeMethods.MSOCM.msoccontextAll, 0, 0);
                    }
                }
                finally
                {
            ...
            internal static ThreadContext FromId(int id)
            {
                ...
                if (context == null && id == WinFormSafeNativeMethods.GetCurrentThreadId())
                {
                    context = new ThreadContext();
                }
            ...
            {
                return totalMessageLoopCount > 0 && baseLoopReason == WinFormNativeMethods.MSOCM.msoloopMain;
            }
            ...
            {
                if (culture == null || culture.LCID != WinFormSafeNativeMethods.GetThreadLocale())
                    culture = new CultureInfo(WinFormSafeNativeMethods.GetThreadLocale());
                return culture;
            }
            ...
            internal bool GetMessageLoop(bool mustBeActive)
            {
                ...
                if (ComponentManager != null && externalComponentManager)
                {
                    ...
                    WinFormUnsafeNativeMethods.IMsoComponent[] activeComponents = new WinFormUnsafeNativeMethods.IMsoComponent[1];
                    if (ComponentManager.FGetActiveComponent(WinFormNativeMethods.MSOCM.msogacActive, activeComponents, null, 0) &&
            ...
            internal System.Threading.ApartmentState OleRequired()
            {
                ...
                if (!GetState(STATE_OLEINITIALIZED))
                {
                    ...
                    int ret = WinFormUnsafeNativeMethods.OleInitialize();
                    ...
                    if (ret == WinFormNativeMethods.RPC_E_CHANGED_MODE)
                    {
            ...
            internal void PostQuit()
            {
                ...
                // to.
                //
                WinFormUnsafeNativeMethods.PostThreadMessage(id, WinFormNativeMethods.WM_QUIT, IntPtr.Zero, IntPtr.Zero);
            ...
            private void RunMessageLoopInner(int reason, ApplicationContext context)
            {
                ...
                if (reason == WinFormNativeMethods.MSOCM.msoloopModalForm && !SystemInformation.UserInteractive)
                {
                ...
                if (reason == WinFormNativeMethods.MSOCM.msoloopMain)
                {
                    ...
                    {
                        throw new InvalidOperationException(DCSR.GetString(DCSR.CantNestMessageLoops));
                    }
                ...
                if (reason == WinFormNativeMethods.MSOCM.msoloopDoEventsModal)
                {
                    localModal = true;
                }

                if (reason == WinFormNativeMethods.MSOCM.msoloopModalForm || reason == WinFormNativeMethods.MSOCM.msoloopModalAlert)
                {
                    fullModal = true;
                    ...
                    // process.
                    hwndOwner = new HandleRef(null, WinFormUnsafeNativeMethods.GetWindowLong(new HandleRef(currentForm, currentForm.Handle), WinFormNativeMethods.GWL_HWNDPARENT));
                    if (hwndOwner.Handle != IntPtr.Zero)
                    {
                        if (WinFormSafeNativeMethods.IsWindowEnabled(hwndOwner))
                        {
                            WinFormSafeNativeMethods.EnableWindow(hwndOwner, false);
                        }
                        else
                        {
                    ...
                    if (currentForm != null &&
                        ...
                        WinFormSafeNativeMethods.IsWindowEnabled(new HandleRef(currentForm, currentForm.Handle)) != modalEnabled)
                    {
                        WinFormSafeNativeMethods.EnableWindow(new HandleRef(currentForm, currentForm.Handle), modalEnabled);
                    }
                }
                ...
                try
                {
                    ...
                    else if (reason == WinFormNativeMethods.MSOCM.msoloopDoEvents ||
                             reason == WinFormNativeMethods.MSOCM.msoloopDoEventsModal)
                    {
                ...
                finally
                {
                    ...
                    if (fullModal)
                    {
                        ...
                        if (hwndOwner.Handle != IntPtr.Zero)
                        {
                            WinFormSafeNativeMethods.EnableWindow(hwndOwner, true);
                        }
                    }
            ...
            public void PublishProcessMessage(WinFormNativeMethods.MSG msg)
            {
                if (this.ProcessFilters(ref msg, out bool modified))
                {
                    return;
                }
            }
            private bool LocalModalMessageLoop(Form form)
            {
                ...
                //            if (msg.hwnd != IntPtr.Zero && WinFormSafeNativeMethods.IsWindowUnicode(new HandleRef(null, msg.hwnd))) {
                //                unicodeWindow = true;
                //                if (!WinFormUnsafeNativeMethods.GetMessageW(ref msg, WinFormNativeMethods.NullHandleRef, 0, 0)) {
                //                    continue;
                //                }
                ...
                //            else {
                //                unicodeWindow = false;
                //                if (!WinFormUnsafeNativeMethods.GetMessageA(ref msg, WinFormNativeMethods.NullHandleRef, 0, 0)) {
                //                    continue;
                //                }
                //            }
                ...
                //            break;
                //        }
                //        else if (!WinFormUnsafeNativeMethods.PeekMessage(ref msg, WinFormNativeMethods.NullHandleRef, 0, 0, WinFormNativeMethods.PM_NOREMOVE)) {
                //            WinFormUnsafeNativeMethods.WaitMessage();
                //        }
                //    }
                //    return continueLoop;
            ...
            internal bool ProcessFilters(ref WinFormNativeMethods.MSG msg, out bool modified)
            {
                bool filtered = false;
            ...
            internal bool ProcessFilters(ref Message m, out bool modified)
            {
                bool filtered = false;
            ...
            ///     mechanism.
            /// </devdoc>
            internal bool PreTranslateMessage(ref WinFormNativeMethods.MSG msg)
            {
                bool modified = false;
                ...
                if (msg.message >= WinFormNativeMethods.WM_KEYFIRST
                        && msg.message <= WinFormNativeMethods.WM_KEYLAST)
                {
                    if (msg.message == WinFormNativeMethods.WM_CHAR)
                    {
                    ...
                    else
                    {
                        ...
                        //
                        IntPtr hwndRoot = WinFormUnsafeNativeMethods.GetAncestor(new HandleRef(null, msg.hwnd), WinFormNativeMethods.GA_ROOT);
                        ...
                        //
                        if (hwndRoot != IntPtr.Zero && WinFormUnsafeNativeMethods.IsDialogMessage(new HandleRef(null, hwndRoot), ref msg))
                        {
                            return true;
                        }
                    }
            ...
            private void RevokeComponent()
            {
                ...
                {
                    int id = componentID;
                    WinFormUnsafeNativeMethods.IMsoComponentManager msocm = componentManager;
            ...
            internal void SetCulture(CultureInfo culture)
            {
                if (culture != null && culture.LCID != WinFormSafeNativeMethods.GetThreadLocale())
                {
                    WinFormSafeNativeMethods.SetThreadLocale(culture.LCID);
                }
            }
            ...
            ///      msodmWriteBe message.
            /// </devdoc>
            bool WinFormUnsafeNativeMethods.IMsoComponent.FDebugMessage(IntPtr hInst, int msg, IntPtr wparam, IntPtr lparam)
            {
            ...
            /// </devdoc>
            bool WinFormUnsafeNativeMethods.IMsoComponent.FPreTranslateMessage(ref WinFormNativeMethods.MSG msg)
            {
            ...
            /// </devdoc>
            void WinFormUnsafeNativeMethods.IMsoComponent.OnEnterState(int uStateID, bool fEnter)
            {
                ...
                if (uStateID == WinFormNativeMethods.MSOCM.msocstateModal)
                {
            ...
            /// </devdoc>
            void WinFormUnsafeNativeMethods.IMsoComponent.OnAppActivate(bool fActive, int dwOtherThreadID)
            {
            }
            ...
            /// </devdoc>
            void WinFormUnsafeNativeMethods.IMsoComponent.OnLoseActivation()
            {
            ...
            ///      pcrinfo.
            /// </devdoc>
            void WinFormUnsafeNativeMethods.IMsoComponent.OnActivationChange(WinFormUnsafeNativeMethods.IMsoComponent component, bool fSameComponent,
                                                  int pcrinfo,
            ...
            /// </devdoc>
            bool WinFormUnsafeNativeMethods.IMsoComponent.FDoIdle(int grfidlef)
            {
                if (idleHandler != null)
                {
            ...
            /// </devdoc>
            bool WinFormUnsafeNativeMethods.IMsoComponent.FContinueMessageLoop(int reason, int pvLoopData, WinFormNativeMethods.MSG[] msgPeeked)
            {
                ...
                else
                {
                    switch (reason)
                    {
                        case WinFormNativeMethods.MSOCM.msoloopFocusWait:

                            // For focus wait, check to see if we are now the active application.
                            //
                            int pid;
                            WinFormSafeNativeMethods.GetWindowThreadProcessId(new HandleRef(null, WinFormUnsafeNativeMethods.GetActiveWindow()), out pid);
                            if (pid == WinFormSafeNativeMethods.GetCurrentProcessId())
                            {
                                continueLoop = false;
                            }
                            break;

                        case WinFormNativeMethods.MSOCM.msoloopModalAlert:
                        case WinFormNativeMethods.MSOCM.msoloopModalForm:
                        ...
                        case WinFormNativeMethods.MSOCM.msoloopDoEvents:
                        case WinFormNativeMethods.MSOCM.msoloopDoEventsModal:
                            // For DoEvents, just see if there are more messages on the queue.
                            //
                            if (!WinFormUnsafeNativeMethods.PeekMessage(ref tempMsg, WinFormNativeMethods.NullHandleRef, 0, 0, WinFormNativeMethods.PM_NOREMOVE))
                            {
                                continueLoop = false;
                            }
            ...
            /// </devdoc>
            bool WinFormUnsafeNativeMethods.IMsoComponent.FQueryTerminate(bool fPromptUser)
            {
                return true;
            }
            ...
            ///      necessary cleanup.
            /// </devdoc>
            void WinFormUnsafeNativeMethods.IMsoComponent.Terminate()
            {
            ...
            ///      exists.
            /// </devdoc>
            IntPtr WinFormUnsafeNativeMethods.IMsoComponent.HwndGetWindow(int dwWhich, int dwReserved)
            {
                return IntPtr.Zero;
            }
        }
        ...
        internal sealed class ParkingWindow : ContainerControl, IArrangedElement
        {
            ...
            private const int WM_CHECKDESTROY = WinFormNativeMethods.WM_USER + 0x01;
            ...
            internal override void RemoveReflectChild()
            {
                ...
                if (childCount == 0)
                {
                    if (IsHandleCreated)
                    {
                        ...
                        int id = WinFormSafeNativeMethods.GetWindowThreadProcessId(new HandleRef(this, HandleInternal), out lpdwProcessId);
                        ...
                        {
                            WinFormUnsafeNativeMethods.PostMessage(new HandleRef(this, HandleInternal), WM_CHECKDESTROY, IntPtr.Zero, IntPtr.Zero);
                        }
                        else
                        {
                            CheckDestroy();
            ...
            private void CheckDestroy()
            {
                if (childCount == 0)
                {
                    IntPtr hwndChild = WinFormUnsafeNativeMethods.GetWindow(new HandleRef(this, Handle), WinFormNativeMethods.GW_CHILD);
            ...
            internal void ParkHandle(HandleRef handle)
            {
                ...
                WinFormUnsafeNativeMethods.SetParent(handle, new HandleRef(this, Handle));
            }
            ...
            internal void UnparkHandle(HandleRef handle)
            {
                if (IsHandleCreated)
                {
                    Debug.Assert(WinFormUnsafeNativeMethods.GetParent(handle) != Handle, "Always set the handle's parent to someone else before calling UnparkHandle");
            ...
            {
                if (m.Msg != WinFormNativeMethods.WM_SHOWWINDOW)
                {
                    await base.WndProc(m);
                    if (m.Msg == WinFormNativeMethods.WM_PARENTNOTIFY)
                    {
                        if (WinFormNativeMethods.Util.LOWORD(unchecked((int)(long)m.WParam)) == WinFormNativeMethods.WM_DESTROY)
                        {
                            WinFormUnsafeNativeMethods.PostMessage(new HandleRef(this, Handle), WM_CHECKDESTROY, IntPtr.Zero, IntPtr.Zero);
                        }
                    }
        ...
        private sealed class ThreadWindows
        {
            ...
            internal ThreadWindows(bool onlyWinForms)
            {
                ...
                WinFormUnsafeNativeMethods.EnumThreadWindows(WinFormSafeNativeMethods.GetCurrentThreadId(),
                                                new WinFormNativeMethods.EnumThreadWindowsCallback(this.Callback),
                                                WinFormNativeMethods.NullHandleRef);
            }
            ...
            private bool Callback(IntPtr hWnd, IntPtr lparam)
            {
                ...
                //
                if (WinFormSafeNativeMethods.IsWindowVisible(new HandleRef(null, hWnd)) && WinFormSafeNativeMethods.IsWindowEnabled(new HandleRef(null, hWnd)))
                {
                    bool add = true;
            ...
            internal void Dispose()
            {
                for (int i = 0; i < windowCount; i++)
                {
                    ...
                    if (WinFormUnsafeNativeMethods.IsWindow(new HandleRef(null, hWnd)))
                    {
                        Control c = Control.FromHandleInternal(hWnd);
                        if (c != null)
                        {
            ...
            internal void Enable(bool state)
            {
                ...
                {
                    activeHwnd = WinFormUnsafeNativeMethods.GetActiveWindow();
                    ...
                    {
                        focusedHwnd = WinFormUnsafeNativeMethods.GetFocus();
                    }
                }
                ...
                for (int i = 0; i < windowCount; i++)
                {
                    ...
                    if (WinFormUnsafeNativeMethods.IsWindow(new HandleRef(null, hWnd))) WinFormSafeNativeMethods.EnableWindow(new HandleRef(null, hWnd), state);
                }
                ...
                if (!onlyWinForms && state)
                {
                    if (activeHwnd != IntPtr.Zero && WinFormUnsafeNativeMethods.IsWindow(new HandleRef(null, activeHwnd)))
                    {
                        WinFormUnsafeNativeMethods.SetActiveWindow(new HandleRef(null, activeHwnd));
                    }

                    if (focusedHwnd != IntPtr.Zero && WinFormUnsafeNativeMethods.IsWindow(new HandleRef(null, focusedHwnd)))
                    {
                        WinFormUnsafeNativeMethods.SetFocus(new HandleRef(null, focusedHwnd));
                    }
                }
            }
        }
        ...
        private class ModalApplicationContext : ApplicationContext
        {
            ...
            public void DisableThreadWindows(bool disable, bool onlyWinForms)
            {
                ...
                if (MainForm != null && MainForm.IsHandleCreated)
                {
                    ...
                    //
                    IntPtr parentHandle = WinFormUnsafeNativeMethods.GetWindowLong(new HandleRef(this, MainForm.Handle), WinFormNativeMethods.GWL_HWNDPARENT);
