    ...
    public class DataGridViewComboBoxCell : DataGridViewCell
    {
        ...
        public virtual int DropDownWidth
        {
            ...
            set
            {
                ...
                if (value < 1)
                {
                    throw new ArgumentOutOfRangeException("DropDownWidth", value, DCSR.GetString(DCSR.DataGridViewComboBoxCell_DropDownWidthOutOfRange, (1).ToString(CultureInfo.CurrentCulture)));
                }
        ...
        public virtual int MaxDropDownItems
        {
            ...
            set
            {
                ...
                {
                    throw new ArgumentOutOfRangeException("MaxDropDownItems", value, DCSR.GetString(DCSR.DataGridViewComboBoxCell_MaxDropDownItemsOutOfRange, (1).ToString(CultureInfo.CurrentCulture), (100).ToString(CultureInfo.CurrentCulture)));
                }
            ...
            if (owningComboBoxColumn != null)
            {
                    ...
                    autoSizeColumnMode != DataGridViewAutoSizeColumnMode.None)
                {
                    if (this.DropDownWidth == 1)
                    {
                        ...
                        WinFormUnsafeNativeMethods.SendMessage(new HandleRef(comboBox, comboBox.Handle), WinFormNativeMethods.CB_SETDROPPEDWIDTH, cachedDropDownWidth, 0);
                    }
                }
                else
                {
                    ...
                    int dropDownWidth = unchecked( (int) (long)WinFormUnsafeNativeMethods.SendMessage(new HandleRef(comboBox, comboBox.Handle), WinFormNativeMethods.CB_GETDROPPEDWIDTH, 0, 0));
                    if (dropDownWidth != this.DropDownWidth)
                    {
                        WinFormUnsafeNativeMethods.SendMessage(new HandleRef(comboBox, comboBox.Handle), WinFormNativeMethods.CB_SETDROPPEDWIDTH, this.DropDownWidth, 0);
                    }
                }
            }
        }
                                                    ...
                                                    DataGridViewDataErrorContexts context)
        {
            ...
            if (value == null || ((this.ValueType != null && !this.ValueType.IsAssignableFrom(value.GetType())) && value != System.DBNull.Value))
            {
                ...
                if (this.DataGridView != null)
                {
                    DataGridViewDataErrorEventArgs dgvdee = new DataGridViewDataErrorEventArgs(
                        new FormatException(DCSR.GetString(DCSR.DataGridViewComboBoxCell_InvalidValue)), this.ColumnIndex,
                        rowIndex, context);
                ...
                !string.IsNullOrEmpty(this.ValueMember) || !string.IsNullOrEmpty(this.DisplayMember))
            {
                ...
                if (!LookupDisplayValue(rowIndex, value, out displayValue))
                {
                    ...
                    {
                        DataGridViewDataErrorEventArgs dgvdee = new DataGridViewDataErrorEventArgs(
                            new ArgumentException(DCSR.GetString(DCSR.DataGridViewComboBoxCell_InvalidValue)), this.ColumnIndex,
                            rowIndex, context);
                        RaiseDataError(dgvdee);
                        if (dgvdee.ThrowException)
                        {
        ...
        private void InitializeDisplayMemberPropertyDescriptor(string displayMember)
        {
            if (this.DataManager != null)
            {
                ...
                else
                {
                    ...
                    {
                        throw new ArgumentException(DCSR.GetString(DCSR.DataGridViewComboBoxCell_FieldNotFound, displayMember));
                    }
                    else
                    {
        ...
        private void InitializeValueMemberPropertyDescriptor(string valueMember)
        {
            if (this.DataManager != null)
            {
                ...
                else
                {
                    ...
                    {
                        throw new ArgumentException(DCSR.GetString(DCSR.DataGridViewComboBoxCell_FieldNotFound, valueMember));
                    }
                    else
                    {
                                                   ...
                                                   TypeConverter valueTypeConverter)
        {
                ...
                !string.IsNullOrEmpty(this.DisplayMember) || !string.IsNullOrEmpty(this.ValueMember))
            {
                ...
                if (!LookupValue(originalValue, out value))
                {
                    ...
                    {
                        throw new FormatException(String.Format(CultureInfo.CurrentCulture, DCSR.GetString(DCSR.Formatter_CantConvert), value, this.DisplayType));
                    }
                }
                return value;
            }
        ...
        public class ObjectCollection : IList 
        {
            ...
            internal void AddRangeInternal(ICollection items) 
            {
                ...
                foreach(object item in items)
                {
                    if (item == null)
                    {
                        throw new InvalidOperationException(DCSR.GetString(DCSR.InvalidNullItemInCollection));
                    }
                }
