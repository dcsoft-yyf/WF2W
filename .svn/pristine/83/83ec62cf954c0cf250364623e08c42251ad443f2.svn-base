...
namespace System.Windows.Forms {
    ...
    public sealed class ToolStripManager {
        ...
        internal class ModalMenuFilter : IMessageModifyAndFilter {
            private HandleRef _activeHwnd = WinFormNativeMethods.NullHandleRef; // the window that was active when we showed the dropdown
            private HandleRef _lastActiveWindow = WinFormNativeMethods.NullHandleRef;         // the window that was last known to be active
                ...
                if (InMenuMode) {
                    try {
                        ...
                        if (ActiveHwnd.Handle != IntPtr.Zero) {
                            ...
                            ActiveHwndInternal = WinFormNativeMethods.NullHandleRef;
                        }
                        ...
                        if (_caretHidden) {
                            _caretHidden = false;
                            WinFormSafeNativeMethods.ShowCaret(WinFormNativeMethods.NullHandleRef);
                        }
            ...
            private void ProcessMouseButtonPressed(IntPtr hwndMouseMessageIsFrom, int x, int y) {
                ...
                for (int i = 0; i < countDropDowns; i++) {
                    ...
                    if (activeToolStrip != null) {
                        ...
                        pt.x = x;
                        pt.y = y;
                        WinFormUnsafeNativeMethods.MapWindowPoints(new HandleRef(activeToolStrip, hwndMouseMessageIsFrom), new HandleRef(activeToolStrip, activeToolStrip.Handle), pt, 1);
            ...
            private void SetActiveToolStripCore(ToolStrip toolStrip) {
                ...
                if (!_caretHidden && toolStrip.IsDropDown && InMenuMode) {
                    _caretHidden = true;
                    WinFormSafeNativeMethods.HideCaret(WinFormNativeMethods.NullHandleRef);
                }
            ...
            private static bool IsChildOrSameWindow(HandleRef hwndParent, HandleRef hwndChild) {
                ...
                if (WinFormUnsafeNativeMethods.IsChild(hwndParent, hwndChild)) {
                    return true;
                }
                return false;
            }
            ...
            private static bool IsKeyOrMouseMessage(Message m) {
                ...
                else if (m.Msg >= WinFormNativeMethods.WM_NCLBUTTONDOWN && m.Msg <= WinFormNativeMethods.WM_NCMBUTTONDBLCLK) {
                    filterMessage = true;
                }
                else if (m.Msg >= WinFormNativeMethods.WM_KEYFIRST && m.Msg <= WinFormNativeMethods.WM_KEYLAST) {
                    filterMessage = true;
                }
            ...
            public bool PreFilterMessage(ref Message m) {
                ...
                HandleRef hwndCurrentActiveWindow = new HandleRef(null, WinFormUnsafeNativeMethods.GetActiveWindow());
                ...
                switch (m.Msg) {
                    ...
                    case WinFormNativeMethods.WM_NCMOUSEMOVE:
                    ...
                    case WinFormNativeMethods.WM_MBUTTONDOWN:
                        ...
                        ProcessMouseButtonPressed(m.HWnd,
                            /*x=*/WinFormNativeMethods.Util.SignedLOWORD(m.LParam),
                            /*y=*/WinFormNativeMethods.Util.SignedHIWORD(m.LParam));
            ...
            private class HostedWindowsFormsMessageHook {
                ...
                private WinFormNativeMethods.HookProc hookProc;
                ...
                private void InstallMessageHook() {
                    lock (this) {
                        ...
                        hookProc = new WinFormNativeMethods.HookProc(this.MessageHookProc);

                        messageHookHandle = WinFormUnsafeNativeMethods.SetWindowsHookEx(WinFormNativeMethods.WH_GETMESSAGE,
                                                                   hookProc,
                ...
                private unsafe IntPtr MessageHookProc(int nCode, IntPtr wparam, IntPtr lparam) {
                    if (nCode == WinFormNativeMethods.HC_ACTION)  {
                        if (isHooked && (int)wparam == WinFormNativeMethods.PM_REMOVE /*only process GetMessage, not PeekMessage*/) {
                            // only process messages we've pulled off the queue
                            WinFormNativeMethods.MSG* msg = (WinFormNativeMethods.MSG*)lparam;
                            if (msg != null) {
                                ...
                                if (Application.ThreadContext.FromCurrent().PreTranslateMessage(ref *msg)) {
                                    msg->message = WinFormNativeMethods.WM_NULL;
                                }
                            }
                        }
                    }

                    return WinFormUnsafeNativeMethods.CallNextHookEx(new HandleRef(this, messageHookHandle), nCode, wparam, lparam);
                }

                private void UninstallMessageHook() {
                    lock (this) {
                        if (messageHookHandle != IntPtr.Zero) {
                            WinFormUnsafeNativeMethods.UnhookWindowsHookEx(new HandleRef(this, messageHookHandle));
                            hookProc = null;
        ...
        internal static bool ProcessCmdKey(ref Message m, Keys keyData) {
            ...
            if (m.Msg == WinFormNativeMethods.WM_SYSKEYDOWN) {
        ...
        internal static bool ProcessMenuKey(ref Message m) {
            ...
            if (intendedControl != null) {
                ...
                if (toplevelControl != null) {
                    IntPtr hMenu = WinFormUnsafeNativeMethods.GetMenu(new HandleRef(toplevelControl, toplevelControl.Handle));
            ...
            if ((char)keyData == ' ') { // dont process system menu 
                ...
                if (WinFormUnsafeNativeMethods.GetKeyState((int)Keys.ShiftKey) < 0 && (keyData == Keys.None)) {
        ...
        public static bool Merge(ToolStrip sourceToolStrip, ToolStrip targetToolStrip) {
            ...
            if (targetToolStrip == sourceToolStrip) {
                throw new ArgumentException(DCSR.GetString(DCSR.ToolStripMergeImpossibleIdentical));
            }
