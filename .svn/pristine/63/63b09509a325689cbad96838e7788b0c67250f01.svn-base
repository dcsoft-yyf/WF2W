...
namespace System.Windows.Forms.ComponentModel.Com2Interop {
    ...
    internal class Com2IVsPerPropertyBrowsingHandler: Com2ExtendedBrowsingHandler {
         ...
         public override Type Interface {
            get {
               return typeof(WinFormNativeMethods.IVsPerPropertyBrowsing);
            }
         }
         
         public static bool AllowChildProperties(Com2PropertyDescriptor propDesc) {
            if (propDesc.TargetObject is WinFormNativeMethods.IVsPerPropertyBrowsing) {
               bool pfHide = false;
               int hr = ((WinFormNativeMethods.IVsPerPropertyBrowsing)propDesc.TargetObject).DisplayChildProperties(propDesc.DISPID, ref pfHide);
               return (hr == WinFormNativeMethods.S_OK && pfHide);
            }
            return false;
         }
         ...
         private void OnGetBaseAttributes(Com2PropertyDescriptor sender, GetAttributesEvent attrEvent){
             WinFormNativeMethods.IVsPerPropertyBrowsing vsObj = sender.TargetObject as WinFormNativeMethods.IVsPerPropertyBrowsing;
             ...
             if (hr == WinFormNativeMethods.S_OK && pHelpString[0] != null){
         ...
         private void OnGetDynamicAttributes(Com2PropertyDescriptor sender, GetAttributesEvent attrEvent){
               ...
               if (sender.TargetObject is WinFormNativeMethods.IVsPerPropertyBrowsing){
                  WinFormNativeMethods.IVsPerPropertyBrowsing vsObj = (WinFormNativeMethods.IVsPerPropertyBrowsing)sender.TargetObject;
                  ...
                  // should we show this
                  if (typeof(WinFormUnsafeNativeMethods.IDispatch).IsAssignableFrom(sender.PropertyType) && sender.CanShow){
                     bool pfDisplay = false;
                     hr = vsObj.DisplayChildProperties(sender.DISPID, ref pfDisplay);
                     if (hr == WinFormNativeMethods.S_OK && pfDisplay){
                           attrEvent.Add(BrowsableAttribute.Yes);
                     }
                  }
               }
               Debug.Assert(sender.TargetObject == null || sender.TargetObject is WinFormNativeMethods.IVsPerPropertyBrowsing, "Object is not " + Interface.Name + "!");
         }

         private void OnCanResetPropertyValue(Com2PropertyDescriptor sender, GetBoolValueEvent boolEvent) {
               if (sender.TargetObject is WinFormNativeMethods.IVsPerPropertyBrowsing) {
                    WinFormNativeMethods.IVsPerPropertyBrowsing target = (WinFormNativeMethods.IVsPerPropertyBrowsing)sender.TargetObject;
         ...
         private void OnGetDisplayName(Com2PropertyDescriptor sender, GetNameItemEvent nameItem){
               if (sender.TargetObject is WinFormNativeMethods.IVsPerPropertyBrowsing){
                  ...
                  if (hr == WinFormNativeMethods.S_OK && pNameString[0] != null){
         ...
         private void OnGetTypeConverterAndTypeEditor(Com2PropertyDescriptor sender, GetTypeConverterAndTypeEditorEvent gveevent) {
            if (sender.TargetObject is WinFormNativeMethods.IVsPerPropertyBrowsing){
                  ...
                  if (sender.CanShow && typeof(WinFormUnsafeNativeMethods.IDispatch).IsAssignableFrom(sender.PropertyType)){
                     WinFormNativeMethods.IVsPerPropertyBrowsing vsObj = (WinFormNativeMethods.IVsPerPropertyBrowsing)sender.TargetObject;
                     ...
                     if (gveevent.TypeConverter is Com2IDispatchConverter){
                        gveevent.TypeConverter = new Com2IDispatchConverter(sender, (hr == WinFormNativeMethods.S_OK && pfResult));
                     }
                     else{
                        gveevent.TypeConverter = new Com2IDispatchConverter(sender, (hr == WinFormNativeMethods.S_OK && pfResult), gveevent.TypeConverter);
                     }
                  }
               }
               Debug.Assert(sender.TargetObject == null || sender.TargetObject is WinFormNativeMethods.IVsPerPropertyBrowsing, "Object is not " + Interface.Name + "!");
         ...
         private void OnShouldSerializeValue(Com2PropertyDescriptor sender, GetBoolValueEvent gbvevent){
               if (sender.TargetObject is WinFormNativeMethods.IVsPerPropertyBrowsing){
                     ...
                     if (hr == WinFormNativeMethods.S_OK && !pfResult){
