...
[System.Reflection.Obfuscation(Exclude = true, ApplyToMembers = false )]
    public class ListView : Control {
        ...
        SRDescription(DCSR.ListViewActivationDescr)
        ]
        public ItemActivation Activation {
            ...
            set {
                ...
                if (this.HotTracking && value != ItemActivation.OneClick) {
                    throw new ArgumentException(DCSR.GetString(DCSR.ListViewActivationMustBeOnWhenHotTrackingIsOn), "value");
                }
        ...
        Localizable(true),
        SRDescription(DCSR.ListViewAlignmentDescr)
        ]
        ...
        SRCategory(DCSR.CatBehavior),
        DefaultValue(false),
        SRDescription(DCSR.ListViewAllowColumnReorderDescr)
        ]
        ...
        SRCategory(DCSR.CatBehavior),
        DefaultValue(true),
        SRDescription(DCSR.ListViewAutoArrangeDescr)
        ]
        ...
        public override Color BackColor {
            ...
            set {
                ...
                if (IsHandleCreated) {
                    SendMessage(WinFormNativeMethods.LVM_SETBKCOLOR, 0, ColorTranslator.ToWin32(BackColor));
                }
            }
        }
        ...
        DefaultValue(false),
        SRDescription(DCSR.ListViewBackgroundImageTiledDescr)
        ]
            ...
            set {
                if (BackgroundImageTiled != value) {
                    ...
                    if (IsHandleCreated && BackgroundImage != null) {
                        ...
                        WinFormNativeMethods.LVBKIMAGE lvbkImage = new WinFormNativeMethods.LVBKIMAGE();
                        lvbkImage.xOffset = 0;
                        lvbkImage.yOffset = 0;

                        if (BackgroundImageTiled)
                            lvbkImage.ulFlags = WinFormNativeMethods.LVBKIF_STYLE_TILE;
                        else
                            lvbkImage.ulFlags = WinFormNativeMethods.LVBKIF_STYLE_NORMAL;

                        lvbkImage.ulFlags  |= WinFormNativeMethods.LVBKIF_SOURCE_URL;
                        lvbkImage.pszImage = this.backgroundImageFileName;
                        lvbkImage.cchImageMax = this.backgroundImageFileName.Length + 1;

                        WinFormUnsafeNativeMethods.SendMessage(new HandleRef(this, this.Handle), WinFormNativeMethods.LVM_SETBKIMAGE, 0, lvbkImage);
                    }
                }
            }
        }
        ...
        SRDescription(DCSR.borderStyleDescr)
        ]
        ...
        SRCategory(DCSR.CatAppearance),
        DefaultValue(false),
        SRDescription(DCSR.ListViewCheckBoxesDescr)
        ]
                ...
                if (this.UseCompatibleStateImageBehavior) {
                    if (CheckBoxes != value) {
                        ...
                        if (value && this.View == View.Tile) {
                            throw new NotSupportedException(DCSR.GetString(DCSR.ListViewCheckBoxesNotSupportedInTileView));
                        }
                    ...
                    if (CheckBoxes != value) {
                        ...
                        if (IsHandleCreated && imageListState != null) {
                            if (CheckBoxes) { // we want custom checkboxes
                                SendMessage(WinFormNativeMethods.LVM_SETIMAGELIST, WinFormNativeMethods.LVSIL_STATE, imageListState.Handle);
                            } else {
                                SendMessage(WinFormNativeMethods.LVM_SETIMAGELIST, WinFormNativeMethods.LVSIL_STATE, IntPtr.Zero);
                            }
                        }
        ...
        SRDescription(DCSR.ListViewColumnsDescr),
        Localizable(true),
        ...
        protected override CreateParams CreateParams {
            ...
            get {
                ...
                cp.ClassName = WinFormNativeMethods.WC_LISTVIEW;

                // Keep the scrollbar if we are just updating styles...
                //
                if (IsHandleCreated) {
                    int currentStyle = unchecked((int)((long)WinFormUnsafeNativeMethods.GetWindowLong(new HandleRef(this, Handle), WinFormNativeMethods.GWL_STYLE)));
                    cp.Style |= (currentStyle & (WinFormNativeMethods.WS_HSCROLL | WinFormNativeMethods.WS_VSCROLL));
                }

                cp.Style |= WinFormNativeMethods.LVS_SHAREIMAGELISTS;

                switch (alignStyle) {
                    case ListViewAlignment.Top:
                        cp.Style |= WinFormNativeMethods.LVS_ALIGNTOP;
                        break;
                    case ListViewAlignment.Left:
                        cp.Style |= WinFormNativeMethods.LVS_ALIGNLEFT;
                        break;
                }

                if (AutoArrange)
                    cp.Style |= WinFormNativeMethods.LVS_AUTOARRANGE;
                ...
                switch (headerStyle) {
                    case ColumnHeaderStyle.None:
                        cp.Style |= WinFormNativeMethods.LVS_NOCOLUMNHEADER;
                        break;
                    case ColumnHeaderStyle.Nonclickable:
                        cp.Style |= WinFormNativeMethods.LVS_NOSORTHEADER;
                        break;
                }

                if (LabelEdit)
                    cp.Style |= WinFormNativeMethods.LVS_EDITLABELS;

                if (!LabelWrap)
                    cp.Style |= WinFormNativeMethods.LVS_NOLABELWRAP;

                if (!HideSelection)
                    cp.Style |= WinFormNativeMethods.LVS_SHOWSELALWAYS;

                if (!MultiSelect)
                    cp.Style |= WinFormNativeMethods.LVS_SINGLESEL;

                if (listItemSorter == null) {
                    switch (sorting) {
                        case SortOrder.Ascending:
                            cp.Style |= WinFormNativeMethods.LVS_SORTASCENDING;
                            break;
                        case SortOrder.Descending:
                            cp.Style |= WinFormNativeMethods.LVS_SORTDESCENDING;
                            break;
                    }
                }

                if (VirtualMode)
                    cp.Style |= WinFormNativeMethods.LVS_OWNERDATA;
        ...
        internal ListViewGroup DefaultGroup {
            get
            {
                if (defaultGroup == null) {
                    defaultGroup = new ListViewGroup(DCSR.GetString(DCSR.ListViewGroupDefaultGroup, "1"));
                }
                return defaultGroup;
            }
        }
        ...
        Browsable(false),
        DesignerSerializationVisibility(DesignerSerializationVisibility.Hidden),
        SRDescription(DCSR.ListViewFocusedItemDescr)
        ]
        public ListViewItem FocusedItem {
            get {
                if (IsHandleCreated) {
                    int displayIndex = unchecked( (int) (long)SendMessage(WinFormNativeMethods.LVM_GETNEXTITEM, -1, WinFormNativeMethods.LVNI_FOCUSED));
                    if (displayIndex > -1)
        ...
        public override Color ForeColor
        {
            ...
            set {
                ...
                if (IsHandleCreated) {
                    SendMessage(WinFormNativeMethods.LVM_SETTEXTCOLOR, 0, ColorTranslator.ToWin32(ForeColor));
                }
            }
        }
        ...
        SRCategory(DCSR.CatAppearance),
        DefaultValue(false),
        SRDescription(DCSR.ListViewFullRowSelectDescr)
        ]
        ...
        SRCategory(DCSR.CatAppearance),
        DefaultValue(false),
        SRDescription(DCSR.ListViewGridLinesDescr)
        ]
        ...
        SRDescription(DCSR.ListViewGroupsDescr),
        MergableProperty(false)
        ]
        ...
        SRDescription(DCSR.ListViewHeaderStyleDescr)
        ]
        ...
        SRCategory(DCSR.CatBehavior),
        DefaultValue(true),
        SRDescription(DCSR.ListViewHideSelectionDescr)
        ]
        ...
        SRCategory(DCSR.CatBehavior),
        DefaultValue(false),
        SRDescription(DCSR.ListViewHotTrackingDescr)
        ]
        ...
        SRCategory(DCSR.CatBehavior),
        DefaultValue(false),
        SRDescription(DCSR.ListViewHoverSelectDescr)
        ]
        public bool HoverSelection {
            ...
            set {
                if (HoverSelection != value) {
                    if (HotTracking && !value) {
                        throw new ArgumentException(DCSR.GetString(DCSR.ListViewHoverMustBeOnWhenHotTrackingIsOn), "value");
                    }
        ...
        Browsable(false),
        DesignerSerializationVisibility(DesignerSerializationVisibility.Hidden),
        SRDescription(DCSR.ListViewInsertionMarkDescr)
        ]
        ...
        SRDescription(DCSR.ListViewItemsDescr),
        MergableProperty(false)
        ]
        ...
        SRCategory(DCSR.CatBehavior),
        DefaultValue(false),
        SRDescription(DCSR.ListViewLabelEditDescr)
        ]
        ...
        DefaultValue(true),
        Localizable(true),
        SRDescription(DCSR.ListViewLabelWrapDescr)
        ]
        ...
        SRCategory(DCSR.CatBehavior),
        DefaultValue(null),
        SRDescription(DCSR.ListViewLargeImageListDescr)
        ]
        public ImageList LargeImageList {
            ...
            set {
                if (value != imageListLarge) {
                    ...
                    if (IsHandleCreated) {
                        SendMessage(WinFormNativeMethods.LVM_SETIMAGELIST, (IntPtr)WinFormNativeMethods.LVSIL_NORMAL, value == null ? IntPtr.Zero: value.Handle);
        ...
        Browsable(false),
        DesignerSerializationVisibility(DesignerSerializationVisibility.Hidden),
        SRDescription(DCSR.ListViewItemSorterDescr)
        ]
        ...
        SRCategory(DCSR.CatBehavior),
        DefaultValue(true),
        SRDescription(DCSR.ListViewMultiSelectDescr)
        ]
        ...
        SRCategory(DCSR.CatBehavior),
        DefaultValue(false),
        SRDescription(DCSR.ListViewOwnerDrawDescr)
        ]
        ...
        SRCategory(DCSR.CatBehavior),
        DefaultValue(true),
        SRDescription(DCSR.ListViewScrollableDescr)
        ]
        ...
        Browsable(false),
        DesignerSerializationVisibility(DesignerSerializationVisibility.Hidden),
        SRDescription(DCSR.ListViewSelectedItemsDescr)
        ]
        ...
        SRCategory(DCSR.CatBehavior),
        DefaultValue(true),
        SRDescription(DCSR.ListViewShowGroupsDescr)
        ]
        ...
        SRCategory(DCSR.CatBehavior),
        DefaultValue(null),
        SRDescription(DCSR.ListViewSmallImageListDescr)
        ]
            ...
            set {
                if (imageListSmall != value) {
                    ...
                    if (IsHandleCreated) {
                        ...
                        SendMessage(WinFormNativeMethods.LVM_SETIMAGELIST, (IntPtr)WinFormNativeMethods.LVSIL_SMALL, value == null ? IntPtr.Zero: value.Handle);
        ...
        SRCategory(DCSR.CatBehavior),
        DefaultValue(false),
        SRDescription(DCSR.ListViewShowItemToolTipsDescr)
        ]
        ...
        SRDescription(DCSR.ListViewSortingDescr)
        ]
        ...
        SRCategory(DCSR.CatBehavior),
        DefaultValue(null),
        SRDescription(DCSR.ListViewStateImageListDescr)
        ]
            ...
            set {
                if (this.UseCompatibleStateImageBehavior) {
                    if (imageListState != value) {
                        ...
                        if (IsHandleCreated)
                            SendMessage(WinFormNativeMethods.LVM_SETIMAGELIST, WinFormNativeMethods.LVSIL_STATE, value == null ? IntPtr.Zero: value.Handle);
                    }
                } else {
                    if (imageListState != value) {
                        ...
                        if (IsHandleCreated) {
                            if (CheckBoxes) {
                                ...
                                SendMessage(WinFormNativeMethods.LVM_SETIMAGELIST, WinFormNativeMethods.LVSIL_STATE, (imageListState == null || imageListState.Images.Count == 0) ? IntPtr.Zero : imageListState.Handle);
                            }
        ...
        [
            SRCategory(DCSR.CatAppearance),
            Browsable(true),
            SRDescription(DCSR.ListViewTileSizeDescr),
        ]
            ...
            get {
                if (tileSize.IsEmpty) {
                    if (this.IsHandleCreated) {
                        ...
                        //
                        WinFormNativeMethods.LVTILEVIEWINFO tileViewInfo = new WinFormNativeMethods.LVTILEVIEWINFO();
                        tileViewInfo.dwMask = WinFormNativeMethods.LVTVIM_TILESIZE;
                        WinFormUnsafeNativeMethods.SendMessage(new HandleRef(this, this.Handle), WinFormNativeMethods.LVM_GETTILEVIEWINFO, 0, tileViewInfo);
            ...
            set {
                if (tileSize != value) {
                    if (value.IsEmpty || value.Height <= 0 || value.Width <= 0) {
                        throw new ArgumentOutOfRangeException("TileSize", DCSR.GetString(DCSR.ListViewTileSizeMustBePositive));
                    }
                    ...
                    if (this.IsHandleCreated) {
                        WinFormNativeMethods.LVTILEVIEWINFO tileViewInfo = new WinFormNativeMethods.LVTILEVIEWINFO();
                        tileViewInfo.dwMask = WinFormNativeMethods.LVTVIM_TILESIZE;
                        tileViewInfo.dwFlags = WinFormNativeMethods.LVTVIF_FIXEDSIZE;
                        tileViewInfo.sizeTile = new WinFormNativeMethods.SIZE(tileSize.Width, tileSize.Height);
                        bool retval = WinFormUnsafeNativeMethods.SendMessage(new HandleRef(this, this.Handle), WinFormNativeMethods.LVM_SETTILEVIEWINFO, 0, tileViewInfo);
        ...
        Browsable(false),
        DesignerSerializationVisibility(DesignerSerializationVisibility.Hidden),
        SRDescription(DCSR.ListViewTopItemDescr)
        ]
        public ListViewItem TopItem {
            get {
                if (viewStyle == View.LargeIcon || viewStyle == View.SmallIcon || viewStyle == View.Tile)
                    throw new InvalidOperationException(DCSR.GetString(DCSR.ListViewGetTopItem));
                ...
                topIndex = unchecked( (int) (long)SendMessage(WinFormNativeMethods.LVM_GETTOPINDEX, 0, 0));
            ...
            set {
                if (viewStyle == View.LargeIcon || viewStyle == View.SmallIcon || viewStyle == View.Tile)
                    throw new InvalidOperationException(DCSR.GetString(DCSR.ListViewSetTopItem));
        ...
        SRDescription(DCSR.ListViewViewDescr)
        ]
        public View View {
            get {
                return viewStyle;
            ...
            set {
                if (value == View.Tile && this.CheckBoxes) {
                    throw new NotSupportedException(DCSR.GetString(DCSR.ListViewTileViewDoesNotSupportCheckBoxes));
                }
                ...
                if (value == View.Tile && VirtualMode) {
                    throw new NotSupportedException(DCSR.GetString(DCSR.ListViewCantSetViewToTileViewInVirtualMode));
                }

                if (viewStyle != value) {
                    viewStyle = value;
                    if (IsHandleCreated && ComctlSupportsVisualStyles) {
                        SendMessage(WinFormNativeMethods.LVM_SETVIEW,(int)viewStyle,0);
                        UpdateGroupView();
        ...
        SRDescription(DCSR.ListViewVirtualListSizeDescr)
        ]
        public int VirtualListSize {
            ...
            set {
                if (value < 0)
                    throw new System.ArgumentException(DCSR.GetString(DCSR.ListViewVirtualListSizeInvalidArgument, "value", (value.ToString(CultureInfo.CurrentCulture))));
                ...
                if (IsHandleCreated && VirtualMode && !DesignMode)
                    SendMessage(WinFormNativeMethods.LVM_SETITEMCOUNT, virtualListSize, 0);
        ...
        DefaultValue(false),
        RefreshProperties(RefreshProperties.Repaint),
        SRDescription(DCSR.ListViewVirtualModeDescr)
        ]
        public bool VirtualMode {
            ...
            set {
                ...
                if (value && Items.Count > 0)
                    throw new InvalidOperationException(DCSR.GetString(DCSR.ListViewVirtualListViewRequiresNoItems));

                if (value && CheckedItems.Count > 0) {
                    throw new InvalidOperationException(DCSR.GetString(DCSR.ListViewVirtualListViewRequiresNoCheckedItems));
                }

                if (value && SelectedItems.Count > 0) {
                    throw new InvalidOperationException(DCSR.GetString(DCSR.ListViewVirtualListViewRequiresNoSelectedItems));
                }

                // Tile view does not work w/ VirtualMode.
                if (value && View == View.Tile) {
                    throw new NotSupportedException(DCSR.GetString(DCSR.ListViewCantSetVirtualModeWhenInTileView));
                }
        ...
        ///    <para>[To be supplied.]</para>
        /// </devdoc>
        [SRCategory(DCSR.CatBehavior), SRDescription(DCSR.ListViewAfterLabelEditDescr)]
        ...
        ///    <para>[To be supplied.]</para>
        /// </devdoc>
        [SRCategory(DCSR.CatBehavior), SRDescription(DCSR.ListViewBeforeLabelEditDescr)]
        ...
        ///    <para>[To be supplied.]</para>
        /// </devdoc>
        [SRCategory(DCSR.CatAction), SRDescription(DCSR.ListViewCacheVirtualItemsEventDescr)]
        ...
        ///    <para>[To be supplied.]</para>
        /// </devdoc>
        [SRCategory(DCSR.CatAction), SRDescription(DCSR.ListViewColumnClickDescr)]
        ...
        /// </devdoc>
        [SRCategory(DCSR.CatPropertyChanged), SRDescription(DCSR.ListViewColumnReorderedDscr)]
        ...
        /// </devdoc>
        [SRCategory(DCSR.CatPropertyChanged), SRDescription(DCSR.ListViewColumnWidthChangedDscr)]
        ...
        /// </devdoc>
        [SRCategory(DCSR.CatPropertyChanged), SRDescription(DCSR.ListViewColumnWidthChangingDscr)]
        ...
        /// </devdoc>
        [SRCategory(DCSR.CatBehavior), SRDescription(DCSR.ListViewDrawColumnHeaderEventDescr)]
        ...
        /// </devdoc>
        [SRCategory(DCSR.CatBehavior), SRDescription(DCSR.ListViewDrawItemEventDescr)]
        ...
        /// </devdoc>
        [SRCategory(DCSR.CatBehavior), SRDescription(DCSR.ListViewDrawSubItemEventDescr)]
        ...
        ///    <para>[To be supplied.]</para>
        /// </devdoc>
        [SRCategory(DCSR.CatAction), SRDescription(DCSR.ListViewItemClickDescr)]
        ...
        ///    <para>[To be supplied.]</para>
        /// </devdoc>
        [SRCategory(DCSR.CatBehavior), SRDescription(DCSR.CheckedListBoxItemCheckDescr)]
        ...
        ///    <para>[To be supplied.]</para>
        /// </devdoc>
        [SRCategory(DCSR.CatBehavior), SRDescription(DCSR.ListViewItemCheckedDescr)]
        ...
        ///    <para>[To be supplied.]</para>
        /// </devdoc>
        [SRCategory(DCSR.CatAction), SRDescription(DCSR.ListViewItemDragDescr)]
        ...
        ///    <para>[To be supplied.]</para>
        /// </devdoc>
        [SRCategory(DCSR.CatAction), SRDescription(DCSR.ListViewItemMouseHoverDescr)]
        ...
        [SRCategory(DCSR.CatBehavior), SRDescription(DCSR.ListViewItemSelectionChangedDescr)]
        ...
        ///    <para>[To be supplied.]</para>
        /// </devdoc>
        [SRCategory(DCSR.CatAction), SRDescription(DCSR.ListViewRetrieveVirtualItemEventDescr)]
        ...
        /// <devdoc>
        /// </devdoc>
        [SRCategory(DCSR.CatAction), SRDescription(DCSR.ListViewSearchForVirtualItemDescr)]
        ...
        ///    <para>[To be supplied.]</para>
        /// </devdoc>
        [SRCategory(DCSR.CatBehavior), SRDescription(DCSR.ListViewSelectedIndexChangedDescr)]
        ...
        [SRCategory(DCSR.CatBehavior), SRDescription(DCSR.ListViewVirtualItemsSelectionRangeChangedDescr)]
        ...
        public void ArrangeIcons(ListViewAlignment value) {
            ...
            switch ((int)value) {
                case WinFormNativeMethods.LVA_DEFAULT:
                case WinFormNativeMethods.LVA_ALIGNLEFT:
                case WinFormNativeMethods.LVA_ALIGNTOP:
                case WinFormNativeMethods.LVA_SNAPTOGRID:
                    if (IsHandleCreated) {
                        WinFormUnsafeNativeMethods.PostMessage(new HandleRef(this, this.Handle), WinFormNativeMethods.LVM_ARRANGE, (int) value, 0);
                    }
                    break;

                default:
                    throw new ArgumentException(DCSR.GetString(DCSR.InvalidArgument,
                                                              "value",
                                                              ((value).ToString())));
            }
        ...
        public void ArrangeIcons() {
            ArrangeIcons((ListViewAlignment)WinFormNativeMethods.LVA_DEFAULT);
        }
        ...
        private int CompensateColumnHeaderResize(Message m, bool columnResizeCancelled) {
            if (this.ComctlSupportsVisualStyles &&
                ...
                this.Items.Count > 0) {
                WinFormNativeMethods.NMHEADER header = (WinFormNativeMethods.NMHEADER) m.GetLParam(typeof(WinFormNativeMethods.NMHEADER));
        ...
        protected override void CreateHandle() {
            if (!RecreatingHandle) {
                ...
                try {
                    ...
                    icc.dwICC = WinFormNativeMethods.ICC_LISTVIEW_CLASSES;
        ...
        unsafe void CustomDraw(ref Message m) {
            ...
            try
            {
                WinFormNativeMethods.NMLVCUSTOMDRAW* nmcd = (WinFormNativeMethods.NMLVCUSTOMDRAW*)m.LParam;
                // Find out which stage we're drawing
                switch (nmcd->nmcd.dwDrawStage)
                {
                    case WinFormNativeMethods.CDDS_PREPAINT:
                        if (OwnerDraw)
                        {
                            m.Result = (IntPtr)(WinFormNativeMethods.CDRF_NOTIFYITEMDRAW);
                            return;
                        }
                        // We want custom draw for this paint cycle
                        m.Result = (IntPtr)(WinFormNativeMethods.CDRF_NOTIFYSUBITEMDRAW | WinFormNativeMethods.CDRF_NEWFONT);
                        ...
                        if (nmcd->dwItemType == WinFormNativeMethods.LVCDI_GROUP)
                        {
                            ...
                            WinFormSafeNativeMethods.SelectObject(new HandleRef(nmcd->nmcd, nmcd->nmcd.hdc), new HandleRef(odCacheFontHandleWrapper, odCacheFontHandleWrapper.Handle));
                            m.Result = (IntPtr)WinFormNativeMethods.CDRF_NEWFONT;
                        }
                        return;
                    ...
                    case WinFormNativeMethods.CDDS_ITEMPREPAINT:
                        ...
                        if (OwnerDraw)
                        {
                            ...
                            {
                                m.Result = (IntPtr)(WinFormNativeMethods.CDRF_NOTIFYSUBITEMDRAW);
                            }
                            else
                            {
                                if (!e.DrawDefault)
                                {
                                    m.Result = (IntPtr)(WinFormNativeMethods.CDRF_SKIPDEFAULT);
                                }
                            }
                        ...
                        goto case (WinFormNativeMethods.CDDS_SUBITEM | WinFormNativeMethods.CDDS_ITEMPREPAINT);

                    case (WinFormNativeMethods.CDDS_SUBITEM | WinFormNativeMethods.CDDS_ITEMPREPAINT):
                        ...
                        if (!HideSelection)
                        {
                            ...
                            if ((realState & WinFormNativeMethods.LVIS_SELECTED) == 0)
                            {
                                state &= ~WinFormNativeMethods.CDIS_SELECTED;
                            }
                        }
                        ...
                        int subitem = ((nmcd->nmcd.dwDrawStage & WinFormNativeMethods.CDDS_SUBITEM) != 0) ? nmcd->iSubItem : 0;
                        ...
                        {
                            haveRenderInfo = true;
                            if (subitem == 0 && (state & WinFormNativeMethods.CDIS_HOT) != 0 && HotTracking)
                            {
                            ...
                            if (subitem > 0 || (state & (WinFormNativeMethods.CDIS_SELECTED | WinFormNativeMethods.CDIS_GRAYED | WinFormNativeMethods.CDIS_HOT | WinFormNativeMethods.CDIS_DISABLED)) == 0)
                            {
                        ...
                        {
                            if ((state & (WinFormNativeMethods.CDIS_SELECTED
                                        | WinFormNativeMethods.CDIS_GRAYED
                                        | WinFormNativeMethods.CDIS_HOT
                                        | WinFormNativeMethods.CDIS_DISABLED)) != 0)
                            {
                                changeColor = false;
                            }
                        }
                        ...
                        if (!haveRenderInfo || subItemFont == null)
                        {
                            ...
                            if (odCacheFont != null)
                            {
                                WinFormSafeNativeMethods.SelectObject(new HandleRef(nmcd->nmcd, nmcd->nmcd.hdc), new HandleRef(null, odCacheFontHandle));
                            }
                        }
                        else
                        {
                    ...
                    default:
                        m.Result = (IntPtr)WinFormNativeMethods.CDRF_DODEFAULT;
                        return;
                }
            }
            catch (Exception e)
        ...
        private void DestroyLVGROUP(WinFormNativeMethods.LVGROUP lvgroup) {
        ...
        private void EnsureDefaultGroup() {
            if (IsHandleCreated && ComctlSupportsVisualStyles && GroupsEnabled) {
                if (SendMessage(WinFormNativeMethods.LVM_HASGROUP, DefaultGroup.ID, 0) == IntPtr.Zero) {
                    UpdateGroupView();
        ...
        public void EnsureVisible(int index) {
            ...
            if (IsHandleCreated)
                WinFormUnsafeNativeMethods.SendMessage(new HandleRef(this, Handle), WinFormNativeMethods.LVM_ENSUREVISIBLE, index, 0);
        }
        ...
        public ListViewItem FindItemWithText(string text, bool includeSubItemsInSearch, int startIndex, bool isPrefixSearch) {
            ...
            {
                throw new ArgumentOutOfRangeException("startIndex", DCSR.GetString(DCSR.InvalidArgument, "startIndex", (startIndex).ToString(CultureInfo.CurrentCulture)));
            }
        ...
        public ListViewItem FindNearestItem(SearchDirectionHint searchDirection, int x, int y)
        {
            if (this.View != View.SmallIcon && this.View != View.LargeIcon) {
                throw new InvalidOperationException(DCSR.GetString(DCSR.ListViewFindNearestItemWorksOnlyInIconView));
            }

            if ( searchDirection < SearchDirectionHint.Left || searchDirection > SearchDirectionHint.Down) {
                throw new ArgumentOutOfRangeException("searchDirection", DCSR.GetString(DCSR.InvalidArgument, "searchDirection", (searchDirection).ToString()));
            }
        ...
        private ListViewItem FindItem(bool isTextSearch, string text, bool isPrefixSearch, Point pt, SearchDirectionHint dir, int startIndex, bool includeSubItemsInSearch) {
            ...
            if (VirtualMode) {
                ...
                WinFormNativeMethods.LVFINDINFO lvFindInfo = new WinFormNativeMethods.LVFINDINFO();
                if (isTextSearch) {
                    lvFindInfo.flags = WinFormNativeMethods.LVFI_STRING;
                    lvFindInfo.flags |= (isPrefixSearch ? WinFormNativeMethods.LVFI_PARTIAL : 0);
                    lvFindInfo.psz = text;
                } else {
                    lvFindInfo.flags = WinFormNativeMethods.LVFI_NEARESTXY;
                    lvFindInfo.ptX = pt.X;
                    lvFindInfo.ptY = pt.Y;
                ...
                int index = (int)WinFormUnsafeNativeMethods.SendMessage(new HandleRef(this, this.Handle),
                                                                 WinFormNativeMethods.LVM_FINDITEM,
        ...
        private void ForceCheckBoxUpdate() {
            ...
            if (CheckBoxes && IsHandleCreated) {
                SendMessage(WinFormNativeMethods.LVM_SETEXTENDEDLISTVIEWSTYLE, WinFormNativeMethods.LVS_EX_CHECKBOXES, 0);
                SendMessage(WinFormNativeMethods.LVM_SETEXTENDEDLISTVIEWSTYLE, WinFormNativeMethods.LVS_EX_CHECKBOXES, WinFormNativeMethods.LVS_EX_CHECKBOXES);
        ...
        private string GenerateRandomName() {
            ...
            try {
                handle = 0;// unchecked ((int) (long) bm.GetHicon());
            } catch {
                bm.Dispose();
            }
        ...
        internal int GetDisplayIndex(ListViewItem item, int lastIndex) {
            ...
            if (IsHandleCreated && !ListViewHandleDestroyed) {
                ...
                WinFormNativeMethods.LVFINDINFO info = new WinFormNativeMethods.LVFINDINFO();
                info.lParam = (IntPtr) item.ID;
                info.flags = WinFormNativeMethods.LVFI_PARAM;

                int displayIndex = -1;

                if (lastIndex != -1) {
                    displayIndex = (int)WinFormUnsafeNativeMethods.SendMessage(new HandleRef(this, Handle), WinFormNativeMethods.LVM_FINDITEM, lastIndex - 1, ref info);
                }

                if (displayIndex == -1) {
                    displayIndex = (int)WinFormUnsafeNativeMethods.SendMessage(new HandleRef(this, Handle), WinFormNativeMethods.LVM_FINDITEM, -1 /* beginning */, ref info);
                }
        ...
        public ListViewItem GetItemAt(int x, int y) {
            WinFormNativeMethods.LVHITTESTINFO lvhi = new WinFormNativeMethods.LVHITTESTINFO();

            lvhi.pt_x = x;
            lvhi.pt_y = y;

            int displayIndex = (int)WinFormUnsafeNativeMethods.SendMessage(new HandleRef(this, Handle), WinFormNativeMethods.LVM_HITTEST, 0, lvhi);

            ListViewItem li = null;
            if (displayIndex >= 0 && ((lvhi.flags & WinFormNativeMethods.LVHT_ONITEM) != 0))
        ...
        internal void GetSubItemAt(int x, int y, out int iItem, out int iSubItem) {
            ...
            int index = (int)WinFormUnsafeNativeMethods.SendMessage(new HandleRef(this, Handle), WinFormNativeMethods.LVM_SUBITEMHITTEST, 0, lvhi);
        ...
        internal Point GetItemPosition(int index) {
            ...
            WinFormUnsafeNativeMethods.SendMessage(new HandleRef(this, Handle), WinFormNativeMethods.LVM_GETITEMPOSITION, index, pt);

            return new Point(pt.x, pt.y);
        }

        internal int GetItemState(int index) {
            return GetItemState(index, WinFormNativeMethods.LVIS_FOCUSED | WinFormNativeMethods.LVIS_SELECTED | WinFormNativeMethods.LVIS_CUT |
                                WinFormNativeMethods.LVIS_DROPHILITED | WinFormNativeMethods.LVIS_OVERLAYMASK |
                                WinFormNativeMethods.LVIS_STATEIMAGEMASK);
        }
        ...
        internal int GetItemState(int index, int mask) {
            ...
            return unchecked( (int) (long)SendMessage(WinFormNativeMethods.LVM_GETITEMSTATE, index, mask));
        }
        ...
        public Rectangle GetItemRect(int index, ItemBoundsPortion portion) {
            ...
            WinFormNativeMethods.RECT itemrect = new WinFormNativeMethods.RECT();
            itemrect.left = (int)portion;
            if (unchecked( (int) (long)SendMessage(WinFormNativeMethods.LVM_GETITEMRECT, index, ref itemrect)) == 0)
                throw new ArgumentException(DCSR.GetString(DCSR.InvalidArgument,
                                                          "index",
        ...
        private WinFormNativeMethods.LVGROUP GetLVGROUP(ListViewGroup group)
        {
            WinFormNativeMethods.LVGROUP lvgroup = new WinFormNativeMethods.LVGROUP();
            lvgroup.mask = WinFormNativeMethods.LVGF_HEADER | WinFormNativeMethods.LVGF_GROUPID | WinFormNativeMethods.LVGF_ALIGN;
            ...
            switch(group.HeaderAlignment) {
                case HorizontalAlignment.Left:
                    lvgroup.uAlign = WinFormNativeMethods.LVGA_HEADER_LEFT;
                    break;
                case HorizontalAlignment.Right:
                    lvgroup.uAlign = WinFormNativeMethods.LVGA_HEADER_RIGHT;
                    break;
                case HorizontalAlignment.Center:
                    lvgroup.uAlign = WinFormNativeMethods.LVGA_HEADER_CENTER;
                    break;
            }
            return lvgroup;
        }
        ...
        internal Rectangle GetSubItemRect(int itemIndex, int subItemIndex, ItemBoundsPortion portion) {
            ...
            if (itemIndex < 0 || itemIndex >= this.Items.Count) {
                throw new ArgumentOutOfRangeException("itemIndex", DCSR.GetString(DCSR.InvalidArgument, "itemIndex", (itemIndex).ToString(CultureInfo.CurrentCulture)));
            }
            int subItemCount = Items[itemIndex].SubItems.Count;

            if (subItemIndex < 0 || subItemIndex >= subItemCount) {
                throw new ArgumentOutOfRangeException("subItemIndex", DCSR.GetString(DCSR.InvalidArgument, "subItemIndex", (subItemIndex).ToString(CultureInfo.CurrentCulture)));
            }
            //valid values are 0x0 to 0x3
            if (!ClientUtils.IsEnumValid(portion, (int)portion, (int)ItemBoundsPortion.Entire, (int)ItemBoundsPortion.ItemOnly))
            {
            ...
            if (unchecked( (int) (long)SendMessage(WinFormNativeMethods.LVM_GETSUBITEMRECT, itemIndex, ref itemrect)) == 0)
                throw new ArgumentException(DCSR.GetString(DCSR.InvalidArgument,
                                                          "itemIndex",
        ...
        public ListViewHitTestInfo HitTest(int x, int y) {
            ...
            if (this.View == View.Details) {
                iItem = unchecked( (int) (long)WinFormUnsafeNativeMethods.SendMessage(new HandleRef(this, this.Handle), WinFormNativeMethods.LVM_SUBITEMHITTEST, 0, lvhi));
            } else {
                iItem = unchecked( (int) (long)WinFormUnsafeNativeMethods.SendMessage(new HandleRef(this, this.Handle), WinFormNativeMethods.LVM_HITTEST, 0, lvhi));
            }

            ListViewItem item = (iItem == -1) ? null : Items[iItem];
            ListViewHitTestLocations location = ListViewHitTestLocations.None;

            if (item == null && (WinFormNativeMethods.LVHT_ABOVE & lvhi.flags) == WinFormNativeMethods.LVHT_ABOVE) {
                location = (ListViewHitTestLocations)((MASK_HITTESTFLAG & lvhi.flags) | (int)ListViewHitTestLocations.AboveClientArea);
            }
            else if (item != null && (WinFormNativeMethods.LVHT_ONITEMSTATEICON & lvhi.flags) == WinFormNativeMethods.LVHT_ONITEMSTATEICON) {
        ...
        private void InvalidateColumnHeaders() {
            if (viewStyle == View.Details && IsHandleCreated) {
                ...
                IntPtr hwndHdr = WinFormUnsafeNativeMethods.SendMessage(new HandleRef(this, Handle), WinFormNativeMethods.LVM_GETHEADER, 0, 0);
                if (hwndHdr != IntPtr.Zero)
                    WinFormSafeNativeMethods.InvalidateRect(new HandleRef(this, hwndHdr), null, true);
            }
        }
        ...
        internal ColumnHeader InsertColumn(int index, ColumnHeader ch, bool refreshSubItems) {
            ...
            if (ch.OwnerListview != null)
                throw new ArgumentException(DCSR.GetString(DCSR.OnlyOneControl, ch.Text), "ch");
            ...
            if (-1 == idx)
                throw new InvalidOperationException(DCSR.GetString(DCSR.ListViewAddColumnFailed));
            ...
            if (ch.ActualImageIndex_Internal != -1 && this.IsHandleCreated && this.View != View.Tile) {
                SetColumnInfo(WinFormNativeMethods.LVCF_IMAGE, ch);
            }
        ...
        private int InsertColumnNative(int index, ColumnHeader ch) {
            WinFormNativeMethods.LVCOLUMN_T lvColumn = new WinFormNativeMethods.LVCOLUMN_T();
            lvColumn.mask = WinFormNativeMethods.LVCF_FMT | WinFormNativeMethods.LVCF_TEXT | WinFormNativeMethods.LVCF_WIDTH;// | NativeMethods.LVCF_ORDER | NativeMethods.LVCF_IMAGE;

            if (ch.OwnerListview != null && ch.ActualImageIndex_Internal != -1) {
                lvColumn.mask |= WinFormNativeMethods.LVCF_IMAGE;
            ...
            return (int)WinFormUnsafeNativeMethods.SendMessage(new HandleRef(this, Handle), WinFormNativeMethods.LVM_INSERTCOLUMN, index, lvColumn);
        }
        ...
        internal void InsertGroupInListView(int index, ListViewGroup group) {
            ...
            if (this.GroupsEnabled) {
                for (int i = 0; i < this.Items.Count; i ++) {
                    ListViewItem item = this.Items[i];
                    WinFormNativeMethods.LVITEM lvItem = new WinFormNativeMethods.LVITEM();
                    lvItem.iItem = item.Index;
                    lvItem.mask = WinFormNativeMethods.LVIF_GROUPID;
                    WinFormUnsafeNativeMethods.SendMessage(new HandleRef(this, this.Handle), WinFormNativeMethods.LVM_GETITEM, 0, ref lvItem);
        ...
        private void InsertGroupNative(int index, ListViewGroup group) {
            ...
            try
            {
                 ...
                 int retval = (int)WinFormUnsafeNativeMethods.SendMessage(new HandleRef(this,this.Handle),WinFormNativeMethods.LVM_INSERTGROUP,index,lvgroup);
        ...
        private void InsertItems(int displayIndex, ListViewItem[] items, bool checkHosting) {
                ...
                if (checkHosting) {
                    for (int i = 0; i < items.Length; i ++) {
                        if (items[i].listView != null) {
                            throw new ArgumentException(DCSR.GetString(DCSR.OnlyOneControl, items[i].Text), "item");
                        }
                    }
                }
            ...
            for (int i = 0; i < items.Length; i++) {
                ...
                if (checkHosting && item.listView != null) {
                    throw new ArgumentException(DCSR.GetString(DCSR.OnlyOneControl, item.Text), "item");
                }
        ...
        private int InsertItemsNative(int index, ListViewItem[] items) {
            ...
            try {
                ...
                //
                SendMessage(WinFormNativeMethods.LVM_SETITEMCOUNT, itemCount, 0);
                ...
                for (int i = 0; i < items.Length; i++) {
                    ...
                    lvItem.Reset();
                    lvItem.mask = WinFormNativeMethods.LVIF_TEXT | WinFormNativeMethods.LVIF_IMAGE | WinFormNativeMethods.LVIF_PARAM | WinFormNativeMethods.LVIF_INDENT;
                    ...
                    if (GroupsEnabled) {
                        ...
                        #if DEBUG
                            Debug.Assert(SendMessage(WinFormNativeMethods.LVM_ISGROUPVIEWENABLED, 0, 0) != IntPtr.Zero, "Groups not enabled");
                            Debug.Assert(SendMessage(WinFormNativeMethods.LVM_HASGROUP, lvItem.iGroupId, 0) != IntPtr.Zero, "Doesn't contain group id: " + lvItem.iGroupId.ToString(CultureInfo.InvariantCulture));
                        #endif
                    }

                    lvItem.mask |= WinFormNativeMethods.LVIF_COLUMNS;
                    ...
                    try {
                        ...
                        insertIndex = (int)WinFormUnsafeNativeMethods.SendMessage(new HandleRef(this, Handle), WinFormNativeMethods.LVM_INSERTITEM, 0, ref lvItem);
                        if (actualIndex == -1) {
                    ...
                    if (-1 == insertIndex) {
                        throw new InvalidOperationException(DCSR.GetString(DCSR.ListViewAddItemFailed));
                    }
        ...
        private void LargeImageListRecreateHandle(object sender, EventArgs e) {
            if (IsHandleCreated) {
                ...
                SendMessage(WinFormNativeMethods.LVM_SETIMAGELIST, (IntPtr) WinFormNativeMethods.LVSIL_NORMAL, handle);
        ...
        private void LvnBeginDrag(MouseButtons buttons, WinFormNativeMethods.NMLISTVIEW nmlv) {
        ...
        protected override void OnFontChanged(EventArgs e) {
            ...
            if (!VirtualMode && IsHandleCreated && AutoArrange) {
                BeginUpdate();
                try {
                    SendMessage(WinFormNativeMethods.LVM_UPDATE, -1, 0);
                } finally {
                    EndUpdate();
                }
            }
        ...
        protected override void OnHandleCreated(EventArgs e) {
            ...
            int version = unchecked((int)(long)SendMessage(WinFormNativeMethods.CCM_GETVERSION, 0, 0));
            if (version < 5) {
                SendMessage(WinFormNativeMethods.CCM_SETVERSION, 5, 0);
            }
            UpdateExtendedStyles();
            RealizeProperties();
            int color = ColorTranslator.ToWin32(BackColor);
            SendMessage(WinFormNativeMethods.LVM_SETBKCOLOR,0,color);
            SendMessage(WinFormNativeMethods.LVM_SETTEXTCOLOR,0,ColorTranslator.ToWin32(base.ForeColor));
            ...
            SendMessage(WinFormNativeMethods.LVM_SETTEXTBKCOLOR, 0,WinFormNativeMethods.CLR_NONE);
            ...
            //
            if (!Scrollable) {
                int style = unchecked((int)((long)WinFormUnsafeNativeMethods.GetWindowLong(new HandleRef(this, Handle), WinFormNativeMethods.GWL_STYLE)));
                style |= WinFormNativeMethods.LVS_NOSCROLL;
                WinFormUnsafeNativeMethods.SetWindowLong(new HandleRef(this, Handle), WinFormNativeMethods.GWL_STYLE, new HandleRef(null, (IntPtr)style));
            }

            // in VirtualMode we have to tell the list view to ask for the list view item's state image index
            if (VirtualMode) {
                int callbackMask = unchecked( (int) (long)WinFormUnsafeNativeMethods.SendMessage(new HandleRef(this, this.Handle), WinFormNativeMethods.LVM_GETCALLBACKMASK, 0, 0));
                callbackMask |= WinFormNativeMethods.LVIS_STATEIMAGEMASK;
                WinFormUnsafeNativeMethods.SendMessage(new HandleRef(this, this.Handle), WinFormNativeMethods.LVM_SETCALLBACKMASK, callbackMask, 0);
            }
        ...
        private unsafe void PositionHeader() {
            IntPtr hdrHWND = WinFormUnsafeNativeMethods.GetWindow(new HandleRef(this, this.Handle), WinFormNativeMethods.GW_CHILD);
            if (hdrHWND != IntPtr.Zero) {

                IntPtr prc   = IntPtr.Zero;
                IntPtr pwpos = IntPtr.Zero;

                prc = Marshal.AllocHGlobal(Marshal.SizeOf(typeof(WinFormNativeMethods.RECT)));
                if (prc == IntPtr.Zero) {
                    return;
                }

                try {
                    pwpos = Marshal.AllocHGlobal(Marshal.SizeOf(typeof(WinFormNativeMethods.WINDOWPOS)));
                    ...
                    WinFormUnsafeNativeMethods.GetClientRect(new HandleRef(this, this.Handle), prc);

                    WinFormNativeMethods.HDLAYOUT hd = new WinFormNativeMethods.HDLAYOUT();
                    hd.prc = prc;
                    hd.pwpos = pwpos;

                    // get the layout information
                    WinFormUnsafeNativeMethods.SendMessage(new HandleRef(this, hdrHWND), WinFormNativeMethods.HDM_LAYOUT, 0, ref hd);

                    // now take the information from the native wpos struct and put it into a managed WINDOWPOS
                    WinFormNativeMethods.WINDOWPOS wpos = (WinFormNativeMethods.WINDOWPOS) Marshal.PtrToStructure(pwpos, typeof(WinFormNativeMethods.WINDOWPOS));

                    // position the header control
                    WinFormSafeNativeMethods.SetWindowPos(new HandleRef(this, hdrHWND),
                                                   new HandleRef(this, wpos.hwndInsertAfter),
                                                   wpos.x,
                                                   wpos.y,
                                                   wpos.cx,
                                                   wpos.cy,
                                                   wpos.flags | WinFormNativeMethods.SWP_SHOWWINDOW);
                } finally {
        ...
        protected void RealizeProperties() {
            ...
            if (c != SystemColors.Window) {
                SendMessage(WinFormNativeMethods.LVM_SETBKCOLOR, 0, ColorTranslator.ToWin32(c));
            }
            c = ForeColor;
            if (c != SystemColors.WindowText)
                SendMessage(WinFormNativeMethods.LVM_SETTEXTCOLOR, 0, ColorTranslator.ToWin32(c));


            if (null != imageListLarge)
                SendMessage(WinFormNativeMethods.LVM_SETIMAGELIST, WinFormNativeMethods.LVSIL_NORMAL, imageListLarge.Handle);

            if (null != imageListSmall)
                SendMessage(WinFormNativeMethods.LVM_SETIMAGELIST, WinFormNativeMethods.LVSIL_SMALL, imageListSmall.Handle);
        ...
        public void RedrawItems(int startIndex, int endIndex, bool invalidateOnly)
        {
            if (this.VirtualMode)
            {
                ...
                {
                    throw new ArgumentOutOfRangeException("endIndex", DCSR.GetString(DCSR.InvalidArgument, "endIndex", (endIndex).ToString(CultureInfo.CurrentCulture)));
                }
            }
            else
            {
            ...
            {
                throw new ArgumentException(DCSR.GetString(DCSR.ListViewStartIndexCannotBeLargerThanEndIndex));
            }
            if (this.IsHandleCreated)
            {
                int retval = (int) WinFormUnsafeNativeMethods.SendMessage(new HandleRef(this, this.Handle),
                                                                   WinFormNativeMethods.LVM_REDRAWITEMS,
                                                                   startIndex, endIndex);
        ...
        private void RemoveGroupNative(ListViewGroup group) {
               ...
               int retval = (int)WinFormUnsafeNativeMethods.SendMessage(new HandleRef(this, this.Handle), WinFormNativeMethods.LVM_REMOVEGROUP, group.ID, IntPtr.Zero);
        ...
        private void Scroll(int fromLVItem, int toLVItem) {
            ...
            WinFormUnsafeNativeMethods.SendMessage(new HandleRef(this, this.Handle), WinFormNativeMethods.LVM_SCROLL, 0, scrollY);
        }
            ...
            if (this.BackgroundImage != null) {
               ...
               // permSet.AddPermission(envPermission);
                ...
                try {
                    ...
                    WinFormUnsafeNativeMethods.GetTempFileName(tempDirName, this.GenerateRandomName(), 0, sb);
                ...
                lvbkImage.ulFlags = WinFormNativeMethods.LVBKIF_SOURCE_NONE;
                this.backgroundImageFileName = String.Empty;
            }
        ...
        internal void SetColumnInfo(int mask, ColumnHeader ch) {
            if (IsHandleCreated) {
                Debug.Assert((mask & ~(WinFormNativeMethods.LVCF_FMT | WinFormNativeMethods.LVCF_TEXT | WinFormNativeMethods.LVCF_IMAGE)) == 0, "Unsupported mask in setColumnInfo");
                WinFormNativeMethods.LVCOLUMN lvColumn = new WinFormNativeMethods.LVCOLUMN();
                lvColumn.mask  = mask;

                if ((mask & WinFormNativeMethods.LVCF_IMAGE) != 0 || (mask & WinFormNativeMethods.LVCF_FMT) != 0) {
                    ...
                    lvColumn.mask |= WinFormNativeMethods.LVCF_FMT;
                    ...
                    if (ch.ActualImageIndex_Internal > -1) {
                        ...
                        lvColumn.iImage = ch.ActualImageIndex_Internal;
                        lvColumn.fmt |= WinFormNativeMethods.LVCFMT_IMAGE;
                    }

                    lvColumn.fmt |= (int) ch.TextAlign;
                }

                if ((mask & WinFormNativeMethods.LVCF_TEXT) != 0) {
                    lvColumn.pszText    = Marshal.StringToHGlobalAuto(ch.Text);
                }

                int retval = (int)WinFormUnsafeNativeMethods.SendMessage(new HandleRef(this, Handle), WinFormNativeMethods.LVM_SETCOLUMN, ch.Index, lvColumn);
                if ((mask & WinFormNativeMethods.LVCF_TEXT) != 0) {
                    Marshal.FreeHGlobal(lvColumn.pszText);
                }

                if (0 == retval)
                    throw new InvalidOperationException(DCSR.GetString(DCSR.ListViewColumnInfoSet));
        ...
        internal void SetColumnWidth(int columnIndex, ColumnHeaderAutoResizeStyle headerAutoResize) {
            if ((columnIndex < 0) ||
                ...
                throw new ArgumentOutOfRangeException("columnIndex", DCSR.GetString(DCSR.InvalidArgument, "columnIndex", (columnIndex).ToString(CultureInfo.CurrentCulture)));
            }
            ...
            if (headerAutoResize == ColumnHeaderAutoResizeStyle.None) {
                ...
                if (width == WinFormNativeMethods.LVSCW_AUTOSIZE_USEHEADER) {
                    headerAutoResize = ColumnHeaderAutoResizeStyle.HeaderSize;
                } else if (width == WinFormNativeMethods.LVSCW_AUTOSIZE) {
            ...
            if (headerAutoResize == ColumnHeaderAutoResizeStyle.HeaderSize) {
                ...
                width = WinFormNativeMethods.LVSCW_AUTOSIZE_USEHEADER;
            } else if (headerAutoResize == ColumnHeaderAutoResizeStyle.ColumnContent) {
                compensate = this.CompensateColumnHeaderResize(columnIndex, false /*columnHeaderResizeCancelled*/);
                width = WinFormNativeMethods.LVSCW_AUTOSIZE;
            }

            if (IsHandleCreated) {
                SendMessage(WinFormNativeMethods.LVM_SETCOLUMNWIDTH, columnIndex, WinFormNativeMethods.Util.MAKELPARAM(width, 0));
            }
            ...
            if (this.IsHandleCreated &&
               (headerAutoResize == ColumnHeaderAutoResizeStyle.ColumnContent ||
                ...
                if (compensate != 0) {
                    ...
                    SendMessage(WinFormNativeMethods.LVM_SETCOLUMNWIDTH, columnIndex, WinFormNativeMethods.Util.MAKELPARAM(newWidth, 0));
                }
            }
        }
        ...
        private void SetColumnWidth(int index, int width) {
            if (IsHandleCreated) {
                SendMessage(WinFormNativeMethods.LVM_SETCOLUMNWIDTH, index, WinFormNativeMethods.Util.MAKELPARAM(width,0));
            }
        }
        ...
        private void SetDisplayIndices(int[] indices) {
            ...
            if (this.IsHandleCreated && !this.Disposing) {
               WinFormUnsafeNativeMethods.SendMessage(new HandleRef(this, this.Handle), WinFormNativeMethods.LVM_SETCOLUMNORDERARRAY, orderedColumns.Length, orderedColumns);
        }
        }
        ...
        internal void SetToolTip(ToolTip toolTip, string toolTipCaption) {
            ...
            IntPtr oldHandle = WinFormUnsafeNativeMethods.SendMessage(new HandleRef(this, this.Handle), WinFormNativeMethods.LVM_SETTOOLTIPS, new HandleRef(toolTip, toolTip.Handle), 0);
            WinFormUnsafeNativeMethods.DestroyWindow(new HandleRef(null, oldHandle));
        }
        ...
        internal void SetItemImage(int index, int image) {
            ...
            if (this.IsHandleCreated) {
                WinFormNativeMethods.LVITEM lvItem = new WinFormNativeMethods.LVITEM();
                lvItem.mask = WinFormNativeMethods.LVIF_IMAGE;
                lvItem.iItem = index;
                lvItem.iImage = image;
                WinFormUnsafeNativeMethods.SendMessage(new HandleRef(this, Handle), WinFormNativeMethods.LVM_SETITEM, 0, ref lvItem);
            }
        }
        ...
        internal void SetItemIndentCount(int index, int indentCount) {
            ...
            if (this.IsHandleCreated) {
                WinFormNativeMethods.LVITEM lvItem = new WinFormNativeMethods.LVITEM();
                lvItem.mask = WinFormNativeMethods.LVIF_INDENT;
                lvItem.iItem = index;
        ...
        internal void SetItemPosition(int index, int x, int y) {
            ...
            pt.x = x;
            pt.y = y;
            WinFormUnsafeNativeMethods.SendMessage(new HandleRef(this, Handle), WinFormNativeMethods.LVM_SETITEMPOSITION32, index, pt);
        }
        ...
        internal void SetItemState(int index, int state, int mask) {
            ...
            if (this.IsHandleCreated) {
                WinFormNativeMethods.LVITEM lvItem = new WinFormNativeMethods.LVITEM();
                lvItem.mask = WinFormNativeMethods.LVIF_STATE;
                lvItem.state = state;
                lvItem.stateMask = mask;
                WinFormUnsafeNativeMethods.SendMessage(new HandleRef(this, Handle), WinFormNativeMethods.LVM_SETITEMSTATE, index, ref lvItem);
            }
        }
        ...
        ///</devdoc>
        private void SetItemText(int itemIndex, int subItemIndex, string text, ref WinFormNativeMethods.LVITEM lvItem) {
            ...
            if (this.View == View.List && subItemIndex == 0) {
                int colWidth = unchecked( (int) (long)WinFormUnsafeNativeMethods.SendMessage(new HandleRef(this, this.Handle), WinFormNativeMethods.LVM_GETCOLUMNWIDTH, 0, 0));
            ...
            lvItem.mask = WinFormNativeMethods.LVIF_TEXT;
            lvItem.iItem    = itemIndex;
            lvItem.iSubItem = subItemIndex;
            lvItem.pszText  = text;

            WinFormUnsafeNativeMethods.SendMessage(new HandleRef(this, Handle), WinFormNativeMethods.LVM_SETITEMTEXT, itemIndex, ref lvItem);
        }
        ...
        internal void SetSelectionMark(int itemIndex) {
            ...
            SendMessage(WinFormNativeMethods.LVM_SETSELECTIONMARK, 0, itemIndex);
        }
        ...
        private void SmallImageListRecreateHandle(object sender, EventArgs e) {
            if (IsHandleCreated) {
                ...
                SendMessage(WinFormNativeMethods.LVM_SETIMAGELIST, (IntPtr) WinFormNativeMethods.LVSIL_SMALL, handle);
        ...
        public void Sort() {
            if (VirtualMode) {
                throw new InvalidOperationException(DCSR.GetString(DCSR.ListViewSortNotAllowedInVirtualListView));
            }

            ApplyUpdateCachedItems();
            if (IsHandleCreated && listItemSorter != null) {
                WinFormNativeMethods.ListViewCompareCallback callback = new WinFormNativeMethods.ListViewCompareCallback(this.CompareFunc);
                WinFormUnsafeNativeMethods.SendMessage(new HandleRef(this, Handle), WinFormNativeMethods.LVM_SORTITEMS, IntPtr.Zero, callback);
            }
        }
        ...
        private void StateImageListRecreateHandle(object sender, EventArgs e) {
            if (IsHandleCreated) {
                ...
                SendMessage(WinFormNativeMethods.LVM_SETIMAGELIST, (IntPtr) WinFormNativeMethods.LVSIL_STATE, handle);
            }
        }
        ...
        protected void UpdateExtendedStyles() {
            ...
            if (IsHandleCreated) {
                int exStyle = 0;
                int exMask = WinFormNativeMethods.LVS_EX_ONECLICKACTIVATE | WinFormNativeMethods.LVS_EX_TWOCLICKACTIVATE |
                             WinFormNativeMethods.LVS_EX_TRACKSELECT | WinFormNativeMethods.LVS_EX_UNDERLINEHOT | WinFormNativeMethods.LVS_EX_ONECLICKACTIVATE |
                             WinFormNativeMethods.LVS_EX_HEADERDRAGDROP | WinFormNativeMethods.LVS_EX_CHECKBOXES |
                             WinFormNativeMethods.LVS_EX_FULLROWSELECT | WinFormNativeMethods.LVS_EX_GRIDLINES |
                             WinFormNativeMethods.LVS_EX_INFOTIP | WinFormNativeMethods.LVS_EX_DOUBLEBUFFER;

                switch (activation) {
                    case ItemActivation.OneClick:
                        exStyle |= WinFormNativeMethods.LVS_EX_ONECLICKACTIVATE;
                        break;
                    case ItemActivation.TwoClick:
                        exStyle |= WinFormNativeMethods.LVS_EX_TWOCLICKACTIVATE;
                        break;
                }

                if (AllowColumnReorder)
                    exStyle |= WinFormNativeMethods.LVS_EX_HEADERDRAGDROP;

                if (CheckBoxes)
                    exStyle |= WinFormNativeMethods.LVS_EX_CHECKBOXES;

                if (DoubleBuffered)
                    exStyle |= WinFormNativeMethods.LVS_EX_DOUBLEBUFFER;

                if (FullRowSelect)
                    exStyle |= WinFormNativeMethods.LVS_EX_FULLROWSELECT;

                if (GridLines)
                    exStyle |= WinFormNativeMethods.LVS_EX_GRIDLINES;

                if (HoverSelection)
                    exStyle |= WinFormNativeMethods.LVS_EX_TRACKSELECT;

                if (HotTracking)
                    exStyle |= WinFormNativeMethods.LVS_EX_UNDERLINEHOT;

                if (ShowItemToolTips)
                    exStyle |= WinFormNativeMethods.LVS_EX_INFOTIP;

                SendMessage(WinFormNativeMethods.LVM_SETEXTENDEDLISTVIEWSTYLE, exMask, exStyle);
                Invalidate();
            }
        }
        ...
        internal void UpdateGroupNative(ListViewGroup group) {
            ...
            try {
                lvgroup = GetLVGROUP(group);
                int retval = (int)WinFormUnsafeNativeMethods.SendMessage(new HandleRef(this, this.Handle),
                                                                  WinFormNativeMethods.LVM_SETGROUPINFO,
                                                                  group.ID,
                                                                  lvgroup);
        ...
        internal void UpdateGroupView() {
            if (IsHandleCreated && ComctlSupportsVisualStyles && !VirtualMode) {
                int retval = unchecked( (int) (long)SendMessage(WinFormNativeMethods.LVM_ENABLEGROUPVIEW, GroupsEnabled ? 1 : 0, 0));
        ...
        private void UpdateTileView() {
            ...
            tileViewInfo.dwMask = WinFormNativeMethods.LVTVIM_COLUMNS;
            tileViewInfo.cLines = this.columnHeaders != null ? this.columnHeaders.Length : 0;

            // the tile view info size
            tileViewInfo.dwMask |= WinFormNativeMethods.LVTVIM_TILESIZE;
            tileViewInfo.dwFlags = WinFormNativeMethods.LVTVIF_FIXEDSIZE;
            tileViewInfo.sizeTile = new WinFormNativeMethods.SIZE(this.TileSize.Width, this.TileSize.Height);

            bool retval = WinFormUnsafeNativeMethods.SendMessage(new HandleRef(this,Handle),WinFormNativeMethods.LVM_SETTILEVIEWINFO,0,tileViewInfo);
            Debug.Assert(retval,"LVM_SETTILEVIEWINFO failed");
        }
        ...
        private void WmNmClick(ref Message m) {
            ...
            if (CheckBoxes) {
                ...
                int displayIndex = (int)WinFormUnsafeNativeMethods.SendMessage(new HandleRef(this, Handle), WinFormNativeMethods.LVM_HITTEST, 0, lvhi);
                if (displayIndex != -1 && (lvhi.flags & WinFormNativeMethods.LVHT_ONITEMSTATEICON) != 0) {
        ...
        private void WmNmDblClick(ref Message m) {
            ...
            if (CheckBoxes) {
                ...
                int displayIndex = (int)WinFormUnsafeNativeMethods.SendMessage(new HandleRef(this, Handle), WinFormNativeMethods.LVM_HITTEST, 0, lvhi);
                if (displayIndex != -1 && (lvhi.flags & WinFormNativeMethods.LVHT_ONITEM) != 0) {
                    ListViewItem clickedItem = Items[displayIndex];
        ...
        private unsafe bool WmNotify(ref Message m) {
            ...
            if (nmhdr->code == WinFormNativeMethods.NM_CUSTOMDRAW && OwnerDraw)
            {
                try
                {
                    WinFormNativeMethods.NMCUSTOMDRAW* nmcd = (WinFormNativeMethods.NMCUSTOMDRAW*)m.LParam;
                    // Find out which stage we're drawing
                        ...
                        case WinFormNativeMethods.CDDS_ITEMPREPAINT:
                            {
                                ...
                                try
                                {
                                    Color foreColor = ColorTranslator.FromWin32(WinFormSafeNativeMethods.GetTextColor(new HandleRef(this, nmcd->hdc)));
                                    Color backColor = ColorTranslator.FromWin32(WinFormSafeNativeMethods.GetBkColor(new HandleRef(this, nmcd->hdc)));
            ...
            if (nmhdr->code == WinFormNativeMethods.NM_RELEASEDCAPTURE && listViewState[LISTVIEWSTATE_columnClicked]) {
                listViewState[LISTVIEWSTATE_columnClicked] = false;
                OnColumnClick(new ColumnClickEventArgs(columnIndex));
            }

            if (nmhdr->code == WinFormNativeMethods.HDN_BEGINTRACKA ||
                nmhdr->code == WinFormNativeMethods.HDN_BEGINTRACKW) {
               ...
               WinFormNativeMethods.NMHEADER nmheader = (WinFormNativeMethods.NMHEADER) m.GetLParam(typeof(WinFormNativeMethods.NMHEADER));
            ...
            if (nmhdr->code == WinFormNativeMethods.HDN_ITEMCHANGINGA || 
                nmhdr->code == WinFormNativeMethods.HDN_ITEMCHANGINGW) {
                WinFormNativeMethods.NMHEADER nmheader = (WinFormNativeMethods.NMHEADER) m.GetLParam(typeof(WinFormNativeMethods.NMHEADER));
                ...
                if (columnHeaders != null && nmheader.iItem < columnHeaders.Length &&
                    ...
                    WinFormNativeMethods.HDITEM2 hdItem = (WinFormNativeMethods.HDITEM2) WinFormUnsafeNativeMethods.PtrToStructure((IntPtr) nmheader.pItem, typeof(WinFormNativeMethods.HDITEM2));
                    int newColumnWidth = ((hdItem.mask & WinFormNativeMethods.HDI_WIDTH) != 0) ? hdItem.cxy : -1;
            ...
            if ((nmhdr->code == WinFormNativeMethods.HDN_ITEMCHANGEDA ||
                nmhdr->code == WinFormNativeMethods.HDN_ITEMCHANGEDW) &&
            ...
            if (nmhdr->code == WinFormNativeMethods.HDN_ENDTRACKA || 
                nmhdr->code == WinFormNativeMethods.HDN_ENDTRACKW) {
            ...
            if (nmhdr->code == WinFormNativeMethods.HDN_ENDDRAG) {
                WinFormNativeMethods.NMHEADER header = (WinFormNativeMethods.NMHEADER) m.GetLParam(typeof(WinFormNativeMethods.NMHEADER));
                if (header.pItem != IntPtr.Zero) {
                    ...
                    // and over what it does in its constructor.
                    WinFormNativeMethods.HDITEM2 hdItem = (WinFormNativeMethods.HDITEM2) WinFormUnsafeNativeMethods.PtrToStructure((IntPtr) header.pItem, typeof(WinFormNativeMethods.HDITEM2));
                    if ((hdItem.mask & WinFormNativeMethods.HDI_ORDER) == WinFormNativeMethods.HDI_ORDER) {
            ...
            if (nmhdr->code == WinFormNativeMethods.HDN_DIVIDERDBLCLICKA ||
                nmhdr->code == WinFormNativeMethods.HDN_DIVIDERDBLCLICKW) {
        ...
        private Font GetListHeaderFont(){
            IntPtr hwndHdr = WinFormUnsafeNativeMethods.SendMessage(new HandleRef(this, Handle), WinFormNativeMethods.LVM_GETHEADER, 0, 0);
            IntPtr hFont = WinFormUnsafeNativeMethods.SendMessage(new HandleRef(this, hwndHdr), WinFormNativeMethods.WM_GETFONT, 0, 0);
        ...
        /// </devdoc>
        /// <internalonly/>
        private int GetIndexOfClickedItem(WinFormNativeMethods.LVHITTESTINFO lvhi) {
            Point pos = Cursor.Position;
            pos = PointToClientInternal(pos);
            lvhi.pt_x = pos.X;
            lvhi.pt_y = pos.Y;
            return (int)WinFormUnsafeNativeMethods.SendMessage(new HandleRef(this, Handle), WinFormNativeMethods.LVM_HITTEST, 0, lvhi);
        ...
        private unsafe void WmReflectNotify(ref Message m) {
            ...
            switch (nmhdr->code) {
                case WinFormNativeMethods.NM_CUSTOMDRAW:
                    CustomDraw(ref m);
                    break;

                case WinFormNativeMethods.LVN_BEGINLABELEDITA:
                case WinFormNativeMethods.LVN_BEGINLABELEDITW: {
                        WinFormNativeMethods.NMLVDISPINFO_NOTEXT nmlvdp = (WinFormNativeMethods.NMLVDISPINFO_NOTEXT)m.GetLParam(typeof(WinFormNativeMethods.NMLVDISPINFO_NOTEXT));
                ...
                case WinFormNativeMethods.LVN_COLUMNCLICK: {
                        WinFormNativeMethods.NMLISTVIEW nmlv = (WinFormNativeMethods.NMLISTVIEW)m.GetLParam(typeof(WinFormNativeMethods.NMLISTVIEW));
                        listViewState[LISTVIEWSTATE_columnClicked] = true;
                        columnIndex = nmlv.iSubItem;
                        break;
                    }

                case WinFormNativeMethods.LVN_ENDLABELEDITA:
                case WinFormNativeMethods.LVN_ENDLABELEDITW: {
                        listViewState[LISTVIEWSTATE_inLabelEdit] = false;
                        WinFormNativeMethods.NMLVDISPINFO nmlvdp = (WinFormNativeMethods.NMLVDISPINFO)m.GetLParam(typeof(WinFormNativeMethods.NMLVDISPINFO));
                ...
                case WinFormNativeMethods.LVN_ITEMACTIVATE:
                    OnItemActivate(EventArgs.Empty);
                    break;

                case WinFormNativeMethods.LVN_BEGINDRAG: {
                ...
                case WinFormNativeMethods.LVN_BEGINRDRAG: {
                    // the items collection was modified while dragging
                    // that means that we can't reliably give the user the item on which the dragging started
                    // so don't tell the user about this operation...
                    // vsWhidbey 235027
                ...
                case WinFormNativeMethods.LVN_ITEMCHANGING: {
                        WinFormNativeMethods.NMLISTVIEW* nmlv = (WinFormNativeMethods.NMLISTVIEW*)m.LParam;
                        if ((nmlv->uChanged & WinFormNativeMethods.LVIF_STATE) != 0) {
                            ...
                            CheckState oldState = (CheckState)(((nmlv->uOldState & WinFormNativeMethods.LVIS_STATEIMAGEMASK) >> 12) == 1 ? 0 : 1);
                            CheckState newState = (CheckState)(((nmlv->uNewState & WinFormNativeMethods.LVIS_STATEIMAGEMASK) >> 12) == 1 ? 0 : 1);
                ...
                case WinFormNativeMethods.LVN_ITEMCHANGED: {
                        WinFormNativeMethods.NMLISTVIEW* nmlv = (WinFormNativeMethods.NMLISTVIEW*)m.LParam;
                        // Check for state changes to the selected state...
                        if ((nmlv->uChanged & WinFormNativeMethods.LVIF_STATE) != 0) {
                            // Because the state image mask is 1-based, a value of 1 means unchecked,
                            // anything else means checked.  We convert this to the more standard 0 or 1
                            CheckState oldValue = (CheckState)(((nmlv->uOldState & WinFormNativeMethods.LVIS_STATEIMAGEMASK) >> 12) == 1 ? 0 : 1);
                            CheckState newValue = (CheckState)(((nmlv->uNewState & WinFormNativeMethods.LVIS_STATEIMAGEMASK) >> 12) == 1 ? 0 : 1);
                            ...
                            int oldState = nmlv->uOldState & WinFormNativeMethods.LVIS_SELECTED;
                            int newState = nmlv->uNewState & WinFormNativeMethods.LVIS_SELECTED;
                ...
                case WinFormNativeMethods.NM_CLICK:
                    WmNmClick(ref m);
                    // FALL THROUGH //
                    goto case WinFormNativeMethods.NM_RCLICK;

                case WinFormNativeMethods.NM_RCLICK:
                    WinFormNativeMethods.LVHITTESTINFO lvhi = new WinFormNativeMethods.LVHITTESTINFO();
                    int displayIndex = GetIndexOfClickedItem(lvhi);

                    MouseButtons button = nmhdr->code == WinFormNativeMethods.NM_CLICK ? MouseButtons.Left : MouseButtons.Right;
                    Point pos = Cursor.Position;
                    pos = PointToClientInternal(pos);
                ...
                case WinFormNativeMethods.NM_DBLCLK:
                    WmNmDblClick(ref m);
                    // FALL THROUGH //
                    goto case WinFormNativeMethods.NM_RDBLCLK;

                case WinFormNativeMethods.NM_RDBLCLK:
                    WinFormNativeMethods.LVHITTESTINFO lvhip = new WinFormNativeMethods.LVHITTESTINFO();
                ...
                case WinFormNativeMethods.LVN_KEYDOWN:
                    if (CheckBoxes) {
                        WinFormNativeMethods.NMLVKEYDOWN lvkd = (WinFormNativeMethods.NMLVKEYDOWN)m.GetLParam(typeof(WinFormNativeMethods.NMLVKEYDOWN));
                ...
                case WinFormNativeMethods.LVN_ODCACHEHINT:
                    // tell the user to prepare the cache:
                    WinFormNativeMethods.NMLVCACHEHINT cacheHint = (WinFormNativeMethods.NMLVCACHEHINT) m.GetLParam(typeof(WinFormNativeMethods.NMLVCACHEHINT));
                    OnCacheVirtualItems(new CacheVirtualItemsEventArgs(cacheHint.iFrom, cacheHint.iTo));
                    break;

                default:
                    if (nmhdr->code == WinFormNativeMethods.LVN_GETDISPINFO) {
                        ...
                        if (this.VirtualMode && m.LParam != IntPtr.Zero) {
                            ...
                            WinFormNativeMethods.NMLVDISPINFO_NOTEXT dispInfo= (WinFormNativeMethods.NMLVDISPINFO_NOTEXT) m.GetLParam(typeof(WinFormNativeMethods.NMLVDISPINFO_NOTEXT));
                            ...
                            if (lvItem == null)
                            {
                                throw new InvalidOperationException(DCSR.GetString(DCSR.ListViewVirtualItemRequired));
                            }

                            lvItem.SetItemIndex(this, dispInfo.item.iItem);
                            if ((dispInfo.item.mask & WinFormNativeMethods.LVIF_TEXT) != 0) {
                                string text;
                                if (dispInfo.item.iSubItem == 0) {
                                    ...
                                    if (lvItem.SubItems.Count <= dispInfo.item.iSubItem) {
                                        throw new InvalidOperationException(DCSR.GetString(DCSR.ListViewVirtualModeCantAccessSubItem));
                                    }
                                    else {
                            ...
                            if ((dispInfo.item.mask & WinFormNativeMethods.LVIF_IMAGE) != 0 && lvItem.ImageIndex != -1) {
                                dispInfo.item.iImage = lvItem.ImageIndex;
                            }

                            if ((dispInfo.item.mask & WinFormNativeMethods.LVIF_INDENT) != 0) {
                            ...
                            */
                            ...
                            if ((dispInfo.item.stateMask & WinFormNativeMethods.LVIS_STATEIMAGEMASK) != 0) {
                    ...
                    else if (nmhdr->code == WinFormNativeMethods.LVN_ODSTATECHANGED) {
                        if (VirtualMode && m.LParam != IntPtr.Zero) {
                            WinFormNativeMethods.NMLVODSTATECHANGE odStateChange = (WinFormNativeMethods.NMLVODSTATECHANGE) m.GetLParam(typeof(WinFormNativeMethods.NMLVODSTATECHANGE));
                            bool selectedChanged = (odStateChange.uNewState & WinFormNativeMethods.LVIS_SELECTED) != (odStateChange.uOldState & WinFormNativeMethods.LVIS_SELECTED);
                            if (selectedChanged) {
                                ...
                                if (!WinFormUnsafeNativeMethods.IsVista) {
                                    iTo--;
                                }
                                ListViewVirtualItemsSelectionRangeChangedEventArgs lvvisrce = new ListViewVirtualItemsSelectionRangeChangedEventArgs(odStateChange.iFrom, iTo, (odStateChange.uNewState & WinFormNativeMethods.LVIS_SELECTED) != 0);
                                OnVirtualItemsSelectionRangeChanged(lvvisrce);
                            }
                        }
                    }
                    else if (nmhdr->code == WinFormNativeMethods.LVN_GETINFOTIP) {
                        if (ShowItemToolTips && m.LParam != IntPtr.Zero) {
                            WinFormNativeMethods.NMLVGETINFOTIP infoTip = (WinFormNativeMethods.NMLVGETINFOTIP) m.GetLParam(typeof(WinFormNativeMethods.NMLVGETINFOTIP));
                            ...
                            if (lvi != null && !string.IsNullOrEmpty(lvi.ToolTipText)) {
                                ...
                                WinFormUnsafeNativeMethods.SendMessage(new HandleRef(this, nmhdr->hwndFrom), WinFormNativeMethods.TTM_SETMAXTIPWIDTH, 0, SystemInformation.MaxWindowTrackSize.Width);
                    ...
                    else if (nmhdr->code == WinFormNativeMethods.LVN_ODFINDITEM) {
                        if (VirtualMode) {
                            WinFormNativeMethods.NMLVFINDITEM nmlvif = (WinFormNativeMethods.NMLVFINDITEM) m.GetLParam(typeof(WinFormNativeMethods.NMLVFINDITEM));

                            if ((nmlvif.lvfi.flags & WinFormNativeMethods.LVFI_PARAM) != 0) {
                                m.Result = (IntPtr) (-1);
                                return;
                            }

                            bool isTextSearch = ((nmlvif.lvfi.flags & WinFormNativeMethods.LVFI_STRING) != 0) ||
                                                ((nmlvif.lvfi.flags & WinFormNativeMethods.LVFI_PARTIAL) != 0);

                            bool isPrefixSearch = (nmlvif.lvfi.flags & WinFormNativeMethods.LVFI_PARTIAL) != 0;
                            ...
                            if ((nmlvif.lvfi.flags & WinFormNativeMethods.LVFI_NEARESTXY) != 0) {
        ...
        protected override async Task WndProc(Message m) {
            ...
            switch (m.Msg) {
                case WinFormNativeMethods.WM_REFLECT + WinFormNativeMethods.WM_NOTIFY:
                ...
                case WinFormNativeMethods.WM_MBUTTONUP:
                    ...
                    if (!ValidationCancelled && listViewState[LISTVIEWSTATE_doubleclickFired] && index != -1) {
                        ...
                        OnMouseDoubleClick(new MouseEventArgs(downButton, 2, WinFormNativeMethods.Util.SignedLOWORD(m.LParam), WinFormNativeMethods.Util.SignedHIWORD(m.LParam), 0));
                    }
                    if (!listViewState[LISTVIEWSTATE_mouseUpFired])
                    {
                        OnMouseUp(new MouseEventArgs(downButton, 1, WinFormNativeMethods.Util.SignedLOWORD(m.LParam), WinFormNativeMethods.Util.SignedHIWORD(m.LParam), 0));
                ...
                case WinFormNativeMethods.WM_PRINT:
                    WmPrint(ref m);
                    break;
                case WinFormNativeMethods.WM_TIMER:
            ...
            public int Count {
                get {
                    if (owner.VirtualMode) {
                        throw new InvalidOperationException(DCSR.GetString(DCSR.ListViewCantAccessCheckedItemsCollectionWhenInVirtualMode));
                    }
        ...
        public class SelectedIndexCollection : IList {
            ...
            public int Count {
                get {
                    if (owner.IsHandleCreated) {
                        return unchecked( (int) (long)owner.SendMessage(WinFormNativeMethods.LVM_GETSELECTEDCOUNT, 0, 0));
                    }
                    else {
            ...
            private int[] IndicesArray {
                get {
                    ...
                    if (owner.IsHandleCreated) {
                        int displayIndex = -1;
                        for (int i = 0; i < count; i++) {
                            int fidx = unchecked( (int) (long)owner.SendMessage(WinFormNativeMethods.LVM_GETNEXTITEM, displayIndex, WinFormNativeMethods.LVNI_SELECTED));
                            ...
                            if (fidx > -1) {
                                ...
                                throw new InvalidOperationException(DCSR.GetString(DCSR.SelectedNotEqualActual));
                        }
                    }
                    else {
                ...
                get {
                    ...
                    if (owner.IsHandleCreated) {
                        ...
                        for(int count = 0; count <= index; count++) {
                            fidx = unchecked( (int) (long)owner.SendMessage(WinFormNativeMethods.LVM_GETNEXTITEM, fidx, WinFormNativeMethods.LVNI_SELECTED));
            ...
            int IList.Add(object value)
            {
                ...
                {
                    throw new ArgumentException(DCSR.GetString(DCSR.InvalidArgument, "value", ((value).ToString())));
                }
            }
            ...
            public int Add(int itemIndex)
            {
                if (this.owner.VirtualMode)
                {
                    ...
                    {
                        this.owner.SetItemState(itemIndex, WinFormNativeMethods.LVIS_SELECTED, WinFormNativeMethods.LVIS_SELECTED);
                        return this.Count;
                    }
                    else
                    {
            ...
            public void Clear()
            {
                ...
                if (this.owner.IsHandleCreated)
                {
                    this.owner.SetItemState(-1, 0, WinFormNativeMethods.LVIS_SELECTED);
                }
            }
            ...
            public void Remove(int itemIndex)
            {
                if (this.owner.VirtualMode)
                {
                    ...
                    if (this.owner.IsHandleCreated)
                    {
                        this.owner.SetItemState(itemIndex, 0, WinFormNativeMethods.LVIS_SELECTED);
                    }
                }
                else
                {
        ...
        public class SelectedListViewItemCollection : IList {
            ...
            private ListViewItem[] SelectedItemArray {
                get {
                    if (owner.IsHandleCreated) {
                        int cnt = unchecked( (int) (long)owner.SendMessage(WinFormNativeMethods.LVM_GETSELECTEDCOUNT, 0, 0));
            ...
            public int Count {
                get {
                    if (owner.VirtualMode) {
                        throw new InvalidOperationException(DCSR.GetString(DCSR.ListViewCantAccessSelectedItemsCollectionWhenInVirtualMode));
                    }
          ...
          private bool IsValidIndex(int index) {
            ...
            int IList.Add(object value) {
                if (value is ColumnHeader) {
                    ...
                    throw new ArgumentException(DCSR.GetString(DCSR.ColumnHeaderCollectionInvalidArgument));
                }
            }
                ...
                if (owner.columnHeaders != null) {
                    if (owner.View == View.Tile) {
                        ...
                        for (int colIdx = owner.columnHeaders.Length-1; colIdx >= 0; colIdx--) {
                            int w = owner.columnHeaders[colIdx].Width; // Update width before detaching from ListView
                            if (owner.IsHandleCreated) {
                                owner.SendMessage(WinFormNativeMethods.LVM_DELETECOLUMN, colIdx, 0);
                            }
                            owner.columnHeaders[colIdx].OwnerListview = null;
                        }
                        owner.columnHeaders = null;
            ...
            public virtual void RemoveAt(int index) {
                ...
                if (owner.IsHandleCreated && this.owner.View != View.Tile) {
                    int retval = unchecked( (int) (long)owner.SendMessage(WinFormNativeMethods.LVM_DELETECOLUMN, index, 0));
           ...
           public virtual bool ContainsKey(string key) {
            ...
            public IEnumerator GetEnumerator() {
                if (this.InnerList.OwnerIsVirtualListView && !this.InnerList.OwnerIsDesignMode) {
                    ...
                    throw new InvalidOperationException(DCSR.GetString(DCSR.ListViewCantGetEnumeratorInVirtualMode));
                }
        ...
        internal class ListViewNativeItemCollection : ListViewItemCollection.IInnerList {
            ...
            public ListViewItem this[int displayIndex] {
                get {
                    ...
                    if (owner.VirtualMode) {
                        ...
                        if (displayIndex < 0 || displayIndex >= owner.itemCount)
                            throw new ArgumentOutOfRangeException("displayIndex", DCSR.GetString(DCSR.InvalidArgument, "displayIndex", (displayIndex).ToString(CultureInfo.CurrentCulture)));
                ...
                set {
                    owner.ApplyUpdateCachedItems();
                    if (owner.VirtualMode) {
                        throw new InvalidOperationException(DCSR.GetString(DCSR.ListViewCantModifyTheItemCollInAVirtualListView));
                    }
            ...
            public ListViewItem Add(ListViewItem value) {
                if (owner.VirtualMode) {
                    throw new InvalidOperationException(DCSR.GetString(DCSR.ListViewCantAddItemsToAVirtualListView));
                } else {
            ...
            private int DisplayIndexToID(int displayIndex) {
                ...
                if (owner.IsHandleCreated && !owner.ListViewHandleDestroyed) {
                    ...
                    WinFormNativeMethods.LVITEM lvItem = new WinFormNativeMethods.LVITEM();
                    lvItem.mask = WinFormNativeMethods.LVIF_PARAM;
                    lvItem.iItem = displayIndex;
                    WinFormUnsafeNativeMethods.SendMessage(new HandleRef(owner, owner.Handle), WinFormNativeMethods.LVM_GETITEM, 0, ref lvItem);
            ...
            public void Clear() {
                if (owner.itemCount > 0) {
                    ...
                    if (owner.IsHandleCreated && !owner.ListViewHandleDestroyed) {
                        ...
                        int nextSelected = (int)WinFormUnsafeNativeMethods.SendMessage(new HandleRef(owner, owner.Handle), WinFormNativeMethods.LVM_GETNEXTITEM, -1, WinFormNativeMethods.LVNI_SELECTED);
                        for (int i = 0; i < count; i++) {
                            ListViewItem item = owner.Items[i];
                            ...
                            if (item != null) {
                                ...
                                if (i == nextSelected) {
                                    ...
                                    nextSelected = (int)WinFormUnsafeNativeMethods.SendMessage(new HandleRef(owner, owner.Handle), WinFormNativeMethods.LVM_GETNEXTITEM, nextSelected, WinFormNativeMethods.LVNI_SELECTED);
                                }
                                else {
                        ...
                        WinFormUnsafeNativeMethods.SendMessage(new HandleRef(owner, owner.Handle), WinFormNativeMethods.LVM_DELETEALLITEMS, 0, 0);
            ...
            public void Remove(ListViewItem item) {
                ...
                if (owner.VirtualMode) {
                    throw new InvalidOperationException(DCSR.GetString(DCSR.ListViewCantRemoveItemsFromAVirtualListView));
                }
            ...
            public void RemoveAt(int index) {
                ...
                if (owner.IsHandleCreated) {
                    ...
                    Debug.Assert(owner.listItemsArray == null, "listItemsArray not null, even though handle created");
                    int retval = unchecked( (int) (long)owner.SendMessage(WinFormNativeMethods.LVM_DELETEITEM, index, 0));
