    ...
    public abstract class TextBoxBase : Control {
        ...
        public bool CanUndo {
            get {
                if (IsHandleCreated) {
                    bool b;
                    b = unchecked( (int) (long)SendMessage(WinFormNativeMethods.EM_CANUNDO, 0, 0)) != 0;
        ...
        protected override CreateParams CreateParams {
            ...
            get {
                ...
                cp.ClassName = "EDIT";
                cp.Style |= WinFormNativeMethods.ES_AUTOHSCROLL | WinFormNativeMethods.ES_AUTOVSCROLL;
                if (!textBoxFlags[hideSelection]) cp.Style |= WinFormNativeMethods.ES_NOHIDESEL;
                if (textBoxFlags[readOnly]) cp.Style |= WinFormNativeMethods.ES_READONLY;
                ...
                if (textBoxFlags[multiline]) {
                    cp.Style |= WinFormNativeMethods.ES_MULTILINE;
                    if (textBoxFlags[wordWrap]) cp.Style &= ~WinFormNativeMethods.ES_AUTOHSCROLL;
                }
        ...
        public virtual int MaxLength {
            ...
            set {
                if (value < 0) {
                    throw new ArgumentOutOfRangeException("MaxLength", DCSR.GetString(DCSR.InvalidLowBoundArgumentEx, "MaxLength", (value).ToString(CultureInfo.CurrentCulture), (0).ToString(CultureInfo.CurrentCulture)));
                }
        ...
        public bool Modified {
            get {
                if (IsHandleCreated) {
                    bool curState = (0 != unchecked( (int) (long)SendMessage(WinFormNativeMethods.EM_GETMODIFY, 0, 0)));
            ...
            set {
                if (Modified != value) {
                    if (IsHandleCreated) {
                        SendMessage(WinFormNativeMethods.EM_SETMODIFY, value ? 1 : 0, 0);
        ...
        DesignerSerializationVisibility(DesignerSerializationVisibility.Hidden),
        SRDescription(DCSR.TextBoxPreferredHeightDescr)
        ]
        ...
        internal void GetSelectionStartAndLength( out int start, out int length ){
            ...
            if( !IsHandleCreated ) {
                ...
                start = 0;
                WinFormUnsafeNativeMethods.SendMessage( new HandleRef( this, Handle ), WinFormNativeMethods.EM_GETSEL, ref start, ref end );
        ...
        public bool ReadOnly {
            ...
            set {
                if (textBoxFlags[readOnly] != value) {
                    ...
                    if (IsHandleCreated) {
                        SendMessage(WinFormNativeMethods.EM_SETREADONLY, value? -1: 0, 0);
                    }
        ...
        internal virtual void SetSelectedTextInternal(string text, bool clearUndo){
            ...
            SendMessage(WinFormNativeMethods.EM_LIMITTEXT, 0, 0);

            if( clearUndo ){
                SendMessage(WinFormNativeMethods.EM_REPLACESEL, 0, text);
                // For consistency with Text, we clear the modified flag
                SendMessage(WinFormNativeMethods.EM_SETMODIFY, 0, 0);
                ClearUndo();
            }
            else{
                SendMessage(WinFormNativeMethods.EM_REPLACESEL, /*undoable*/ -1, text);
            }

            // Re-enable user input.
            SendMessage(WinFormNativeMethods.EM_LIMITTEXT, maxLength, 0);
        }
        ...
        public virtual int SelectionLength {
            ...
            set {
                if (value < 0){
                    throw new ArgumentOutOfRangeException("SelectionLength", DCSR.GetString(DCSR.InvalidArgument, "SelectionLength", value.ToString(CultureInfo.CurrentCulture)));
                }
                ...
                this.LogWF2WPropertyValue("SelectionLength", value);
            }
        }
        ...
        public int SelectionStart {
            ...
            set {
                ...
                this.LogWF2WPropertyValue("SelectionStart", value);
            }
        }
        ...
        public virtual int TextLength {
            get {
                ...
                if (IsHandleCreated && Marshal.SystemDefaultCharSize == 2) {
                    return DCWin32API.JSRuntime.Invoke<int>("__DCWin32API.GetWindowTextLength", this.Handle.ToInt32());// WinFormSafeNativeMethods.GetWindowTextLength(new HandleRef(this, Handle));
                }
                else {
                    return Text.Length;
                }
        ...
        internal void ForceWindowText(string value) {
            ...
            try {
                if (IsHandleCreated) {
                    ...
                    //WinFormUnsafeNativeMethods.SetWindowText(new HandleRef(this, Handle), value);
                }
                else {
                    if (value.Length == 0) {
                        Text = null;
        ...
        public bool WordWrap {
            ...
            set {
                ...
                this.LogWF2WPropertyValue("WordWrap", value);
            }
        }
        ...
        public void ClearUndo() {
            if (IsHandleCreated) {
                SendMessage(WinFormNativeMethods.EM_EMPTYUNDOBUFFER, 0, 0);
            }
        }
        ...
        public void Copy() {
            SendMessage(WinFormNativeMethods.WM_COPY, 0, 0);
        }
        ...
        /// </devdoc>
        public void Cut() {
            SendMessage(WinFormNativeMethods.WM_CUT, 0, 0);
        }
        ...
        public void Paste() {
            ...
            SendMessage(WinFormNativeMethods.WM_PASTE, 0, 0);
        }
        ...
        protected override void OnMouseUp(MouseEventArgs mevent) {
            ...
            if (mevent.Button == MouseButtons.Left) {
                //if (!ValidationCancelled && WinFormUnsafeNativeMethods.WindowFromPoint(pt.X, pt.Y) == Handle) 
                {
        ...
        public virtual int GetCharIndexFromPosition(Point pt) {
            int longPoint = WinFormNativeMethods.Util.MAKELONG(pt.X, pt.Y);
            int index = (int)WinFormUnsafeNativeMethods.SendMessage(new HandleRef(this, Handle), WinFormNativeMethods.EM_CHARFROMPOS, 0, longPoint);
            index = WinFormNativeMethods.Util.LOWORD(index);
        ...
        public virtual int GetLineFromCharIndex(int index) {
            return unchecked( (int) (long)SendMessage(WinFormNativeMethods.EM_LINEFROMCHAR, index, 0));
        }
        ...
        public virtual Point GetPositionFromCharIndex(int index) {
            ...
            int i = (int)WinFormUnsafeNativeMethods.SendMessage(new HandleRef(this, Handle), WinFormNativeMethods.EM_POSFROMCHAR, index, 0);
            return new Point(WinFormNativeMethods.Util.SignedLOWORD(i), WinFormNativeMethods.Util.SignedHIWORD(i));
        }
        ...
        public int GetFirstCharIndexFromLine(int lineNumber) {
            if (lineNumber < 0) {
                throw new ArgumentOutOfRangeException("lineNumber", DCSR.GetString(DCSR.InvalidArgument, "lineNumber", lineNumber.ToString(CultureInfo.CurrentCulture)));
            }
            return unchecked( (int) (long)SendMessage(WinFormNativeMethods.EM_LINEINDEX, lineNumber, 0));
        }
        ...
        public int GetFirstCharIndexOfCurrentLine() {
            return unchecked( (int) (long)SendMessage(WinFormNativeMethods.EM_LINEINDEX, -1, 0));
        }
        ...
        public void ScrollToCaret() {
            if (IsHandleCreated) {
                ...
                try {
                    if (WinFormUnsafeNativeMethods.SendMessage(new HandleRef(this, Handle), RichTextBoxConstants.EM_GETOLEINTERFACE, 0, out editOle) != 0) {

                        editOlePtr = Marshal.GetIUnknownForObject(editOle);

                        if (editOlePtr != IntPtr.Zero) {
                            IntPtr iTextDocument = IntPtr.Zero;
                            Guid iiTextDocumentGuid = typeof(WinFormUnsafeNativeMethods.ITextDocument).GUID;

                            try {
                                Marshal.QueryInterface(editOlePtr, ref iiTextDocumentGuid, out iTextDocument);

                                WinFormUnsafeNativeMethods.ITextDocument textDocument = Marshal.GetObjectForIUnknown(iTextDocument) as WinFormUnsafeNativeMethods.ITextDocument;
                                ...
                                if (textDocument != null) {
                                    ...
                                    WinFormUnsafeNativeMethods.ITextRange textRange = textDocument.Range(this.WindowText.Length - 1, this.WindowText.Length - 1);
                                    textRange.ScrollIntoView(0);   // 0 ==> tomEnd

                                    // 2. Get the first visible line.
                                    int firstVisibleLine = unchecked( (int) (long)SendMessage(WinFormNativeMethods.EM_GETFIRSTVISIBLELINE, 0, 0));
        ...
        internal virtual void SelectInternal(int start, int length, int textLen) {
            ...
            if (IsHandleCreated) {
                ...
                SendMessage(WinFormNativeMethods.EM_SETSEL, s, e);
                // 
        ...
        internal void SetSelectionOnHandle() {
            ...
            if (textBoxFlags[setSelectionOnHandleCreated]) {
                ...
                int start, end;
                AdjustSelectionStartAndEnd(this.selectionStart, this.selectionLength, out start, out end, -1);
                SendMessage(WinFormNativeMethods.EM_SETSEL, start, end);
            }
        }
        ...
        /// </devdoc>
        public void Undo() {
            SendMessage(WinFormNativeMethods.EM_UNDO, 0, 0);
        }
        ...
        private void WmReflectCommand(ref Message m) {
            if (!textBoxFlags[codeUpdateText] && !textBoxFlags[creatingHandle]) {
                if (WinFormNativeMethods.Util.HIWORD(m.WParam) == WinFormNativeMethods.EN_CHANGE && CanRaiseTextChangedEvent) {
                    OnTextChanged(EventArgs.Empty);
                }
                else if (WinFormNativeMethods.Util.HIWORD(m.WParam) == WinFormNativeMethods.EN_UPDATE) {
        ...
        void WmSetFont(ref Message m) {
            base.WndProc(m);
            if (!textBoxFlags[multiline]) {
                SendMessage(WinFormNativeMethods.EM_SETMARGINS, WinFormNativeMethods.EC_LEFTMARGIN | WinFormNativeMethods.EC_RIGHTMARGIN, 0);
            }
        }
        ...
        void WmGetDlgCode(ref Message m) {
            ...
            if (AcceptsTab) {
                ...
                m.Result = (IntPtr)(unchecked( (int) (long)m.Result) | WinFormNativeMethods.DLGC_WANTTAB);
            }
            else {
                Debug.WriteLineIf(Control.ControlKeyboardRouting.TraceVerbose, "TextBox doesn't want tabs");
                m.Result = (IntPtr)(unchecked( (int) (long)m.Result) & ~(WinFormNativeMethods.DLGC_WANTTAB | WinFormNativeMethods.DLGC_WANTALLKEYS));
            }
        }
