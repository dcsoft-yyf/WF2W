...
namespace System.Drawing {
    ...
    public sealed class Icon : MarshalByRefObject, ISerializable, ICloneable, IDisposable {
        ...
        public string CreateBlobUrl(bool autoDeleteBlobUrl = false)
        {
            this._AutoDeleteBlobUrl = autoDeleteBlobUrl;
            if (this._BlobUrl == null)
            {
                var bs = this.ToByteArray();
                if (bs != null && bs.Length > 0)
                {
                    this._BlobUrl = DCTextUtils.EventCreateBlobUrl(bs, "image/x-icon");
                }
            }
        ...
        public Icon(byte[] bsData )
        {
            if(bsData == null) {
                throw new ArgumentException(DCSR.GetString(DCSR.InvalidArgument, "bsData", "null"));
            }
            SetData(bsData);
        }
        private void SetData(byte[] bsData )
        {
            ...
            this.iconData = bsData;
            try
            {
                if (this.iconData.Length >= 6)
                {
                    // idReserved(0), idType(1=icon), idCount
                    int idReserved = this.iconData[0] | (this.iconData[1] << 8);
                    int idType = this.iconData[2] | (this.iconData[3] << 8);
                    int idCount = this.iconData[4] | (this.iconData[5] << 8);
                    if (idReserved == 0 && idType == 1 && idCount > 0 && this.iconData.Length >= 6 + 16)
                    {
                        int entryOffset = 6; // first ICONDIRENTRY
                        byte w = this.iconData[entryOffset + 0];
                        byte h = this.iconData[entryOffset + 1];
                        int width = w == 0 ? 256 : w;
            ...
            catch { /* ignore parsing errors; iconSize will be resolved lazily */ }
        }
        ...
        internal Icon(IntPtr handle, bool takeOwnership) {
            if (handle == IntPtr.Zero) {
                throw new ArgumentException(DCSR.GetString(DCSR.InvalidGDIHandle, (typeof(Icon)).Name));
            }
            this.handle = handle;
        ...
        public Icon(Icon original, int width, int height) : this() {
            if (original == null) {
                throw new ArgumentException(DCSR.GetString(DCSR.InvalidArgument, "original", "null"));
            }
        ...
        private static Icon ExtractAssociatedIcon(string filePath, int index) {
            if (filePath == null) {
                throw new ArgumentException(DCSR.GetString(DCSR.InvalidArgument, "filePath", "null"));
            }
            ...
            if (uri.IsUnc) {
                throw new ArgumentException (DCSR.GetString (DCSR.InvalidArgument, "filePath", filePath));
            }
            if (uri.IsFile)
            {
        ...
        private unsafe void Initialize(int width, int height) {
            if (iconData == null) {
                throw new InvalidOperationException(DCSR.GetString(DCSR.IllegalState, GetType().Name));
            }
            // Managed-only ICO header parsing to choose best entry by size/bit depth
            if (iconData.Length < 6) {
                throw new ArgumentException(DCSR.GetString(DCSR.InvalidPictureType, "picture", "Icon"));
            }
            int idReserved = iconData[0] | (iconData[1] << 8);
            int idType = iconData[2] | (iconData[3] << 8);
            int idCount = iconData[4] | (iconData[5] << 8);
            ...
            {
                int off = 6 + i * 16;
                if (off + 16 > iconData.Length) break;
                int w = iconData[off + 0]; w = (w == 0 ? 256 : w);
                int h = iconData[off + 1]; h = (h == 0 ? 256 : h);
                int planes = iconData[off + 4] | (iconData[off + 5] << 8);
                int bitc = iconData[off + 6] | (iconData[off + 7] << 8);
                int bpp = bitc != 0 ? bitc : (iconData[off + 2] != 0 ? 4 : 8);
                int delta = (width > 0 && height > 0) ? Math.Abs(w - width) + Math.Abs(h - height) : -(w * h); // prefer larger if no desired size
                if (delta < bestDelta || (delta == bestDelta && bpp >= selBitDepth))
                {
                    bestDelta = delta;
                    bestIndex = i;
                    selBitDepth = bpp;
            ...
            int eoff = 6 + bestIndex * 16;
            int bw = iconData[eoff + 0]; bw = (bw == 0 ? 256 : bw);
            int bh = iconData[eoff + 1]; bh = (bh == 0 ? 256 : bh);
            iconSize = new Size(bw, bh);
            bestBitDepth = selBitDepth;
            bestBytesInRes = iconData[eoff + 8] | (iconData[eoff + 9] << 8) | (iconData[eoff + 10] << 16) | (iconData[eoff + 11] << 24);
            bestImageOffset = iconData[eoff + 12] | (iconData[eoff + 13] << 8) | (iconData[eoff + 14] << 16) | (iconData[eoff + 15] << 24);
        ...
        public override string ToString() {
            return DCSR.GetString(DCSR.toStringIcon);
        }
