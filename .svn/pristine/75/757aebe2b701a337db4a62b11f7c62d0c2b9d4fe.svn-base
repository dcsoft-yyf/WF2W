...
namespace System.Windows.Forms {
    ...
    public sealed class Cursor : IDisposable, ISerializable {
        ...
        private string _HtmlName = null;
        [System.Reflection.Obfuscation(Exclude = true, ApplyToMembers = false)]
        public string HtmlName
        {
            get { return this._HtmlName; }
        }
        internal Cursor(string htmlName)
        {
        ...
        internal Cursor(SerializationInfo info, StreamingContext context) {
            ...
            for (; sie.MoveNext();) {
                ...
                if (String.Equals(sie.Name, "CursorData", StringComparison.OrdinalIgnoreCase) ){
                    ...
                    if (cursorData != null) {
                        LoadPicture(new WinFormUnsafeNativeMethods.ComStreamFromDataStream(new MemoryStream(cursorData)));
                    }
                }
        ...
        public Cursor(IntPtr handle) {
            ...
            if (handle == IntPtr.Zero) {
                throw new ArgumentException(DCSR.GetString(DCSR.InvalidGDIHandle, (typeof(Cursor)).Name));
            }
        ...
        internal static Rectangle ClipInternal
        {
            get {
                ...
                //WinFormSafeNativeMethods.GetClipCursor(ref r);
                //return Rectangle.FromLTRB(r.left, r.top, r.right, r.bottom);
            }
            set {
                // 无法实现
                //if (value.IsEmpty) {
                //    WinFormUnsafeNativeMethods.ClipCursor(null);
                //}
                //else {
                //    WinFormNativeMethods.RECT rcClip = WinFormNativeMethods.RECT.FromXYWH(value.X, value.Y, value.Width, value.Height);
                //    WinFormUnsafeNativeMethods.ClipCursor(ref rcClip);
                //}
            }
        }
        ...
        internal static Cursor CurrentInternal {
            get {
                IntPtr curHandle = WinFormSafeNativeMethods.GetCursor();
            ...
            set {
                ...
                WinFormUnsafeNativeMethods.SetCursor(new HandleRef(value, handle));
            }
        }
        ...
        public IntPtr Handle {
            get {
                if (handle == IntPtr.Zero) {
                    throw new ObjectDisposedException(DCSR.GetString(DCSR.ObjectDisposed, GetType().Name));
                }
                return handle;
            }
        }
        ...
        public Point HotSpot
        {
            get {
                ...
                WinFormNativeMethods.ICONINFO info = new WinFormNativeMethods.ICONINFO();
                ...
                try {
                    WinFormSafeNativeMethods.GetIconInfo(new HandleRef(this, currentIcon.Handle), info);
                    ...
                    if (info.hbmMask  != IntPtr.Zero) {
                      ...
                      WinFormSafeNativeMethods.ExternalDeleteObject(new HandleRef(null, info.hbmMask));
                      info.hbmMask  = IntPtr.Zero;
                    }
                    if (info.hbmColor != IntPtr.Zero) {
                      // ExternalDelete to prevent Handle underflow
                      WinFormSafeNativeMethods.ExternalDeleteObject(new HandleRef(null, info.hbmColor));
        ...
        public static Point Position {
            ...
            set {
                ...
                WinFormUnsafeNativeMethods.SetCursorPos(value.X, value.Y);
            }
        }
        ...
        public Size Size {
            get {
                if (cursorSize.IsEmpty) {
                     cursorSize = new Size(
                                         WinFormUnsafeNativeMethods.GetSystemMetrics(WinFormNativeMethods.SM_CXCURSOR),
                                         WinFormUnsafeNativeMethods.GetSystemMetrics(WinFormNativeMethods.SM_CYCURSOR)
                                         );
        ...
        public IntPtr CopyHandle() {
            Size sz = Size;
            return WinFormSafeNativeMethods.CopyImage(new HandleRef(this, Handle), WinFormNativeMethods.IMAGE_CURSOR, sz.Width, sz.Height, 0);
        }
        ...
        private void DestroyHandle() {
            if (ownHandle) {
                WinFormUnsafeNativeMethods.DestroyCursor(new HandleRef(this, handle));
            }
        }
            ...
            try { // want finally clause to release dc
                ...
                if (stretch) {
                    ...
                    if (targetWidth == imageWidth && targetHeight == imageHeight
                        && imageX == 0 && imageY == 0 && rop == WinFormNativeMethods.SRCCOPY
                        && imageWidth == cursorSize.Width && imageHeight == cursorSize.Height) {
                        WinFormSafeNativeMethods.DrawIcon(new HandleRef(graphics, dc), targetX, targetY, new HandleRef(this, handle));
                        return;
                    }
                    ...
                    // screen
                    //
                    if (imageX == 0 && imageY == 0 && rop == WinFormNativeMethods.SRCCOPY
                ...
                if (rop == WinFormNativeMethods.SRCCOPY) {
                    ...
                    //
                    WinFormSafeNativeMethods.IntersectClipRect(new HandleRef(this, Handle), targetX, targetY, targetX+clipWidth, targetY+clipHeight);
                    WinFormSafeNativeMethods.DrawIconEx(new HandleRef(graphics, dc), targetX - imageX, targetY - imageY,
                                       new HandleRef(this, handle), drawWidth, drawHeight, 0, WinFormNativeMethods.NullHandleRef, WinFormNativeMethods.DI_NORMAL);
        ...
        void ISerializable.GetObjectData(SerializationInfo si, StreamingContext context) {
            if (cursorData != null) {
                ...
                throw new SerializationException(DCSR.GetString(DCSR.CursorNonSerializableHandle));
            }
        }
        ...
        public static void Hide() {
            ...
            WinFormUnsafeNativeMethods.ShowCursor(false);
        }
        ...
        private void LoadFromResourceId(int nResourceId) {
           ...
           try {
               ...
               handle = WinFormSafeNativeMethods.LoadCursor(WinFormNativeMethods.NullHandleRef, nResourceId);
           }
           catch (Exception e) {
               handle = IntPtr.Zero;
        ...
        private Size GetIconSize(IntPtr iconHandle) { 
            ...
            WinFormNativeMethods.ICONINFO info = new WinFormNativeMethods.ICONINFO();
            WinFormSafeNativeMethods.GetIconInfo(new HandleRef(this, iconHandle), info);
            WinFormNativeMethods.BITMAP bmp = new WinFormNativeMethods.BITMAP();

            if (info.hbmColor != IntPtr.Zero) {
                WinFormUnsafeNativeMethods.GetObject(new HandleRef(null, info.hbmColor), Marshal.SizeOf(typeof(WinFormNativeMethods.BITMAP)), bmp);
                WinFormSafeNativeMethods.IntDeleteObject(new HandleRef(null, info.hbmColor));
                iconSize = new Size(bmp.bmWidth, bmp.bmHeight);
            }
            else if (info.hbmMask != IntPtr.Zero) {
                WinFormUnsafeNativeMethods.GetObject(new HandleRef(null, info.hbmMask), Marshal.SizeOf(typeof(WinFormNativeMethods.BITMAP)), bmp);
                iconSize = new Size(bmp.bmWidth, bmp.bmHeight / 2);
            }
            
            if (info.hbmMask != IntPtr.Zero) {
                WinFormSafeNativeMethods.IntDeleteObject(new HandleRef(null, info.hbmMask));
            }
            return iconSize;            
        }
        ...
        /// </devdoc>
        private  void LoadPicture(WinFormUnsafeNativeMethods.IStream stream) {

            if (stream == null) {
                throw new ArgumentNullException("stream");
            }
            try {
                Guid g = typeof(WinFormUnsafeNativeMethods.IPicture).GUID;
                WinFormUnsafeNativeMethods.IPicture picture = null;

                //new SecurityPermission(SecurityPermissionFlag.UnmanagedCode).Assert();
                
                try {
                    picture = WinFormUnsafeNativeMethods.OleCreateIPictureIndirect(null, ref g, true);
                    WinFormUnsafeNativeMethods.IPersistStream ipictureAsIPersist = (WinFormUnsafeNativeMethods.IPersistStream)picture;
                    ipictureAsIPersist.Load(stream);
                    
                    if (picture != null && picture.GetPictureType() == WinFormNativeMethods.Ole.PICTYPE_ICON) {
                        IntPtr cursorHandle = picture.GetHandle();
                        Size picSize = GetIconSize(cursorHandle); 
                        handle = WinFormSafeNativeMethods.CopyImageAsCursor(new HandleRef(this, cursorHandle), WinFormNativeMethods.IMAGE_CURSOR,
                            picSize.Width, picSize.Height, 0);
                        ownHandle = true;
                    }
                    else {
                        throw new ArgumentException(DCSR.GetString(DCSR.InvalidPictureType,
                                                          "picture",
                                                          "Cursor"), "picture");
                    }
                }
                ...
                Debug.Fail(e.ToString());
                throw new ArgumentException(DCSR.GetString(DCSR.InvalidPictureFormat), "stream", e);
            }
        }
        ...
        internal void SavePicture(Stream stream) {
            ...
            if(this.resourceId != 0) {
                throw new FormatException(DCSR.GetString(DCSR.CursorCannotCovertToBytes));
            }
            try {
                ...
                Debug.Fail(e.ToString());
                throw new InvalidOperationException(DCSR.GetString(DCSR.InvalidPictureFormat));
            }
        }
        ...
        public static void Show() {
            WinFormUnsafeNativeMethods.ShowCursor(true);
        }
        ...
        {
            if (this._HtmlName != null && this._HtmlName.Length > 0)
            {
